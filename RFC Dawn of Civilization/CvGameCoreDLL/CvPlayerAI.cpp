...
void CvPlayerAI::AI_init()
{
	...
	if ((GC.getInitCore().getSlotStatus(getID()) == SS_TAKEN) || (GC.getInitCore().getSlotStatus(getID()) == SS_COMPUTER))
	{
		...
		//AI_setCivicTimer(((getMaxAnarchyTurns() == 0) ? (MIN_REVOLUTION_TURNS * 2) : CIVIC_CHANGE_DELAY) / 2);
...
{
	PROFILE_FUNC();
	GC.countFunctionCall("CvPlayerAI::AI_doTurnPre()");
...
{
	PROFILE_FUNC();
	GC.countFunctionCall("CvPlayerAI::AI_doTurnPost()");
	if (isHuman())
	{
		return;
	}
...
void CvPlayerAI::AI_conquerCity(CvCity* pCity, PlayerTypes ePreviousOwner, PlayerTypes eHighestCulturePlayer, int iCaptureGold)
{
	...
	if (canRaze(pCity))
	{
		...
		if ((iX == 67 && iY == 48) || //Carthage
			...
			(iX == 90 && iY == 45) || //Ur
			(iX == 81 && iY == 53) || //Gordium
			(iX == 84 && iY == 53) || //Hattusas
			(iX == 89 && iY == 53) || //Artaxata
			(iX == 79 && iY == 18) || //Zimbabwe
		...
		// Leoreth: added more 600 AD cities here
			...
			(iX == 74 && iY == 45) ||     //Bengazi
			(iX == 77 && iY == 44) ||     //Alexandria
			(iX == 79 && iY == 43) ||     //Cairo
			(iX == 94 && iY == 45) ||     //Shiraz
			(iX == 97 && iY == 53) ||     //Merv
			(iX == 100 && iY == 54) ||    //Samarkand
			(iX == 125 && iY == 56) ||	//Beijing
			(iX == 86 && iY == 54) ||	//Trabzon
			(iX == 82 && iY == 53)) {	//Konya
			iRazeValue -= 30;
		}
...
/************************************************************************************************/
	...
	{
		iValue += 40;
		iValue += (FRESH_WATER_HEALTH_CHANGE * 30);
	}
...
//	argsList.add(true);
//	lResult = -1;
//	GC.callPythoFunction(PYGameModule, "AI_chooseTech", argsList.makeFunctionArgs(), &lResult);
...
int CvPlayerAI::AI_getPeaceAttitude(PlayerTypes ePlayer) const
{
	int iAttitudeChange;
	iAttitudeChange = 0;
	if ((GET_PLAYER(ePlayer).isHuman()) && GC.m_iCVPLAYERAI_ATTITUDE_BONUS != 0)
	{
		iAttitudeChange += GC.m_iCVPLAYERAI_ATTITUDE_BONUS; //mediv01 外交态度红利
	}
...
int CvPlayerAI::AI_maxGoldTrade(PlayerTypes ePlayer) const
{
	...
	else
	{
		...
		iMaxGold -= (iMaxGold % DIPLOMACY_VALUE_REMAINDER);
	}

	if (CVPLAYERAI_CAN_TRADE_GOLD_UNLIMITED == 1) {  //无限制交易金币
		iMaxGold = getGold();
	}
...
int CvPlayerAI::AI_maxGoldPerTurnTrade(PlayerTypes ePlayer) const
{
	...
	if (isHuman() || (GET_PLAYER(ePlayer).getTeam() == getTeam()))
	{
		iMaxGoldPerTurn = (calculateGoldRate() + (getGold() / PEACE_TREATY_LENGTH));
	...
	else
	{
		...
		if (isHuman()) {
			if (GC.m_iCVPLAYERAI_CAN_TRADE_GOLD_TURN_BASE_ON_POPULATION > 0) {  //无限制交易金币
				iMaxGoldPerTurn += getTotalPopulation() / GC.m_iCVPLAYERAI_CAN_TRADE_GOLD_TURN_BASE_ON_POPULATION;
			}
		}
	}
	...
	if (isHuman()) {
		if (GC.m_iCVPLAYERAI_CAN_TRADE_GOLD_TURN_UNLIMITED_MULTI > 0) {  //无限制交易金币
			iMaxGoldPerTurn = getTotalPopulation() * GC.m_iCVPLAYERAI_CAN_TRADE_GOLD_TURN_UNLIMITED_MULTI;
			goldrate = (calculateGoldRate() + 1) * GC.m_iCVPLAYERAI_CAN_TRADE_GOLD_TURN_UNLIMITED_MULTI + iResearchSurplus;
		}
	}
...
{
	int iValue = iGoldPerTurn * PEACE_TREATY_LENGTH;
...
int CvPlayerAI::AI_bonusTradeVal(BonusTypes eBonus, PlayerTypes ePlayer, int iChange) const
{
	...
	return (iValue * PEACE_TREATY_LENGTH);
}
...
DenialTypes CvPlayerAI::AI_bonusTrade(BonusTypes eBonus, PlayerTypes ePlayer) const
{
	...
	if (GC.m_iCVPLAYERAI_CAN_ALWAYS_TRADE_RESOURCE == 1) {// mediv01 
		if (GET_PLAYER(ePlayer).isHuman()) {
			return NO_DENIAL;
		}
	}
...
DenialTypes CvPlayerAI::AI_cityTrade(CvCity* pCity, PlayerTypes ePlayer) const
{
	...
	if (GC.m_iCVPLAYERAI_CAN_ALWAYS_TRADE_CITY == 1) {//mediv01 
		if (GET_PLAYER(ePlayer).isHuman()) {
...
void CvPlayerAI::AI_doCommerce()
{
	...
	if (isCommerceFlexible(COMMERCE_CULTURE))
	{
		if (getNumCities() > 0)
		{
			...
			iIdealPercent -= (iIdealPercent % COMMERCE_PERCENT_CHANGE_INCREMENTS);
	...
	if (isCommerceFlexible(COMMERCE_RESEARCH))
	{
		...
		else
		{
			...
			{
				changeCommercePercent(COMMERCE_RESEARCH, COMMERCE_PERCENT_CHANGE_INCREMENTS);
			...
			{
				setCommercePercent(COMMERCE_RESEARCH, COMMERCE_PERCENT_CHANGE_INCREMENTS);
			}
			...
			{
				case MALI:
					changeCommercePercent(COMMERCE_RESEARCH, -4*(COMMERCE_PERCENT_CHANGE_INCREMENTS)); //-40%
					break;
			}
			//Rhye - end
		}
	...
	if (isCommerceFlexible(COMMERCE_ESPIONAGE) && !bFirstTech)
	{
		...
		else
		{
			...
			{
				changeCommercePercent(COMMERCE_RESEARCH, -COMMERCE_PERCENT_CHANGE_INCREMENTS);
				changeCommercePercent(COMMERCE_ESPIONAGE, COMMERCE_PERCENT_CHANGE_INCREMENTS);
	...
	//					break;
	//				}
					if (getCommercePercent(COMMERCE_RESEARCH) * 2 <= (getCommercePercent(COMMERCE_ESPIONAGE) + COMMERCE_PERCENT_CHANGE_INCREMENTS) * 3)
					{
						break;
					}
				}
...
void CvPlayerAI::AI_doCivics()
{
	...
	if (canRevolution(paeBestCivic))
	{
		...
		AI_setCivicTimer((getMaxAnarchyTurns() == 0) ? (MIN_REVOLUTION_TURNS * 2) : CIVIC_CHANGE_DELAY);
	}
...
{
	PROFILE_FUNC();
	GC.countFunctionCall("CvPlayerAI::AI_doDiplo()");
...
//	argsList.add(getID());
//	long lResult=0;
//	GC.callPythoFunction(PYGameModule, "AI_doDiplo", argsList.makeFunctionArgs(), &lResult);
//	if (lResult == 1)
//	{
//		return;
//	}
					...
					if (iI != getID())
					{
						if (GET_PLAYER((PlayerTypes)iI).getTeam() != getTeam())
						{
							for(pLoopDeal = GC.getGameINLINE().firstDeal(&iLoop); pLoopDeal != NULL; pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop))
							{
								...
								{
									if ((GC.getGameINLINE().getGameTurn() - pLoopDeal->getInitialGameTurn()) >= (PEACE_TREATY_LENGTH * 2))
									{
										bCancelDeal = false;
										...
										if (bCancelDeal)
										{
											if (canContact((PlayerTypes)iI) && AI_isWillingToTalk((PlayerTypes)iI))
											{
												if (GET_PLAYER((PlayerTypes)iI).isHuman())
												{
													...
													{
														if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_VASSAL_DEAL") > 0) {
															continue;
														}
													...
													{
														if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_CANCEL_VASSAL_DEAL") > 0) {
															continue;
														}
							...
							if (GET_PLAYER((PlayerTypes)iI).getTeam() == getTeam() || GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isVassal(getTeam()))
							{
								...
								if (eBestGiveBonus != NO_BONUS)
								{
									...
									if (GET_PLAYER((PlayerTypes)iI).isHuman())
									{
										if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
										{
											if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_GIVE_RESOURCE_DEAL") > 0) {
												continue;
											}
											pDiplo = new CvDiploParameters(getID());
											FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
							...
							if (GET_PLAYER((PlayerTypes)iI).getTeam() != getTeam() && GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isVassal(getTeam()))
							{
								...
								if (eBestGiveTech != NO_TECH)
								{
									...
									if (GET_PLAYER((PlayerTypes)iI).isHuman())
									{
										if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
										{
											if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_GIVE_TECH_DEAL") > 0) {
												continue;
											}
											pDiplo = new CvDiploParameters(getID());
											FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
								...
								if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isVassal(getTeam()))
								{
									...
									if (eBestGiveBonus != NO_BONUS)
									{
										...
										if (GET_PLAYER((PlayerTypes)iI).isHuman())
										{
											if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
											{
												if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_ASK_TRIBUTE_DEAL") > 0) {
													continue;
												}
													...
													if (iCount >= (iPossibleCount / 2))
													{
														...
														if (canTradeItem(((PlayerTypes)iI), item, true))
														{
															...
															if (GET_PLAYER((PlayerTypes)iI).isHuman())
															{
																//if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
																{
																	if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_GIVE_CITY_DEAL") > 0) {
																		continue;
																	}
																	pDiplo = new CvDiploParameters(getID());
																	FAssertMsg(pDiplo != NULL, "pDiplo must be valid");
											...
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_PERMANENT_ALLIANCE), "AI Diplo Alliance") == 0)
											{
												...
												if (canTradeItem(((PlayerTypes)iI), item, true) && GET_PLAYER((PlayerTypes)iI).canTradeItem(getID(), item, true))
												{
													...
													if (GET_PLAYER((PlayerTypes)iI).isHuman())
													{
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_SIGN_PERMANENT_ALLIANCE_DEAL") > 0) {
																continue;
															}
											...
											if (!bOffered)
											{
												...
												if (canTradeItem((PlayerTypes)iI, item, true))
												{
													...
													if (GET_PLAYER((PlayerTypes)iI).isHuman())
													{
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_OFFER_VASSAL_DEAL") > 0) {
																continue;
															}
															AI_changeContactTimer(((PlayerTypes)iI), CONTACT_PERMANENT_ALLIANCE, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_PERMANENT_ALLIANCE));
															pDiplo = new CvDiploParameters(getID());
											...
											if (GET_PLAYER((PlayerTypes)iI).canConvert(getStateReligion()))
											{
												if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_RELIGION_PRESSURE) == 0)
												{
													if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_RELIGION_PRESSURE), "AI Diplo Religion Pressure") == 0)
													{
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_ASK_RELIGION_PRESSURE") > 0) {
																continue;
															}
													...
													if (GET_PLAYER((PlayerTypes)iI).canRevolution(NULL))
													{
														if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_CIVIC_PRESSURE) == 0)
														{
															if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_CIVIC_PRESSURE), "AI Diplo Civic Pressure") == 0)
															{
																if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
																{
																	if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_ASK_CIVIC_PRESSURE") > 0) {
																		continue;
																	}
											...
											if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_JOIN_WAR) == 0)
											{
												if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_JOIN_WAR), "AI Diplo Join War") == 0)
												{
													...
													if (eBestTeam != NO_TEAM)
													{
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_ASK_JOIN_WAR") > 0) {
																continue;
															}
											...
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_STOP_TRADING), "AI Diplo Stop Trading") == 0)
											{
												...
												if ((eBestTeam != NO_TEAM) && GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(eBestTeam) && !GET_TEAM(eBestTeam).isVassal(GET_PLAYER((PlayerTypes)iI).getTeam()))
												{
													if (GET_PLAYER((PlayerTypes)iI).canStopTradingWithTeam(eBestTeam))
													{
														...
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_ASK_STOP_TRADING") > 0) {
																continue;
															}
											...
											if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_ASK_FOR_HELP) == 0)
											{
												if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_ASK_FOR_HELP), "AI Diplo Ask For Help") == 0)
												{
													...
													if (eBestReceiveTech != NO_TECH)
													{
														...
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_ASK_TECH") > 0) {
																continue;
															}
												...
												if (AI_getAttitude((PlayerTypes)iI) <= GC.getLeaderHeadInfo(GET_PLAYER((PlayerTypes)iI).getPersonalityType()).getDemandTributeAttitudeThreshold())
												{
													if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_DEMAND_TRIBUTE) == 0)
													{
														if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_DEMAND_TRIBUTE), "AI Diplo Demand Tribute") == 0)
														{
															...
															iReceiveGold -= (iReceiveGold % DIPLOMACY_VALUE_REMAINDER);
															...
															if (iReceiveGold > 50)
															{
																...
																if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
																{
																	if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_ASK_GOLD") > 0) {
																		continue;
																	}
													...
													if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_DEMAND_TRIBUTE) == 0)
													{
														if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_DEMAND_TRIBUTE), "AI Diplo Demand Tribute") == 0)
														{
															if (GET_TEAM(getTeam()).AI_mapTradeVal(GET_PLAYER((PlayerTypes)iI).getTeam()) > 100)
															{
																...
																if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
																{
																	if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_ASK_MAP") > 0) {
																		continue;
																	}
																	AI_changeContactTimer(((PlayerTypes)iI), CONTACT_DEMAND_TRIBUTE, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_DEMAND_TRIBUTE));
																	pDiplo = new CvDiploParameters(getID());
													...
													if (AI_getContactTimer(((PlayerTypes)iI), CONTACT_DEMAND_TRIBUTE) == 0)
													{
														if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_DEMAND_TRIBUTE), "AI Diplo Demand Tribute") == 0)
														{
															...
															if (eBestReceiveBonus != NO_BONUS)
															{
																...
																if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
																{
																	if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_ASK_RESOURCE") > 0) {
																		continue;
																	}
																	AI_changeContactTimer(((PlayerTypes)iI), CONTACT_DEMAND_TRIBUTE, GC.getLeaderHeadInfo(getPersonalityType()).getContactDelay(CONTACT_DEMAND_TRIBUTE));
																	pDiplo = new CvDiploParameters(getID());
											...
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_OPEN_BORDERS), "AI Diplo Open Borders") == 0)
											{
												...
												if (canTradeItem(((PlayerTypes)iI), item, true) && GET_PLAYER((PlayerTypes)iI).canTradeItem(getID(), item, true))
												{
													...
													if (GET_PLAYER((PlayerTypes)iI).isHuman())
													{
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_SIGN_OPEN_TRADE_DEAL") > 0) {
																continue;
															}
											...
											if (GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getContactRand(CONTACT_DEFENSIVE_PACT), "AI Diplo Defensive Pact") == 0)
											{
												...
												if (canTradeItem(((PlayerTypes)iI), item, true) && GET_PLAYER((PlayerTypes)iI).canTradeItem(getID(), item, true))
												{
													...
													if (GET_PLAYER((PlayerTypes)iI).isHuman())
													{
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_SIGN_DEFENSIVE_PACT_DEAL") > 0) {
																continue;
															}
													...
													if (!(GET_PLAYER((PlayerTypes)iI).isHuman()) || (iOurValue >= iTheirValue))
													{
														if ((iOurValue > ((iTheirValue * 2) / 3)) && (iTheirValue > ((iOurValue * 2) / 3)))
														{
															...
															if (GET_PLAYER((PlayerTypes)iI).isHuman())
															{
																if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
																{
																	if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_SIGN_TECH_TRADE_DEAL") > 0) {
																		continue;
																	}
														...
														if (iGold >= iTheirValue && iTheirValue > 0)
														{
															...
															if (canTradeItem((PlayerTypes)iI, item, true))
															{
																...
																if (GET_PLAYER((PlayerTypes)iI).isHuman())
																{
																	if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
																	{
																		if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_SIGN_SLAVE_TRADE_DEAL") > 0) {
																			continue;
																		}
												...
												if (eBestGiveBonus != NO_BONUS)
												{
													if (!(GET_PLAYER((PlayerTypes)iI).isHuman()) || (AI_bonusTradeVal(eBestReceiveBonus, ((PlayerTypes)iI), -1) >= GET_PLAYER((PlayerTypes)iI).AI_bonusTradeVal(eBestGiveBonus, getID(), 1)))
													{
														...
														if (GET_PLAYER((PlayerTypes)iI).isHuman())
														{
															if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
															{
																if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_SIGN_BONUS_TRADE_DEAL") > 0) {
																	continue;
																}
											...
											if (GET_PLAYER((PlayerTypes)iI).canTradeItem(getID(), item, true) && canTradeItem(((PlayerTypes)iI), item, true))
											{
												if (!(GET_PLAYER((PlayerTypes)iI).isHuman()) || (GET_TEAM(getTeam()).AI_mapTradeVal(GET_PLAYER((PlayerTypes)iI).getTeam()) >= GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).AI_mapTradeVal(getTeam())))
												{
													...
													if (GET_PLAYER((PlayerTypes)iI).isHuman())
													{
														if (!(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
														{
															if (GC.getDefineINT("CVPLAYERAI_AI_DODIPIO_CANNOT_DO_SIGN_MAP_TRADE_DEAL") > 0) {
																continue;
															}
...
int CvPlayerAI::AI_eventValue(EventTypes eEvent, const EventTriggeredData& kTriggeredData) const
{
	...
	if (kEvent.isCityEffect())
	{
		...
		iValue -= (kEvent.getHurryAnger() * 6 * HURRY_ANGER_DIVISOR * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getHurryConscriptAngerPercent()) / 100;
	...
	else if (!kEvent.isOtherPlayerCityEffect())
	{
		...
		iValue -= (iNumCities * kEvent.getHurryAnger() * 6 * HURRY_ANGER_DIVISOR * GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getHurryConscriptAngerPercent()) / 100;
	...
	if (NO_PLAYER != kTriggeredData.m_eOtherPlayer)
	{
		...
		if (NO_BONUS != kEvent.getBonusGift())
		{
			...
			iDiploValue += (iBonusValue * PEACE_TREATY_LENGTH) / 60;
		}
...
bool CvPlayerAI::AI_advancedStartPlaceExploreUnits(bool bLand)
{
	...
	for (pLoopCity = firstCity(&iLoop); pLoopCity != NULL; pLoopCity = nextCity(&iLoop))
	{
		...
		if (pLoopPlot == NULL)
		{
			...
			pLoopArea = bLand ? pLoopCity->area() : pLoopPlot->waterArea();
