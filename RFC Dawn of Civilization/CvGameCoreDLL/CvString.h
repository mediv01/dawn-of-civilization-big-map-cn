...
// wide string
class CvWString2 : public std::wstring
{
public:
	CvWString2() {}
	CvWString2(const std::string& s) { Copy(s.c_str()); }
	CvWString2(const CvWString2& s) { *this = s; }
	CvWString2(const char* s) { Copy(s); }
	CvWString2(const wchar* s) { if (s) *this = s; }
	//	CvWString2(const __wchar_t* s) { if (s) *this = s; }
	CvWString2(const std::wstring& s) { assign(s.c_str()); }
#ifndef _USRDLL
	// FString conversion, if not in the DLL
	CvWString2(const FStringA& s) { Copy(s.GetCString()); }
	CvWString2(const FStringW& s) { assign(s.GetCString()); }
#endif
	~CvWString2() {}
	...
	wchar& operator[](std::wstring::size_type i) { return std::wstring::operator[](i); }
	const wchar operator[](int i) const { return std::wstring::operator[](i); }
	const CvWString2& operator=(const wchar* s) { if (s) assign(s); else clear();	return *this; }
	const CvWString2& operator=(const std::wstring& s) { assign(s.c_str());	return *this; }
	const CvWString2& operator=(const std::string& w) { Copy(w.c_str());	return *this; }
	const CvWString2& operator=(const CvWString2& w) { assign(w.c_str());	return *this; }
#ifndef _USRDLL
	// FString conversion, if not in the DLL
	const CvWString2& operator=(const FStringW& s) { assign(s.GetCString());	return *this; }
	const CvWString2& operator=(const FStringA& w) { Copy(w.GetCString());	return *this; }
#endif
	const CvWString2& operator=(const char* w) { Copy(w);	return *this; }
	...
	static bool formatv(std::wstring& out, const wchar* fmt, va_list args);
	static bool format(std::wstring& out, const wchar* fmt, ...);
	static CvWString2 format(const wchar* fmt, ...);
	static std::wstring formatv(const wchar* fmt, va_list args);
};


//#define WIDEPTR(s) (s ? CvWString2(s).c_str() : NULL)

/*
inline CvWString2 operator+(const CvWString2& s, const CvWString2& t) { return (std::wstring&)s + (std::wstring&)t; }
inline CvWString2 operator+(const CvWString2& s, const wchar* t) { return (std::wstring&)s + std::wstring(t); }
inline CvWString2 operator+(const wchar* s, const CvWString2& t) { return std::wstring(s) + std::wstring(t); }
*/
//CvString2 operator+( const CvString2& s, const CvString2& t) { return (std::string&)s + (std::string&)t; }

class CvWString2Buffer
{
public:
	CvWString2Buffer()
	{
	...
	void append(const CvWString2& szString)
	{
		append(szString.GetCString());
	}

	void append(const CvWString2Buffer& szStringBuffer)
	{
		append(szStringBuffer.m_pBuffer);
	}

	void assign(const CvWString2& szString)
	{
		assign(szString.GetCString());
	}
...
//
class CvString2 : public std::string
{
public:
	CvString2() {}
	CvString2(int iLen) { reserve(iLen); }
	CvString2(const char* s) { operator=(s); }
	CvString2(const std::string& s) { assign(s.c_str()); }
	explicit CvString2(const std::wstring& s) { Copy(s.c_str()); }		// don't want accidental conversions down to narrow strings
	~CvString2() {}
	...
	// implicit conversion
	operator const char* () const { return c_str(); }
	//	operator const CvWString2() const 	{ return CvWString2(c_str()); }							

	// operators
	char& operator[](int i) { return std::string::operator[](i); }
	char& operator[](std::string::size_type i) { return std::string::operator[](i); }
	const char operator[](int i) const { return std::string::operator[](i); }
	const CvString2& operator=(const char* s) { if (s) assign(s); else clear();	return *this; }
	const CvString2& operator=(const std::string& s) { assign(s.c_str());	return *this; }
	//	const CvString2& operator=( const std::wstring& w) { Copy(w.c_str());	return *this; }		// don't want accidental conversions down to narrow strings
	//	const CvString2& operator=( const wchar* w) { Copy(w);	return *this; }	
	...
	void getTokens(const CvString2& delimiters, std::vector<CvString2>& tokensOut) const;

	// static helpers
	static bool formatv(std::string& out, const char* fmt, va_list args);
	static bool format(std::string& out, const char* fmt, ...);
	static CvString2 format(const char* fmt, ...);
	static std::string formatv(const char* fmt, va_list args);
};

//////////////////////////////////////////////////////////////////////////
// INLINES
// Don't move these into a cpp file, since I don't want CvString2 to be part of the DLL, MT
//////////////////////////////////////////////////////////////////////////

inline int CvString2::Replace(char chOld, char chNew)
{
	int i, iCnt = 0;
	for (i = 0; i < (int)size(); i++)
	{
...
inline void CvString2::getTokens(const CvString2& delimiters, std::vector<CvString2>& tokensOut) const
{
	//tokenizer code taken from http://www.digitalpeer.com/id/simple
	...
	while (CvString2::npos != pos || CvString2::npos != lastPos)
	{
		// found a token, parse it.
		CvString2 token = substr(lastPos, pos - lastPos);
		tokensOut.push_back(token);
...
// static
//
inline bool CvString2::formatv(std::string& out, const char* fmt, va_list args)
{
	char buf[2048];
	char* pbuf = buf;
	int len = 0;
	...
	if (attempts == kMaxAttempts)
	{
		// dxPrintNL( "CvString2::formatv - Max reallocs occurred while formatting string. Result is likely truncated!", 0 );
	}
...
// static
//
inline bool CvWString2::formatv(std::wstring& out, const wchar* fmt, va_list args)
{
	wchar buf[2048];
	wchar* pbuf = buf;
	int len = 0;
...
// static
//
inline std::wstring CvWString2::formatv(const wchar* fmt, va_list args)
{
	std::wstring result;
	formatv(result, fmt, args);
	return result;
...
// static
//
inline CvWString2 CvWString2::format(const wchar* fmt, ...)
{
	std::wstring result;
	va_list args;
	va_start(args, fmt);
	formatv(result, fmt, args);
	va_end(args);
	return CvWString2(result);
}

//
// static
//
inline bool CvWString2::format(std::wstring& out, const wchar* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	bool r = formatv(out, fmt, args);
...
//
//
inline void CvWString2::Format(LPCWSTR lpszFormat, ...)
{
	std::wstring result;
	va_list args;
	va_start(args, lpszFormat);
...
// static
//
inline std::string CvString2::formatv(const char* fmt, va_list args)
{
	std::string result;
	formatv(result, fmt, args);
	return result;
...
// static
//
inline CvString2 CvString2::format(const char* fmt, ...)
{
	std::string result;
	va_list args;
	va_start(args, fmt);
	formatv(result, fmt, args);
	va_end(args);
	return CvString2(result);
}

//
// static
//
inline bool CvString2::format(std::string& out, const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	bool r = formatv(out, fmt, args);
...
//
//
inline void CvString2::Format(LPCSTR lpszFormat, ...)
{
	std::string result;
	va_list args;
	va_start(args, lpszFormat);
