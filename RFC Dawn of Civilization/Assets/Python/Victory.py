...
lSouthAsianCivs = [iIndia, iChina, iTamils, iIndonesia, iKhmer, iBurma, iVietnam, iPhilippines, iMughals, iThailand]
...
def setup():
    ...
    if utils.getScenario() == i600AD:
        for iPlayer in [iNubia, iEthiopia, iTamils, iMississippi]:
            loseAll(iPlayer)
...
def checkTurn(iGameTurn, iPlayer):
    if not gcgame.isVictoryValid(7): return

    if iPlayer >= iNumPlayers: return

    if iGameTurn == utils.getScenarioStartTurn(): return

    if not gcgetPlayer(iPlayer).isAlive(): return
    ...
    if iPlayer == iEgypt:
        ...
        if iGameTurn == utils.getTurnForYear(-100):
            expire(iEgypt, 1)

        # third goal: have 5000 culture in 170 AD
        if iGameTurn == utils.getTurnForYear(170):
        ...
        if iGameTurn == utils.getTurnForYear(-700):
        ...
        # first goal: have open borders with another civilization by 1600 BC
        if isPossible(iHarappa, 0):
            if countOpenBorders(iHarappa) >= 1:
                win(iHarappa, 0)
        ...
        if not pNorteChico.isReborn():
            ...
            if iGameTurn == utils.getTurnForYear(-1800):
                if isPossible(iNorteChico, 0):
                    if (pNorteChico.getCapitalCity().getCulture(iNorteChico) >= gc.getCultureLevelInfo(3).getSpeedThreshold(gcgame.getGameSpeedType())):
                        win(iNorteChico, 0)
                    else:
                        lose(iNorteChico, 0)
            ...
            # third goal: Have a capital with 5 populations and 6 buildings in 1400 BC
            if iGameTurn == utils.getTurnForYear(-1400):
        ...
        # NorteChico
            ...
            if isPossible(iNorteChico, 1):
                bInca = iGameTurn >= utils.getTurnForYear(tBirth[iInca]) and isControlledOrVassalized(iNorteChico, Areas.getCoreArea(iInca, True))
                if bInca:
                    win(iNorteChico, 1)

            if iGameTurn == utils.getTurnForYear(1475):
        ...
        if isPossible(iNubia, 0):
            bPyramids = isBuildingInCity((78, 37), iPyramids)
            bControlsEgypt = isControlled(iNubia, Areas.getCoreArea(iMamluks, False))
            if bControlsEgypt:
                data.iNubiaEgyptYears += gcgame.getTurnYear(iGameTurn) - gcgame.getTurnYear(iGameTurn - 1)
                if bPyramids and data.iNubiaEgyptYears >= 1000:
                    win(iNubia, 0)

            if iGameTurn == utils.getTurnForYear(-656):
                if not bPyramids:
                    expire(iNubia, 0)
        ...
        if iGameTurn == utils.getTurnForYear(1365):
        ...
        if iGameTurn == utils.getTurnForYear(1821):
            if isPossible(iNubia, 2):
                iBestIslamicCommerceOutput = getBestPlayer(iNubia, playerCommerceOutput, getReligionPlayers([iIslam]))
                bPalace = isBuildingInCity((81, 35), iPalace)
                bIslamicMosque = isBuildingInCity((81, 35), iIslamicCathedral)
                bOrthodoxCathedral = isBuildingInCity((81, 35), iOrthodoxCathedral)
                bJewishSanhedrin = isBuildingInCity((81, 35), iJewishCathedral)
                if iBestIslamicCommerceOutput == iNubia and bIslamicMosque and bOrthodoxCathedral and bJewishSanhedrin:
                    win(iNubia, 2)
                else:
                    lose(iNubia, 2)
        ...
        if isPossible(iChina, 2):
            if data.iChineseGoldenAgeTurns >= utils.getTurns(40):
                win(iChina, 2)
        ...
        # second goal: control Egypt, Phoenicia, Babylonia and Persia in 300 BC
        if iGameTurn == utils.getTurnForYear(-300):
        ...
        if iGameTurn == utils.getTurnForYear(-250):
            expire(iGreece, 2)
        ...
        if iGameTurn == utils.getTurnForYear(1200):
        ...
        if iGameTurn == utils.getTurnForYear(-400):
            expire(iOlmecs, 0)
    ...
    # third goal: be the first to discover Compass
        ...
        if iGameTurn == utils.getTurnForYear(200):
        ...
        if iGameTurn in [utils.getTurnForYear(-200), utils.getTurnForYear(-1), utils.getTurnForYear(60)]:
            if not isBestPlayer(iCeltia, playerTotalCulture):
                lose(iCeltia, 2)
            elif iGameTurn == utils.getTurnForYear(60):
                win(iCeltia, 2)
        ...
        if not pPersia.isReborn():
            ...
            if iGameTurn == utils.getTurnForYear(140):
                expire(iPersia, 0)
            ...
            if iGameTurn == utils.getTurnForYear(350):
                expire(iPersia, 1)
        ...
        # Iran
            ...
            if iGameTurn == utils.getTurnForYear(1650):
        ...
        if iGameTurn == utils.getTurnForYear(320):
    ...
    # third goal: be first to discover Theology, Machinery and Civil Service
        ...
        if not pMaya.isReborn():
            ...
            if iGameTurn == utils.getTurnForYear(900):
                expire(iMaya, 1)
        ...
        # Colombia
            ...
            if iGameTurn == utils.getTurnForYear(1870):
            ...
            if iGameTurn == utils.getTurnForYear(1920):
        ...
        if isPossible(iYuezhi, 1):
            if numTradeConnections(iPlayer) >= 4:
                win(iYuezhi, 1)

            if iGameTurn == utils.getTurnForYear(200):
                expire(iYuezhi, 1)

        # third goal: Build the Salsal Buddha and eight temples by 400 CE
        if isPossible(iYuezhi, 2):
            iTemples = 0

            for iReligion in range(iNumReligions):
                iTemple = iJewishTemple + 4 * iReligion
                iTemples += getNumBuildings(iPlayer, iTemple)

            if data.getWonderBuilder(iSalsalBuddha) == iPlayer and iTemples >= 8:
                win(iYuezhi, 2)
        ...
        if iGameTurn == utils.getTurnForYear(210):
            expire(iXiongnu, 0)
        ...
        if iGameTurn == utils.getTurnForYear(560):
            expire(iXiongnu, 2)
        ...
        if isPossible(iEthiopia, 1):
            ...
            if gcgame.isReligionFounded(iOrthodoxy) and iGameTurn > gcgame.getReligionGameTurnFounded(iOrthodoxy) + utils.getTurns(5):
        ...
        if iGameTurn == utils.getTurnForYear(550):
        ...
        if iGameTurn == utils.getTurnForYear(500):
        ...
        if iGameTurn == utils.getTurnForYear(1070):
    ...
    # third goal: sink 20 enemy ships
        ...
        if iGameTurn == utils.getTurnForYear(1200):
            bLargest = isBestCity(iByzantium, (79, 55), cityPopulation)
            bCulture = isBestCity(iByzantium, (79, 55), cityCulture)
        ...
        if iGameTurn == utils.getTurnForYear(1450):
            ...
            bNearEast = getNumCitiesInArea(iByzantium, utils.getPlotList(tNearEastTL, tNearEastBR, tNearEastExceptions)) >= 3
        ...
        # third goal: Have three cities with developing culture and 5 population by 1100 AD
        ...
        if iGameTurn == utils.getTurnForYear(1940):
    ...
    # third goal: be the first to discover 5 modern techs
        ...
        if iGameTurn == utils.getTurnForYear(1050):
        ...
        if isPossible(iTurks, 2):
            ...
            if iGameTurn <= utils.getTurnForYear(900):
                if not data.tFirstTurkicCapital and capital.getCulture(iTurks) >= gc.getCultureLevelInfo(3).getSpeedThreshold(gcgame.getGameSpeedType()):
                    data.tFirstTurkicCapital = tCapital

            if iGameTurn <= utils.getTurnForYear(1100):
                if data.tFirstTurkicCapital and not data.tSecondTurkicCapital and tCapital != data.tFirstTurkicCapital and capital.getCulture(iTurks) >= gc.getCultureLevelInfo(4).getSpeedThreshold(gcgame.getGameSpeedType()):
                    data.tSecondTurkicCapital = tCapital

            if iGameTurn <= utils.getTurnForYear(1400):
                if tCapital != data.tFirstTurkicCapital and tCapital != data.tSecondTurkicCapital and data.tFirstTurkicCapital and data.tSecondTurkicCapital and capital.getCulture(iTurks) >= gc.getCultureLevelInfo(5).getSpeedThreshold(
                        gcgame.getGameSpeedType()):
                    win(iTurks, 2)
        ...
        if isPossible(iArabia, 2):
            if gcgame.calculateReligionPercent(iIslam) >= 30.0:
                win(iArabia, 2)
        ...
        if isPossible(iTibet, 1):
            if gcgame.calculateReligionPercent(iBuddhism) >= 25.0:
                win(iTibet, 1)
        ...
        if iGameTurn == utils.getTurnForYear(1211):
            iProphets = 0
            iScientists = 0
            iArtists = 0
            for city in utils.getCityList(iPlayer):
        ...
        if iGameTurn == utils.getTurnForYear(1580):
        ...
        if iGameTurn == utils.getTurnForYear(1031):
            expire(iKhazars, 0)
        ...
        if iGameTurn == utils.getTurnForYear(1241):
        ...
        if iGameTurn == utils.getTurnForYear(1259):
            expire(iChad, 0)

        if iGameTurn == utils.getTurnForYear(1380):
            expire(iChad, 1)
        ...
        if iGameTurn == utils.getTurnForYear(1603):
            expire(iChad, 2)
        ...
        if iGameTurn == utils.getTurnForYear(1650):
            fReligionPercent = gcgame.calculateReligionPercent(iCatholicism)
        ...
        if iGameTurn == utils.getTurnForYear(1700):
            if getCityCulture(iFrance, (61, 60)) >= utils.getTurns(50000):
                win(iFrance, 0)
            else:
                lose(iFrance, 0)
        ...
        if iGameTurn == utils.getTurnForYear(1900):
            expire(iFrance, 2)
        ...
        if isPossible(iMuisca, 1):
            ...
            for iLoopPlayer in range(iNumPlayers):
                iTradeGold += gcgetPlayer(iLoopPlayer).getGoldPerTurnByPlayer(iMuisca)
        ...
        if iGameTurn == utils.getTurnForYear(1540):
            expire(iMuisca, 1)
        ...
        if iGameTurn == utils.getTurnForYear(1265):
        ...
        if iGameTurn == utils.getTurnForYear(1229):
        ...
        if iGameTurn == utils.getTurnForYear(1815):
            expire(iEngland, 0)
        ...
        if isPossible(iEngland, 2):
            if isConnectedByRailroad(iEngland, (72, 11), lNorthernEgypt):
                if utils.getGameTurn() >= utils.getTurnForYear(1860):
                    win(iEngland, 2)
                else:
                    bNAmerica = getNumCitiesInArea(iEngland, utils.getPlotList(tNorthAmericaTL, tNorthAmericaBR)) >= 5
                    bSCAmerica = getNumCitiesInArea(iEngland, utils.getPlotList(tSouthCentralAmericaTL, tSouthCentralAmericaBR)) >= 3
        ...
        if iGameTurn == utils.getTurnForYear(1550):
            if isPossible(iHolyRome, 0):
        ...
        if iGameTurn == utils.getTurnForYear(1327):
            expire(iKievanRus, 0)
        ...
        if iGameTurn == utils.getTurnForYear(1867):
        ...
        if iGameTurn == utils.getTurnForYear(1380):
            if isPossible(iMamluks, 1):
                lLowerNile = [(x, y) for (x, y) in utils.getPlotList(tLowerNileTL, tLowerNileBR) if gcmap.plot(x, y).isRiver()]
                if isBestCity(iMamluks, (79, 43), cityPopulation) and countPopulationInArea(iMamluks, lLowerNile) >= 30:
                    win(iMamluks, 1)
                else:
                    lose(iMamluks, 1)
        ...
        if iGameTurn == utils.getTurnForYear(1350):
            expire(iMali, 0)
        ...
        if iGameTurn == utils.getTurnForYear(1700) and isPossible(iMali, 2):
        ...
        if iGameTurn == utils.getTurnForYear(1930):
            expire(iItaly, 2)
        ...
        if iGameTurn == utils.getTurnForYear(1600):
            if isPossible(iNigeria, 0):
                lAfricaCivs = [iLoopPlayer for iLoopPlayer in range(iNumPlayers) if gcgetPlayer(iLoopPlayer).getCapitalCity().getRegionID() in lAfrica]
        ...
        if iGameTurn == utils.getTurnForYear(1660):
            expire(iMughals, 1)
        ...
        if not pAztecs.isReborn():
            ...
            if iGameTurn == utils.getTurnForYear(1520):
                if isBestCity(iAztecs, (17, 43), cityPopulation):
                    win(iAztecs, 0)
                else:
                    lose(iAztecs, 0)
        ...
        # Mexico
            ...
            if iGameTurn == utils.getTurnForYear(1880):
                expire(iAztecs, 0)
            ...
            if iGameTurn == utils.getTurnForYear(1960):
                if isBestCity(iAztecs, (17, 43), cityPopulation):
                    win(iAztecs, 2)
                else:
                    lose(iAztecs, 2)
        ...
        if iGameTurn == utils.getTurnForYear(1700):
            if isBestCity(iThailand, (119, 36), cityPopulation) or isBestCity(iThailand, (119, 36), cityPopulation):
                win(iThailand, 1)
            else:
                lose(iThailand, 1)
    ...
    # third goal: enter the Industrial Era before anyone enters the Modern Era
        ...
        if iGameTurn == utils.getTurnForYear(1970):
            expire(iSweden, 2)
        ...
        if iGameTurn == utils.getTurnForYear(1745):
        ...
        if iGameTurn == utils.getTurnForYear(1775):
    ...
    # third goal: be the first to complete the tech tree
        ...
        if iGameTurn == utils.getTurnForYear(1990):
            expire(iAmerica, 2)
        ...
        if iGameTurn == utils.getTurnForYear(2000):
            expire(iArgentina, 2)
    ...
    if utils.getHumanID() == iPlayer:
        ...
        if iVictoryType == iCatholicism:
            if gcgame.getSecretaryGeneral(1) == iPlayer:
                data.iPopeTurns += 1
            ...
            PaganReligionName = gc.getCivilizationInfo(gcgetPlayer(iPlayer).getCivilizationType()).getPaganReligionName(0)
            PaganReligionName = localText.getText(CvUtil.convertToStr(PaganReligionName), ())  # wunshare
...
def checkHistoricalVictory(iPlayer):
    ...
    if not data.players[iPlayer].bHistoricalGoldenAge:
        if countAchievedGoals(iPlayer) >= 2:
            ...
            iGoldenAgeTurns = gcgetPlayer(iPlayer).getGoldenAgeLength()
            if not gcgetPlayer(iPlayer).isAnarchy(): iGoldenAgeTurns += 1

            gcgetPlayer(iPlayer).changeGoldenAgeTurns(iGoldenAgeTurns)

            if pPlayer.isHuman():
                utils.addMessage(iPlayer, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_VICTORY_INTERMEDIATE", ()), "", 0, "", utils.ColorTypes(iPurple), -1, -1, True, True)
    ...
    if not isDiscovered(iTech):
        ...
        for iLoopPlayer in dTechGoals.keys():
            ...
            if iTech in lTechs:
                ...
                elif checkTechGoal(iLoopPlayer, lTechs):
...
def onBuildingBuilt(iPlayer, iBuilding):
    if not gcgame.isVictoryValid(7): return False
    ...
    if isWonder(iBuilding) and not isWonderBuilt(iBuilding):
        ...
        for iLoopPlayer in dWonderGoals.keys():
            ...
            if iBuilding in lWonders:
                if iPlayer != iLoopPlayer:
                    lose(iLoopPlayer, iGoal)
                elif bCanWin and checkWonderGoal(iLoopPlayer, lWonders):
                    win(iLoopPlayer, iGoal)
    ...
    if iPlayer == iChina:
        if isPossible(iChina, 0):
            if iBuilding in [iConfucianCathedral, iTaoistCathedral, iBuddhistCathedral]:
                ...
                if iConfucian >= 3 and iTaoist >= 3 and iBuddhist >= 2:
                    win(iChina, 0)
    ...
    # first Roman goal: build 6 Barracks, 5 Aqueducts, 4 Arenas and 3 Forums by 100 AD
        ...
        if isPossible(iRome, 0):
            if iBuilding in [iCastra, iAqueduct, iArena, iForum]:
                iNumCastras = getNumBuildings(iRome, iCastra)
                ...
                if iNumCastras >= 6 and iNumAqueducts >= 5 and iNumArenas >= 4 and iNumForums >= 3:
                    win(iRome, 0)
...
def onTradeMission(iPlayer, iX, iY, iGold):
    ...
    # first Mande goal: conduct a trade mission in your state religion's holy city by 1350 AD
        ...
        if isPossible(iMali, 0):
            ...
            if iStateReligion != -1:
                pHolyCity = gcgame.getHolyCity(iStateReligion)
    ...
    # first Kievan Rus goal: Conduct two trade or diplomatic missions with the most prominent (top scoring) European civilization by 1327 AD
        ...
        if isPossible(iKievanRus, 2):
            if gcmap.plot(iX, iY).getOwner() in lCivGroups[0]:
                lTopCivs = getTopCivsInGroup(0, [iKievanRus])
                if gcmap.plot(iX, iY).getOwner() in lTopCivs:
...
def onDiplomaticMission(iPlayer, iX, iY, bMadePeace):
    ...
    if iPlayer == iKhazars:
        if isPossible(iKhazars, 0):
            plot = gcmap.plot(iX, iY)
            if plot.getRegionID() in lEurope:
                if gcgetPlayer(plot.getOwner()).getStateReligion() == iIslam:
                    win(iKhazars, 0)
    ...
    if iPlayer == iChad:
        if isPossible(iChad, 0):
            ...
            elif gcmap.plot(iX, iY).isPlotCity():
                iOwner = gcmap.plot(iX, iY).isPlotCity().getOwner()
    ...
    for iCiv in lCivGroups[iGroup]:
        if gcgetPlayer(iCiv).isAlive():
            if iCiv not in lExclude:
                iCurrentScore = gcgetPlayer(iCiv).calculateScore(False, False)
...
def onFirstContact(iPlayer, iHasMetPlayer):
    ...
    if isPossible(iMuisca, 0):
        ...
        if utils.getGameTurn() > utils.getTurnForYear(tBirth[iAztecs]) + 2 and utils.getGameTurn() < utils.getTurnForYear(1800):
            iOldWorldCiv = -1
            iNewWorldCiv = -1
    ...
    if not pMaya.isReborn() and isPossible(iMaya, 2):
        ...
        if iPlayer == iMaya or iHasMetPlayer == iMaya:
            if iPlayer == iMaya and iHasMetPlayer in lCivGroups[0]:
                iEuropean = iHasMetPlayer
            elif iHasMetPlayer == iMaya and iPlayer in lCivGroups[0]:
                iEuropean = iPlayer
            else:
                return
...
def onPlayerChangeStateReligion(iPlayer, iStateReligion):
    ...
    if iPlayer == iEthiopia:
        if iStateReligion == iOrthodoxy:
            if gcgame.isReligionFounded(iOrthodoxy):
                if utils.getGameTurn() <= gcgame.getReligionGameTurnFounded(iOrthodoxy) + utils.getTurns(5):
                    data.bEthiopiaConverted = True


def onRevolution(iPlayer):
    if gcgetPlayer(iPlayer).getCivics(iCivicsReligion) == iTolerance:
        if isPossible(iHungary, 1):
            if iPlayer == iHungary:
...
def checkReligiousGoal(iPlayer, iGoal):
    ...
    if iVictoryType == -1:
        ...
        # second Jewish goal: have legendary culture in the Jewish holy city
            ...
            pHolyCity = gcgame.getHolyCity(iJudaism)
        ...
        if iGoal == 0:
            ...
            if checkTechGoal(iPlayer, lProtestantTechs):
                return 1
            elif data.lFirstDiscovered[iCivilLiberties] not in [iPlayer, -1] or data.lFirstDiscovered[iSocialContract] not in [iPlayer, -1] or data.lFirstDiscovered[iLogistics] not in [iPlayer, -1]:
                return 0
        ...
        if iGoal == 0:
            iCount = 0
            pHolyCity = gcgame.getHolyCity(iHinduism)
            for iGreatPerson in lGreatPeople:
        ...
        # second Confucian goal: have five wonders in the Confucian holy city
            ...
            pHolyCity = gcgame.getHolyCity(iConfucianism)
        ...
        # third Taoist goal: have legendary culture in the Tao holy city
            ...
            pHolyCity = gcgame.getHolyCity(iTaoism)
            if pHolyCity.getOwner() == iPlayer and pHolyCity.getCultureLevel() >= 6: return 1
        ...
        if iGoal == 1:
            fReligionPercent = gcgame.calculateReligionPercent(iZoroastrianism)
            if fReligionPercent >= 10.0: return 1

        # third Zoroastrian goal: have legendary culture in the Zoroastrian holy city
        elif iGoal == 2:
            pHolyCity = gcgame.getHolyCity(iZoroastrianism)
            if pHolyCity.getOwner() == iPlayer and pHolyCity.getCultureLevel() >= 6: return 1
        ...
        # second Pagan goal: depends on Pagan religion
            ...
            paganReligion = localText.getText(CvUtil.convertToStr(paganReligion), ())  # wunshare
        ...
        # third Secular goal: make sure the five most advanced civilizations are secular
            ...
            iCount = 0
            lAdvancedPlayers = utils.getSortedList([iLoopPlayer for iLoopPlayer in range(iNumPlayers) if gcgetPlayer(iLoopPlayer).isAlive() and not utils.isAVassal(iLoopPlayer)],
                                                   lambda iLoopPlayer: gcgetTeam(iLoopPlayer).getTotalTechValue(), True)
            for iLoopPlayer in lAdvancedPlayers[:5]:
                if gcgetPlayer(iLoopPlayer).getCivics(iCivicsReligion) == iSecularism:
                    iCount += 1
            if iCount >= 5: return 1
...
def lose(iPlayer, iGoal):
    ...
    if utils.getHumanID() == iPlayer and utils.getGameTurn() > utils.getScenarioStartTurn() and AlertsOpt.isShowUHVFailPopup():
...
def win(iPlayer, iGoal):
    ...
    data.players[iPlayer].lGoalTurns[iGoal] = utils.getGameTurn()
    if utils.getHumanID() == iPlayer and utils.getGameTurn() > utils.getScenarioStartTurn() and AlertsOpt.isShowUHVFailPopup():
        utils.show('您的第 %d 个历史任务完成了！' % (iGoal + 1))
...
def getBestCity(iPlayer, tPlot, function):
    x, y = tPlot
    # if not gcmap.plot(x, y).isCity(): return None

    bestCity = gcmap.plot(x, y).getPlotCity()
...
def playerTechs(iPlayer):
    iValue = 0
    for iTech in range(iNumTechs):
        if gcgetTeam(iPlayer).isHasTech(iTech):
            iValue += gc.getTechInfo(iTech).getResearchCost()
    return iValue


def playerRealPopulation(iPlayer):
    return gcgetPlayer(iPlayer).getRealPopulation()


def playerCultureOutput(iPlayer):
    return gcgetPlayer(iPlayer).getCommerceRate(CommerceTypes.COMMERCE_CULTURE)
...
def playerResearchOutput(iPlayer):
    return gcgetPlayer(iPlayer).getCommerceRate(CommerceTypes.COMMERCE_RESEARCH)


def playerFoodOutput(iPlayer):
    return gcgetPlayer(iPlayer).calculateTotalYield(YieldTypes.YIELD_FOOD)


def playerProductionOutput(iPlayer):
    return gcgetPlayer(iPlayer).calculateTotalYield(YieldTypes.YIELD_PRODUCTION)


def playerCommerceOutput(iPlayer):
    return gcgetPlayer(iPlayer).calculateTotalYield(YieldTypes.YIELD_COMMERCE)
...
def getNumBuildings(iPlayer, iBuilding):
    return gcgetPlayer(iPlayer).countNumBuildings(iBuilding)


def getPopulationPercent(iPlayer, cityFunction=None):
    iTotalPopulation = gcgame.getTotalPopulation()
    if iTotalPopulation == 0: return 0.0

    iPopulation = 0

    if cityFunction is None:
        iPopulation = gcgetTeam(iPlayer).getTotalPopulation()
    else:
        for city in utils.getCityList(iPlayer):
...
def getLandPercent(iPlayer):
    iTotalLand = gcmap.getLandPlots()
    iOurLand = gcgetPlayer(iPlayer).getTotalLand()
...
def checkOwnedCiv(iPlayer, iOwnedPlayer):
    ...
    return (iPlayerCities >= 2 and iPlayerCities > iOwnedCities) or (iPlayerCities >= 1 and not gcgetPlayer(iOwnedPlayer).isAlive()) or (iPlayerCities >= 1 and iOwnedPlayer == iCarthage)
...
def isControlledOrVassalized(iPlayer, lPlots):
    ...
    for city in utils.getAreaCities(lPlots):
        ...
        if iOwner not in lOwners and iOwner < iNumPlayers:
            lOwners.append(iOwner)
    for iLoopPlayer in range(iNumPlayers):
        if gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isVassal(iPlayer):
...
def countResources(iPlayer, iBonus):
    ...
    for iLoopPlayer in range(iNumPlayers):
        if iLoopPlayer != iPlayer:
            pLoopPlayer = gcgetPlayer(iLoopPlayer)
            if pLoopPlayer.isAlive() and gcgetTeam(pLoopPlayer.getTeam()).isVassal(iPlayer):
...
def isStateReligionInArea(iReligion, tTopLeft, tBottomRight):
    ...
    for iPlayer in range(iNumPlayers):
        if gcgetPlayer(iPlayer).getStateReligion() == iReligion:
            for city in utils.getCityList(iPlayer):
...
def isConnectedByRailroad(iPlayer, tStart, lTargets):
    if not gcgetTeam(iPlayer).isHasTech(iRailroad): return False
...
def isConnectedByRoute(iPlayer, tStart, lTargets):
    if not gcgetTeam(iPlayer).isHasTech(iLeverage): return False
...
def countPlayersWithAttitudeAndCriteria(iPlayer, eAttitude, function):
    return len([iOtherPlayer for iOtherPlayer in range(iNumPlayers) if gcgetPlayer(iPlayer).canContact(iOtherPlayer) and gcgetPlayer(iOtherPlayer).AI_getAttitude(iPlayer) >= eAttitude and function(iOtherPlayer)])
...
def countPlayersWithAttitudeInGroup(iPlayer, eAttitude, lOtherPlayers):
    return countPlayersWithAttitudeAndCriteria(iPlayer, eAttitude, lambda x: not gcgetTeam(gcgetPlayer(x).getTeam()).isAVassal())
...
def countCityWonders(iPlayer, tPlot, bIncludeObsolete=False):
    ...
    for iWonder in lWonders:
        ...
        if not bIncludeObsolete and iObsoleteTech != -1 and gcgetTeam(iPlayer).isHasTech(iObsoleteTech): continue
...
def getTotalCulture(lPlayers):
    ...
    for iPlayer in lPlayers:
        iTotalCulture += gcgetPlayer(iPlayer).countTotalCulture()
    return iTotalCulture


def countImprovements(iPlayer, iImprovement):
    if iImprovement <= 0: return 0
    return gcgetPlayer(iPlayer).getImprovementCount(iImprovement)
...
def isAtPeace(iPlayer):
    for iLoopPlayer in range(iNumPlayers):
        if gcgetPlayer(iLoopPlayer).isAlive() and gcgetTeam(iPlayer).isAtWar(iLoopPlayer):
            return False
    return True
...
def isCompleteTechTree(iPlayer):
    if gcgetPlayer(iPlayer).getCurrentEra() < iGlobal: return False
...
def countBestCitiesReligion(iReligion, function, iNumCities):
    ...
    for city in lCities[:iNumCities]:
        if city.isHasReligion(iReligion) and gcgetPlayer(city.getOwner()).getStateReligion() == iReligion:
            iCount += 1
...
def countGoodRelationPlayers(iPlayer, iAttitudeThreshold):
    ...
    for iLoopPlayer in range(iNumPlayers):
        if iPlayer != iLoopPlayer and tPlayer.isHasMet(iLoopPlayer):
            if gcgetPlayer(iLoopPlayer).AI_getAttitude(iPlayer) >= iAttitudeThreshold:
                iCount += 1
    return iCount
...
def calculateShrineIncome(iPlayer, iReligion):
    ...
    iThreshold = 20
    if getNumBuildings(iPlayer, iDomeOfTheRock) > 0 and not gcgetTeam(iPlayer).isHasTech(iCivilLiberties): iThreshold = 40

    return min(iThreshold, gcgame.countReligionLevels(iReligion))
...
def getApostolicVotePercent(iPlayer):
    iTotal = 0
    for iLoopPlayer in range(iNumPlayers):
        iTotal += gcgetPlayer(iLoopPlayer).getVotes(16, 1)

    if iTotal == 0: return 0.0

    return gcgetPlayer(iPlayer).getVotes(16, 1) * 100.0 / iTotal
...
def isTradeConnected(iPlayer):
    for iOtherPlayer in range(iNumPlayers):
        if iPlayer != iOtherPlayer and gcgetPlayer(iPlayer).canContact(iOtherPlayer) and gcgetPlayer(iPlayer).canTradeNetworkWith(iOtherPlayer):
            return True

    return False


def numTradeConnections(iPlayer):
    iNumConnections = 0
...
def getGlobalTreasury():
    ...
    for iPlayer in range(iNumPlayers):
        iTreasury += gcgetPlayer(iPlayer).getGold()
...
def calculateAlliedPercent(iPlayer, function):
    pTeam = gcgetTeam(gcgetPlayer(iPlayer).getTeam())
    ...
    for iLoopPlayer in range(iNumPlayers):
        pLoopPlayer = gcgetPlayer(iLoopPlayer)
        pLoopTeam = gcgetTeam(pLoopPlayer.getTeam())
        ...
        if iLoopPlayer == iPlayer or pLoopTeam.isVassal(gcgetPlayer(iPlayer).getTeam()) or pTeam.isDefensivePact(pLoopPlayer.getTeam()):
            iAlliedValue += iValue
...
def calculateAlliedCommercePercent(iPlayer):
    return calculateAlliedPercent(iPlayer, lambda x: gcgetPlayer(x).calculateTotalCommerce())


def calculateAlliedPowerPercent(iPlayer):
    return calculateAlliedPercent(iPlayer, lambda x: gcgetPlayer(x).getPower())


def countRegionReligion(iReligion, lRegions):
    lCities = [gcmap.plot(x, y).getPlotCity() for (x, y) in utils.getRegionPlots(lRegions) if gcmap.plot(x, y).isCity()]
...
def countVassals(iPlayer, lPlayers=None, iReligion=-1):
    lVassals = [iVassal for iVassal in range(iNumPlayers) if gcgetTeam(iVassal).isVassal(iPlayer) and (not lPlayers or iVassal in lPlayers) and (iReligion < 0 or gcgetPlayer(iVassal).getStateReligion() == iReligion)]
    return len(lVassals)
...
def isMonopoly(iPlayer, iBonus, lPlots, bIncludeVassals=True):
    bOwnSelf = gcgetPlayer(iPlayer).getNumAvailableBonuses(iBonus) > 0
...
def countCultureBuildings(city, bIncludeWonders=True, bIncludeObsolete=False):
    ...
    for iBuilding in range(iNumBuildings):
        ...
        if not bIncludeObsolete and iObsoleteTech != -1 and gcgetTeam(city.getOwner()).isHasTech(iObsoleteTech): continue
        bIsCulture = False
...
def getReligionHappiness(iPlayer):
    iStateReligion = gcgetPlayer(iPlayer).getStateReligion()
    iHappiness = 0
    ...
    bTolerant = gcgetPlayer(iPlayer).getCivics(iCivicsReligion) in [iTolerance, iSecularism]
    for city in utils.getCityList(iPlayer):
        if bTolerant:
...
def getURVHelp(iPlayer, iGoal):
    ...
    if iVictoryType == iJudaism:
        if iGoal == 0:
            ...
            holyCity = gcgame.getHolyCity(iJudaism)
            aHelp.append(getIcon(holyCity.getOwner() == iPlayer) + localText.getText("TXT_KEY_VICTORY_CONTROL_HOLY_CITY", (holyCity.getName(),)) + ' ' + getIcon(holyCity.getCultureLevel() >= 6) + localText.getText(
                "TXT_KEY_VICTORY_LEGENDARY_CULTURE_CITY", (holyCity.getName(),)))
        elif iGoal == 2:
            iFriendlyRelations = countPlayersWithAttitudeAndReligion(iPlayer, AttitudeTypes.ATTITUDE_FRIENDLY, iJudaism)
        ...
        if iGoal == 0:
            ...
            for city in lCultureCities:
                aHelp.append(getIcon(city.isHasReligion(iOrthodoxy) and gcgetPlayer(city.getOwner()).getStateReligion() == iOrthodoxy) + city.getName())
        elif iGoal == 2:
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bCivilLiberties) + localText.getText("TXT_KEY_TECH_CIVIL_LIBERTIES", ()) + ' ' + getIcon(bConstitution) + localText.getText("TXT_KEY_TECH_CONSTITUTION", ()) + ' ' + getIcon(bEconomics) + localText.getText(
                "TXT_KEY_TECH_ECONOMICS", ()))
        elif iGoal == 1:
            iMerchants = countReligionSpecialists(iProtestantism, iSpecialistGreatMerchant)
            iEngineers = countReligionSpecialists(iProtestantism, iSpecialistGreatEngineer)
        ...
        if iGoal == 0:
            ...
            for iGreatPerson in lGreatPeople:
                iCount += pHolyCity.getFreeSpecialistCount(iGreatPerson)
            aHelp.append(getIcon(iCount >= 7) + localText.getText("TXT_KEY_VICTORY_CITY_GREAT_PEOPLE", (gcgame.getHolyCity(iIslam).getName(), iCount, 7)))
        elif iGoal == 2:
            iCount = 0
            for iReligion in range(iNumReligions):
                iCount += getNumBuildings(iPlayer, iShrine + 4 * iReligion)
        ...
        if iGoal == 0:
            ...
            for iGreatPerson in lGreatPeople:
                if pHolyCity.getFreeSpecialistCount(iGreatPerson) > 0:
                    iCount += 1
            aHelp.append(getIcon(iCount >= 5) + localText.getText("TXT_KEY_VICTORY_CITY_DIFFERENT_GREAT_PEOPLE", (gcgame.getHolyCity(iHinduism).getName(), iCount, 5)))
        elif iGoal == 1:
        ...
        if iGoal == 0:
            ...
            holyCity = gcgame.getHolyCity(iConfucianism)
            iCount = countCityWonders(iPlayer, (holyCity.getX(), holyCity.getY()), True)
            aHelp.append(getIcon(holyCity.getOwner() == iPlayer) + localText.getText("TXT_KEY_VICTORY_CONTROL_HOLY_CITY", (holyCity.getName(),)) + ' ' + getIcon(iCount >= 5) + localText.getText("TXT_KEY_VICTORY_HOLY_CITY_WONDERS",
                                                                                                                                                                                                  (holyCity.getName(), iCount, 5)))
        elif iGoal == 2:
            iUnitCombatMelee = gc.getInfoTypeForString("UNITCOMBAT_MELEE")
            iUnitCombatGunpowder = gc.getInfoTypeForString("UNITCOMBAT_GUN")
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bConfucianShrine) + localText.getText("TXT_KEY_BUILDING_CONFUCIAN_SHRINE", ()) + ' ' + getIcon(bTaoistShrine) + localText.getText("TXT_KEY_BUILDING_TAOIST_SHRINE", ()) + ' ' + getIcon(
            ...
            holyCity = gcgame.getHolyCity(iTaoism)
            aHelp.append(getIcon(holyCity.getOwner() == iPlayer) + localText.getText("TXT_KEY_VICTORY_CONTROL_HOLY_CITY", (holyCity.getName(),)) + ' ' + getIcon(holyCity.getCultureLevel() >= 6) + localText.getText(
                "TXT_KEY_VICTORY_LEGENDARY_CULTURE_CITY", (holyCity.getName(),)))
        ...
        if iGoal == 0:
            ...
            holyCity = gcgame.getHolyCity(iZoroastrianism)
            aHelp.append(getIcon(holyCity.getOwner() == iPlayer) + localText.getText("TXT_KEY_VICTORY_CONTROL_HOLY_CITY", (holyCity.getName(),)) + ' ' + getIcon(holyCity.getCultureLevel() >= 6) + localText.getText(
                "TXT_KEY_VICTORY_LEGENDARY_CULTURE_CITY", (holyCity.getName(),)))
...
def getPaganGoalHelp(iPlayer):
    pPlayer = gcgetPlayer(iPlayer)
    paganReligion = gc.getCivilizationInfo(gcgetPlayer(iPlayer).getCivilizationType()).getPaganReligionName(0)
    paganReligion = localText.getText(CvUtil.convertToStr(paganReligion), ())  # wunshare
    ...
    if paganReligion == "Anunnaki":
        ...
        return getIcon(iNumPearls >= 4) + localText.getText("TXT_KEY_VICTORY_AVAILABLE_RESOURCES", (gc.getBonusInfo(iPearls).getText().lower(), iNumPearls, 4)) + ' ' + getIcon(iOceanTiles >= 50) + localText.getText(
            "TXT_KEY_VICTORY_CONTROLLED_OCEAN_TILES", (iOceanTiles, 50))
        ...
        return getIcon(iNumIvory >= 8) + localText.getText("TXT_KEY_VICTORY_AVAILABLE_RESOURCES", (gc.getBonusInfo(iIvory).getText().lower(), iNumIvory, 8)) + ' ' + getIcon(iNumGems >= 6) + localText.getText(
            "TXT_KEY_VICTORY_AVAILABLE_RESOURCES", (gc.getBonusInfo(iGems).getText().lower(), iNumGems, 6))

    elif paganReligion == "Midewiwin":
        iNumCorn = pPlayer.getNumAvailableBonuses(iCorn)
        iNumFarms = countImprovements(iPlayer, iFarm)

        return getIcon(iNumCorn >= 4) + localText.getText("TXT_KEY_VICTORY_AVAILABLE_RESOURCES", (gc.getBonusInfo(iCorn).getText().lower(), iNumCorn, 4)) + ' ' + getIcon(iNumFarms >= 20) + localText.getText("TXT_KEY_VICTORY_NUM_STRING", (
            "TXT_KEY_IMPROVEMENT_FARM", iNumFarms, 20))
        ...
        return getIcon(iNumCrabs >= 3) + localText.getText("TXT_KEY_VICTORY_AVAILABLE_RESOURCES", (gc.getBonusInfo(iCrab).getText().lower(), iNumCrabs, 3)) + ' ' + getIcon(iNumFish >= 3) + localText.getText(
            "TXT_KEY_VICTORY_AVAILABLE_RESOURCES", (gc.getBonusInfo(iFish).getText().lower(), iNumFish, 3)) + ' ' + getIcon(iNumWhales >= 3) + localText.getText("TXT_KEY_VICTORY_AVAILABLE_RESOURCES",
                                                                                                                                                                 (gc.getBonusInfo(iWhales).getText().lower(), iNumWhales, 3))
...
def getUHVHelp(iPlayer, iGoal):
    ...
    if data.players[iPlayer].lGoals[iGoal] == 1:
        ...
        iTurnYear = gcgame.getTurnYear(iWinTurn)
        if iTurnYear < 0:
    ...
    if iPlayer == iEgypt:
        if iGoal == 0:
            ...
            iCulturePerTurn = max(playerCultureOutput(iPlayer), 1)
            iCultureGapTurn = (utils.getTurns(500) - iCulture) / iCulturePerTurn
            gameTurnExpect = max(0, iCultureGapTurn)
            expireTurn = max(utils.getTurnForYear(-850) - utils.getGameTurn(), 0)
            aHelp.append("预计还需 %d 回合 ， 回合剩余：%d" % (gameTurnExpect, expireTurn))
        elif iGoal == 1:
            ...
            aHelp.append(getIcon(bPyramids) + localText.getText("TXT_KEY_BUILDING_PYRAMIDS", ()) + getIcon(bLibrary) + localText.getText("TXT_KEY_BUILDING_GREAT_LIBRARY", ()) + getIcon(bLighthouse) + localText.getText(
                "TXT_KEY_BUILDING_GREAT_LIGHTHOUSE", ()))
            UHVGoalYear = -100
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        elif iGoal == 2:
            iCulture = pEgypt.countTotalCulture()
            ...
            iCulturePerTurn = max(playerCultureOutput(iPlayer), 1)
            iCultureGapTurn = (utils.getTurns(5000) - iCulture) / iCulturePerTurn
            gameTurnExpect = max(0, iCultureGapTurn)
            expireTurn = max(utils.getTurnForYear(170) - utils.getGameTurn(), 0)
            aHelp.append("预计还需 %d 回合 ， 回合剩余：%d" % (gameTurnExpect, expireTurn))
        ...
        if iGoal == 1:
            ...
            iNumGranaries = getNumBuildings(iHarappa, iGranary)
            iNumSmokehouses = getNumBuildings(iHarappa, iSmokehouse)
            aHelp.append(getIcon(iNumReservoirs >= 3) + localText.getText("TXT_KEY_VICTORY_NUM_RESERVOIRS", (iNumReservoirs, 3)) + ' ' + getIcon(iNumGranaries >= 2) + localText.getText("TXT_KEY_VICTORY_NUM_GRANARIES",
                                                                                                                                                                                         (iNumGranaries, 2)) + ' ' + getIcon(
                iNumSmokehouses >= 2) + localText.getText("TXT_KEY_VICTORY_NUM_SMOKEHOUSES", (iNumSmokehouses, 2)))
            UHVGoalYear = -1500
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        elif iGoal == 2:
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bConstruction) + localText.getText("TXT_KEY_TECH_CONSTRUCTION", ()) + ' ' + getIcon(bArithmetics) + localText.getText("TXT_KEY_TECH_ARITHMETICS", ()) + ' ' + getIcon(bWriting) + localText.getText(
                "TXT_KEY_TECH_WRITING", ()))
            ...
            aHelp.append('巴比伦免费科技的UP还剩 %d 次' % gcgetPlayer(iPlayer).getFreeTechsOnDiscovery())
        elif iGoal == 1:
            tcity = (89, 47)
            iscity = gcmap.plot(tcity[0], tcity[1]).isCity()
            txt = getIcon(iscity) + ' 是否在UHV判定点拥有城市'
            pBestCity = getBestCity(iBabylonia, tcity, cityPopulation)
            bBestCity = isBestCity(iBabylonia, tcity, cityPopulation)

            aHelp.append(txt + getIcon(bBestCity) + localText.getText("TXT_KEY_VICTORY_MOST_POPULOUS_CITY", (pBestCity.getName(),)))
            UHVGoalYear = -850
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        elif iGoal == 2:
            ...
            iscity = gcmap.plot(tcity[0], tcity[1]).isCity()
            txt = getIcon(iscity) + ' 是否在UHV判定点拥有城市'
            pBestCity = getBestCity(iBabylonia, tcity, cityCulture)
            bBestCity = isBestCity(iBabylonia, tcity, cityCulture)
            aHelp.append(txt + getIcon(bBestCity) + localText.getText("TXT_KEY_VICTORY_MOST_CULTURED_CITY", (pBestCity.getName(),)))
            UHVGoalYear = -700
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        ...
        if not pNorteChico.isReborn():
            if iGoal == 0:
                ...
                iRequiredCulture = gc.getCultureLevelInfo(3).getSpeedThreshold(gcgame.getGameSpeedType())
                ...
                iCulturePerTurn = max(playerCultureOutput(iPlayer), 1)
                iCultureGapTurn = (iRequiredCulture - iCulture) / iCulturePerTurn
                gameTurnExpect = max(0, iCultureGapTurn)
                expireTurn = max(utils.getTurnForYear(-1800) - utils.getGameTurn(), 0)
                aHelp.append("预计还需 %d 回合 ， 回合剩余：%d" % (gameTurnExpect, expireTurn))
            ...
            if iGoal == 2:
                ...
                aHelp.append(getIcon(iPop >= 5) + localText.getText("TXT_KEY_VICTORY_CAPITAL_POPULATION", (capital.getName(), iPop, 5)) + ' ' + getIcon(iBuildings >= 5) + localText.getText("TXT_KEY_VICTORY_CAPITAL_BUILDINGS",
                                                                                                                                                                                             (capital.getName(), iBuildings + 1, 6)))
                expireTurn = max(utils.getTurnForYear(-1400) - utils.getGameTurn(), 0)
                aHelp.append(" 回合剩余：%d" % (expireTurn))
        else:
            if iGoal == 0:
        ...
        if iGoal == 0:
            ...
            aHelp.append(
                getIcon(bPyramids) + localText.getText("TXT_KEY_BUILDING_PYRAMIDS", ()) + ' ' + getIcon(bEgypt) + localText.getText("TXT_KEY_CIV_EGYPT_SHORT_DESC", ()) + ' ' + getIcon(data.iNubiaEgyptYears >= 1000) + localText.getText(
                    "TXT_KEY_UHV_YEARS", (data.iNubiaEgyptYears, 1000)))
            UHVGoalYear = -656
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        elif iGoal == 1:
            ...
            aHelp.append(getIcon(iNumOrthodoxCathedrals >= 2) + localText.getText("TXT_KEY_VICTORY_ORTHODOX_CATHEDRALS", (iNumOrthodoxCathedrals, 2)) + ' ' + getIcon(iNumPleasedOrBetterChristians >= 5) + localText.getText(
                "TXT_KEY_VICTORY_PLEASED_OR_FRIENDLY_CHRISTIANS", (iNumPleasedOrBetterChristians, 5)))
            UHVGoalYear = 1365
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        elif iGoal == 2:
            ...
            bOrthodoxCathedral = isBuildingInCity((81, 35), iOrthodoxCathedral)
            bJewishSanhedrin = isBuildingInCity((81, 35), iJewishCathedral)
            aHelp.append(getIcon(iBestIslamicCommerceOutput == iNubia) + localText.getText("TXT_KEY_VICTORY_MOST_ISLAMIC_COMMERCE_CIV", (str(gcgetPlayer(iBestIslamicCommerceOutput).getCivilizationShortDescriptionKey()),)) + ' ' + getIcon(
                bPalace) + localText.getText("TXT_KEY_VICTORY_PALACE_IN_SENNAR", ()) + ' ' + getIcon(bIslamicMosque) + localText.getText("TXT_KEY_VICTORY_ISLAMICMOSQUE_IN_SENNAR", ()) + ' ' + getIcon(bOrthodoxCathedral) + localText.getText(
                "TXT_KEY_VICTORY_ORTHODOXCATHEDRAL_IN_SENNAR", ()) + ' ' + getIcon(bJewishSanhedrin) + localText.getText("TXT_KEY_VICTORY_JEWISHSANHEDRIN_IN_SENNAR", ()))
            UHVGoalYear = 1821
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        ...
        if iGoal == 0:
            ...
            aHelp.append(
                getIcon(iConfucianCounter >= 3) + localText.getText("TXT_KEY_VICTORY_NUM_CONFUCIAN_ACADEMIES", (iConfucianCounter, 3)) + ' ' + getIcon(iBuddhistCounter >= 2) + localText.getText("TXT_KEY_VICTORY_NUM_BUDDHIST_CATHEDRALS",
                                                                                                                                                                                                  (iBuddhistCounter, 2)) + ' ' + getIcon(
                    iTaoistCounter >= 3) + localText.getText("TXT_KEY_VICTORY_NUM_TAOIST_PAGODAS", (iTaoistCounter, 3)))
            UHVGoalYear = 1000
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        elif iGoal == 1:
            ...
            aHelp.append(getIcon(bCompass) + localText.getText("TXT_KEY_TECH_COMPASS", ()) + ' ' + getIcon(bPaper) + localText.getText("TXT_KEY_TECH_PAPER", ()) + ' ' + getIcon(bGunpowder) + localText.getText("TXT_KEY_TECH_GUNPOWDER",
                                                                                                                                                                                                                 ()) + ' ' + getIcon(
                bPrintingPress) + localText.getText("TXT_KEY_TECH_PRINTING", ()))
        elif iGoal == 2:
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bMathematics) + localText.getText("TXT_KEY_TECH_MATHEMATICS", ()) + ' ' + getIcon(bLiterature) + localText.getText("TXT_KEY_TECH_LITERATURE", ()) + ' ' + getIcon(bAesthetics) + localText.getText(
                "TXT_KEY_TECH_AESTHETICS", ()))
            ...
            bBabylonia = checkOwnedCiv(iGreece, iBabylonia)
            bPersia = checkOwnedCiv(iGreece, iPersia)
            aHelp.append(getIcon(bEgypt) + localText.getText("TXT_KEY_CIV_EGYPT_SHORT_DESC", ()) + ' ' + getIcon(bPhoenicia) + localText.getText("TXT_KEY_CIV_PHOENICIA_SHORT_DESC", ()) + ' ' + getIcon(bBabylonia) + localText.getText(
                "TXT_KEY_CIV_BABYLONIA_SHORT_DESC", ()) + ' ' + getIcon(bPersia) + localText.getText("TXT_KEY_CIV_PERSIA_SHORT_DESC", ()))
            expireTurn = max(utils.getTurnForYear(-300) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        elif iGoal == 2:
            ...
            aHelp.append(getIcon(bParthenon) + localText.getText("TXT_KEY_BUILDING_PARTHENON", ()) + ' ' + getIcon(bColossus) + localText.getText("TXT_KEY_BUILDING_COLOSSUS", ()) + ' ' + getIcon(bStatueOfZeus) + localText.getText(
                "TXT_KEY_BUILDING_STATUE_OF_ZEUS", ()) + ' ' + getIcon(bArtemis) + localText.getText("TXT_KEY_BUILDING_TEMPLE_OF_ARTEMIS", ()))
            UHVGoalYear = -250
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        ...
        if iGoal == 1:
            ...
            aHelp.append(
                getIcon(bArithmetics) + localText.getText("TXT_KEY_TECH_ARITHMETICS", ()) + ' ' + getIcon(bWriting) + localText.getText("TXT_KEY_TECH_WRITING", ()) + ' ' + getIcon(bCalendar) + localText.getText("TXT_KEY_TECH_CALENDAR", ()))
            UHVGoalYear = -400
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
    elif iPlayer == iCarthage:
        if iGoal == 0:
            ...
            bItaly = isControlled(iCarthage, utils.getPlotList(Areas.tNormalArea[iItaly][0], Areas.tNormalArea[iItaly][1], [(62, 48), (63, 48), (63, 46)]))
            bIberia = isControlled(iCarthage, Areas.getNormalArea(iSpain, False))
        ...
        if iGoal == 1:
            ...
            txt = "目标是否完成：" + getIcon(bControlled)
            aHelp.append(txt)
            txt = localText.getText("TXT_KEY_VICTORY_CELTIA_CONTROL_GALLIA", (iGallia, 3)) + getIcon(iGallia >= 3)
            txt += "   "
            txt += localText.getText("TXT_KEY_VICTORY_CELTIA_CONTROL_GERMANIA", (iGermania, 3)) + getIcon(iGermania >= 3)
            txt += "   "
            txt += localText.getText("TXT_KEY_VICTORY_CELTIA_CONTROL_ITALIA", (iItalia, 1)) + getIcon(iItalia >= 1)
            txt += "   "
            txt += localText.getText("TXT_KEY_VICTORY_IBERIA", (iIberia, 1)) + getIcon(iIberia >= 1)
            txt += "   "
            txt += localText.getText("TXT_KEY_VICTORY_CELTIA_CONTROL_BRITANNIA", (iBritannia, 1)) + getIcon(iBritannia >= 1)
            aHelp.append(txt)
            UHVGoalYear = -1
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        ...
        if iGoal == 2:
            ...
            aHelp.append(getIcon(iBestCiv == iPlayer) + localText.getText("TXT_KEY_VICTORY_MOST_CULTURED_CIVILIZATION", (str(gcgetPlayer(iBestCiv).getCivilizationShortDescriptionKey()),)))

            UHVGoalYear = -200
            if utils.getGameTurn() <= utils.getTurnForYear(-1) and utils.getGameTurn() >= utils.getTurnForYear(-200):
                UHVGoalYear = -1
            elif utils.getGameTurn() > utils.getTurnForYear(-1):
                UHVGoalYear = 60

            CeltiaCultue = playerTotalCulture(iPlayer)
            CeltiaCultuePerTurn = max(playerCultureOutput(iPlayer), 1)
            BestPlayerCulture = playerTotalCulture(iBestCiv)
            BestPlayerCulturePerTurn = max(playerCultureOutput(iBestCiv), 1)

            iCultureGapTurn = (BestPlayerCulture - CeltiaCultue) / (CeltiaCultuePerTurn - BestPlayerCulturePerTurn)
            gameTurnExpect = max(0, iCultureGapTurn)

            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)

            txt = '我们的文化值：%d(%d)，最强对手的文化值：%d(%d)' % (CeltiaCultue, CeltiaCultuePerTurn, BestPlayerCulture, BestPlayerCulturePerTurn)
            aHelp.append(txt)
            aHelp.append("预计还需 %d 回合 ， 离 %d 年还剩 %d 回合" % (gameTurnExpect, UHVGoalYear, expireTurn))
        ...
        if iGoal == 0 or iGoal == 1:
            ...
            aHelp.append(getIcon(bHawaii) + localText.getText("TXT_KEY_VICTORY_HAWAII", ()) + getIcon(bNewZealand) + localText.getText("TXT_KEY_VICTORY_NEW_ZEALAND", ()) + getIcon(bMarquesas) + localText.getText("TXT_KEY_VICTORY_MARQUESAS",
                                                                                                                                                                                                                    ()) + getIcon(
                bEasterIsland) + localText.getText("TXT_KEY_VICTORY_EASTER_ISLAND", ()))
            UHVGoalYear = 800
            if iGoal == 1:
                UHVGoalYear = 1000
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
        ...
        if not pPersia.isReborn():
            ...
            if iGoal == 0:
                ...
                bTransoxania = isControlled(iPersia, utils.getPlotList(tTransoxaniaTL, tTransoxaniaBR))
                bNWIndia = isControlled(iPersia, utils.getPlotList(tNWIndiaTL, tNWIndiaBR, tNWIndiaExceptions))
                aHelp.append(getIcon(bMesopotamia) + localText.getText("TXT_KEY_VICTORY_MESOPOTAMIA", ()) + ' ' + getIcon(bTransoxania) + localText.getText("TXT_KEY_VICTORY_TRANSOXANIA", ()) + ' ' + getIcon(bNWIndia) + localText.getText(
        ...
        if iGoal == 0:
            ...
            iNumForums = getNumBuildings(iRome, iForum)
            aHelp.append(
                getIcon(iNumCastras >= 6) + localText.getText("TXT_KEY_VICTORY_NUM_BARRACKS", (iNumCastras, 6)) + ' ' + getIcon(iNumAqueducts >= 5) + localText.getText("TXT_KEY_VICTORY_NUM_AQUEDUCTS", (iNumAqueducts, 5)) + ' ' + getIcon(
                    iNumArenas >= 4) + localText.getText("TXT_KEY_VICTORY_NUM_ARENAS", (iNumArenas, 4)) + ' ' + getIcon(iNumForums >= 3) + localText.getText("TXT_KEY_VICTORY_NUM_FORUMS", (iNumForums, 3)))
        elif iGoal == 1:
            ...
            aHelp.append(getIcon(iCitiesSpain >= 2) + localText.getText("TXT_KEY_VICTORY_ROME_CONTROL_SPAIN", (iCitiesSpain, 2)) + ' ' + getIcon(iCitiesFrance >= 3) + localText.getText("TXT_KEY_VICTORY_ROME_CONTROL_FRANCE",
                                                                                                                                                                                         (iCitiesFrance, 3)) + ' ' + getIcon(
                iCitiesEngland >= 1) + localText.getText("TXT_KEY_VICTORY_ROME_CONTROL_ENGLAND", (iCitiesEngland, 1)))
            aHelp.append(getIcon(iCitiesCarthage >= 2) + localText.getText("TXT_KEY_VICTORY_ROME_CONTROL_CARTHAGE", (iCitiesCarthage, 2)) + ' ' + getIcon(iCitiesByzantium >= 4) + localText.getText("TXT_KEY_VICTORY_ROME_CONTROL_BYZANTIUM",
                                                                                                                                                                                                     (iCitiesByzantium, 4)) + ' ' + getIcon(
            ...
            aHelp.append(getIcon(bArchitecture) + localText.getText("TXT_KEY_TECH_ARCHITECTURE", ()) + ' ' + getIcon(bPolitics) + localText.getText("TXT_KEY_TECH_POLITICS", ()) + ' ' + getIcon(bScholarship) + localText.getText(
                "TXT_KEY_TECH_SCHOLARSHIP", ()))
    ...
    # Maya goals have no stages
        ...
        if iGoal == 2:
            ...
            bSalsalBuddha = data.getWonderBuilder(iSalsalBuddha) == iPlayer

            aHelp.append(getIcon(bSalsalBuddha) + localText.getText("TXT_KEY_BUILDING_SALSAL_BUDDHA", ()) + ' ' + getIcon(iTemples >= 8) + localText.getText("TXT_KEY_VICTORY_NUM_STRING", ("TXT_KEY_VICTORY_TEMPLES", iTemples, 8)))
        ...
        if iGoal == 1:
            ...
            aHelp.append(getIcon(iLeadingCiv == iXiongnu) + localText.getText("TXT_KEY_VICTORY_LARGEST_CIV", (gcgetPlayer(iLeadingCiv).getCivilizationShortDescription(0),)) + " (" + str(int(iLeaderScore)) + ")")
        if iGoal == 2:
            aHelp.append(
                getIcon(data.iXiongnuCityTernary == 1 or isWon(iXiongnu, 2)) + localText.getText("TXT_KEY_UHV_AREA_29", ()) + ' ' + getIcon(data.iXiongnuCityTernary == 2 or isWon(iXiongnu, 2)) + localText.getText("TXT_KEY_UHV_AREA_110",
                                                                                                                                                                                                                     ()))
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bKivalliq) + localText.getText("TXT_KEY_VICTORY_KIVALLIQ", ()) + ' ' + getIcon(bKalaallitNunaat) + localText.getText("TXT_KEY_VICTORY_KALAALLIT_NUNAAT", ()) + ' ' + getIcon(bQikiqtaaluk) + localText.getText(
                "TXT_KEY_VICTORY_QIKIQTAALUK", ()) + ' ' + getIcon(bNunavik) + localText.getText("TXT_KEY_VICTORY_NUNAVIK", ()))
            UHVGoalYear = 900
            expireTurn = max(utils.getTurnForYear(UHVGoalYear) - utils.getGameTurn(), 0)
            aHelp.append(" 回合剩余：%d" % (expireTurn))
        ...
        if iGoal == 2:
            ...
            aHelp.append(
                getIcon(iBestPlayerOwner == iPlayer) + localText.getText("TXT_KEY_VICTORY_MOST_WATER_OWN_CIV", (gcgetPlayer(iBestPlayerOwner).getCivilizationShortDescription(0),)) + " (" + str(iBestScoreOwner) + ")" + ' ' + getIcon(
                    iBestPlayerWorker == iPlayer) + localText.getText("TXT_KEY_VICTORY_MOST_WATER_WORK_CIV", (gcgetPlayer(iBestPlayerWorker).getCivilizationShortDescription(0),)) + " (" + str(iBestScoreWorker) + ")")
            lNumWater = countControlledTerrain(iPlayer, [iCoast, iOcean], True)
            aHelp.append(" 我们拥有 %d个水格，有%d个水格有市民工作" % (lNumWater[0],lNumWater[1]))
        ...
        if iGoal == 0:
            ...
            aHelp.append(
                getIcon(bMississippiRiver) + localText.getText("TXT_KEY_VICTORY_MISSISSIPPI_RIVER", ()) + ' ' + getIcon(bOhioRiver) + localText.getText("TXT_KEY_VICTORY_OHIO_RIVER", ()) + ' ' + getIcon(bGreatLakes) + localText.getText(
                    "TXT_KEY_VICTORY_GREAT_LAKES", ()))
        elif iGoal == 1:
            bSerpentMound = data.getWonderBuilder(iSerpentMound) == iMississippi
            iNumEffigyMound = getNumBuildings(iPlayer, iEffigyMound)

            aHelp.append(getIcon(bSerpentMound) + localText.getText("TXT_KEY_BUILDING_SERPENT_MOUND", ()) + ' ' + getIcon(iNumEffigyMound >= 7) + localText.getText("TXT_KEY_VICTORY_NUM_STRING",
                                                                                                                                                                    ("TXT_KEY_BUILDING_MISSISSIPPI_EFFIGY_MOUND", iNumEffigyMound, 7)))
            ...
            aHelp.append(getIcon(bPalace) + localText.getText("TXT_KEY_BUILDING_PALACE", ()) + ' ' + getIcon(iNumMerchant >= 2) + localText.getText("TXT_KEY_VICTORY_GREAT_MERCHANTS_IN_CITY",
                                                                                                                                                    (pMississippi.getCapitalCity().getName(), iNumMerchant, 2)))
        ...
        if iGoal == 0:
            bGateOfTheSun = data.getWonderBuilder(iGateOfTheSun) == iTiwanaku
            iGreatProphets = countCitySpecialists(iTiwanaku, (pTiwanaku.getCapitalCity().getX(), pTiwanaku.getCapitalCity().getY()), iSpecialistGreatProphet)
            aHelp.append(getIcon(bGateOfTheSun) + localText.getText("TXT_KEY_BUILDING_GATE_OF_THE_SUN", ()) + ' ' + getIcon(iGreatProphets >= 1) + localText.getText("TXT_KEY_VICTORY_GREAT_PROPHETS_SETTLED",
                                                                                                                                                                     (pTiwanaku.getCapitalCity().getName(), iGreatProphets, 1)))
        ...
        if iGoal == 0:
            ...
            aHelp.append(
                getIcon(bGold) + localText.getText("TXT_KEY_BONUS_GOLD", ()) + ' ' + getIcon(bDye) + localText.getText("TXT_KEY_BONUS_DYE", ()) + ' ' + getIcon(bCotton) + localText.getText("TXT_KEY_BONUS_COTTON", ()) + ' ' + getIcon(
                    bSheep) + localText.getText("TXT_KEY_BONUS_SHEEP", ()) + getIcon(iCulture >= utils.getTurns(500)) + localText.getText("TXT_KEY_VICTORY_TOTAL_CULTURE", (iCulture, utils.getTurns(500))))
            ...
            aHelp.append(getIcon(iNumBarracks >= 3) + localText.getText("TXT_KEY_VICTORY_NUM_BARRACKS", (iNumBarracks, 3)) + ' ' + getIcon(iNumColcas >= 3) + localText.getText("TXT_KEY_VICTORY_NUM_COLCAS", (iNumColcas, 3)) + ' ' + getIcon(
                bRoute) + localText.getText("TXT_KEY_VICTORY_CONNECTED", ()))
        ...
        if iGoal == 0:
            ...
            pBestPopCity = getBestCity(iByzantium, (79, 55), cityPopulation)
            bBestPopCity = isBestCity(iByzantium, (79, 55), cityPopulation)
            pBestCultureCity = getBestCity(iByzantium, (79, 55), cityCulture)
            bBestCultureCity = isBestCity(iByzantium, (79, 55), cityCulture)
            aHelp.append(getIcon(bBestPopCity) + localText.getText("TXT_KEY_VICTORY_MOST_POPULOUS_CITY", (pBestPopCity.getName(),)) + ' ' + getIcon(bBestCultureCity) + localText.getText("TXT_KEY_VICTORY_MOST_CULTURED_CITY",
                                                                                                                                                                                          (pBestCultureCity.getName(),)))
        elif iGoal == 2:
            ...
            iNearEast = getNumCitiesInArea(iByzantium, utils.getPlotList(tNearEastTL, tNearEastBR, tNearEastExceptions))
            aHelp.append(getIcon(iBalkans >= 3) + localText.getText("TXT_KEY_VICTORY_BALKANS", (iBalkans, 3)) + ' ' + getIcon(iNorthAfrica >= 3) + localText.getText("TXT_KEY_VICTORY_NORTH_AFRICA", (iNorthAfrica, 3)) + ' ' + getIcon(
                iNearEast >= 3) + localText.getText("TXT_KEY_VICTORY_NEAR_EAST", (iNearEast, 3)))
        ...
        if iGoal == 0:
            ...
            bIndonesia = isControlledOrVassalized(iJapan, utils.getPlotList(tIndonesiaTL, tIndonesiaBR))
            bPhilippines = isControlledOrVassalized(iJapan, utils.getPlotList(tPhilippinesTL, tPhilippinesBR))
            aHelp.append(getIcon(bKorea) + localText.getText("TXT_KEY_CIV_KOREA_SHORT_DESC", ()) + ' ' + getIcon(bManchuria) + localText.getText("TXT_KEY_VICTORY_MANCHURIA", ()) + ' ' + getIcon(bChina) + localText.getText(
                "TXT_KEY_CIV_CHINA_SHORT_DESC", ()))
            aHelp.append(getIcon(bIndochina) + localText.getText("TXT_KEY_VICTORY_INDOCHINA", ()) + ' ' + getIcon(bIndonesia) + localText.getText("TXT_KEY_CIV_INDONESIA_SHORT_DESC", ()) + ' ' + getIcon(bPhilippines) + localText.getText(
            ...
            aHelp.append(getIcon(iGlobalTechs >= 8) + localText.getText("TXT_KEY_VICTORY_TECHS_FIRST_DISCOVERED", (gc.getEraInfo(iGlobal).getText(), iGlobalTechs, 8)) + ' ' + getIcon(iDigitalTechs >= 8) + localText.getText(
                "TXT_KEY_VICTORY_TECHS_FIRST_DISCOVERED", (gc.getEraInfo(iDigital).getText(), iDigitalTechs, 8)))
        ...
        if iGoal == 0:
            ...
            if pTurks.getNumCities() > 0:
                ...
                iRequiredCulture = gc.getCultureLevelInfo(iCultureLevel).getSpeedThreshold(gcgame.getGameSpeedType())
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(iMostAdvancedCiv == iArabia) + localText.getText("TXT_KEY_VICTORY_MOST_ADVANCED_CIV", (str(gcgetPlayer(iMostAdvancedCiv).getCivilizationShortDescriptionKey()),)))
        elif iGoal == 1:
            bEgypt = isControlledOrVassalized(iArabia, Areas.getCoreArea(iEgypt, False))
            bMaghreb = isControlledOrVassalized(iArabia, utils.getPlotList(tCarthageTL, tCarthageBR))
            bMesopotamia = isControlledOrVassalized(iArabia, Areas.getCoreArea(iBabylonia, False))
            ...
            bSpain = isControlledOrVassalized(iArabia, Areas.getNormalArea(iSpain, False))
            aHelp.append(
                getIcon(bEgypt) + localText.getText("TXT_KEY_CIV_EGYPT_SHORT_DESC", ()) + ' ' + getIcon(bMaghreb) + localText.getText("TXT_KEY_VICTORY_MAGHREB", ()) + ' ' + getIcon(bSpain) + localText.getText("TXT_KEY_CIV_SPAIN_SHORT_DESC",
                                                                                                                                                                                                                 ()))
        ...
        if iGoal == 0:
            ...
            fReligionPercent = gcgame.calculateReligionPercent(iBuddhism)
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bHighest) + localText.getText("TXT_KEY_VICTORY_HIGHEST_POPULATION_CIV", (gcgetPlayer(iHighestCiv).getCivilizationShortDescription(0),)))
        elif iGoal == 1:
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bShwedagon) + localText.getText("TXT_KEY_BUILDING_SHWEDAGON_PAYA", ()) + ' ' + getIcon(iTemples >= 2) + localText.getText("TXT_KEY_VICTORY_NUM_BUDDHIST_TEMPLES", (iTemples, 2)) + ' ' + getIcon(
                iMonasteries >= 2) + localText.getText("TXT_KEY_VICTORY_NUM_BUDDHIST_MONASTERIES", (iMonasteries, 2)))
        if iGoal == 1:
            iProphets = 0
            iScientists = 0
            iArtists = 0
            ...
            aHelp.append(
                getIcon(iProphets >= 1) + localText.getText("TXT_KEY_UNIT_GREAT_PROPHET", ()) + ' ' + getIcon(iScientists >= 1) + localText.getText("TXT_KEY_UNIT_GREAT_SCIENTIST", ()) + ' ' + getIcon(iArtists >= 1) + localText.getText(
                    "TXT_KEY_UNIT_GREAT_ARTIST", ()) + ' ' + getIcon(iSpecialists >= 5) + localText.getText("TXT_KEY_VICTORY_PROPHETS_SCIENTISTS_ARTISTS", (iSpecialists, 5)))
        if iGoal == 2:
            bIndochina = isControlled(iBurma, utils.getPlotList(tIndochinaTL, tIndochinaBR, tIndochinaExceptions))
        ...
        if iGoal == 1:
            ...
            aHelp.append(getIcon(iHappiness >= 10) + localText.getText("TXT_KEY_VICTORY_RELIGION_HAPPINESS", (iHappiness, 10)) + ' ' + getIcon(iLeader == iPlayer) + localText.getText("TXT_KEY_VICTORY_JEWISH_POPULATION", (
                gcgetPlayer(iLeader).getCivilizationShortDescription(0),)) + " (" + str(iPopulation) + ")")
        if iGoal == 2:
        ...
        if iGoal == 0:
            aHelp.append(getIcon(data.iChadDiplomacyMissions >= 1) + localText.getText("TXT_KEY_VICTORY_DIPLOMATIC_MISSIONS", (data.iChadDiplomacyMissions, 1)) + ' ' + getIcon(data.iChadTradeMissions >= 3) + localText.getText(
                "TXT_KEY_VICTORY_TRADE_MISSIONS", (data.iChadTradeMissions, 3)))
        if iGoal == 1:
            aHelp.append(
                getIcon(data.iChadSlaves >= 5) + localText.getText("TXT_KEY_VICTORY_SLAVES_SOLD", (data.iChadSlaves, 5)) + ' ' + getIcon(data.iChadStrategicBonuses >= 5) + localText.getText("TXT_KEY_VICTORY_STRATEGIC_BONUSES_BOUGHT",
                                                                                                                                                                                              (data.iChadStrategicBonuses, 5)))
        if iGoal == 2:
            lAfricaCivs = getCivsWithHoldingsInRegion(lAfrica)
            ...
            bNigeria = isControlled(iChad, utils.getPlotList(tNigeriaTL, tNigeriaBR))
            bLibya = isControlled(iChad, utils.getPlotList(tLibyaTL, tLibyaBR))
            aHelp.append(getIcon(iBestAfricanHoldingArmy == iChad) + localText.getText("TXT_KEY_VICTORY_STRONGEST_ARMY_AFRICA_HOLDINGS", (str(gcgetPlayer(iBestAfricanHoldingArmy).getCivilizationShortDescriptionKey()),)) + ' ' + getIcon(
                bLibya) + localText.getText("TXT_KEY_VICTORY_LIBYA", ()) + ' ' + getIcon(bNigeria) + localText.getText("TXT_KEY_VICTORY_NIGERIA", ()) + ' ' + getIcon(bCameroon) + localText.getText("TXT_KEY_VICTORY_CAMEROON", ()))
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(iMaghreb >= 3) + localText.getText("TXT_KEY_VICTORY_MAGHREB_MOORS", (iMaghreb, 3)) + ' ' + getIcon(iIberia >= 2) + localText.getText("TXT_KEY_VICTORY_IBERIA", (iIberia, 2)) + ' ' + getIcon(
                iWestAfrica >= 2) + localText.getText("TXT_KEY_VICTORY_WEST_AFRICA", (iWestAfrica, 2)))
        elif iGoal == 1:
            bMezquita = data.getWonderBuilder(iMezquita) == iMoors
            iCounter = 0
            iCounter += countCitySpecialists(iMoors, (57, 49), iSpecialistGreatProphet)
        ...
        if iGoal == 1:
            ...
            aHelp.append(getIcon(fReligionPercent >= 30.0) + localText.getText("TXT_KEY_VICTORY_SPREAD_RELIGION_PERCENT", (gc.getReligionInfo(iCatholicism).getTextKey(), str(u"%.2f%%" % fReligionPercent), str(30))) + ' ' + getIcon(
                bNoProtestants) + localText.getText("TXT_KEY_VICTORY_NO_PROTESTANTS", ()))

    elif iPlayer == iFrance:
        if iGoal == 0:
            iCulture = getCityCulture(iFrance, (61, 60))
            ...
            fNorthAmerica = iNorthAmerica * 100.0 / iTotalNorthAmerica
            aHelp.append(
                getIcon(fEurope >= 40.0) + localText.getText("TXT_KEY_VICTORY_EUROPEAN_TERRITORY", (str(u"%.2f%%" % fEurope), str(40))) + ' ' + getIcon(fNorthAmerica >= 40.0) + localText.getText("TXT_KEY_VICTORY_NORTH_AMERICAN_TERRITORY",
                                                                                                                                                                                                   (str(u"%.2f%%" % fNorthAmerica), str(40))))
        elif iGoal == 2:
            ...
            aHelp.append(getIcon(bNotreDame) + localText.getText("TXT_KEY_BUILDING_NOTRE_DAME", ()) + ' ' + getIcon(bVersailles) + localText.getText("TXT_KEY_BUILDING_VERSAILLES", ()) + ' ' + getIcon(bLouvre) + localText.getText(
                "TXT_KEY_BUILDING_LOUVRE", ()))
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(iLeader == iYemen) + localText.getText("TXT_KEY_VICTORY_MOST_CAPITAL_CULTURE_BUILDINGS_CIV", (gcgetPlayer(iLeader).getCivilizationShortDescription(0),)) + " (" + str(iLeaderScore) + ")")
        ...
        if iGoal == 0:
            ...
            bWatPreahPisnulok = data.getWonderBuilder(iWatPreahPisnulok) == iKhmer
            aHelp.append(getIcon(iNumBuddhism >= 4) + localText.getText("TXT_KEY_VICTORY_NUM_BUDDHIST_MONASTERIES", (iNumBuddhism, 4)) + ' ' + getIcon(iNumHinduism >= 4) + localText.getText("TXT_KEY_VICTORY_NUM_HINDU_MONASTERIES",
                                                                                                                                                                                              (iNumHinduism, 4)) + ' ' + getIcon(
                bWatPreahPisnulok) + localText.getText("TXT_KEY_BUILDING_WAT_PREAH_PISNULOK", ()))
        elif iGoal == 1:
        ...
        if iGoal == 1:
            ...
            aHelp.append(
                getIcon(iAveragePlayer == iMuisca) + localText.getText("TXT_KEY_VICTORY_HIGHEST_AVERAGE_POPULATION_NEW_WORLD", (gcgetPlayer(iAveragePlayer).getCivilizationShortDescription(0),)) + " (" + str(int(iAverageScore)) + ")")
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(iNAmerica >= 5) + localText.getText("TXT_KEY_VICTORY_ENGLAND_CONTROL_NORTH_AMERICA", (iNAmerica, 5)) + ' ' + getIcon(iAsia >= 6) + localText.getText("TXT_KEY_VICTORY_ENGLAND_CONTROL_ASIA",
                                                                                                                                                                                      (iAsia, 6)) + ' ' + getIcon(
                iAfrica >= 4) + localText.getText("TXT_KEY_VICTORY_ENGLAND_CONTROL_AFRICA", (iAfrica, 4)))
            ...
            bSunset = ((iNAmerica >= 5) and (iSCAmerica >= 3) and (iOceania >= 6) and (iAfrica) >= 4 and (iAsia >= 6)) or utils.getGameTurn() >= utils.getTurnForYear(1860)
        ...
        if iGoal == 0:
            ...
            aHelp.append(
                getIcon(bSaintPeters) + localText.getText("TXT_KEY_BUILDING_CATHOLIC_SHRINE", ()) + ' ' + getIcon(bAnastasis) + localText.getText("TXT_KEY_BUILDING_ORTHODOX_SHRINE", ()) + ' ' + getIcon(bAllSaints) + localText.getText(
                    "TXT_KEY_BUILDING_PROTESTANT_SHRINE", ()))
        elif iGoal == 1:
        ...
        if iGoal == 0:
            ...
            bLevant = isCultureControlled(iMamluks, utils.getPlotList(tLevantTL, tLevantBR), True)
            bMesopotamia = isControlled(iMamluks, Areas.getCoreArea(iBabylonia, False))
            aHelp.append(getIcon(bNorthAfrica) + localText.getText("TXT_KEY_VICTORY_NORTH_AFRICA_MAM", ()) + ' ' + getIcon(bHejaz) + localText.getText("TXT_KEY_VICTORY_HEJAZ", ()) + ' ' + getIcon(bLevant) + localText.getText(
                "TXT_KEY_VICTORY_LEVANT", ()) + ' ' + getIcon(bMesopotamia) + localText.getText("TXT_KEY_VICTORY_MESOPOTAMIA", ()))
        elif iGoal == 1:
            lLowerNile = [(x, y) for (x, y) in utils.getPlotList(tLowerNileTL, tLowerNileBR) if gcmap.plot(x, y).isRiver()]
            pBestCity = getBestCity(iMamluks, (79, 43), cityPopulation)
            bBestCity = isBestCity(iMamluks, (79, 43), cityPopulation)
            ...
            aHelp.append(getIcon(iMostCultureOutputCiv == iMamluks) + localText.getText("TXT_KEY_VICTORY_HIGHEST_CULTURE_OUTPUT_CIV", (str(gcgetPlayer(iMostCultureOutputCiv).getCivilizationShortDescriptionKey()),)) + ' ' + getIcon(
                iMostResearchOutputCiv == iMamluks) + localText.getText("TXT_KEY_VICTORY_HIGHEST_RESEARCH_OUTPUT_CIV", (str(gcgetPlayer(iMostResearchOutputCiv).getCivilizationShortDescriptionKey()),)))
        ...
        if iGoal == 1:
            ...
            iThreshold = 7500
            if utils.getGameTurn() > utils.getTurnForYear(1500): iThreshold = 15000
        ...
        if iGoal == 0:
            ...
            bGreatZimbabwe = data.getWonderBuilder(iGreatZimbabwe) == iZimbabwe
            aHelp.append(getIcon(iNumCastles >= 4) + localText.getText("TXT_KEY_VICTORY_NUM_CASTLES", (iNumCastles, 4)) + ' ' + getIcon(iNumKraals >= 4) + localText.getText("TXT_KEY_VICTORY_NUM_KRAALS", (iNumKraals, 4)) + ' ' + getIcon(
                bGreatZimbabwe) + localText.getText("TXT_KEY_BUILDING_GREAT_ZIMBABWE", ()))
        elif iGoal == 1:
            lSubSaharanAfrica = utils.getPlotList(tSubSaharaTL, tSubSaharaBR, tSubSaharaExceptions)
            bOwnGold, bNotOthersGold = isMonopoly(iZimbabwe, iGold, lSubSaharanAfrica)
            bOwnSilver, bNotOthersSilver = isMonopoly(iZimbabwe, iSilver, lSubSaharanAfrica)
            bOwnGems, bNotOthersGems = isMonopoly(iZimbabwe, iGems, lSubSaharanAfrica)
            bOwnIvory, bNotOthersIvory = isMonopoly(iZimbabwe, iIvory, lSubSaharanAfrica)
            aHelp.append(getIcon(bOwnGold) + localText.getText("TXT_KEY_VICTORY_MONOPOLY_OWN", (gc.getBonusInfo(iGold).getDescription(),)) + ' ' + getIcon(bNotOthersGold) + localText.getText("TXT_KEY_VICTORY_MONOPOLY_OTHERS_NOT_OWN", (
                gc.getBonusInfo(iGold).getDescription(),)) + ' ' + \
                         getIcon(bOwnSilver) + localText.getText("TXT_KEY_VICTORY_MONOPOLY_OWN", (gc.getBonusInfo(iSilver).getDescription(),)) + ' ' + getIcon(bNotOthersSilver) + localText.getText("TXT_KEY_VICTORY_MONOPOLY_OTHERS_NOT_OWN",
                                                                                                                                                                                                     (gc.getBonusInfo(
                                                                                                                                                                                                         iSilver).getDescription(),)))
            aHelp.append(getIcon(bOwnGems) + localText.getText("TXT_KEY_VICTORY_MONOPOLY_OWN", (gc.getBonusInfo(iGems).getDescription(),)) + ' ' + getIcon(bNotOthersGems) + localText.getText("TXT_KEY_VICTORY_MONOPOLY_OTHERS_NOT_OWN", (
                gc.getBonusInfo(iGems).getDescription(),)) + ' ' + \
                         getIcon(bOwnIvory) + localText.getText("TXT_KEY_VICTORY_MONOPOLY_OWN", (gc.getBonusInfo(iIvory).getDescription(),)) + ' ' + getIcon(bNotOthersIvory) + localText.getText("TXT_KEY_VICTORY_MONOPOLY_OTHERS_NOT_OWN",
                                                                                                                                                                                                  (gc.getBonusInfo(iIvory).getDescription(),)))
        elif iGoal == 2:
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bSanMarcoBasilica) + localText.getText("TXT_KEY_BUILDING_SAN_MARCO_BASILICA", ()) + ' ' + getIcon(bSistineChapel) + localText.getText("TXT_KEY_BUILDING_SISTINE_CHAPEL", ()) + ' ' + getIcon(
                bSantaMariaDelFiore) + localText.getText("TXT_KEY_BUILDING_SANTA_MARIA_DEL_FIORE", ()))
        elif iGoal == 1:
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(iLargestArmy == iNigeria) + localText.getText("TXT_KEY_VICTORY_LARGEST_ARMY_AFRICA", (str(gcgetPlayer(iLargestArmy).getCivilizationShortDescriptionKey()),)))
        elif iGoal == 1:
        ...
        if iGoal == 0:
            iLeadingCiv = getBestPlayer(iPlayer, getLandPercent)
            iLeaderScore = getLandPercent(iLeadingCiv)
            aHelp.append(getIcon(iLeadingCiv == iLithuania) + localText.getText("TXT_KEY_VICTORY_LARGEST_CIV", (gcgetPlayer(iLeadingCiv).getCivilizationShortDescription(0),)) + " (" + str(int(iLeaderScore)) + ")")
        if iGoal == 1:
        ...
        if iGoal == 2:
            ...
            bPrussia = isControlledOrVassalized(iLithuania, Areas.getCoreArea(iPrussia))
            bRussia = isControlledOrVassalized(iLithuania, Areas.getCoreArea(iRussia))
            aHelp.append(getIcon(bAustria) + localText.getText("TXT_KEY_CIV_AUSTRIA_SHORT_DESC", ()) + ' ' + getIcon(bPrussia) + localText.getText("TXT_KEY_CIV_GERMANY_PRUSSIA", ()) + ' ' + getIcon(bRussia) + localText.getText(
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bRedFort) + localText.getText("TXT_KEY_BUILDING_RED_FORT", ()) + ' ' + getIcon(bShalimarGardens) + localText.getText("TXT_KEY_BUILDING_SHALIMAR_GARDENS", ()) + ' ' + getIcon(bTajMahal) + localText.getText(
                "TXT_KEY_BUILDING_TAJ_MAHAL", ()))
        elif iGoal == 2:
        ...
        if not pAztecs.isReborn():
            if iGoal == 0:
                pBestCity = getBestCity(iAztecs, (17, 43), cityPopulation)
                bBestCity = isBestCity(iAztecs, (17, 43), cityPopulation)
            ...
            if iGoal == 0:
                ...
                iStateReligion = pAztecs.getStateReligion()
                if iStateReligion >= 0:
                    iStateReligionCathedral = 4 * iStateReligion + iJewishCathedral
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bCairo) + localText.getText("TXT_KEY_VICTORY_CAIRO", ()) + ' ' + getIcon(bMecca) + localText.getText("TXT_KEY_VICTORY_MECCA", ()) + ' ' + getIcon(bBaghdad) + localText.getText("TXT_KEY_VICTORY_BAGHDAD",
                                                                                                                                                                                                                 ()) + ' ' + getIcon(
                bVienna) + localText.getText("TXT_KEY_VICTORY_VIENNA", ()))
        elif iGoal == 2:
        ...
        if iGoal == 0:
            ...
            pBestCity = getBestCity(iThailand, (119, 36), cityPopulation)
            bBestCity = isBestCity(iThailand, (119, 36), cityPopulation)
            if not bBestCity:
                pBestCity = getBestCity(iThailand, (119, 37), cityPopulation)
                bBestCity = isBestCity(iThailand, (119, 37), cityPopulation)
            aHelp.append(getIcon(bBestCity) + localText.getText("TXT_KEY_VICTORY_MOST_POPULOUS_CITY", (pBestCity.getName(),)))
        elif iGoal == 2:
            bSouthAsia = isAreaOnlyCivs(tSouthAsiaTL, tSouthAsiaBR, lSouthAsianCivs)
            aHelp.append(getIcon(bSouthAsia) + localText.getText("TXT_KEY_VICTORY_NO_SOUTH_ASIAN_COLONIES", ()))
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(iMostFoodCiv == iManchuria) + localText.getText("TXT_KEY_VICTORY_MOST_FOOD_CIV", (str(gcgetPlayer(iMostFoodCiv).getCivilizationShortDescriptionKey()),)) + ' ' + getIcon(
                iMostProductionCiv == iManchuria) + localText.getText("TXT_KEY_VICTORY_MOST_PRODUCTION_CIV", (str(gcgetPlayer(iMostProductionCiv).getCivilizationShortDescriptionKey()),)) + ' ' + getIcon(
                iMostCommerceCiv == iManchuria) + localText.getText("TXT_KEY_VICTORY_MOST_COMMERCE_CIV", (str(gcgetPlayer(iMostCommerceCiv).getCivilizationShortDescriptionKey()),)))
        elif iGoal == 2:
            bAllIndustrialTechs = data.lFirstCompleted[iIndustrial] == iManchuria
            iGlobalTechs = countFirstDiscovered(iGermany, iGlobal)
            aHelp.append(
                getIcon(bAllIndustrialTechs) + localText.getText("TXT_KEY_VICTORY_ALL_ERA_TECHS", (gc.getEraInfo(iIndustrial).getText(),)) + ' ' + getIcon(iGlobalTechs >= 8) + localText.getText("TXT_KEY_VICTORY_TECHS_FIRST_DISCOVERED", (
                    gc.getEraInfo(iGlobal).getText(), iGlobalTechs, 8)))
        ...
        if iGoal == 0:
            ...
            bEngland = checkOwnedCiv(iGermany, iEngland)
            bScandinavia = checkOwnedCiv(iGermany, iVikings)
            aHelp.append(getIcon(bRome) + localText.getText("TXT_KEY_CIV_ITALY_SHORT_DESC", ()) + ' ' + getIcon(bFrance) + localText.getText("TXT_KEY_CIV_FRANCE_SHORT_DESC", ()) + ' ' + getIcon(bScandinavia) + localText.getText(
            ...
            iGlobalTechs = countFirstDiscovered(iGermany, iGlobal)
            aHelp.append(getIcon(iIndustrialTechs >= 8) + localText.getText("TXT_KEY_VICTORY_TECHS_FIRST_DISCOVERED", (gc.getEraInfo(iIndustrial).getText(), iIndustrialTechs, 8)) + ' ' + getIcon(iGlobalTechs >= 8) + localText.getText(
                "TXT_KEY_VICTORY_TECHS_FIRST_DISCOVERED", (gc.getEraInfo(iGlobal).getText(), iGlobalTechs, 8)))
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(bStatueOfLiberty) + localText.getText("TXT_KEY_BUILDING_STATUE_OF_LIBERTY", ()) + ' ' + getIcon(bBrooklynBridge) + localText.getText("TXT_KEY_BUILDING_BROOKLYN_BRIDGE", ()) + ' ' + getIcon(
                bEmpireState) + localText.getText("TXT_KEY_BUILDING_EMPIRE_STATE_BUILDING", ()))
            aHelp.append(
                getIcon(bGoldenGateBridge) + localText.getText("TXT_KEY_BUILDING_GOLDEN_GATE_BRIDGE", ()) + ' ' + getIcon(bPentagon) + localText.getText("TXT_KEY_BUILDING_PENTAGON", ()) + ' ' + getIcon(bUnitedNations) + localText.getText(
                    "TXT_KEY_BUILDING_UNITED_NATIONS", ()))
        elif iGoal == 2:
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(iSlavePlantations >= 8) + localText.getText("TXT_KEY_VICTORY_NUM_IMPROVEMENTS", (gc.getImprovementInfo(iSlavePlantation).getText(), iSlavePlantations, 8)) + ' ' + getIcon(iPastures >= 4) + localText.getText(
                "TXT_KEY_VICTORY_NUM_IMPROVEMENTS", (gc.getImprovementInfo(iPasture).getText(), iPastures, 4)))
        elif iGoal == 1:
            ...
            aHelp.append(getIcon(bWembley) + localText.getText("TXT_KEY_BUILDING_WEMBLEY", ()) + ' ' + getIcon(bCristoRedentor) + localText.getText("TXT_KEY_BUILDING_CRISTO_REDENTOR", ()) + ' ' + getIcon(bItaipuDam) + localText.getText(
                "TXT_KEY_BUILDING_ITAIPU_DAM", ()))
        elif iGoal == 2:
            ...
            aHelp.append(getIcon(iForestPreserves >= 20) + localText.getText("TXT_KEY_VICTORY_NUM_IMPROVEMENTS", (gc.getImprovementInfo(iForestPreserve).getText(), iForestPreserves, 20)) + ' ' + getIcon(bNationalPark) + localText.getText(
        ...
        if iGoal == 0:
            ...
            aHelp.append(getIcon(iAustraliaCities >= 7) + localText.getText("TXT_KEY_VICTORY_AUSTRALIA_CONTROL_AUSTRALIA", (iAustraliaCities, 7)) + ' ' + getIcon(iNewZealandCities >= 2) + localText.getText(
                "TXT_KEY_VICTORY_AUSTRALIA_CONTROL_NEW_ZEALAND", (iNewZealandCities, 2)) + ' ' + getIcon(iGuineaCities >= 1) + localText.getText("TXT_KEY_VICTORY_AUSTRALIA_CONTROL_GUINEA", (iGuineaCities, 1)) + ' ' + getIcon(
                iPacificCities >= 3) + localText.getText("TXT_KEY_VICTORY_AUSTRALIA_CONTROL_PACIFIC", (iPacificCities, 3)))
        elif iGoal == 1:
        ...
        if iGoal == 0:
            ...
            aHelp.append(
                getIcon(pBestCity.getOwner() == iPlayer) + localText.getText("TXT_KEY_VICTORY_MOST_RESEARCH_CITY", (pBestCity.getName(),)) + ' ' + getIcon(iResearchTurns >= 10) + localText.getText("TXT_KEY_VICTORY_MOST_RESEARCH_TURNS",
                                                                                                                                                                                                     (iResearchTurns, 10)))
        elif iGoal == 2:
