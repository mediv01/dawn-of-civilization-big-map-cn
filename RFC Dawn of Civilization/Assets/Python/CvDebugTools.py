...
class CvDebugTools:
    ...
    def setDebugMode(self, bVal):
        ...
        GlobalCyInterface.addImmediateMessage("CvDebugTools.setDebugMode set to %s" % self.bDebugMode, "")
    ...
    def applyEffectViewer(self, playerID, userData, popupReturn):
        px, py = userData
        self.pEffectPlot = GlobalCyMap.plot(px, py)
    ...
    def initUnitPicker(self, argsList):
        ...
        if (iOwner == PlayerTypes.NO_PLAYER):
            iOwner = gcgame.getActivePlayer()
        popup.createPythonPullDown("Choose a Player")
        # popup.addPullDownString(gcgetPlayer(iOwner).getName(), iOwner) #Rhye
        popup.addPullDownString(gcgetPlayer(iOwner).getCivilizationShortDescription(0), iOwner)  # Rhye
        for i in range(gc.getMAX_PLAYERS()):
            if (gcgetPlayer(i).isEverAlive()):
                if (i != iOwner):
                    # popup.addPullDownString(gcgetPlayer(i).getName(), i) #Rhye
                    popup.addPullDownString(gcgetPlayer(i).getCivilizationShortDescription(0), i)  # Rhye
    ...
    def resetUnitMovement(self):
        ...
        if g_bDebugMode == 0:
            return
        for i in range(gc.getMAX_PLAYERS()):
            (unit, iter) = gcgetPlayer(i).firstUnit(False)
            while (unit):
                unit.setMoves(0)
                (unit, iter) = gcgetPlayer(i).nextUnit(iter, False)
    ...
    def wonderMovie(self):
        ...
        for i in range(gc.getNumProjectInfos()):
            ...
            if (len(szArtDef) > 0):
                szMovieFile = GlobalCyArtFileMgr.getMovieArtInfo(szArtDef).getPath()
            if (szMovieFile != None and len(szMovieFile) > 0):
