...
class GameData:
    ...
    def setup(self):
        ...
        self.iSeed = gcgame.getSorenRandNum(100, "Random delay")
        ...
        self.iObserveModeBenchmark_Array = [1, 1]
    ...
    def isPlayerEnabled(self, iPlayer):
        if gcgame.getActivePlayer() == iPlayer: return True
    ...
    def getSecedingCities(self, iPlayer):
        ...
        return [city for city in [gcgetPlayer(iPlayer).getCity(i) for i in self.dSecedingCities[iPlayer]] if (city.getX(), city.getY()) != (-1, -1)]
    ...
    def setStabilityLevel(self, iPlayer, iValue):
        if gcgame.getActivePlayer() == iPlayer:  # if utils.getHumanID() == iPlayer
            iOldStabilityLevel = self.getStabilityLevel(iPlayer)
            if iValue == iStabilityCollapsing:
                szTempBuffer = GlobalCyTranslator.getText("TXT_KEY_STABILITY_COLLAPSING", ())
            elif iValue == iStabilityUnstable:
                szTempBuffer = GlobalCyTranslator.getText("TXT_KEY_STABILITY_UNSTABLE", ())
            elif iValue == iStabilityShaky:
                szTempBuffer = GlobalCyTranslator.getText("TXT_KEY_STABILITY_SHAKY", ())
            elif iValue == iStabilityStable:
                szTempBuffer = GlobalCyTranslator.getText("TXT_KEY_STABILITY_STABLE", ())
            elif iValue == iStabilitySolid:
                szTempBuffer = GlobalCyTranslator.getText("TXT_KEY_STABILITY_SOLID", ())
            else:
                szTempBuffer = GlobalCyTranslator.getText("TXT_KEY_STABILITY_SOLID", ())

            if iValue > iOldStabilityLevel:
                GlobalCyInterface.addMessage(gcgame.getActivePlayer(), True, iDuration, (GlobalCyTranslator.getText("TXT_KEY_STABILITY_IMPROVING", ()) + " " + szTempBuffer), "", 0, "", ColorTypes(iWhite), -1, -1, True, True)
            elif iValue < iOldStabilityLevel:
                GlobalCyInterface.addMessage(gcgame.getActivePlayer(), True, iDuration, (GlobalCyTranslator.getText("TXT_KEY_STABILITY_WORSENING", ()) + " " + szTempBuffer), "", 0, "", ColorTypes(iWhite), -1, -1, True, True)
