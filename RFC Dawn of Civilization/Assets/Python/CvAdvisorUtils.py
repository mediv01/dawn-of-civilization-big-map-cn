...
def featPopup(iPlayer):
    if (not gcgetPlayer(iPlayer).isOption(PlayerOptionTypes.PLAYEROPTION_ADVISOR_POPUPS)):
        return False
    if (not gcgetPlayer(iPlayer).isHuman()):
        return False
    if (gcgame.isNetworkMultiPlayer()):
        return False
    if (gcgame.getElapsedGameTurns() == 0):
        return False
    return True


def populationFeat(iPlayer, eFeat, szText):
    if (not gcgetPlayer(iPlayer).isFeatAccomplished(eFeat)):

        gcgetPlayer(iPlayer).setFeatAccomplished(eFeat, True)

        if (featPopup(iPlayer)):
            popupInfo = CyPopupInfo()
            popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
            popupInfo.setData1(eFeat)
            popupInfo.setText(localText.getText(szText, (gcgetPlayer(iPlayer).getCivilizationDescriptionKey(),)))
...
def unitBuiltFeats(pCity, pUnit):
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_ARCHER)):

        if (pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_ARCHER")):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_ARCHER, True)

            if (featPopup(pCity.getOwner()) and (gcgame.getStartYear() == gc.getDefineINT("START_YEAR"))):
                popupInfo = CyPopupInfo()
                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
    ...
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_MOUNTED)):
        ...
        if (pUnit.getUnitCombatType() in [gc.getInfoTypeForString("UNITCOMBAT_LIGHT_CAVALRY"), gc.getInfoTypeForString("UNITCOMBAT_HEAVY_CAVALRY")]):
            ...
            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_MOUNTED, True)
    ...
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_MELEE)):

        if (pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_MELEE")):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_MELEE, True)
    ...
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_SIEGE)):

        if (pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_SIEGE")):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_SIEGE, True)
    ...
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_GUN)):

        if (pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_GUN")):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_GUN, True)
    ...
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_ARMOR)):

        if (pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_ARMOR")):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_ARMOR, True)
    ...
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_HELICOPTER)):

        if (pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_HELICOPTER")):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_HELICOPTER, True)
    ...
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_NAVAL)):

        if (pUnit.getUnitCombatType() == gc.getInfoTypeForString("UNITCOMBAT_NAVAL")):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNITCOMBAT_NAVAL, True)
    ...
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNIT_PRIVATEER)):

        if (pUnit.getUnitType() == gc.getInfoTypeForString("UNIT_PRIVATEER")):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNIT_PRIVATEER, True)
    ...
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_UNIT_SPY)):

        if (pUnit.getUnitType() == gc.getInfoTypeForString("UNIT_SPY")):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_UNIT_SPY, True)
...
def buildingBuiltFeats(pCity, iBuildingType):
    if (not gcgetPlayer(pCity.getOwner()).isFeatAccomplished(FeatTypes.FEAT_NATIONAL_WONDER)):

        if (isNationalWonderClass(gc.getBuildingInfo(iBuildingType).getBuildingClassType())):

            gcgetPlayer(pCity.getOwner()).setFeatAccomplished(FeatTypes.FEAT_NATIONAL_WONDER, True)
...
def endTurnFeats(iPlayer):
    lRealPopulation = gcgetPlayer(iPlayer).getRealPopulation()
    ...
    if (not gcgetPlayer(iPlayer).isFeatAccomplished(FeatTypes.FEAT_TRADE_ROUTE)):

        apCityList = PyPlayer(iPlayer).getCityList()
        for pCity in apCityList:
            if (not pCity.isCapital()):
                if (pCity.isConnectedToCapital(iPlayer)):
                    gcgetPlayer(iPlayer).setFeatAccomplished(FeatTypes.FEAT_TRADE_ROUTE, True)

                    if (featPopup(iPlayer) and (gcgame.getStartYear() == gc.getDefineINT("START_YEAR"))):
                        popupInfo = CyPopupInfo()
                        popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
    ...
    pCapitalCity = gcgetPlayer(iPlayer).getCapitalCity()

    if (not pCapitalCity.isNone()):

        if (not gcgetPlayer(iPlayer).isFeatAccomplished(FeatTypes.FEAT_COPPER_CONNECTED)):

            iBonus = gc.getInfoTypeForString("BONUS_COPPER")
            if (iBonus != BonusTypes.NO_BONUS):
                if (pCapitalCity.hasBonus(iBonus)):
                    gcgetPlayer(iPlayer).setFeatAccomplished(FeatTypes.FEAT_COPPER_CONNECTED, True)
        ...
        if (not gcgetPlayer(iPlayer).isFeatAccomplished(FeatTypes.FEAT_HORSE_CONNECTED)):

            iBonus = gc.getInfoTypeForString("BONUS_HORSE")
            if (iBonus != BonusTypes.NO_BONUS):
                if (pCapitalCity.hasBonus(iBonus)):
                    gcgetPlayer(iPlayer).setFeatAccomplished(FeatTypes.FEAT_HORSE_CONNECTED, True)
        ...
        if (not gcgetPlayer(iPlayer).isFeatAccomplished(FeatTypes.FEAT_IRON_CONNECTED)):

            iBonus = gc.getInfoTypeForString("BONUS_IRON")
            if (iBonus != BonusTypes.NO_BONUS):
                if (pCapitalCity.hasBonus(iBonus)):
                    gcgetPlayer(iPlayer).setFeatAccomplished(FeatTypes.FEAT_IRON_CONNECTED, True)
        ...
        if (not gcgetPlayer(iPlayer).isFeatAccomplished(FeatTypes.FEAT_LUXURY_CONNECTED)):

            for iI in range(gc.getNumBonusInfos()):
                if (gc.getBonusInfo(iI).getHappiness() > 0):
                    if (pCapitalCity.hasBonus(iI)):
                        gcgetPlayer(iPlayer).setFeatAccomplished(FeatTypes.FEAT_LUXURY_CONNECTED, True)
        ...
        if (not gcgetPlayer(iPlayer).isFeatAccomplished(FeatTypes.FEAT_FOOD_CONNECTED)):

            for iI in range(gc.getNumBonusInfos()):
                if (gc.getBonusInfo(iI).getHealth() > 0):
                    if (pCapitalCity.hasBonus(iI)):
                        gcgetPlayer(iPlayer).setFeatAccomplished(FeatTypes.FEAT_FOOD_CONNECTED, True)
        ...
        if (not gcgetPlayer(iPlayer).isFeatAccomplished(FeatTypes.FEAT_CORPORATION_ENABLED)):

            for iI in range(gc.getNumBuildingInfos()):
                eCorporation = gc.getBuildingInfo(iI).getFoundsCorporation()
                if eCorporation != -1 and not gcgame.isCorporationFounded(eCorporation):
                    bValid = True
                    eTeam = gcgetPlayer(iPlayer).getTeam()
                    if not gcgetTeam(eTeam).isHasTech(gc.getBuildingInfo(iI).getPrereqAndTech()):
                        bValid = False
                    if bValid:
                        for iPrereq in range(gc.getDefineINT("NUM_BUILDING_AND_TECH_PREREQS")):
                            if not gcgetTeam(eTeam).isHasTech(gc.getBuildingInfo(iI).getPrereqAndTechs(iPrereq)):
                                bValid = False
                                break
                    if bValid:
                        gcgetPlayer(iPlayer).setFeatAccomplished(FeatTypes.FEAT_CORPORATION_ENABLED, True)

                        szBonusList = u""
                        bFirst = True
                        for iPrereq in range(NUM_CORPORATION_PREREQ_BONUSES):
...
def cityAdvise(pCity, iPlayer):
    ...
    if (gcgetPlayer(iPlayer).isOption(PlayerOptionTypes.PLAYEROPTION_ADVISOR_POPUPS) and gcgetPlayer(iPlayer).isHuman() and not gcgame.isNetworkMultiPlayer()):

        if (gcgame.getGameTurn() % 40 == pCity.getGameTurnFounded() % 40):
            if (not pCity.getID() in g_listNoLiberateCities):
                ...
                if (eLiberationPlayer != -1):
                    ...
                    popupInfo.setText(localText.getText("TXT_KEY_POPUP_LIBERATION_DEMAND", (pCity.getNameKey(), gcgetPlayer(eLiberationPlayer).getCivilizationDescriptionKey(), gcgetPlayer(eLiberationPlayer).getNameKey())))
                ...
                elif (gcgetPlayer(iPlayer).canSplitEmpire() and gcgetPlayer(iPlayer).canSplitArea(pCity.area().getID()) and pCity.AI_cityValue() < 0):
                    popupInfo = CyPopupInfo()
                    popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
                    popupInfo.setData1(pCity.getID())
        ...
        if (pCity.isProduction()):
            ...
            if (not pCity.isProductionUnit() and (pCity.getOrderQueueLength() <= 1)):
                ...
                if (gcgame.getGameTurn() + 1) % 40 == pCity.getGameTurnFounded() % 40:

                    if ((gcgame.getElapsedGameTurns() < 200) and (pCity.getPopulation() > 2) and (gcgetPlayer(iPlayer).AI_totalAreaUnitAIs(pCity.area(), UnitAITypes.UNITAI_SETTLE) == 0) and not gcgetPlayer(
                            iPlayer).AI_isFinancialTrouble() and (pCity.area().getBestFoundValue(iPlayer) > 0)):
                        ...
                        for iI in range(gc.getNumUnitClassInfos()):
                            ...
                            if (not isLimitedUnitClass(iI)):
                                ...
                                eLoopUnit = gc.getCivilizationInfo(gcgetPlayer(iPlayer).getCivilizationType()).getCivilizationUnits(iI)
                                    ...
                                    if (gc.getUnitInfo(eLoopUnit).getDomainType() == DomainTypes.DOMAIN_LAND):
                                        ...
                                        if pCity.canTrain(eLoopUnit, False, False):
                                            ...
                                            if (pCity.getFirstUnitOrder(eLoopUnit) == -1):
                                                ...
                                                iValue = gcgetPlayer(iPlayer).AI_unitValue(eLoopUnit, UnitAITypes.UNITAI_SETTLE, pCity.area())
                ...
                if (gcgame.getGameTurn() + 5) % 40 == pCity.getGameTurnFounded() % 40:
                                    ...
                                    if (gc.getUnitInfo(eLoopUnit).getDomainType() == DomainTypes.DOMAIN_LAND):
                                        ...
                                        if pCity.canTrain(eLoopUnit, False, False):
                                            ...
                                            if (pCity.getFirstUnitOrder(eLoopUnit) == -1):
                                                ...
                                                iValue = gcgetPlayer(iPlayer).AI_unitValue(eLoopUnit, UnitAITypes.UNITAI_WORKER, pCity.area())
                ...
                if (gcgame.getGameTurn() + 9) % 40 == pCity.getGameTurnFounded() % 40:
                                ...
                                if (eLoopUnit != UnitTypes.NO_UNIT):
                                    ...
                                    if (gc.getUnitInfo(eLoopUnit).getDomainType() == DomainTypes.DOMAIN_LAND):
                                        ...
                                        if pCity.canTrain(eLoopUnit, False, False):
                                            ...
                                            iValue = (gcgetPlayer(iPlayer).AI_unitValue(eLoopUnit, UnitAITypes.UNITAI_CITY_DEFENSE, pCity.area()) * 2)
                                            iValue += gcgetPlayer(iPlayer).AI_unitValue(eLoopUnit, UnitAITypes.UNITAI_ATTACK, pCity.area())
                ...
                if (gcgame.getGameTurn() + 12) % 40 == pCity.getGameTurnFounded() % 40:

                    if ((gcgetPlayer(iPlayer).AI_totalAreaUnitAIs(pCity.area(), UnitAITypes.UNITAI_MISSIONARY) == 0) and (gcgetTeam(gcgetPlayer(iPlayer).getTeam()).getAtWarCount(True) == 0)):

                        eStateReligion = gcgetPlayer(iPlayer).getStateReligion()

                        if (eStateReligion != ReligionTypes.NO_RELIGION):

                            if (gcgetPlayer(iPlayer).getHasReligionCount(eStateReligion) < (gcgetPlayer(iPlayer).getNumCities() / 2)):
                                        ...
                                        if (gc.getUnitInfo(eLoopUnit).getDomainType() == DomainTypes.DOMAIN_LAND):
                                            ...
                                            if (gc.getUnitInfo(eLoopUnit).getReligionSpreads(eStateReligion)):
                                                ...
                                                if pCity.canTrain(eLoopUnit, False, False):
                                                    ...
                                                    iValue = gcgetPlayer(iPlayer).AI_unitValue(eLoopUnit, UnitAITypes.UNITAI_MISSIONARY, pCity.area())
            ...
            if (not pCity.isProductionBuilding() and (pCity.getOrderQueueLength() <= 1)):
                ...
                if (pCity.healthRate(False, 0) < 0):
                    ...
                    if (gcgame.getGameTurn() + 2) % 40 == pCity.getGameTurnFounded() % 40:
                        ...
                        for iI in range(gc.getNumBuildingClassInfos()):
                            ...
                            if (not isLimitedWonderClass(iI)):
                                ...
                                eLoopBuilding = gc.getCivilizationInfo(gcgetPlayer(iPlayer).getCivilizationType()).getCivilizationBuildings(iI)
                ...
                if (pCity.angryPopulation(0) > 0):
                    ...
                    if (gcgame.getGameTurn() + 3) % 40 == pCity.getGameTurnFounded() % 40:
                ...
                if ((gcgame.getGameTurn < 100) and (gcgetTeam(gcgetPlayer(iPlayer).getTeam()).getHasMetCivCount(True) > 0) and (pCity.getBuildingDefense() == 0)):

                    if (gcgame.getGameTurn() + 4) % 40 == pCity.getGameTurnFounded() % 40:
                ...
                if (pCity.getMaintenance() >= 8):
                    ...
                    if (gcgame.getGameTurn() + 6) % 40 == pCity.getGameTurnFounded() % 40:
                ...
                if (pCity.getCommerceRate(CommerceTypes.COMMERCE_CULTURE) == 0 and not pCity.isOccupation()):
                    ...
                    if (gcgame.getGameTurn() + 7) % 40 == pCity.getGameTurnFounded() % 40:
                ...
                if (pCity.getBaseCommerceRate(CommerceTypes.COMMERCE_GOLD) > 10):
                    ...
                    if (gcgame.getGameTurn() + 8) % 40 == pCity.getGameTurnFounded() % 40:
                ...
                if (pCity.getBaseCommerceRate(CommerceTypes.COMMERCE_RESEARCH) > 10):
                    ...
                    if (gcgame.getGameTurn() + 10) % 40 == pCity.getGameTurnFounded() % 40:
                ...
                if (pCity.countNumWaterPlots() > 10):
                    ...
                    if (gcgame.getGameTurn() + 11) % 40 == pCity.getGameTurnFounded() % 40:
