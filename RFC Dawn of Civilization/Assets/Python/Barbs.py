...
tMinorCities = (
    ...
    (1158, (68, 59), iIndependent, 'München', 3, iCrossbowman, 2),  # München
    ...
    (1200, (86, 54), iIndependent2, 'Trapezon', 4, iCrossbowman, 3), # Trapezon
...
iHandicapOld = (gcgame.getHandicapType() - 1)
...
class Barbs:
    ...
    def checkTurn(self, iGameTurn):
        ...
        # handicap level modifier
        iHandicap = gc.getHandicapInfo(gcgame.getHandicapType()).getBarbarianSpawnModifier()
        ...
        if utils.isYearIn(-210, 400):
            ...
            if pKhitan.isAlive():  # balanced fixed
                ...
                if utils.getGameTurn() % 3 == 0 and gcgame.getSorenRandNum(3, 'iIronpagoda') > 2:
        ...
        if utils.isYearIn(-1000, 1200):
            iUnit = iArcher
            iStrength = iHandicap
            if iGameTurn >= utils.getTurnForYear(-500): iUnit = iAxeman
            if iGameTurn >= utils.getTurnForYear(0): iStrength += 1
            if iGameTurn >= utils.getTurnForYear(200): iUnit = iSwordsman
        ...
        if not gcgetPlayer(iMoors).isAlive():
        ...
        # 	gcmap.plot(19, 41).setImprovementType(iHut)
    ...
    def foundMinorCities(self, iGameTurn):
        for i in range(len(tMinorCities)):
            iYear, tPlot, iPlayer, sName, iPopulation, iUnitType, iNumUnits = tMinorCities[i] # Add BuildingList
            if iGameTurn < utils.getTurnForYear(iYear): return
            if iGameTurn > utils.getTurnForYear(iYear) + 10: continue
            ...
            if sName == 'Hamburg': lReligions = [iCatholicism]  
            if sName == 'Trapezon': lReligions = [iOrthodoxy]            	          	
    ...
    def canFoundCity(self, sName):
        if sName == 'Kanchipuram' and utils.getHumanID() == iTamils:
            return False
        elif sName == 'Tanjapuri' and gcgetPlayer(iTamils).isAlive():
            return False
        elif sName == 'Ji' and utils.getHumanID() == iChina:
            return False
        #elif sName == 'Hamburg' and (utils.getHumanID() == iHolyRome or data.iSeed % 4 == 0):
            #return False
        #elif sName == 'Lübeck' and (utils.getHumanID() == iHolyRome or data.iSeed % 4 != 0):
            #return False
        elif sName == 'Rasa' and gcgetPlayer(iTibet).isAlive():
            return False
    ...
    def foundCity(self, iPlayer, tPlot, sName, iPopulation, iUnitType=-1, iNumUnits=-1, lReligions=[]):
        ...
        if (len(sName) > 0 and PYTHON_SHOW_MINOR_CITY_TEXT == 1):
            ...
            if (PYTHON_LOG_ON_MAIN_MINOR == 1):
                utils.logwithid(iIndependent, tem_text)
        ...
        if plot.isCity():
            ...
            if sName in ['Tucume']:
                ...
                plot.changeCulture(iPlayer, 20 * (gcgame.getCurrentEra() + 1), True)
                city.changeCulture(iPlayer, 20 * (gcgame.getCurrentEra() + 1), True)
    ...
    def clearUnits(self, iPlayer, tPlot):  # Unused
        ...
        capital = gcgetPlayer(utils.getHumanID()).getCapitalCity()
    ...
    def killNeighbours(self, tCoords):  # Unused
        ...
        for (x, y) in utils.surroundingPlots(tCoords):
            killPlot = GlobalCyMap.getPlot(x, y)
    ...
    def checkSpawn(self, iPlayer, iUnitType, iNumUnits, tTL, tBR, spawnFunction, iTurn, iPeriod, iRest, lAdj=[]):
        ...
        if iTurn % utils.getTurns(iPeriod) == iRest - 3:
            if (len(sAdj) > 0 and PYTHON_SHOW_BARBARIAN_TEXT == 1):
                # a= localText.getText(sAdj, ())
                name = lAdj[0]
                civname = name
                # a=GlobalCyTranslator.getText(str(name ), (name ,))
                ...
                civname_show = " "
                if civname[0:4] == 'TXT_':
                    civname_show = GlobalCyTranslator.getText(civname, ()) + " "
        ...
        if iTurn % utils.getTurns(iPeriod) == iRest:
            if (len(sAdj) > 0 and PYTHON_SHOW_BARBARIAN_TEXT == 1):
                ...
                tem_text = utils.getText('TXT_KEY_PYTHON_LOGGER_CHINESE_BEFORE_AD') + str(gcgame.getGameTurnYear()) + utils.getText('TXT_KEY_PYTHON_LOGGER_CHINESE_BARBS_AND_YEAR') + civname_show + utils.getText(
                    'TXT_KEY_PYTHON_LOGGER_CHINESE_BARBS_START_TO_INVADE2') + regionname + utils.getText('TXT_KEY_PYTHON_LOGGER_CHINESE_BARBS_LOCATION') + str(x1) + "," + str(y1) + ") To (" + str(x2) + "," + str(y2) + ")"

                utils.addMessage(gcgame.getActivePlayer(), False, iDuration, tem_text, "", 0, "", utils.ColorTypes(iRed), -1, -1, True, True)
                if (PYTHON_LOG_ON_MAIN_BARBS == 1):
                    utils.log(tem_text)
    ...
    def possibleTile(self, tPlot, bWater, bTerritory, bBorder, bImpassable, bNearCity):
        ...
        if [(i, j) for (i, j) in lSurrounding if gcmap.plot(i, j).isCity()]: return False
        ...
        if bBorder and not [(i, j) for (i, j) in lSurrounding if gcmap.plot(i, j).getOwner() != plot.getOwner()]: return False

        # near a city if specified (next to cities excluded above)
        if bNearCity and not [(i, j) for (i, j) in utils.surroundingPlots(tPlot, 2, lambda (a, b): not gcmap.plot(a, b).isCity())]: return False

        # not on landmasses without cities
        if not bWater and gcmap.getArea(plot.getArea()).getNumCities() == 0: return False
    ...
    def includesActiveHuman(self, lPlayers):
        return utils.getHumanID() in lPlayers and tBirth[utils.getHumanID()] <= gcgame.getGameTurnYear()
