...
class UniquePowers:
    ...
    def checkTurn(self, iGameTurn):
        if iGameTurn >= utils.getTurnForYear(tBirth[iRussia]) and pRussia.isAlive():
            self.russianUP()

        if iGameTurn >= utils.getTurnForYear(tBirth[iAmerica]) + utils.getTurns(5):
            self.checkImmigration()

        if iGameTurn >= utils.getTurnForYear(tBirth[iKhazars]) and pKhazars.isAlive():
            self.doJewishKingdom()

        # if iGameTurn >= utils.getTurnForYear(tBirth[iIndonesia]) and pIndonesia.isAlive():
        #	self.indonesianUP()

        if iGameTurn >= utils.getTurnForYear(tBirth[iPhilippines]) and pPhilippines.isAlive():
            utils.doPhilippineEmbassy()

        if iGameTurn >= utils.getTurnForYear(tBirth[iSwahili]) and pSwahili.isAlive():
            self.swahiliDhow()

        if iGameTurn >= utils.getTurnForYear(tBirth[iTeotihuacan]) and pTeotihuacan.isAlive():
    ...
    def vikingUP(self, argsList):
        ...
        if (iOwner == iVikings and utils.getGameTurn() <= utils.getTurnForYear(1500)) or pWinningUnit.getUnitType() == iCorsair:
            if cLosingUnit.getDomainType() == DomainTypes.DOMAIN_SEA:
                ...
                utils.addMessage(iOwner, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_VIKING_NAVAL_UP", (iGold, sAdjective, pLosingUnit.getNameKey())), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)
    ...
    def tatarUP(self, argsList):
        ...
        if (iOwner == iTatar) and (pWinningUnit.getUnitType() == iLancer or pWinningUnit.getUnitType() == iMangudai or pWinningUnit.getUnitType() == iTatarNoyan or pWinningUnit.getUnitType() == iKeshik):
            if cLosingUnit.getDomainType() == DomainTypes.DOMAIN_LAND:
                ...
                gcgetPlayer(iOwner).changeGold(iGold)
                sAdjective = gcgetPlayer(pLosingUnit.getOwner()).getCivilizationAdjectiveKey()
                utils.addMessage(iOwner, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_TATAR_YOKE_UP", (iGold, sAdjective, pLosingUnit.getNameKey())), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)
    ...
    def romanConquestUP(self, iEnemy, iNumTargets=1, lPreferredTargetRegions=[]):  # Unused
        ...
        # utils.debugTextPopup("Roman conquerors against "+GlobalCyTranslator.getText(str(gcgetPlayer(iEnemy).getCivilizationShortDescriptionKey()), ()))

        utils.addMessage(iRome, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_ROMAN_CONQUESTS", (gcgetPlayer(iEnemy).getCivilizationShortDescriptionKey(),)), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)
        utils.addMessage(iEnemy, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_ROMAN_CONQUESTS_TARGET", ()), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)
    ...
    def greekConquestUP(self, iEnemy, iNumTargets=1):  # Unused
        ...
        utils.addMessage(iEnemy, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_GREEK_CONQUESTS_TARGET", ()), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)
    ...
    def tamilConquestUP(self, iEnemy, iNumTargets=1):  # Unused
        ...
        utils.addMessage(iEnemy, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_TAMIL_CONQUESTS_TARGET", ()), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)

    # ------------------ARABIAN U.P.-------------------

    def arabianUP(self, city):
        # pArabia = gcgetPlayer(iArabia)
    ...
    def aztecUP(self, argsList):  # Real Slavery by Sevo
        ...
        pWinningPlayer = gcgetPlayer(iWinningPlayer)
        ...
        iRandom = gcgame.getSorenRandNum(100, 'capture chance')
        if iRandom < 35:
            ...
            utils.addMessage(iWinningPlayer, True, 15, GlobalCyTranslator.getText("TXT_KEY_UP_ENSLAVE_WIN", ()), 'SND_REVOLTEND', 1, 'Art/Units/slave/button_slave.dds', utils.ColorTypes(8), pWinningUnit.getX(), pWinningUnit.getY(), True,
                                     True)
            utils.addMessage(iLosingPlayer, True, 15, GlobalCyTranslator.getText("TXT_KEY_UP_ENSLAVE_LOSE", ()), 'SND_REVOLTEND', 1, 'Art/Units/slave/button_slave.dds', utils.ColorTypes(7), pWinningUnit.getX(), pWinningUnit.getY(), True,
                                     True)
    ...
    def russianUP(self):
        # pRussia = gcgetPlayer(iRussia)
        # teamRussia = gcgetTeam(pRussia.getTeam())
    ...
    def setMongolAI(self):  # Unused
        pCity = gcmap.plot(data.lLatestRazeData[3], data.lLatestRazeData[4])
                    ...
                    if iOwnerNear in [iOldOwner, iIndependent, iIndependent2]:
                        ...
                        if cityNear.getPopulation() <= data.lLatestRazeData[2] and not cityNear.isCapital():
                            ...
                            if bUnitsApproaching:
                                ...
                                utils.addMessage(iOwnerNear, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_TERROR1", ()) + " " + cityNear.getName() + " " + GlobalCyTranslator.getText("TXT_KEY_UP_TERROR2", ()), "", 0, "",
                                                         utils.ColorTypes(iWhite), -1, -1, True, True)
                                utils.addMessage(iMongolia, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_TERROR1", ()) + " " + cityNear.getName() + " " + GlobalCyTranslator.getText("TXT_KEY_UP_TERROR2", ()), "", 0, "",
                                                         utils.ColorTypes(iWhite), -1, -1, True, True)
    ...
    def mongolUP(self, city):
        ...
        if city.getPopulation() >= 4:
            utils.addMessage(iMongolia, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_MONGOL_HORDE", ()), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)
    ...
    def checkImmigration(self):
        ...
        if data.iImmigrationTimer == 0:
            self.doImmigration()
            self.doAliyah()
            iRandom = gcgame.getSorenRandNum(5, 'random')
    ...
    def doImmigration(self):
        ...
        for iMigration in range(iNumMigrations):
            ...
            if gcgetPlayer(iSourcePlayer).isStateReligion():
                iReligion = gcgetPlayer(iSourcePlayer).getStateReligion()
                if sourceCity.isHasReligion(iReligion) and not targetCity.isHasReligion(iReligion):
                    iRandom = gcgame.getSorenRandNum(3, 'random religion spread')
                    if iRandom == 0:
                        targetCity.setHasReligion(iReligion, True, True, True)

            # notify affected players
            if utils.getHumanID() == iSourcePlayer:
                utils.addMessage(iSourcePlayer, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_EMIGRATION", (sourceCity.getName(),)), "", InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT, gc.getUnitInfo(iSettler).getButton(),
                                         utils.ColorTypes(iYellow), sourceCity.getX(), sourceCity.getY(), True, True)
            elif utils.getHumanID() == iTargetPlayer:
                utils.addMessage(iTargetPlayer, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_IMMIGRATION", (targetCity.getName(),)), "", InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT, gc.getUnitInfo(iSettler).getButton(),
                                         utils.ColorTypes(iYellow), x, y, True, True)
    ...
    def canadianUP(self, city):
        ...
        if utils.getHumanID() == city.getOwner():
            utils.addMessage(city.getOwner(), False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_MULTICULTURALISM", (city.getName(), gc.getUnitInfo(iGreatPerson).getText(), iPopulation)), "",
                                     InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT, gc.getUnitInfo(iGreatPerson).getButton(), utils.ColorTypes(iGreen), city.getX(), city.getY(), True, True)
            ...
            if plot.getOwner() == iIndonesia:
                for iUnit in range(plot.getNumUnits()):
                    ...
                    if unit.getDomainType() == DomainTypes.DOMAIN_SEA:
                        ...
                        if iOwner < iNumPlayers and iOwner != iIndonesia and not gcgetTeam(iOwner).isAtWar(iIndonesia):
                            iNumUnits += 1

        if iNumUnits > 0:
            iGold = 5 * iNumUnits
            gcgetPlayer(iIndonesia).changeGold(iGold)
            if utils.getHumanID() == iIndonesia:
                utils.addMessage(iIndonesia, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_INDONESIAN_UP", (iGold,)), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)

    # Mughal UP: receives 50% of building cost as culture when building is completed
    def mughalUP(self, city, iBuilding):
        iCost = gcgetPlayer(iMughals).getBuildingProductionNeeded(iBuilding)
        ...
        if unitList:
            ...
            if iCount > 0:
                ...
                if utils.getHumanID() == iSwahili:
                    utils.addMessage(iSwahili, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_SWAHILI_DHOW_GOLD", (iGold,)), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)
    ...
    def teotihuacanArtisan(self):
        ...
        if unitList:
            ...
            if utils.getHumanID() == iTeotihuacan and totalCulture > 0:
                ...
                utils.addMessage(iTeotihuacan, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_TEOTIHUACAN_ARTISAN_CULTURE", (totalCulture,)), "", 0, "", utils.ColorTypes(iWhite), -1, -1, True, True)
    ...
    def doAliyah(self):
        ...
        for iPlayer in range(iNumPlayers):
            ...
            if gcgetTeam(pPlayer.getTeam()).isAtWar(iIsrael):
                for city in utils.getCityList(iPlayer):
        ...
        for iMigration in range(iNumMigrations):
            ...
            # notify affected players
            if utils.getHumanID() == iSourcePlayer:
                utils.addMessage(iSourcePlayer, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_EMIGRATION_ALIYAH", (sourceCity.getName(),)), "", InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT,
                                         gc.getUnitInfo(iSettler).getButton(), utils.ColorTypes(iYellow), sourceCity.getX(), sourceCity.getY(), True, True)
            elif utils.getHumanID() == iTargetPlayer:
                utils.addMessage(iTargetPlayer, False, iDuration,
                                         GlobalCyTranslator.getText("TXT_KEY_UP_IMMIGRATION_ALIYAH", (sourceCity.getName(), gcgetPlayer(iSourcePlayer).getCivilizationShortDescriptionKey(), targetCity.getName())), "",
                                         InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT, gc.getUnitInfo(iSettler).getButton(), utils.ColorTypes(iYellow), x, y, True, True)

    def computeAliyahBonus(self):
        capital = gcgetPlayer(iIsrael).getCapitalCity()

        # count all cities with Judaism in civs who have open borders with Israel
        openjewishcities = 0
        for iPlayer in range(iNumPlayers):
            if gcgetTeam(iPlayer).isOpenBorders(iIsrael):
    ...
    def doJewishKingdom(self):
        ...
        for iMigration in range(iNumMigrations):
            ...
            # notify affected players
            if utils.getHumanID() == iSourcePlayer:
                utils.addMessage(iSourcePlayer, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_EMIGRATION_JEWISH_KINGDOM", (sourceCity.getName(),)), "", InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT,
                                         gc.getUnitInfo(iSettler).getButton(), utils.ColorTypes(iYellow), sourceCity.getX(), sourceCity.getY(), True, True)
            elif utils.getHumanID() == iTargetPlayer:
                utils.addMessage(iTargetPlayer, False, iDuration, GlobalCyTranslator.getText("TXT_KEY_UP_IMMIGRATION_JEWISH_KINGDOM", (targetCity.getName(),)), "", InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT,
                                         gc.getUnitInfo(iSettler).getButton(), utils.ColorTypes(iYellow), x, y, True, True)
        ...
        if iPlayer == iIndonesia and pIndonesia.getNumCities() == 1:
            ...
            for iLoopPlayer in range(iNumPlayers):
                if gcgetPlayer(iLoopPlayer).isAlive():
                    iCompanyCount += gcgetPlayer(iLoopPlayer).countCorporations(iSpiceRoute)
    ...
    def onUnitBuildImprovement(self, pUnit, iBuild):
        ...
        if pUnit.getUnitType() == iAyllu:
            gcgetPlayer(pUnit.getOwner()).getCapitalCity().changeCulture(pUnit.getOwner(), utils.getTurns(10), True)
            if (pUnit.getOwner() is utils.getHumanID()):
                utils.info("修建设施完成，成功增加文化！")
