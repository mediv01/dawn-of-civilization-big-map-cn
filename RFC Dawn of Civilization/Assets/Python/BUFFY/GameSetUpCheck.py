...
def saveGameStart():
    ...
    """
    if not GlobalCyGame.isGameMultiPlayer() and not MapFinder.isActive():
...
def getWarningMessage():
    ...
    if GlobalCyGame.getWarningStatus() == 1:
...
def checkCRCs(fileName):
    ...
    """
    global crcResult
    crcResult = GlobalCyGame.checkCRCs(fileName, modCRC, dllCRC, shaderCRC, pythonCRC, xmlCRC)
    return crcResult


def isXOTMScenario():
    mapName = gcmap.getMapScriptName().upper()
...
def getWorldWrapSettingOK():
    ...
    """
    map = gcmap
    mapScript = gcmap.getMapScriptName()
    worldWrapOK = True
...
def isMapScriptOK():
    return hofAllowedMapScripts.has_key(gcmap.getMapScriptName())


def isMapSizeOK():
    size = gcmap.getWorldSize()
...
def isOpponentCountTooLow(count):
    try:
        return count < minHOFOpponents[gcmap.getWorldSize()]
    except:
        return True
...
def isOpponentCountTooHigh(count):
    try:
        return count > maxHOFOpponents[gcmap.getWorldSize()]
    except:
        return True
