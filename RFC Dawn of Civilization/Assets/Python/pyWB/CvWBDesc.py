...
def getPlayer(idx):
    ...
    if (gcgetPlayer(idx).isAlive()):
        return gcgetPlayer(idx)
    return None
...
class CvGameDesc:
    ...
    def apply(self):
        ...
        gcgame.setStartYear(self.iStartYear)

    def write(self, f):
        "write out game data"
        f.write("BeginGame\n")
        f.write("\tEra=%s\n" % (gc.getEraInfo(gcgame.getStartEra()).getType(),))
        f.write("\tSpeed=%s\n" % (gc.getGameSpeedInfo(gcgame.getGameSpeedType()).getType(),))
        f.write("\tCalendar=%s\n" % (gc.getCalendarInfo(gcgame.getCalendar()).getType(),))
    ...
    def write(self, f, idx):
        ...
        for i in xrange(gc.getMAX_TEAMS()):
            if pTeam.isVassal(i):
                ...
                if iType == 1:
                    f.write("\tVassalOfTeam=%d, Type=%s, (%s)\n" % (i, "FREE", gcgetTeam(i).getName().encode(fileencoding)))
                elif iType == 0:
                    f.write("\tVassalOfTeam=%d, Type=%s, (%s)\n" % (i, "CAP", gcgetTeam(i).getName().encode(fileencoding)))
...
class CvPlayerDesc:
    def __init__(self):
        ...
        self.civType = "NONE"
        STANDARD_HANDICAP = 1
        self.handicap = gc.getHandicapInfo(STANDARD_HANDICAP).getType()
        ...
        if pPlayer.getCivilizationType() != CivilizationTypes.NO_CIVILIZATION:
            ...
            for i in range(gc.getMAX_PLAYERS()):
                if pPlayer.AI_getAttitudeExtra(i) != 0:
                    f.write("\tAttitudePlayer=%d, AttitudeExtra=%d\n, (%s)" % (i, pPlayer.AI_getAttitudeExtra(i), gcgetPlayer(i).getName().encode(fileencoding)))
...
class CvUnitDesc:
    ...
    def write(self, f, unit, plot):
        ...
        f.write("\tBeginUnit\n")
        f.write("\t\tUnitType=%s, UnitOwner=%d, (%s)\n" % (Info.getType(), unit.getOwner(), gcgetPlayer(unit.getOwner()).getName().encode(fileencoding)))
    ...
    def apply(self):
        ...
        if (player):
            ...
            if unit:
                if (self.szName != None):
                    unit.setName(CvUtil.convertToUnicode(self.szName))
                # leader unit type
...
class CvCityDesc:
    ...
    def write(self, f, plot):
        ...
        f.write("\tBeginCity\n")
        f.write("\t\tCityOwner=%d, (%s)\n" % (city.getOwner(), gcgetPlayer(city.getOwner()).getName().encode(fileencoding)))
        ...
        for iPlayerLoop in range(gc.getMAX_PLAYERS()):
            ...
            if (iPlayerCulture > 0):
                f.write("\t\tPlayer%dCulture=%d, (%s)\n" % (iPlayerLoop, iPlayerCulture, gcgetPlayer(iPlayerLoop).getName().encode(fileencoding)))
    ...
    def apply(self, bSpecial):
        ...
        if (self.name != None):
            self.city.setName(CvUtil.convertToUnicode(self.name), False)
        ...
        for holyCityRel in (self.holyCityReligions):
            ...
            gcgame.setHolyCity(religionTypeNum, self.city, False)
        ...
        for headquarters in (self.headquarterCorporations):
            ...
            gcgame.setHeadquarters(corporationTypeNum, self.city, False)
...
class CvPlotDesc:
    ...
    def preApply(self):
        ...
        plot = GlobalCyMap.plot(self.iX, self.iY)
    ...
    def write(self, f, plot):
        ...
        for iTeamLoop in range(gc.getMAX_CIV_TEAMS()):
            if gcgetTeam(iTeamLoop).isAlive():
    ...
    def apply(self):
        ...
        if (self.szLandmark != ""):
            CyEngine().addLandmark(GlobalCyMap.plot(self.iX, self.iY), "%s" % (self.szLandmark))
...
class CvSignDesc:
    ...
    def apply(self):
        plot = GlobalCyMap.plot(self.iPlotX, self.iPlotY)
    ...
    def write(self, f, sign):
        ...
        f.write("\tplotX=%d\n" % (sign.getPlot().getX(),))
        f.write("\tplotY=%d\n" % (sign.getPlot().getY(),))
        f.write("\tplayerType=%d, (%s)\n" % (sign.getPlayerType(), gcgetPlayer(sign.getPlayerType()).getName().encode(fileencoding)))
...
class CvWBDesc:
    ...
    def applyMap(self):
        ...
        GlobalCyMap.rebuild(self.mapDesc.iGridW, self.mapDesc.iGridH, self.mapDesc.iPrimeMeridian, self.mapDesc.iEquator, self.mapDesc.iTopLatitude, self.mapDesc.iBottomLatitude, self.mapDesc.bWrapX, self.mapDesc.bWrapY,
                        WorldSizeTypes(worldSizeType), ClimateTypes(climateType), SeaLevelTypes(seaLevelType), 0, None)
        ...
        if str(self.mapDesc.bRandomizeResources).lower() != "false":
            for iPlotLoop in range(GlobalCyMap.numPlots()):
                pPlot = GlobalCyMap.plotByIndex(iPlotLoop)
        ...
        for iPlayerLoop in xrange(len(self.playersDesc)):
            ...
            if (pPlayer.getLeaderType() != -1 and str(pWBPlayer.bRandomStartLocation).lower() != "false"):
                ...
                if ((pWBPlayer.iStartingX != -1) and (pWBPlayer.iStartingY != -1)):
                    pPlayer.setStartingPlot(GlobalCyMap.plot(pWBPlayer.iStartingX, pWBPlayer.iStartingY), True)
    ...
    def applyInitialItems(self):
        for iTeamLoop in xrange(len(self.teamsDesc)):
            ...
            for item in pWBTeam.bVassalOfTeamList:
                gcgetTeam(item[0]).assignVassal(iTeamLoop, item[1])
        ...
        for iTeamLoop in xrange(len(self.teamsDesc)):
            ...
            if pWBTeam.bRevealMap:
                GlobalCyMap.setRevealedPlots(iTeamLoop, True, False)
        ...
        for iPlotLoop in range(self.mapDesc.numPlotsWritten):
            ...
            pPlot = GlobalCyMap.plot(pWBPlot.iX, pWBPlot.iY)
