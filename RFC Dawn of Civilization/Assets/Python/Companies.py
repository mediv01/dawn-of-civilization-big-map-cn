...
class Companies:
    ...
    def checkCompany(self, iCompany, iGameTurn):
        if (iCompany in [iTransSaharanRoute, iSpiceRoute, iSilkRoute] and iGameTurn > utils.getTurnForYear(1500)) or (iCompany == iTradingCompany and iGameTurn > utils.getTurnForYear(1800)) or (
                iCompany == iTextileIndustry and iGameTurn > utils.getTurnForYear(1920)):
            iMaxCompanies = 0
        else:
        ...
        for iLoopPlayer in range(iNumPlayers):
            if gcgetPlayer(iLoopPlayer).isAlive():
                iCompanyCount += gcgetPlayer(iLoopPlayer).countCorporations(iCompany)
        ...
        for iPlayer in range(iNumPlayers):
            if gcgetTeam(gcgetPlayer(iPlayer).getTeam()).isHasTech(tCompanyTechs[iCompany]) and (gcgetTeam(gcgetPlayer(iPlayer).getTeam()).isHasTech(iEconomics) or iCompany <= iTradingCompany):
                for city in utils.getCityList(iPlayer):
                    iValue = self.getCityValue(city, iCompany)
                    if iValue > 0:
                        cityValueList.append((city, iValue * 10 + gcgame.getSorenRandNum(10, 'random bonus')))
    ...
    def getCityValue(self, city, iCompany):
        ...
        ownerTeam = gcgetTeam(owner.getTeam())
        ...
        if iCompany == iSpiceRoute:
            ...
            if not self.isCityInArea(tPlot, tSpiceIndiaTL, tSpiceIndiaBR) and not self.isCityInArea(tPlot, tSpiceIndonesiaTL, tSpiceIndonesiaBR) and not self.isCityInArea(tPlot, tSpiceAfricaTL, tSpiceAfricaBR):
                return -1
        if iCompany == iTradingCompany:
            if not self.isCityInArea(tPlot, tCaribbeanTL, tCaribbeanBR) and not self.isCityInArea(tPlot, tSubSaharanAfricaTL, tSubSaharanAfricaBR) and not self.isCityInArea(tPlot, tSouthAsiaTL, tSouthAsiaBR) and not (
                    city.isHasRealBuilding(iTradingCompanyBuilding) or city.isHasRealBuilding(iIberianTradingCompanyBuilding)):
                return -1
        ...
        # competition
        if iCompany == iCerealIndustry and city.isHasCorporation(iFishingIndustry):
            iValue /= 2
        elif iCompany == iFishingIndustry and city.isHasCorporation(iCerealIndustry):
            iValue /= 2
        elif iCompany == iSteelIndustry and city.isHasCorporation(iTextileIndustry):
            iValue /= 2
        elif iCompany == iTextileIndustry and city.isHasCorporation(iSteelIndustry):
            iValue /= 2
        elif iCompany == iOilIndustry and city.isHasCorporation(iComputerIndustry):
            iValue /= 2
        elif iCompany == iComputerIndustry and city.isHasCorporation(iOilIndustry):
            iValue /= 2
