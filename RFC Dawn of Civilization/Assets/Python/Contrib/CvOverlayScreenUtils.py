...
#	"""
#	localTxt = GlobalCyTranslator
#	artMgr = GlobalCyArtFileMgr
...
class CvOverlayScreenUtils:
    ...
    def mouseOverPlot(self, argsList):
        # return 1 to consume the input
        # GlobalCyInterface.addImmediateMessage("Mouse Over Plot called", "")
        screenEnum = argsList[0]
...
class OverlaySign:
    ...
    def addSymbolIcon(self, symbolIndex):
        ...
        """
        self.text += u"%c" % gcgame.getSymbolID(symbolIndex)
    ...
    def addSign(self):
        ...
        """
        if not self.isAdded:
            if gcmap.plot(self.plotX, self.plotY):
                self.plot = gcmap.plot(self.plotX, self.plotY)
            else:
                BugUtil.debug("Invalid Plot for sign at " + str(self.plotX) + "," + str(self.plotY))
            self.playerType = gcgame.getActivePlayer()
