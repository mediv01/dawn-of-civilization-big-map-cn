...
class CvTechSplashScreen:
    ...
    def interfaceScreen(self, iTech):
        ...
        #		screen.setHelpTextArea(self.W_HELP_AREA, FontTypes.SMALL_FONT, self.X_UNIT_PANE, self.Y_UNIT_PANE, self.Z_HELP_AREA, 1, GlobalCyArtFileMgr.getInterfaceArtInfo("POPUPS_BACKGROUND_TRANSPARENT").getPath(), True, True, CvUtil.FONT_LEFT_JUSTIFY, 0 )
        #		screen.addDDSGFC( "TechSplashBackground", GlobalCyArtFileMgr.getInterfaceArtInfo("TEMP_BG").getPath(), 0, 0, self.W_SCREEN, self.H_SCREEN, WidgetTypes.WIDGET_GENERAL, -1, -1 )
        ...
        szSpecialText = GlobalCyGameTextMgr.getTechHelp(self.iTech, True, False, False, True, -1)[1:]
        ...
        for j in range(CyGlobalContext().getNumBuildInfos()):
            ...
            if (bTechFound == 1):
                if (CyGlobalContext().getBuildInfo(j).getImprovement() == -1):
                    ...
                    screen.attachImageButton(panelName3, "", CyGlobalContext().getBuildInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_IMPROVEMENT,
                                             CyGlobalContext().getBuildInfo(j).getImprovement(), 1, False)
