...
class AbstractMoreCiv4lertsEvent(object):
    ...
    def _addMessage(self, iPlayer, szString, szIcon, iFlashX, iFlashY, bOffArrow, bOnArrow, iColor):
        ...
        utils.addMessage(iPlayer, True, eventMessageTimeLong,
                                 szString, None, 0, szIcon, utils.ColorTypes(iColor),
...
class MoreCiv4lertsEvent(AbstractMoreCiv4lertsEvent):
    ...
    def OnCityBuilt(self, argsList):
        city = argsList[0]
        iPlayer = city.getOwner()
        iActivePlayer = gcgame.getActivePlayer()
    ...
    def CheckForAlerts(self, iActivePlayer, activeTeam, BeginTurn):
        ...
        DomVictory = 3
        popGrowthCount = 0
        currentTurn = utils.getGameTurn()
        activePlayer = gcgetPlayer(iActivePlayer)
        ...
        if (self.getCheckForDomPopVictory() or (BeginTurn and self.getCheckForCityBorderExpansion())):
            ...
            for loopPlayer in range(len(teamPlayerList)):
                ...
                for loopCity in range(len(lCity)):
                    city = gcgetPlayer(loopPlayer).getCity(loopCity)
        ...
        if (self.getCheckForDomVictory() and gcgame.isVictoryValid(DomVictory)):

            # Population Limit
            if (self.getCheckForDomPopVictory()):
                VictoryPopPercent = 0.0
                VictoryPopPercent = gcgame.getAdjustedPopulationPercent(DomVictory) * 1.0
            ...
            if (self.getCheckForDomLandVictory()):
                ...
                VictoryLandPercent = gcgame.getAdjustedLandPercent(DomVictory) * 1.0
            ...
            for iLoopPlayer, currentTechs in techsByPlayer.iteritems():
                ...
                if (newTechs):
                    ...
                    message = localText.getText("TXT_KEY_MORECIV4LERTS_NEW_TECH_AVAIL",
                                                (gcgetPlayer(iLoopPlayer).getCivilizationShortDescription(0), szNewTechs))
                    self._addMessageNoIcon(iActivePlayer, message)
                ...
                if (removedTechs):
                    ...
                    message = localText.getText("TXT_KEY_MORECIV4LERTS_TECH_NOT_AVAIL",
                                                (gcgetPlayer(iLoopPlayer).getCivilizationShortDescription(0), szRemovedTechs))
                    self._addMessageNoIcon(iActivePlayer, message)
            ...
            for iLoopPlayer, currentTrades in tradesByPlayer.iteritems():
                ...
                if (newTrades):
                    ...
                    message = localText.getText("TXT_KEY_MORECIV4LERTS_NEW_BONUS_AVAIL",
                                                (gcgetPlayer(iLoopPlayer).getCivilizationShortDescription(0), szNewTrades))
                    self._addMessageNoIcon(iActivePlayer, message)
                ...
                if (removedTrades):
                    ...
                    message = localText.getText("TXT_KEY_MORECIV4LERTS_BONUS_NOT_AVAIL",
                                                (gcgetPlayer(iLoopPlayer).getCivilizationShortDescription(0), szRemovedTrades))
                    self._addMessageNoIcon(iActivePlayer, message)
    ...
    def buildPlayerString(self, players):
        ...
        names = [gcgetPlayer(iPlayer).getCivilizationShortDescription(0) for iPlayer in players]
        names.sort()
        return u", ".join(names)
