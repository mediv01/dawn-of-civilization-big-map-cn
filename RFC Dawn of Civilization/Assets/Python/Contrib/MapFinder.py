...
def regenerate():
    if canRegenerate():
        if GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_SHOW:
            GlobalCyInterface.setShowInterface(InterfaceVisibility.INTERFACE_SHOW)
...
def showInterface():
    ...
    if not savedInterfaceMode:
        savedInterfaceMode = GlobalCyInterface.getShowInterface()
    GlobalCyInterface.setShowInterface(InterfaceVisibility.INTERFACE_SHOW)
...
def restoreInterface():
    ...
    if savedInterfaceMode:
        GlobalCyInterface.setShowInterface(savedInterfaceMode)
...
def matchRules():
    ...
    iMaxX = gcmap.getGridWidth()
    iMaxY = gcmap.getGridHeight()
    bWrapX = gcmap.isWrapX()
    bWrapY = gcmap.isWrapY()
...
def save():
    ...
    gcgame.takeJPEGScreenShot(screenFile)
...
def loadCodeText(root):
    ...
    lLang = []
    CodeText = {}
    iLang = gcgame.getCurrentLanguage()
