...
def addMessage(iPlayer, szString, szIcon, iFlashX=-1, iFlashY=-1, bOffArrow=False, bOnArrow=False):
    ...
    utils.addMessage(iPlayer, True, eventMessageTimeLong,
                             szString, None, InterfaceMessageTypes.MESSAGE_TYPE_INFO,
                             szIcon, utils.ColorTypes(-1),
...
class AbstractCityAlertManager(AbstractStatefulAlert):
    ...
    def onCityAcquiredAndKept(self, argsList):
        ...
        if (iPlayer == gcgame.getActivePlayer()):
            self._resetCity(city)

    def onCityLost(self, argsList):
        'City Lost'
        city = argsList[0]
        iPlayer = gcgame.getActivePlayer()
...
class CanHurryPopulation(AbstractCanHurry):
    ...
    def _passesTest(self, city):
        if PYTHON_CIV4_ALERT_ON_HURRY_ONLY_IN_HURRYANGER_ZERO:
            return city.canHurry(self.keHurryType, False) and city.getHurryAngerTimer()==0
        else:
            return city.canHurry(self.keHurryType, False)
    ...
    def check(self):
        ...
        for eTeam, eNewEnemy in newEnemies.iteritems():
            if activeTeam.isHasMet(eTeam):
                ...
                if eOldEnemy != -1 and not gcgetTeam(eOldEnemy).isAlive():
                    eOldEnemy = -1
                    enemies[eTeam] = -1
                ...
                if eOldEnemy != eNewEnemy:
                    ...
                    if eNewEnemy == -1:
                        if eOldEnemy == eActiveTeam:
                            message = BugUtil.getText("TXT_KEY_CIV4LERTS_ON_YOU_NO_WORST_ENEMY", gcgetPlayer(eTeam).getCivilizationShortDescription(0))
                        else:
                            message = BugUtil.getText("TXT_KEY_CIV4LERTS_ON_NO_WORST_ENEMY",
                                                      (gcgetPlayer(eTeam).getCivilizationShortDescription(0), gcgetPlayer(eOldEnemy).getCivilizationShortDescription(0)))
                    elif eOldEnemy == -1:
                        message = None  # handled below
                        if eNewEnemy not in delayedMessages:
                            delayedMessages[eNewEnemy] = gcgetPlayer(eTeam).getCivilizationShortDescription(0)
                        else:
                            delayedMessages[eNewEnemy] += u", " + gcgetPlayer(eTeam).getCivilizationShortDescription(0)
                    else:
                        if eOldEnemy == eActiveTeam:
                            message = BugUtil.getText("TXT_KEY_CIV4LERTS_ON_SWITCH_WORST_ENEMY_FROM_YOU",
                                                      (gcgetPlayer(eTeam).getCivilizationShortDescription(0), gcgetPlayer(eNewEnemy).getCivilizationShortDescription(0)))
                            ...
                            message = BugUtil.getText("TXT_KEY_CIV4LERTS_ON_SWITCH_WORST_ENEMY",
                                                      (gcgetPlayer(eTeam).getCivilizationShortDescription(0), gcgetPlayer(eNewEnemy).getCivilizationShortDescription(0), gcgetPlayer(eOldEnemy).getCivilizationShortDescription(0)))
                    if message:
                        addMessageNoIcon(eActivePlayer, message)
        for eEnemy, haters in delayedMessages.iteritems():
            if eActiveTeam == eEnemy:
                ...
                message = BugUtil.getText("TXT_KEY_CIV4LERTS_ON_WORST_ENEMY", (haters, gcgetPlayer(eEnemy).getCivilizationShortDescription(0)))
            addMessageNoIcon(eActivePlayer, message)
