...
def sdGetTimeInt(turn):
    TurnTable = GlobalCyGameTextMgr.getTimeStr(turn, False).split(' ')
...
def sdGameYearsInt():
    yearsBC = sdGetTimeInt(gcgame.getStartTurn())
    if (yearsBC < 0):
        yearsBC = yearsBC - (yearsBC * 2)
    else:
        yearsBC = 0
    yearsAD = 0
    for i in range(gc.getGameSpeedInfo(gcgame.getGameSpeedType()).getNumTurnIncrements()):
        yearsAD += gc.getGameSpeedInfo(gcgame.getGameSpeedType()).getGameTurnInfo(i).iNumGameTurnsPerIncrement
    yearsAD = sdGetTimeInt(yearsAD)
    if (yearsAD < sdGetTimeInt(utils.getGameTurn())):
        yearsAD = sdGetTimeInt(utils.getGameTurn())
