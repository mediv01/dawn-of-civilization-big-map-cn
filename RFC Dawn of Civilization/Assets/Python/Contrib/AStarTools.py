...
class AStar:
    ...
    def getRealXDist(self, x1, x2):
        d1 = abs(x1 - x2)
        d2 = abs(min(x1, x2) + GlobalCyMap.getGridWidth() - max(x1, x2))
        return min(d1, d2)

    # returns the minimum y-distance, considering the y-axis over-/underflow
    def getRealYDist(self, y1, y2):
        d1 = abs(y1 - y2)
        d2 = abs(min(y1, y2) + GlobalCyMap.getGridHeight() - max(y1, y2))
        return min(d1, d2)
    ...
    def fnCheckLandBridge(self, pPlot, x, y):
        pTestPlot = GlobalCyMap.plot(pPlot.getX() + x, pPlot.getY() + y)
        if not (pPlot.isCity() or pTestPlot.isCity()):
            pTestPlot = GlobalCyMap.plot(pPlot.getX() + x, pPlot.getY())
            if (pTestPlot.isCoastalLand()):
                pTestPlot = GlobalCyMap.plot(pPlot.getX(), pPlot.getY() + y)
                if (pTestPlot.isCoastalLand()):
                    return True
        return False
    ...
    def getNewPlot(self, pPlot, dx, dy):
        ...
        # check x underflow
        if x < 0:
            x += GlobalCyMap.getGridWidth()
        # check x overflow
        elif x >= GlobalCyMap.getGridWidth():
            x -= GlobalCyMap.getGridWidth()
        # check y underflow
        if y < 0:
            y += GlobalCyMap.getGridHeight()
        # check y overflow
        elif y >= GlobalCyMap.getGridHeight():
            y -= GlobalCyMap.getGridHeight()
        return GlobalCyMap.plot(x, y)
    ...
    def getArea(self):
        ...
        if self.bResultValid:
            for i in range(self.pCloseList.len()):
                ...
                iCosts = self.pCloseList.getGCosts(GlobalCyMap.plot(lAPlot[0], lAPlot[1]))
...
class AStarMoveArea:
        ...
        for i in range(iNumUnits):
            ...
            if (iInvisibleType != InvisibleTypes.NO_INVISIBLE):
                for j in range(self.pUnit.getNumSeeInvisibleTypes()):
                    ...
                    continue
            if (gcgetTeam(iTeam).isBarbarian()):
                bBarbarian = True
            elif (gcgetTeam(iTeam).isAtWar(self.iActivePlayerTeam)):
                bEnemy = True
            else:
                bNeutral = True
        if bEnemy:
    ...
    def checkAdjacentRevealed(self, pPlot):
        for dx in range(-1, 2):
            for dy in range(-1, 2):
                if not (dx == 0 and dy == 0):
                    if GlobalCyMap.plot(pPlot.getX() + dx, pPlot.getY() + dy).isRevealed(self.iActivePlayerTeam, False):
                        return True
        return False

    # handles the color for a plot
    def setPlotColor(self, x, y, iCosts):
        pPlot = GlobalCyMap.plot(x, y)
        tPlot = (x, y)
    ...
    def highlightMoveArea(self, pUnit):
        ...
        pActivePlayer = gcgetPlayer(self.iActivePlayer)
