...
class Plague:
    ...
    def setup(self):
        ...
        if utils.getScenario() == i3000BC:
            data.lGenericPlagueDates[0] = utils.getTurnForYear(400) + utils.variation(20)

        data.lGenericPlagueDates[1] = utils.getTurnForYear(1300) + utils.variation(20)

        # Avoid interfering with the Indian UHV
        if utils.getHumanID() == iIndia and data.lGenericPlagueDates[1] <= utils.getTurnForYear(1200):
            data.lGenericPlagueDates[1] = utils.getTurnForYear(1200) + 1

        if utils.getScenario != i1700AD:
            data.lGenericPlagueDates[2] = utils.getTurnForYear(1650) + utils.variation(20)

        data.lGenericPlagueDates[3] = utils.getTurnForYear(1850) + utils.variation(20)

        undoPlague = gcgame.getSorenRandNum(8, 'undo')
        if undoPlague <= 3:
    ...
    def checkTurn(self, iGameTurn):
        ...
        for iPlague, iPlagueDate in enumerate(data.lGenericPlagueDates):
            if iGameTurn == iPlagueDate:
                # print ("new plague")
                if (PYTHON_LOG_ON_MAIN_PLAGUE == 1):
                    utils.log('Start Plague')
                    utils.log_plague('Start Plague')
    ...
    def checkPlayerTurn(self, iGameTurn, iPlayer):
        if data.bNoPlagues:
            return
        if (PLAGUE_DONNOT_INFECT_HUMAN == 1):
    ...
    def startPlague(self, iPlagueCounter):
            ...
            if pPlayer.isAlive():
                if self.isVulnerable(iPlayer):
                    ...
                    if pPlayer.calculateTotalCityHealthiness() > 0:
                        iHealth += gcgame.getSorenRandNum(40, 'random modifier')
        ...
        if iWorstCiv != -1:
            ...
            if (PYTHON_LOG_ON_MAIN_PLAGUE == 1):
                utils.logwithid_plague(iWorstCiv, 'starting plague with iWorstHealth: ' + str(iWorstHealth))
            pWorstCiv = gcgetPlayer(iWorstCiv)
    ...
    def preStopPlague(self, iPlayer):
        ...
        if cityList:
            ...
            for city in cityList:
                if gcgame.getSorenRandNum(100, 'roll') > 30 - 5 * city.healthRate(False, 0) + iModifier:
    ...
    def losePopulation(self, city):
        ...
        iRand = gcgame.getSorenRandNum(100, "Lose population from plague")
    ...
    def spreadToVassals(self, iPlayer):
        ...
        for iLoopPlayer in range(iNumMajorPlayers):
            if gcgetTeam(gcgetPlayer(iPlayer).getTeam()).isVassal(iLoopPlayer) or gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isVassal(iPlayer):
                if self.isVulnerable(iLoopPlayer) and data.players[iLoopPlayer].iPlagueCountdown > 2:
                    if gcgetPlayer(iLoopPlayer).getNumCities() > 0:
                        capital = gcgetPlayer(iLoopPlayer).getCapitalCity()
                        self.spreadPlague(iLoopPlayer)
                        if (PYTHON_LOG_ON_MAIN_PLAGUE == 1):
                            utils.logwithid_plague(iPlayer, ' plague is spreading to vassals')
        ...
        for iTradeRoute in range(city.getTradeRoutes()):
            ...
            if not tradeCity.isNone():
                ...
                if not tradeCity.isHasRealBuilding(iPlague):
                    if (PYTHON_LOG_ON_MAIN_PLAGUE == 1):
                        utils.logwithid_plague(iPlayer, ' plague is spreading through trade route')
                    if iPlayer == iOwner:
    ...
    def spreadBetweenCities(self, iPlayer, lSourceCities, lTargetCities):
        ...
        for targetCity in lTargetCities:
            if [city for city in lSourceCities if targetCity.isConnectedTo(city) and utils.calculateDistance(targetCity.getX(), targetCity.getY(), city.getX(), city.getY()) <= 6]:
                ...
                if (PYTHON_LOG_ON_MAIN_PLAGUE == 1):
                    utils.logwithid_plague(iPlayer, ' plague is spreading between city')
                return
    ...
    def killUnitsByPlague(self, city, pPlot, baseValue, iDamage, iPreserveDefenders):
        ...
        teamOwner = gcgetTeam(gcgetPlayer(city.getOwner()).getTeam())

        # deadly plague when human player isn't born yet, will speed up the loading
        if utils.getGameTurn() < utils.getTurnForYear(tBirth[utils.getHumanID()]) + utils.getTurns(20):
            iDamage += 10
            baseValue -= 5
        ...
        if iNumUnitsInAPlot > 0:
            for iUnit in reversed(range(iNumUnitsInAPlot)):
                ...
                if gcgetPlayer(unit.getOwner()).isHuman():
                    ...
                    if iPreserveDefenders > 0:
                        if utils.isDefenderUnit(unit):
                            iPreserveDefenders -= 1
                            if pPlot.getNumUnits() > iPreserveDefenders or gcgetTeam(gcgetPlayer(unit.getOwner()).getTeam()).isAtWar(iHuman):
                                pass
                            else:
                                iMaxDamage = 50
                                if unit.workRate(100) > 0 and not unit.canFight(): iMaxDamage = 100
                ...
                if utils.isMortalUnit(unit):  # another human handicap inside
                    ...
                    if gcgame.getSorenRandNum(100, 'roll') > iThreshold:
                        # Leoreth: disable damange announcements
                        # if iDamage - unit.getExperience()/10 - unit.baseCombatStr()*3/7 >= 100 - unit.getDamage():
                        #	if unit.getOwner() != iOwner and gcgetPlayer(unit.getOwner()).isHuman():
                        #		utils.addMessage(unit.getOwner(), False, iDuration/2, GlobalCyTranslator.getText("TXT_KEY_PLAGUE_PROCESS_UNIT", ()) + " " + city.getName(), "AS2D_PLAGUE", 0, "", utils.ColorTypes(iLime), -1, -1, True, True)
    ...
    def infectCity(self, city):
        if city.getOwner() == iCongo and gcgame.getGameTurnYear() <= 1650:
            return  # Leoreth: don't let plague mess up the UHV
        elif city.getOwner() == iMali and gcgame.getGameTurnYear() <= 1500:
        ...
        if gcgetPlayer(city.getOwner()).isHuman():
            utils.addMessage(city.getOwner(), True, iDuration / 2, GlobalCyTranslator.getText("TXT_KEY_PLAGUE_SPREAD_CITY", ()) + " " + city.getName() + "!", "AS2D_PLAGUE", 0, "", utils.ColorTypes(iLime), -1, -1, True, True)
        for (x, y) in utils.surroundingPlots((city.getX(), city.getY()), 2):
    ...
    def isVulnerable(self, iPlayer):
        ...
        if gcgetTeam(pPlayer.getTeam()).isHasTech(iMicrobiology): return False
        ...
        if data.players[iPlayer].iPlagueCountdown == 0:  # vulnerable
            if not gcgetTeam(pPlayer.getTeam()).isHasTech(iMicrobiology):
    ...
    def spreadPlague(self, iPlayer):
        ...
        if (PYTHON_LOG_ON_MAIN_PLAGUE == 1):
            utils.logwithid_plague(iPlayer, ' is spreading plague')
    ...
    def infectCitiesNear(self, iPlayer, startingX, startingY):
        for city in utils.getCityList(iPlayer):
            if utils.calculateDistance(city.getX(), city.getY(), startingX, startingY) <= 3:
                if (PYTHON_LOG_ON_MAIN_PLAGUE == 1):
                    utils.logwithid_plague(iPlayer, ' is infecting near city')
                self.infectCity(city)
                self.announceForeignPlagueSpread(city)
    ...
    def onCityAcquired(self, iOldOwner, iNewOwner, city):
        if city.hasBuilding(iPlague):
            if not data.players[iOldOwner].bFirstContactPlague:  # don't infect if first contact plague
                if data.players[iNewOwner].iPlagueCountdown <= 0 and utils.getGameTurn() > utils.getTurnForYear(tBirth[iNewOwner]) + utils.getTurns(
                        iImmunity):  # skip immunity in this case (to prevent expoiting of being immune to conquer weak civs), but not for the new born civs
                    if not gcgetTeam(gcgetPlayer(iNewOwner).getTeam()).isHasTech(iMicrobiology):  # but not permanent immunity
        ...
        if utils.getGameTurn() > utils.getTurnForYear(tBirth[iAztecs]) + 2 and utils.getGameTurn() < utils.getTurnForYear(1800):
            ...
            if iOldWorldCiv != -1 and iNewWorldCiv != -1:
                        ...
                        if not teamInuit.isHasTech(iBiology):
                            ...
                            if city:
                                ...
                                if iHealth < 10:  # no spread for iHealth >= 70 years
                                    iHealth /= 10
                                    if gcgame.getSorenRandNum(100, 'roll') > 30 + 5 * iHealth:
                ...
                if data.players[iNewWorldCiv].iPlagueCountdown == 0:  # vulnerable
                    # print ("vulnerable", iNewWorldCiv)
                    if not gcgetTeam(pNewWorldCiv.getTeam()).isHasTech(iBiology):
                        ...
                        if city:
                            ...
                            if iHealth < 10:  # no spread for iHealth >= 70 years
                                ...
                                if gcgame.getSorenRandNum(100, 'roll') > 30 + 5 * iHealth:
                                    ...
                                    if (PYTHON_LOG_ON_MAIN_PLAGUE == 1):
                                        utils.logwithid_plague(iHasMetTeamY, ' is infecting new world civ city')
                                    self.infectCity(city)
                                    self.announceForeignPlagueSpread(city)
    ...
    def announceForeignPlagueSpread(self, city):
        iOwner = city.getOwner()
        iHuman = utils.getHumanID()
        if gcgetPlayer(iHuman).canContact(iOwner) and iHuman != iOwner and city.isRevealed(iHuman, False):
            utils.addMessage(iHuman, False, iDuration / 2, GlobalCyTranslator.getText("TXT_KEY_PLAGUE_SPREAD_CITY", ()) + " " + city.getName() + " (" + gcgetPlayer(iOwner).getCivilizationAdjective(0) + ")", "AS2D_PLAGUE", 0, "",
                                     utils.ColorTypes(iLime), -1, -1, True, True)
