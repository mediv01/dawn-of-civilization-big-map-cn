...
class Religions:
    ...
    def checkTurn(self, iGameTurn):
        ...
        if utils.getHumanID() != iIndia:
            if iGameTurn == utils.getTurnForYear(-2000) + 1:
                if not gcgame.isReligionFounded(iHinduism):
                    if not gcmap.plot(92, 39).getPlotCity().isNone():
    ...
    def foundReligion(self, tPlot, iReligion):
        ...
        plot = gcmap.plot(x, y)
        if plot.isCity():
            gcgame.setHolyCity(iReligion, plot.getPlotCity(), True)
            return True

        return False

    def onReligionFounded(self, iReligion, iFounder):
        if utils.getGameTurn() == utils.getScenarioStartTurn(): return
    ...
    def spreadReligion(self, tCoords, iNum, iMissionary):  # Unused
        x, y = tCoords
        if not tCoords or not gcmap.plot(x, y).isCity(): return
    ...
    def getTargetCities(self, lCities, iReligion):
        return [city for city in lCities if not city.isHasReligion(iReligion) and gcgetPlayer(city.getOwner()).getSpreadType(city.plot(), iReligion) > ReligionSpreadTypes.RELIGION_SPREAD_NONE]

    def selectHolyCity(self, tTL, tBR, tPreferredCity=None, bAIOnly=True):
        if tPreferredCity:
            x, y = tPreferredCity
            if gcmap.plot(x, y).isCity():
                if not bAIOnly or utils.getHumanID() != gcmap.plot(x, y).getPlotCity().getOwner():
    ...
    def checkLateReligionFounding(self, iReligion, iTech):
        ...
        for iPlayer in range(iNumPlayers):
            if gcgetPlayer(iPlayer).isAlive():
                iPlayerCount += 1
                if gcgetTeam(gcgetPlayer(iPlayer).getTeam()).isHasTech(iTech):
                    iPrereqCount += 1
    ...
    def checkJudaism(self, iGameTurn):
        if gcgame.isReligionFounded(iJudaism): return

        if iGameTurn == utils.getTurnForYear(-1500) - utils.getTurns(data.iSeed % 5):
    ...
    def spreadReligionToRegion(self, iReligion, lRegions, iGameTurn, iStartDate, iInterval, iOffset):
        ...
        if iGameTurn < utils.getTurnForYear(iStartDate): return
        if iGameTurn > utils.getTurnForYear(1948): return
    ...
    def spreadIslamIndonesia(self, iGameTurn):
        if not gcgame.isReligionFounded(iIslam): return
        if not pIndonesia.isAlive(): return
        if not (utils.getTurnForYear(1300) <= iGameTurn <= utils.getTurnForYear(1600)): return

        if iGameTurn % utils.getTurns(15) != utils.getTurns(4): return

        lIndonesianContacts = [iPlayer for iPlayer in range(iNumPlayers) if pIndonesia.canContact(iPlayer) and gcgetPlayer(iPlayer).getStateReligion() == iIslam]
    ...
    def checkBuddhism(self, iGameTurn):
        if gcgame.isReligionFounded(iBuddhism): return

        if iGameTurn == utils.getTurnForYear(-300) - 5 + utils.getTurns(data.iSeed % 10):
            self.foundReligion(self.selectHolyCity(tBuddhistTL, tBuddhistBR), iBuddhism)

    ## ORTHODOXY

    def checkChristianity(self, iGameTurn):
        if not gcgame.isReligionFounded(iJudaism): return
        if gcgame.isReligionFounded(iOrthodoxy): return

        iOffset = utils.getTurns(data.iSeed % 15)

        if iGameTurn == utils.getTurnForYear(0) + iOffset:
            pHolyCity = gcgame.getHolyCity(iJudaism)

            if pHolyCity.getOwner() != utils.getHumanID() and gcgame.getSorenRandNum(2, "Holy city?") == 0:
    ...
    def foundBuddhism(self, city):
        gcgetPlayer(city.getOwner()).foundReligion(iBuddhism, iBuddhism, True)

    ## CATHOLICISM

    def checkSchism(self, iGameTurn):
        if not gcgame.isReligionFounded(iOrthodoxy): return
        if gcgame.isReligionFounded(iCatholicism): return

        if gcgame.countReligionLevels(iOrthodoxy) < 10: return
        ...
        for iPlayer in range(iNumTotalPlayersB):
            ...
            if iStateReligion == iOrthodoxy:
                lStateReligionCities.extend(lCities)
            elif gcgetPlayer(iPlayer).isMinorCiv() or gcgetPlayer(iPlayer).isBarbarian():
                lMinorCities.extend(lCities)
            elif iStateReligion == -1:
                lNoStateReligionCities.extend(lCities)
            else:
                lDifferentStateReligionCities.extend(lCities)
        ...
        if lOrthodoxCapitals:
            pOrthodoxCapital = utils.getHighestEntry(lOrthodoxCapitals, lambda city: gcgetPlayer(city.getOwner()).getScoreHistory(iGameTurn))
        else:
            pOrthodoxCapital = gcgame.getHolyCity(iOrthodoxy)
    ...
    def schism(self, pOrthodoxCapital, pCatholicCapital, lReplace, lDistance):
        ...
        if gcgetPlayer(utils.getHumanID()).getStateReligion() == iOrthodoxy and utils.getGameTurn() >= utils.getTurnForYear(tBirth[utils.getHumanID()]):
            utils.popup(GlobalCyTranslator.getText("TXT_KEY_SCHISM_TITLE", ()), GlobalCyTranslator.getText("TXT_KEY_SCHISM_MESSAGE", (pCatholicCapital.getName(),)), ())
        ...
        for iPlayer in range(iNumPlayers):
            ...
            if pPlayer.isAlive() and pPlayer.getStateReligion() == iOrthodoxy:
                ...
                if 2 * len(lConvertedCities) >= gcgetPlayer(iPlayer).getNumCities():
                    gcgetPlayer(iPlayer).setLastStateReligion(iCatholicism)
    ...
    def reformationPopup(self):
        self.showPopup(7624, GlobalCyTranslator.getText("TXT_KEY_REFORMATION_TITLE", ()), GlobalCyTranslator.getText("TXT_KEY_REFORMATION_MESSAGE", ()),
                       (GlobalCyTranslator.getText("TXT_KEY_REFORMATION_1", ()), GlobalCyTranslator.getText("TXT_KEY_REFORMATION_2", ()), GlobalCyTranslator.getText("TXT_KEY_REFORMATION_3", ())))
    ...
    def onTechAcquired(self, iPlayer, iTech):
        ...
        if iTech == iAcademia:
            if gcgetPlayer(iPlayer):  # wunshare : why will getPlayer() is None ??
                if gcgetPlayer(iPlayer).getStateReligion() == iCatholicism:
                    if not gcgame.isReligionFounded(iProtestantism):
                        gcgetPlayer(iPlayer).foundReligion(iProtestantism, iProtestantism, True)
                        self.reformation()
    ...
    def onBuildingBuilt(self, city, iPlayer, iBuilding):
        ...
        if iBuilding == iOrthodoxCathedral:
            ...
            pOrthodoxHolyCity = gcgame.getHolyCity(iOrthodoxy)

            if pOrthodoxHolyCity.getOwner() != iPlayer:
                self.foundReligion((city.getX(), city.getY()), iCatholicism)
                pCatholicHolyCity = gcgame.getHolyCity(iCatholicism)
    ...
    def chooseProtestantism(self, iCiv):
        iRand = gcgame.getSorenRandNum(100, 'Protestantism Choice')
        return iRand >= getCatholicPreference(iCiv)

    def isProtestantAnyway(self, iCiv):
        iRand = gcgame.getSorenRandNum(100, 'Protestantism anyway')
            ...
            if data.players[iPlayer].iReformationDecision == 2:
                for iTargetPlayer in range(iNumPlayers):
                    if data.players[iTargetPlayer].iReformationDecision == 0 and utils.getHumanID() != iTargetPlayer and iTargetPlayer != iNetherlands and not utils.isAVassal(iTargetPlayer):
                        gcgetTeam(iPlayer).declareWar(iTargetPlayer, True, WarPlanTypes.WARPLAN_DOGPILE)
    ...
    def reformationyes(self, iCiv):  # Unused
        for city in utils.getCityList(iCiv):
            ...
            if city.getPopulation() > 7:
                iRand = gcgame.getSorenRandNum(100, "Reformation residual")
    ...
    def reformationno(self, iCiv):  # Unused
        ...
        for city in cityList:
            if city.isHasReligion(iCatholicism):
                rndnum = gcgame.getSorenRandNum(100, 'ReformationAnyway')
