...
def testunit(iPlayer, iUnit):
    unit = gcgetPlayer(iPlayer).initUnit(utils.getUniqueUnit(iPlayer, iUnit), 0, 0, UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
    print (getName(unit))


def create(iPlayer, iUnit, (x, y)):
    gcgetPlayer(iPlayer).createGreatPeople(utils.getUniqueUnit(iPlayer, iUnit), True, True, x, y)


def getAlias(iCiv, iType, iEra):
    if iCiv in [iCivHarappa, iCivTamils]:
        return iCivIndia
    elif iCiv == iCivAssyria:
        return iCivBabylonia
    elif iCiv == iCivHittites:
        return iCivGreece
    elif iCiv == iCivIran:
        return iCivPersia
    elif iCiv in [iCivManchuria, iCivKhitan]:
        return iCivChina
    elif iCiv == iCivTeotihuacan:
        return iCivMaya
    elif iCiv == iCivNovgorod:
        return iCivRussia
    elif iCiv == iCivLithuania:
        return iCivPoland
    elif iCiv == iCivTatar:
        return iCivMongols  # wunshare: bugfix
...
def getEraNames(iCiv, iType, iEra):
    ...
    lBefore = [sName for sName in lNames[:iOffset] if not gcgame.isGreatPersonBorn(sName)]
    lAfter = [sName for sName in lNames[iOffset:] if not gcgame.isGreatPersonBorn(sName)]
...
def onGreatPersonBorn(unit, iPlayer, city, bAnnounceBirth=True):
    sName = getName(unit)
    if sName:
        gcgame.addGreatPersonBornName(sName)
    ...
    if bAnnounceBirth:
        if iPlayer not in [iIndependent, iIndependent2, iBarbarian]:
            pDisplayCity = city
            if pDisplayCity.isNone(): pDisplayCity = gcmap.findCity(unit.getX(), unit.getY(), PlayerTypes.NO_PLAYER, TeamTypes.NO_TEAM, False, False, TeamTypes.NO_TEAM, DirectionTypes.NO_DIRECTION, CyCity())

            sCity = "%s (%s)" % (pDisplayCity.getName(), gcgetPlayer(pDisplayCity.getOwner()).getCivilizationShortDescription(0))
            ...
            for iLoopPlayer in range(iNumPlayers):
                if gcgetPlayer(iLoopPlayer).isAlive():
                    if unit.plot().isRevealed(gcgetPlayer(iLoopPlayer).getTeam(), False):
                        utils.addMessage(iLoopPlayer, False, iDuration, sMessage, "AS2D_UNIT_GREATPEOPLE", InterfaceMessageTypes.MESSAGE_TYPE_MAJOR_EVENT, unit.getButton(), utils.ColorTypes(gc.getInfoTypeForString("COLOR_UNIT_TEXT")),
                                                 unit.getX(), unit.getY(), True, True)
                    else:
                        utils.addMessage(iLoopPlayer, False, iDuration, sUnrevealedMessage, "AS2D_UNIT_GREATPEOPLE", InterfaceMessageTypes.MESSAGE_TYPE_MAJOR_EVENT, "", utils.ColorTypes(gc.getInfoTypeForString("COLOR_UNIT_TEXT")), -1, -1,
                                                 False, False)
