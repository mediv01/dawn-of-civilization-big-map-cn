...
def createMap(iPlayer, iReborn):
    ...
    try:
        writer = csv.writer(file)
        writer.writerow((gcgetPlayer(iPlayer).getPlayerTextColorR(), gcgetPlayer(iPlayer).getPlayerTextColorG(), gcgetPlayer(iPlayer).getPlayerTextColorB()))
...
def getForeignCorePlots(iPlayer, iReborn):
    ...
    for iLoopPlayer in range(iNumPlayers):
        if iLoopPlayer != iPlayer:
            if utils.canEverRespawn(iLoopPlayer, utils.getTurnForYear(iSpawnDate)) or tBirth[iLoopPlayer] > iSpawnDate:
