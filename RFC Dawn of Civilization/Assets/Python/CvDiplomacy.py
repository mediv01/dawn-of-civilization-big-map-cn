...
class CvDiplomacy:
    ...
    def determineResponses(self, eComment):
        ...
        # Or if the AI is no longer a vassal
            ...
            if (gcgetTeam(gcgame.getActiveTeam()).canDeclareWar(gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam())):
        ...
        # If they reject our demand
                ...
                # Otherwise this is a player offer to the AI
                    ...
                    if (self.diploScreen.ourOfferEmpty() == 0 and self.diploScreen.theirOfferEmpty() == 0):
                        ...
                        if (not self.diploScreen.atWar()):
                            if (gcgame.getActiveTeam() != gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam()):
                    ...
                    # Otherwise they have something on the table and we dont
                        ...
                        # Otherwise (during peacetime) ask what they want for our item or demand they give it to us
                            ...
                            if (gcgame.getActiveTeam() == gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam()):
                                ...
                                if (gcgetTeam(gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam()).isVassal(gcgame.getActiveTeam()) and self.diploScreen.theirVassalTribute()):
                                    self.addUserComment("USER_DIPLOCOMMENT_VASSAL_TRIBUTE", -1, -1)
                                elif (gcgetPlayer(self.diploScreen.getWhoTradingWith()).AI_getAttitude(gcgame.getActivePlayer()) >= AttitudeTypes.ATTITUDE_PLEASED):
                                    self.addUserComment("USER_DIPLOCOMMENT_ASK", -1, -1)
                                elif (gcgetTeam(gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam()).isVassal(gcgame.getActiveTeam()) or gcgetTeam(gcgame.getActiveTeam()).canDeclareWar(
                                        gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam())):
            ...
            if (gcgame.getActiveTeam() == gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam() or gcgetTeam(gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam()).isVassal(gcgame.getActiveTeam())):
                self.addUserComment("USER_DIPLOCOMMENT_RESEARCH", -1, -1)

            if (gcgetTeam(gcgame.getActiveTeam()).AI_shareWar(gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam())):
            ...
            for i in range(gc.getNumTechInfos()):
                if (gcgetPlayer(self.diploScreen.getWhoTradingWith()).canResearch(i, False)):
            ...
            for i in range(gc.getMAX_CIV_PLAYERS()):
                if (gcgetPlayer(i).isAlive()):
                    if ((i != gcgame.getActivePlayer()) and (i != self.diploScreen.getWhoTradingWith())):
                        if (gcgetTeam(gcgame.getActiveTeam()).isHasMet(gcgetPlayer(i).getTeam()) and gcgetTeam(gcgetPlayer(self.diploScreen.getWhoTradingWith()).getTeam()).isHasMet(gcgetPlayer(i).getTeam())):
                            self.addUserComment("USER_DIPLOCOMMENT_ATTITUDE_PLAYER", i, -1, gcgetPlayer(i).getNameKey())
        ...
        # The default...
            ...
            if (gcgetPlayer(gcgame.getActivePlayer()).canTradeWith(self.diploScreen.getWhoTradingWith())):
    ...
    def setAIComment(self, eComment, *args):
        ...
        if DebugLogging:
            print
            "CvDiplomacy.setAIComment: %s" % (eComment,)
            if (len(args)):
                print
                "args", args
    ...
    def filterUserResponse(self, diploInfo):
        ...
        theirPlayer = gcgetPlayer(self.diploScreen.getWhoTradingWith())
        ...
        for i in range(diploInfo.getNumResponses()):
            ...
            if (self.isUsed(diploInfo.getAttitudeTypes, i, AttitudeTypes.NUM_ATTITUDE_TYPES)):
                att = theirPlayer.AI_getAttitude(GlobalCyGame.getActivePlayer())
    ...
    def handleUserResponse(self, eComment, iData1, iData2):
        ...
        # if we are demanding something
            ...
            if (diploScreen.offerDeal()):
                ...
                print
                "asdf " + str(self.diploScreen.getWhoTradingWith()) + " " + str(gcgame.getActivePlayer())

                if gcgame.getActivePlayer() in [iXiongnu]:
                    vic.onPlayerDemandAccepted(gcgetPlayer(self.diploScreen.getWhoTradingWith()), gcgame.getActivePlayer())
        ...
        # if we are demanding something with the threat of war
            ...
            if (diploScreen.offerDeal()):
                ...
                print
                "asdf " + str(gcgetPlayer(self.diploScreen.getWhoTradingWith())) + " " + str(gcgame.getActivePlayer())
        ...
        # If we reject their demand
            ...
            if (gcgetPlayer(self.diploScreen.getWhoTradingWith()).AI_demandRebukedWar(gcgame.getActivePlayer())):
                self.setAIComment(self.getCommentID("AI_DIPLOCOMMENT_DECLARE_WAR"))
                diploScreen.diploEvent(DiploEventTypes.DIPLOEVENT_DEMAND_WAR, -1, -1)
            else:
        ...
        # If we want to ask them to what their attitude is on a specific player
            ...
            eAttitude = gcgetPlayer(self.diploScreen.getWhoTradingWith()).AI_getAttitude(iData1)

            if (eAttitude == AttitudeTypes.ATTITUDE_FURIOUS):
                self.setAIComment(self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_FURIOUS"), gcgetPlayer(iData1).getNameKey())
            elif (eAttitude == AttitudeTypes.ATTITUDE_ANNOYED):
                self.setAIComment(self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_ANNOYED"), gcgetPlayer(iData1).getNameKey())
            elif (eAttitude == AttitudeTypes.ATTITUDE_CAUTIOUS):
                self.setAIComment(self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_CAUTIOUS"), gcgetPlayer(iData1).getNameKey())
            elif (eAttitude == AttitudeTypes.ATTITUDE_PLEASED):
                self.setAIComment(self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_PLEASED"), gcgetPlayer(iData1).getNameKey())
            else:
                self.setAIComment(self.getCommentID("AI_DIPLOCOMMENT_ATTITUDE_PLAYER_FRIENDLY"), gcgetPlayer(iData1).getNameKey())
