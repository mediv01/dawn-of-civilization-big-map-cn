            ...
            if (pHeadSelectedCity):
                ...
                for i in range(gc.getNumBonusInfos()):
                    ...
                    if (pHeadSelectedCity.hasBonus(i)):
                        ...
                        if (iHappiness != 0):
                            if (iHappiness > 0):
                                szTempBuffer = u"<font=1>+%d%c</font>" % (iHappiness, GlobalCyGame.getSymbolID(FontSymbols.HAPPY_CHAR))
                            else:
                                szTempBuffer = u"<font=1>+%d%c</font>" % (-iHappiness, GlobalCyGame.getSymbolID(FontSymbols.UNHAPPY_CHAR))

                            if (iHealth > 0):
                                szTempBuffer += u"<font=1>, +%d%c</font>" % (iHealth, GlobalCyGame.getSymbolID(FontSymbols.HEALTHY_CHAR))
                        ...
                        if (iHealth != 0 and bHandled == False):
                            if (iHealth > 0):
                                ...
                                szTempBuffer = u"<font=1>+%d%c</font>" % (-iHealth, GlobalCyGame.getSymbolID(FontSymbols.UNHEALTHY_CHAR))
                ...
                szBuffer = u"%d%% %s" % (pHeadSelectedCity.plot().calculateCulturePercent(pHeadSelectedCity.getOwner()), gcgetPlayer(pHeadSelectedCity.getOwner()).getCivilizationAdjective(0))
                ...
                for h in range(gc.getMAX_PLAYERS()):
                    if (gcgetPlayer(h).isAlive()):
                        iPercent = pHeadSelectedCity.plot().calculateCulturePercent(h)
                        if (iPercent > 0):
                            screen.setStackedBarColorsRGB("NationalityBar", iWhichBar, gcgetPlayer(h).getPlayerTextColorR(), gcgetPlayer(h).getPlayerTextColorG(), gcgetPlayer(h).getPlayerTextColorB(),
                                                          gcgetPlayer(h).getPlayerTextColorA())
                            if (iRemainder <= 0):
                        ...
                        while (j > -1):
                            ...
                            if (not GlobalCyInterface.isScoresMinimized() or gcgame.getActivePlayer() == ePlayer):
                                ...
                                if (gcgetPlayer(ePlayer).isEverAlive() and not gcgetPlayer(ePlayer).isBarbarian()
                                    ...
                                    if (not gcgetPlayer(ePlayer).isMinorCiv() or ScoreOpt.isShowMinorCivs()):
                                        # BUG - Minor Civs - end
                                            ...
                                            if (gcgetPlayer(ePlayer).isAlive()):
                                                ...
                                                if (gcgetTeam(eTeam).isAlive()):
                                                    ...
                                                    if (gcgetPlayer(ePlayer).getStateReligion() != -1):
                                                        if (gcgetPlayer(ePlayer).hasHolyCity(gcgetPlayer(ePlayer).getStateReligion())):
                                                ...
                                                if (((gcgetPlayer(ePlayer).getTeam() == gcgame.getActiveTeam()) and (gcgetTeam(gcgame.getActiveTeam()).getNumMembers() > 1)) or (
                                                    ...
                                                    if (gcgetPlayer(ePlayer).getCurrentResearch() != -1):
                                                        ...
                                                        szBuffer = szBuffer + szTempBuffer
                                                        if (bAlignIcons):
                                                            scores.setResearch(gcgetPlayer(ePlayer).getCurrentResearch(), gcgetPlayer(ePlayer).getResearchTurnsLeft(gcgetPlayer(ePlayer).getCurrentResearch(), True))
