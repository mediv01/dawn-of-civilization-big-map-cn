...
class CvMilitaryAdvisor:
    ...
    def interfaceScreen(self):
        ...
        self.H_MAP = (self.W_MAP * GlobalCyMap.getGridHeight()) / GlobalCyMap.getGridWidth()
        if (self.H_MAP > self.H_MAP_MAX):
            self.W_MAP = (self.H_MAP_MAX * GlobalCyMap.getGridWidth()) / GlobalCyMap.getGridHeight()
        ...
        iOldMode = GlobalCyInterface.getShowInterface()
        GlobalCyInterface.setShowInterface(InterfaceVisibility.INTERFACE_MINIMAP_ONLY)
        screen.updateMinimapVisibility()
        GlobalCyInterface.setShowInterface(iOldMode)
    ...
    def drawCombatExperience(self):
        ...
        if (gcgetPlayer(self.iActivePlayer).greatPeopleThreshold(True) > 0):
            iExperience = gcgetPlayer(self.iActivePlayer).getCombatExperience()

            screen = self.getScreen()
            screen.addStackedBarGFC(self.GREAT_GENERAL_BAR_ID, self.X_GREAT_GENERAL_BAR, self.Y_GREAT_GENERAL_BAR, self.W_GREAT_GENERAL_BAR, self.H_GREAT_GENERAL_BAR, InfoBarTypes.NUM_INFOBAR_TYPES, WidgetTypes.WIDGET_HELP_GREAT_GENERAL,
                                    -1, -1)
            ...
            screen.setBarPercentage(self.GREAT_GENERAL_BAR_ID, InfoBarTypes.INFOBAR_STORED, float(iExperience) / float(gcgetPlayer(self.iActivePlayer).greatPeopleThreshold(True)))
            screen.setLabel(self.GREAT_GENERAL_LABEL_ID, "", localText.getText("TXT_KEY_MISC_COMBAT_EXPERIENCE", ()), CvUtil.FONT_CENTER_JUSTIFY, self.X_GREAT_GENERAL_BAR + self.W_GREAT_GENERAL_BAR / 2, self.Y_GREAT_GENERAL_BAR + 6, 0,
                            FontTypes.GAME_FONT, WidgetTypes.WIDGET_HELP_GREAT_GENERAL, -1, -1)
    ...
    def resetMinimapColor(self):
        screen = self.getScreen()
        for iX in range(gcmap.getGridWidth()):
            for iY in range(gcmap.getGridHeight()):
    ...
    def isSelectedUnit(self, iPlayer, iUnitId, bIndirect):
        if (bIndirect):
            if -1 in self.selectedGroupList:
                return True
            unit = gcgetPlayer(iPlayer).getUnit(iUnitId)
    ...
    def refreshUnitSelection(self, bReload):
        ...
        if (bReload):
            if (self.bUnitDetails):
                ...
                screen.setLabel(self.UNIT_BUTTON_LABEL_ID, "", localText.getText("TXT_KEY_MILITARY_ADVISOR_UNIT_TOGGLE_OFF", ()), CvUtil.FONT_LEFT_JUSTIFY, self.X_TEXT + self.MAP_MARGIN + 22, self.Y_TEXT + self.MAP_MARGIN / 2 + 2, 0,
                ...
                screen.setLabel(self.UNIT_BUTTON_LABEL_ID, "", localText.getText("TXT_KEY_MILITARY_ADVISOR_UNIT_TOGGLE_ON", ()), CvUtil.FONT_LEFT_JUSTIFY, self.X_TEXT + self.MAP_MARGIN + 22, self.Y_TEXT + self.MAP_MARGIN / 2 + 2, 0,
                                FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
            ...
            for iPlayer in range(gc.getMAX_PLAYERS()):
                ...
                if (player.isAlive()):
                    ...
                    for loopUnit in unitList:
                        ...
                        plot = loopUnit.plot()
                        if (not plot.isNone()):
                            bVisible = plot.isVisible(gcgetPlayer(self.iActivePlayer).getTeam(), False) and not loopUnit.isInvisible(gcgetPlayer(self.iActivePlayer).getTeam(), False)
        ...
        for iUnit in range(gc.getNumUnitInfos()):
            if (len(self.unitsList[iUnit][2]) > 0):
                ...
                for loopUnit in self.unitsList[iUnit][2]:
                    ...
                    if (self.bUnitDetails):
                        szDescription = GlobalCyGameTextMgr.getSpecificUnitHelp(loopUnit, True, False)
                    ...
                    player = PyPlayer(iPlayer)
                    iColor = gc.getPlayerColorInfo(gcgetPlayer(iPlayer).getPlayerColor()).getColorTypePrimary()
                    ...
                    if (self.isSelectedUnit(loopUnit.getOwner(), loopUnit.getID(), True) and (iPlayer in self.selectedPlayerList)):
                        ...
                        if (player.getTeam().isAtWar(gcgetPlayer(self.iActivePlayer).getTeam())):
                            iColor = gc.getInfoTypeForString("COLOR_RED")
                        elif (gcgetPlayer(iPlayer).getTeam() != gcgetPlayer(self.iActivePlayer).getTeam()):
    ...
    def refresh(self, bReload):
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            ...
            if (player.isAlive() and (gcgetTeam(player.getTeam()).isHasMet(gcgetPlayer(self.iActivePlayer).getTeam()) or gcgame.isDebugMode())):
        ...
        for iIndex in range(iNumLeaders):
            ...
            if (bReload):
                if player.isBarbarian():
                    ...
                    # szButton = gc.getLeaderHeadInfo(gcgetPlayer(iLoopPlayer).getLeaderType()).getButton() #Rhye
                    szButton = gc.getCivilizationInfo(gcgetPlayer(iLoopPlayer).getCivilizationType()).getButton()  # Rhye
                screen.addCheckBoxGFC(self.getLeaderButton(iLoopPlayer), szButton, ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), x, y, iButtonSize, iButtonSize, WidgetTypes.WIDGET_MINIMAP_HIGHLIGHT, 2, iLoopPlayer,
