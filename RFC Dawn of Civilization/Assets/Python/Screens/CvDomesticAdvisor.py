...
class CvDomesticAdvisor:
    ...
    def interfaceScreen(self):
        ...
        screen.setText("DomesticExit", "Background", localText.getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper(), CvUtil.FONT_RIGHT_JUSTIFY, self.nScreenWidth - 25, self.nScreenHeight - 45, -0.1, FontTypes.TITLE_FONT,
        ...
        # if (bCanLiberate or gcgetPlayer(gcgame.getActivePlayer()).canSplitEmpire()):
    ...
    def drawHeaders(self):
        ...
        # Happiness Column
        screen.setTableColumnHeader("CityListBackground", 3, "<font=2>" + (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.HAPPY_CHAR)) + "</font>", (40 * self.nTableWidth) / self.nNormalizedTableWidth)

        # Health Column
        screen.setTableColumnHeader("CityListBackground", 4, "<font=2>" + (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.HEALTHY_CHAR)) + "</font>", (40 * self.nTableWidth) / self.nNormalizedTableWidth)
        ...
        # Trade Column
        screen.setTableColumnHeader("CityListBackground", 11, "<font=2>" + (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.TRADE_CHAR)) + "</font>", (35 * self.nTableWidth) / self.nNormalizedTableWidth)

        # Maintenance Column
        screen.setTableColumnHeader("CityListBackground", 12, "<font=2>" + (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.BAD_GOLD_CHAR)) + "</font>", (40 * self.nTableWidth) / self.nNormalizedTableWidth)

        # Great Person Column
        screen.setTableColumnHeader("CityListBackground", 13, "<font=2>" + (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR)) + "</font>", (70 * self.nTableWidth) / self.nNormalizedTableWidth)

        # Garrison Column
        screen.setTableColumnHeader("CityListBackground", 14, "<font=2>" + (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.DEFENSE_CHAR)) + "</font>", (35 * self.nTableWidth) / self.nNormalizedTableWidth)
    ...
    def updateTable(self, pLoopCity, i):
        ...
        if pLoopCity.isCapital():
            szName += (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.STAR_CHAR))
        elif pLoopCity.isGovernmentCenter():
            szName += (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.SILVER_STAR_CHAR))
        ...
        iCultureTimes100 = pLoopCity.getCultureTimes100(GlobalCyGame.getActivePlayer())
        ...
        if iGreatPersonRate > 0:
            iGPPLeft = gcgetPlayer(gcgame.getActivePlayer()).greatPeopleThreshold(False) - pLoopCity.getGreatPeopleProgress()
            if iGPPLeft > 0:
        ...
        if pLoopCity.getLiberationPlayer(False) != -1:
            screen.setTableText("CityListBackground", 16, i, "<font=2>" + (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.OCCUPATION_CHAR)) + "</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
    ...
    def drawSpecialists(self):
        ...
        for i in range(gc.getNumSpecialistInfos()):
            if (gc.getSpecialistInfo(i).isVisible()):
                ...
                screen.setLabel(szName, "Background", "", CvUtil.FONT_LEFT_JUSTIFY, self.nFirstSpecialistX + (self.nSpecialistDistance * i) + self.nSpecTextOffsetX, self.nSpecialistY + self.nSpecTextOffsetY, 0, FontTypes.GAME_FONT,
    ...
    def updateSpecialists(self):
        ...
        if (GlobalCyInterface.isOneCitySelected()):

            city = GlobalCyInterface.getHeadSelectedCity()
            ...
            for i in range(gc.getNumSpecialistInfos()):
                if (gc.getSpecialistInfo(i).isVisible()):
                    ...
                    szName = "SpecialistText" + str(i)
                    screen.setLabel(szName, "Background", str(city.getSpecialistCount(i)) + "/" + str(city.getMaxSpecialistCount(i)), CvUtil.FONT_LEFT_JUSTIFY, self.nFirstSpecialistX + (self.nSpecialistDistance * i) + self.nSpecTextOffsetX,
                                    self.nSpecialistY + self.nSpecTextOffsetY, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
                    screen.show(szName)
    ...
    def updateAppropriateCitySelection(self):
        nCities = gcgetPlayer(gcgame.getActivePlayer()).getNumCities()
        screen = CyGInterfaceScreen("DomesticAdvisor", CvScreenEnums.DOMESTIC_ADVISOR)
