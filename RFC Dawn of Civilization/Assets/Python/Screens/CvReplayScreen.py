...
class CvReplayScreen:
    ...
    def showScreen(self, bFromHallOfFame):
        ...
        if not bFromHallOfFame:
            self.replayInfo = GlobalCyGame.getReplayInfo()
            if self.replayInfo.isNone():
                self.replayInfo = CyReplayInfo()
                self.replayInfo.createInfo(gcgame.getActivePlayer())
        ...
        screen.setLabel(self.szHeader, "Background", u"<font=4b>" + localText.getText("TXT_KEY_REPLAY_SCREEN_TITLE", ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.Y_TITLE, self.Z_CONTROLS, FontTypes.TITLE_FONT,
        ...
        screen.addGraphWidget(self.szGraph, "Background", ArtFileMgr.getInterfaceArtInfo("POPUPS_BACKGROUND_TRANSPARENT").getPath(), self.X_GRAPH, self.Y_GRAPH, self.Z_CONTROLS, self.W_GRAPH, self.H_GRAPH, WidgetTypes.WIDGET_GENERAL, -1,
                              -1)
    ...
    def showEvents(self, iTurn, bSilent):
        ...
        szTurnDate = GlobalCyGameTextMgr.getDateStr(self.iTurn, False, self.replayInfo.getCalendar(), self.replayInfo.getStartYear(), self.replayInfo.getGameSpeed())
        ...
        for iLoopEvent in events:
            ...
            szEventDate = GlobalCyGameTextMgr.getDateStr(self.replayInfo.getReplayMessageTurn(iLoopEvent), False, self.replayInfo.getCalendar(), self.replayInfo.getStartYear(), self.replayInfo.getGameSpeed())
        ...
        while (iLoopTurn <= self.iTurn):
            ...
            for iLoopPlayer in range(self.replayInfo.getNumPlayers()):
                if gcgetPlayer(iLoopPlayer).isMinorCiv(): continue  # Leoreth
