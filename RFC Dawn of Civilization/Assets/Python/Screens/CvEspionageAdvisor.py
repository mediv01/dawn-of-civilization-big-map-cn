...
class CvEspionageAdvisor:
    ...
    def interfaceScreen(self):
        ...
        self.pActivePlayer = gcgetPlayer(self.iActivePlayer)
        self.iActiveTeam = self.pActivePlayer.getTeam()
        self.pActiveTeam = gcgetTeam(self.iActiveTeam)
        ...
        screen.addDDSGFC("EspionageBackground", GlobalCyArtFileMgr.getInterfaceArtInfo('SCREEN_BG_OPAQUE').getPath(), 0, 0, self.W_SCREEN, self.H_SCREEN, WidgetTypes.WIDGET_GENERAL, -1, -1)
        ...
        if GlobalCyGame.isDebugMode():
            ...
            for iPlayer in xrange(gc.getMAX_CIV_PLAYERS()):
                if gcgetPlayer(iPlayer).isAlive():
                    ...
                    screen.addPullDownString("DebugPlayerSelect", utils.getLeaderNameOrCivName(iPlayer), iPlayer, iPlayer, False)
    ...
    def updateLeftPanel(self):
        ...
        if self.pActivePlayer.isCommerceFlexible(CommerceTypes.COMMERCE_ESPIONAGE):
            screen.setButtonGFC("EspionagePlus", "", "", self.X_LEFT_PANE + 17, self.Y_LEFT_PANE + 1, 20, 20, WidgetTypes.WIDGET_GENERAL, CommerceTypes.COMMERCE_ESPIONAGE, gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS"),
                                ButtonStyles.BUTTON_STYLE_CITY_PLUS)
            screen.setButtonGFC("EspionageMinus", "", "", self.X_LEFT_PANE + 37, self.Y_LEFT_PANE + 1, 20, 20, WidgetTypes.WIDGET_GENERAL, CommerceTypes.COMMERCE_ESPIONAGE, -gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS"),
                                ButtonStyles.BUTTON_STYLE_CITY_MINUS)

        szRivalLabel = GlobalCyTranslator.changeTextColor("vs. You", gc.getInfoTypeForString('COLOR_SELECTED_TEXT'))
        screen.setLabel("EspionageRivalLabel", "EspionageHeader", u"<font=2>" + szRivalLabel + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_RIVAL_STATS + 20, self.Y_LEFT_PANE + 4, -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1,
                        -1)
        szPlayerLabel = GlobalCyTranslator.changeTextColor("vs. Them", gc.getInfoTypeForString('COLOR_SELECTED_TEXT'))
        screen.setLabel("EspionagePlayerLabel", "EspionageHeader", u"<font=2>" + szPlayerLabel + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_PLAYER_STATS + 20, self.Y_LEFT_PANE + 4, -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL,
                        -1, -1)
        ...
        for iRival in range(iNumPlayers):
            pRival = gcgetPlayer(iRival)
            iRivalTeam = pRival.getTeam()
            pRivalTeam = gcgetTeam(iRivalTeam)
            ...
            if pRival.isAlive() and self.pActiveTeam.isHasMet(iRivalTeam):
                ...
                screen.addCheckBoxGFCAt(PlayerPanel, LeaderIcon, gc.getLeaderHeadInfo(pRival.getLeaderType()).getButton(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), 0, 0, self.ICON_SIZE, self.ICON_SIZE,
                                        WidgetTypes.WIDGET_ESPIONAGE_SELECT_PLAYER, iRival, self.iActivePlayer, ButtonStyles.BUTTON_STYLE_LABEL, False)
                screen.setState(LeaderIcon, self.iTargetPlayer == iRival)

                # Leader Info
                # szLeaderName =  u"<font=3b>" + pRival.getName() + u"</font>"
                from RFCUtils import utils
                szLeaderName = u"<font=3b>" + utils.getLeaderNameOrCivName(iRival) + u"</font>"
                ...
                # Weight Buttons
                screen.setImageButtonAt("WeightIncrease" + str(iRival), PlayerPanel, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_PLUS").getPath(), self.X_PLAYER_INFO, self.Y_PLAYER_INFO + (self.LINE_SPACING * 2),
                                        self.WEIGHT_BUTTON_SIZE, self.WEIGHT_BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, iRivalTeam + 1, self.iIncrement)
                screen.setImageButtonAt("WeightDecrease" + str(iRival), PlayerPanel, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_MINUS").getPath(), self.X_PLAYER_INFO + self.WEIGHT_BUTTON_SIZE,
                                        self.Y_PLAYER_INFO + (self.LINE_SPACING * 2), self.WEIGHT_BUTTON_SIZE, self.WEIGHT_BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, iRivalTeam + 1, self.iIncrement)
                ...
                screen.setLabelAt("RivalAccumulated" + str(iRival), PlayerPanel, u"<font=2>" + szRivalAmount + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_RIVAL_STATS, self.Y_PLAYER_INFO, -0.1, FontTypes.SMALL_FONT,
                ...
                screen.setLabelAt("PlayerAccumulated" + str(iRival), PlayerPanel, u"<font=2>" + szPlayerAmount + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_PLAYER_STATS, self.Y_PLAYER_INFO, -0.1, FontTypes.SMALL_FONT,
                                  WidgetTypes.WIDGET_GENERAL, -1, -1)

                # Espionage Cost Modifier
                szCost = GlobalCyTranslator.changeTextColor("Cost: ", gc.getInfoTypeForString('COLOR_SELECTED_TEXT'))
                szRivalModifier = " - "
                ...
                if self.pActivePlayer.canDoEspionageMission(iSeeEspionage, iRival, pRival.getCapitalCity().plot(), -1):
                    ...
                    if iRivalTurns > 0:
                        ...
                        szRivalModifier = GlobalCyTranslator.changeTextColor(str(iRivalModifier) + "%", gc.getInfoTypeForString('COLOR_POSITIVE_TEXT'))
                        szRivalModifier += u" (%d)" % iRivalTurns
                    elif iRivalModifier < 100:
                        szRivalModifier = szCost + GlobalCyTranslator.changeTextColor(str(iRivalModifier) + "%", gc.getInfoTypeForString('COLOR_NEGATIVE_TEXT'))
                    else:
                        szRivalModifier = szCost + u"%d%%" % iRivalModifier

                screen.setLabelAt("RivalEspionageMultiplier" + str(iRival), PlayerPanel, u"<font=2>" + szRivalModifier + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_RIVAL_STATS, self.Y_PLAYER_INFO + (self.LINE_SPACING * 2), -0.1,
                ...
                if iPlayerTurns > 0:
                    ...
                    szPlayerModifier = GlobalCyTranslator.changeTextColor(str(iPlayerModifier) + "%", gc.getInfoTypeForString('COLOR_NEGATIVE_TEXT'))
                    szPlayerModifier += u" (%d)" % iPlayerTurns
                elif iPlayerModifier > 100:
                    szPlayerModifier = szCost + GlobalCyTranslator.changeTextColor(str(iPlayerModifier) + "%", gc.getInfoTypeForString('COLOR_NEGATIVE_TEXT'))
                else:
                    szPlayerModifier = szCost + u"%d%%" % iPlayerModifier

                screen.setLabelAt("PlayerEspionageMultiplier" + str(iRival), PlayerPanel, u"<font=2>" + szPlayerModifier + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_PLAYER_STATS, self.Y_PLAYER_INFO + (self.LINE_SPACING * 2), -0.1,
                                  FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def updateRightPanel(self):
        ...
        if self.iTargetPlayer > -1:
            pTargetPlayer = gcgetPlayer(self.iTargetPlayer)
            ...
            while (loopCity):
                if loopCity.isRevealed(self.iActiveTeam, False):
                    ...
                    if loopCity.isCapital():
                        szStatus = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.STAR_CHAR)
                    elif loopCity.isGovernmentCenter():
                        szStatus = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.SILVER_STAR_CHAR)
                    else:
                        szStatus = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)

                    # Name
                    szName = loopCity.getName()
                    if self.iTargetCity == loopCity.getID():
                        szName = GlobalCyTranslator.changeTextColor(szName, gc.getInfoTypeForString('COLOR_HIGHLIGHT_TEXT'))
                    ...
                    if self.iMission > -1:
                        ...
                        if iCost > -1:
                            if iEspionage < iCost:
                                szCost = GlobalCyTranslator.changeTextColor(str(iCost), gc.getInfoTypeForString('COLOR_NEGATIVE_TEXT'))
                            else:
                                szCost = GlobalCyTranslator.changeTextColor(str(iCost), gc.getInfoTypeForString('COLOR_POSITIVE_TEXT'))
            ...
            # Go to City Button
            screen.setButtonGFC("ViewButton", "Go to City", "", self.X_GO_TO_CITY, self.Y_GO_TO_CITY, self.W_GO_TO_CITY, self.H_GO_TO_CITY, WidgetTypes.WIDGET_GO_TO_CITY, self.iTargetPlayer, self.iTargetCity,
            ...
            screen.setButtonGFC("InvestigateButton", "Investigate City", "", self.X_INVESTIGATE_CITY, self.Y_INVESTIGATE_CITY, self.W_INVESTIGATE_CITY, self.H_INVESTIGATE_CITY, WidgetTypes.WIDGET_ZOOM_CITY, self.iTargetPlayer,
                                self.iTargetCity, ButtonStyles.BUTTON_STYLE_STANDARD)
            ...
            for iLoopMission in xrange(gc.getNumEspionageMissionInfos()):
                ...
                if pMission.getCost() > -1:
                    ...
                    if iLoopMission == self.iMission:
                        szMission = GlobalCyTranslator.changeTextColor(pMission.getDescription(), gc.getInfoTypeForString('COLOR_HIGHLIGHT_TEXT'))
                    else:
    ...
    def updateEspionageWeights(self):
        ...
        szEspionagePercent += GlobalCyTranslator.getText("TXT_KEY_MISC_POS_GOLD_PER_TURN", (self.pActivePlayer.getCommerceRate(CommerceTypes.COMMERCE_ESPIONAGE),))
        screen.setLabel("EspionageInvestment", "EspionageHeader", u"<font=2>" + szEspionagePercent + "</font>", CvUtil.FONT_LEFT_JUSTIFY, self.X_PLAYER_INFO + 20, self.Y_LEFT_PANE + 3, -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL,
                        -1, -1)
        ...
        for iRival in xrange(gc.getMAX_CIV_PLAYERS()):
            ...
            if pRival.isAlive() and self.pActiveTeam.isHasMet(iRivalTeam):
                ...
                # Espionage Weight
                szWeight = GlobalCyTranslator.changeTextColor("Weight: ", gc.getInfoTypeForString('COLOR_SELECTED_TEXT'))
                iWeight = self.pActivePlayer.getEspionageSpendingWeightAgainstTeam(iRivalTeam)
                if iWeight > 0:
                    szWeight += GlobalCyTranslator.changeTextColor(str(iWeight), gc.getInfoTypeForString('COLOR_POSITIVE_TEXT'))
                else:
                    szWeight += str(iWeight)

                screen.setLabelAt("EspionageWeight" + str(iRival), PlayerPanel, u"<font=2>" + szWeight + "</font>", CvUtil.FONT_LEFT_JUSTIFY, self.X_PLAYER_INFO + (self.WEIGHT_BUTTON_SIZE * 2) + 2,
                                  self.Y_PLAYER_INFO + (self.LINE_SPACING * 2), -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
                ...
                screen.setLabelAt("RivalEspionageRate" + str(iRival), PlayerPanel, u"<font=2>" + szRivalRate + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_RIVAL_STATS, self.Y_PLAYER_INFO + self.LINE_SPACING, -0.1, FontTypes.SMALL_FONT,
                                  WidgetTypes.WIDGET_GENERAL, -1, -1)
                ...
                if iPlayerRate > 0:
                    if iWeight > 0:
                        szPlayerRate = GlobalCyTranslator.changeTextColor(u"+%d/Turn" % iPlayerRate, gc.getInfoTypeForString('COLOR_POSITIVE_TEXT'))
                    else:
                        szPlayerRate = u"+%d/Turn" % iPlayerRate

                screen.setLabelAt("PlayerEspionageRate" + str(iRival), PlayerPanel, u"<font=2>" + szPlayerRate + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_PLAYER_STATS, self.Y_PLAYER_INFO + self.LINE_SPACING, -0.1, FontTypes.SMALL_FONT,
                                  WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def changeEspionageWeight(self, iTargetTeam, iChange):
        ...
        GlobalCyInterface.setDirty(InterfaceDirtyBits.Espionage_Advisor_DIRTY_BIT, True)

    def resetEspionageWeights(self):
        iActivePlayer = GlobalCyGame.getActivePlayer()
        ...
        for iRival in xrange(gc.getMAX_CIV_PLAYERS()):
            ...
            if pRival.isAlive() and gcgetTeam(iActiveTeam).isHasMet(iRivalTeam):
                iChange = -1 * gcgetPlayer(iActivePlayer).getEspionageSpendingWeightAgainstTeam(iRivalTeam) + PYTHON_ESPION_RESET_DEFAULT_WEIGHT
    ...
    def handleInput(self, inputClass):
        ...
        if inputClass.getFunctionName() == "EspionagePlus" or inputClass.getFunctionName() == "EspionageMinus":
            ...
            screen.hideScreen()
            pCity = gcgetPlayer(inputClass.getData1()).getCity(inputClass.getData2())
            ...
            if len(self.CitySpies) > 0:
                ...
                for pSpy in self.CitySpies:
                    if not pSpy.hasMoved():
                        pSelectedUnit = pSpy
                        break
                GlobalCyInterface.selectUnit(pSelectedUnit, True, True, False)

        elif inputClass.getButtonType() == WidgetTypes.WIDGET_ZOOM_CITY:
            screen.hideScreen()
            GlobalCyInterface.selectCity(gcgetPlayer(inputClass.getData1()).getCity(inputClass.getData2()), True)
    ...
    def update(self, fDelta):
        if GlobalCyInterface.isDirty(InterfaceDirtyBits.Espionage_Advisor_DIRTY_BIT):
            self.updateEspionageWeights()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.Espionage_Advisor_DIRTY_BIT, False)
        return

    def drawEspionageExperience(self):
        if (gcgetPlayer(self.iActivePlayer).greatPeopleThreshold(True) > 0):
            screen = self.getScreen()

            iExperience = gcgetPlayer(self.iActivePlayer).getEspionageExperience()
            ...
            screen.setBarPercentage("Great Spy Bar", InfoBarTypes.INFOBAR_STORED, float(iExperience) / float(gcgetPlayer(self.iActivePlayer).greatSpyThreshold()))

            screen.setLabel("Great Spy Text", "", localText.getText("TXT_KEY_MISC_ESPIONAGE_EXPERIENCE", ()), CvUtil.FONT_CENTER_JUSTIFY, self.X_GREAT_SPY_BAR + self.W_GREAT_SPY_BAR / 2, self.Y_GREAT_SPY_BAR + self.H_GREAT_SPY_BAR / 3 - 2,
                            0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_HELP_GREAT_SPY, -1, -1)
