...
class CvExoticForeignAdvisor(CvForeignAdvisor.CvForeignAdvisor):
            ...
            for j in range(gc.getMAX_PLAYERS()):
                if (gcgetPlayer(j).isAlive()):
                    ...
                    if j == self.iActiveLeader:
                        bSelected = True
                    screen.addPullDownString(self.szDropdownName, gcgetPlayer(j).getCivilizationShortDescription(0), j, j, bSelected)
    ...
    def drawContents(self, bInitial):
        ...
        self.objActiveLeader = gcgetPlayer(self.iActiveLeader)
        self.iActiveTeam = self.objActiveLeader.getTeam()
        self.objActiveTeam = gcgetTeam(self.iActiveTeam)
        ...
        for i in range(len(self.ORDER_LIST)):
            ...
            if (self.iScreen != self.SCREEN_DICT[szScreen]):
                screen.setText(szTextId, "", u"<font=4>" + localText.getText(self.TXT_KEY_DICT[szScreen], ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_FOREIGN_ADVISOR,
                               self.SCREEN_DICT[szScreen], -1)
            else:
                screen.setText(szTextId, "", u"<font=4>" + localText.getColorText(self.TXT_KEY_DICT[szScreen], (), gc.getInfoTypeForString("COLOR_YELLOW")).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0,
        ...
        for j in range(nNumPLayers):
            ...
            for i in range(gcgame.getIndexAfterLastDeal()):
                ...
                if (deal.getFirstPlayer() == iLoopPlayer and deal.getSecondPlayer() == self.iActiveLeader and not deal.isNone()) or (deal.getSecondPlayer() == iLoopPlayer and deal.getFirstPlayer() == self.iActiveLeader):
                    szDealText = GlobalCyGameTextMgr.getDealString(deal, iLoopPlayer)
    ...
    def drawInfoOriginal(self, bInitial):
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            if (gcgetPlayer(iLoopPlayer).isAlive() and iLoopPlayer != self.iActiveLeader and (
                ...
                nPlayerReligion = gcgetPlayer(iLoopPlayer).getStateReligion()
                objReligion = gc.getReligionInfo(nPlayerReligion)

                objLeaderHead = gc.getLeaderHeadInfo(gcgetPlayer(iLoopPlayer).getLeaderType())
                ...
                if (nPlayerReligion != -1):
                    ...
                    if (gcgetPlayer(iLoopPlayer).hasHolyCity(nPlayerReligion)):
                ...
                for nCivicOption in ltCivicOptions:
                    nCivic = gcgetPlayer(iLoopPlayer).getCivics(nCivicOption)
                ...
                if not gcgame.isOption(GameOptionTypes.GAMEOPTION_RANDOM_PERSONALITIES):
                    nFavoriteCivic = objLeaderHead.getFavoriteCivic()
                    if (nFavoriteCivic != -1) and (not gcgame.isOption(GameOptionTypes.GAMEOPTION_RANDOM_PERSONALITIES)):
    ...
    def drawInfoImproved(self, bInitial):
        ...
        for headerText in (BugUtil.getPlainText("TXT_KEY_FOREIGN_ADVISOR_ABBR_LEADER"),
                           ...
                           u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.RELIGION_CHAR)),
                           u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.TRADE_CHAR)),
                           u"%c%c" % (GlobalCyGame.getSymbolID(FontSymbols.TRADE_CHAR), gc.getYieldInfo(YieldTypes.YIELD_COMMERCE).getChar()),
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            if (iLoopPlayer != self.iActiveLeader):
                objLoopPlayer = gcgetPlayer(iLoopPlayer)
                if (self.objActiveTeam.isHasMet(objLoopPlayer.getTeam()) or gcgame.isDebugMode()):
    ...
    def drawInfoRow(self, screen, mainPanelName, iLoopPlayer, ePanelStyle):
        ...
        objLoopTeam = gcgetTeam(iLoopTeam)
        bIsActivePlayer = (iLoopPlayer == self.iActiveLeader)
        if (objLoopPlayer.isAlive()
                # and (self.objActiveTeam.isHasMet(iLoopTeam) or gcgame.isDebugMode())
            ...
            itemName = self.getNextWidgetName()
            if (not self.objActiveTeam.isHasMet(iLoopTeam) and not gcgame.isDebugMode()):
    ...
    def initializeGlance(self):
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            if (gcgetPlayer(iLoopPlayer).isAlive()
                    and (gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isHasMet(gcgetPlayer(self.iActiveLeader).getTeam())
                         or gcgame.isDebugMode())
                    and not gcgetPlayer(iLoopPlayer).isBarbarian()
                    and not gcgetPlayer(iLoopPlayer).isMinorCiv()):

                #				ExoticForPrint ("Player = %d" % iLoopPlayer)
                self.ltPlayerMet[iLoopPlayer] = True

                for nHost in range(gc.getMAX_PLAYERS()):
                    if (gcgetPlayer(nHost).isAlive()
                            and nHost != self.iActiveLeader
                            and (gcgetTeam(gcgetPlayer(nHost).getTeam()).isHasMet(gcgetPlayer(self.iActiveLeader).getTeam())
                                 or gcgame.isDebugMode())
                            and not gcgetPlayer(nHost).isBarbarian()
                            and not gcgetPlayer(nHost).isMinorCiv()):
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            if self.ltPlayerMet[iLoopPlayer]:
                if (iLoopPlayer != self.iActiveLeader):
                    ...
                    screen.addCheckBoxGFCAt(panelName, szName, gc.getLeaderHeadInfo(gcgetPlayer(iLoopPlayer).getLeaderType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                            self.X_GLANCE_OFFSET + (self.X_Spread * nCount), self.Y_GLANCE_OFFSET, self.GLANCE_BUTTON_SIZE, self.GLANCE_BUTTON_SIZE, WidgetTypes.WIDGET_LEADERHEAD, iLoopPlayer, self.iActiveLeader,
    ...
    def drawGlanceRows(self, screen, mainPanelName, bSorted=False, nPlayer=1):
        ...
        for i in range(self.nCount):
            ...
            if nCount > 8:
                screen.attachImageButton(playerPanelName, "", gc.getLeaderHeadInfo(gcgetPlayer(iLoopPlayer).getLeaderType()).getButton(), GenericButtonSizes.BUTTON_SIZE_32, WidgetTypes.WIDGET_LEADERHEAD, iLoopPlayer, self.iActiveLeader,
                                         False)
            else:
                screen.attachImageButton(playerPanelName, "", gc.getLeaderHeadInfo(gcgetPlayer(iLoopPlayer).getLeaderType()).getButton(), GenericButtonSizes.BUTTON_SIZE_46, WidgetTypes.WIDGET_LEADERHEAD, iLoopPlayer, self.iActiveLeader,
                                         False)
    ...
    def drawResourceDeals(self, bInitial):
        screen = self.getScreen()
        activePlayer = gcgetPlayer(self.iActiveLeader)
        self.initTradeTable()
        ...
        for iLoopBonus in range(gc.getNumBonusInfos()):
            ...
            for iLoopPlayer in range(gc.getMAX_PLAYERS()):
                currentPlayer = gcgetPlayer(iLoopPlayer)
                if (currentPlayer.isAlive() and not currentPlayer.isBarbarian()
                        and not currentPlayer.isMinorCiv()
                        and gcgetTeam(currentPlayer.getTeam()).isHasMet(activePlayer.getTeam())
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            ...
            if (currentPlayer.isAlive() and not currentPlayer.isBarbarian() and not currentPlayer.isMinorCiv()
                ...
                # gold
                if (gcgetTeam(activePlayer.getTeam()).isGoldTrading() or gcgetTeam(currentPlayer.getTeam()).isGoldTrading()):
                    sAmount = str(gcgetPlayer(iLoopPlayer).AI_maxGoldPerTurnTrade(self.iActiveLeader))
                ...
                if (self.RES_SHOW_ACTIVE_TRADE):
                    amount = 0
                    for iLoopDeal in range(gcgame.getIndexAfterLastDeal()):
                        deal = gcgame.getDeal(iLoopDeal)
                        # BUG - Kill Deal - start
                        if not deal.isNone():
    ...
    def drawTechDeals(self, bInitial):
        ...
        activeTeam = gcgetTeam(iActiveTeam)
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            ...
            currentTeam = gcgetTeam(iLoopTeam)

            if (currentPlayer.isAlive() and not currentPlayer.isBarbarian() and not currentPlayer.isMinorCiv()
                    and gcgetTeam(currentPlayer.getTeam()).isHasMet(activePlayer.getTeam())
                    and iLoopPlayer != self.iActiveLeader):
                message = ""
                if (not gcgetTeam(activePlayer.getTeam()).isTechTrading() and not gcgetTeam(currentPlayer.getTeam()).isTechTrading()):
                ...
                if (gcgetTeam(activePlayer.getTeam()).isGoldTrading() or gcgetTeam(currentPlayer.getTeam()).isGoldTrading()):
                    sAmount = str(gcgetPlayer(iLoopPlayer).AI_maxGoldTrade(self.iActiveLeader))
                    self.techIconGrid.setText(currentRow, iTechColGold, sAmount)

                if (gcgetTeam(activePlayer.getTeam()).isTechTrading() or gcgetTeam(currentPlayer.getTeam()).isTechTrading()):
                    ...
                    for iLoopTech in range(gc.getNumTechInfos()):
                        ...
                        elif (gcgetTeam(activePlayer.getTeam()).isHasTech(iLoopTech) and currentPlayer.canResearch(iLoopTech, False)):
                        ...
                        elif (gcgetTeam(currentPlayer.getTeam()).isHasTech(iLoopTech) and activePlayer.canResearch(iLoopTech, False)):
