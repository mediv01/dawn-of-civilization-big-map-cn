...
class WBStoredDataScreen:
    ...
    def interfaceScreen(self):
        ...
        screen.setLabel("StoredDataHeader", "Background", "<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_WB_STOREDDATA", ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, screen.getXResolution() / 2, 20, -0.1, FontTypes.TITLE_FONT,
                        WidgetTypes.WIDGET_GENERAL, -1, -1)
        screen.setText("StoredDataExit", "Background", "<font=4>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper() + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, screen.getXResolution() - 30, screen.getYResolution() - 42, -0.1,
        ...
        screen.setButtonGFC("RestoreBackup", GlobalCyTranslator.getText("TXT_KEY_WB_RESTORE_SD_BACKUP", ()), "", 20, 50 + 2 * 35, iWidth, 30, WidgetTypes.WIDGET_PYTHON, 22013, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
        screen.setButtonGFC("CreateBackup", GlobalCyTranslator.getText("TXT_KEY_WB_CREATE_SD_BACKUP", ()), "", 20, 50 + 3 * 35, iWidth, 30, WidgetTypes.WIDGET_PYTHON, 22014, -1, ButtonStyles.BUTTON_STYLE_STANDARD)

        # Warning texts
        sWarning = "<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_WB_STOREDDATA_WARNING", ()).upper() + "</font>"
        ...
        screen.addPullDownString("SelectMode", GlobalCyTranslator.getText("TXT_KEY_WB_GLOBAL", ()), 0, 0, iSelectedMode == 0)
        screen.addPullDownString("SelectMode", GlobalCyTranslator.getText("TXT_KEY_WB_CIVS", ()), 1, 1, iSelectedMode == 1)

        # Civ selection
        screen.addDropDownBoxGFC("SelectCiv", 20, 50 + 6 * 35 + 100, iWidth, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
        screen.addPullDownString("SelectCiv", GlobalCyTranslator.getText("TXT_KEY_CULTURELEVEL_NONE", ()), -1, -1, iSelectedCiv == -1)
        for i in range(iNumPlayers):
            screen.addPullDownString("SelectCiv", GlobalCyTranslator.getText(str(gcgetPlayer(i).getCivilizationShortDescriptionKey()), ()), i, i, iSelectedCiv == i)
        ...
        screen.addPullDownString("CurrentPage", GlobalCyTranslator.getText("TXT_KEY_PITBOSS_GAME_OPTIONS", ()), 10, 10, False)
        screen.addPullDownString("CurrentPage", GlobalCyTranslator.getText("TXT_KEY_INFO_SCREEN", ()), 11, 11, False)
        screen.addPullDownString("CurrentPage", GlobalCyTranslator.getText("TXT_KEY_WB_STOREDDATA", ()), 12, 12, True)
    ...
    def placeCivButton(self):
        ...
        if iSelectedCiv == -1:
            ArtKey = GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_CANCEL").getPath()
            screen.addDDSGFC("CivButtonButton", ArtKey, iX + 10, 85 + iHeight - iSize2 + 10, iSize, iSize, WidgetTypes.WIDGET_GENERAL, -1, -1)
        else:
            ArtKey = gc.getCivilizationInfo(gcgetPlayer(iSelectedCiv).getCivilizationType()).getButton()
            screen.addDDSGFC("CivButtonButton", ArtKey, iX + 10, 85 + iHeight - iSize2 + 10, iSize, iSize, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIV, gcgetPlayer(iSelectedCiv).getCivilizationType(), -1)
        ...
        for i in range(len(lInts)):
            ...
            if item == "iPlayer":
                ...
                if item == "iStabilityLevel":
                    sText += u" (%s)" % GlobalCyTranslator.getText(StabilityLevelTexts[scriptDict[item]], ())
                elif item in ["iNextTurnAIWar"]:
                    sText += u" (Turn %s)" % utils.getTurnForYear(scriptDict[item])
    ...
    def placeListTables(self):
        ...
        screen.addDDSGFCAt("WBArrow", "MainBG", GlobalCyArtFileMgr.getInterfaceArtInfo("LINE_ARROW").getPath(), screen.getXResolution() / 4 + iWidth + 20, screen.getYResolution() - 50 - iHeightMax / 2, 60, 24, WidgetTypes.WIDGET_GENERAL, -1,
                           -1, False)
        ...
        for i in range(len(lSelectedList)):
            ...
            if item == "lWarTrend":
                sText = GlobalCyTranslator.getText(str(gcgetPlayer(i).getCivilizationShortDescriptionKey()), ())
                screen.setTableText("WBListTableTwo", 0, i, sText, gc.getCivilizationInfo(gcgetPlayer(i).getCivilizationType()).getButton(), WidgetTypes.WIDGET_PYTHON, 22009, i, CvUtil.FONT_LEFT_JUSTIFY)
                ...
                sText = GlobalCyTranslator.getText(str(gcgetPlayer(lSecondaryCivs[i]).getCivilizationShortDescriptionKey()), ())
                screen.setTableText("WBListTableTwo", 0, i, sText, gc.getCivilizationInfo(gcgetPlayer(lSecondaryCivs[i]).getCivilizationType()).getButton(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
            elif item == "lFirstContactConquerors":  # New world civs conquerors
                sText = GlobalCyTranslator.getText(str(gcgetPlayer(lCivBioNewWorld[i]).getCivilizationShortDescriptionKey()), ())
                screen.setTableText("WBListTableTwo", 0, i, sText, gc.getCivilizationInfo(gcgetPlayer(lCivBioNewWorld[i]).getCivilizationType()).getButton(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
            elif item == "lFirstContactMongols":  # Mongol conquerors
                sText = GlobalCyTranslator.getText(str(gcgetPlayer(lMongolCivs[i]).getCivilizationShortDescriptionKey()), ())
                screen.setTableText("WBListTableTwo", 0, i, sText, gc.getCivilizationInfo(gcgetPlayer(lMongolCivs[i]).getCivilizationType()).getButton(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                ...
                sText = GlobalCyTranslator.getText(StabilityTypesTexts[i], ())
                screen.setTableText("WBListTableTwo", 0, i, sText, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
            elif item == "lWarStartTurn":
                sText = GlobalCyTranslator.getText(str(gcgetPlayer(i).getCivilizationShortDescriptionKey()), ())
                screen.setTableText("WBListTableTwo", 0, i, sText, gc.getCivilizationInfo(gcgetPlayer(i).getCivilizationType()).getButton(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
            elif item == "lConquest":
                sAttacker = GlobalCyTranslator.getText(str(gcgetPlayer(AIWars.lConquests[i][1]).getCivilizationShortDescriptionKey()), ())
                sDefender = GlobalCyTranslator.getText(str(gcgetPlayer(AIWars.lConquests[i][2]).getCivilizationShortDescriptionKey()), ())
            ...
            if bCiv:
                ...
                if lSelectedList[i] == -1:
                    screen.setTableText("WBListTableTwo", 1, i, sText, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_CANCEL").getPath(), WidgetTypes.WIDGET_PYTHON, 22008, i, CvUtil.FONT_LEFT_JUSTIFY)
                else:
                    screen.setTableText("WBListTableTwo", 1, i, sText, gc.getCivilizationInfo(gcgetPlayer(lSelectedList[i]).getCivilizationType()).getButton(), WidgetTypes.WIDGET_PYTHON, 22008, i, CvUtil.FONT_LEFT_JUSTIFY)
            else:
                sText = str(lSelectedList[i])
                if item in ["lGenericPlagueDates", "lWarStartTurn"]:
                    iYear = gcgame.getTurnYear(lSelectedList[i])
                    if iYear < 0:
                        sBC = GlobalCyTranslator.getText("TXT_KEY_BC", ())
                    else:
                        sBC = GlobalCyTranslator.getText("TXT_KEY_AD", ())
                    sText += u" (%d %s)" % (abs(gcgame.getTurnYear(lSelectedList[i])), sBC)
                elif item == "lGoals":
                    lVicConditionTexts = ["TXT_KEY_VICTORY_SCREEN_NOTYET", "TXT_KEY_VICTORY_GOAL_FAILED", "TXT_KEY_VICTORY_GOAL_ACCOMPLISHED"]
                    sText += u" (%s)" % GlobalCyTranslator.getText(lVicConditionTexts[lSelectedList[i] + 1], ())
        ...
        if inputClass.getFunctionName() == "CurrentPage":
            ...
            if iSelectedMode == 0:
                for iPlayer in range(iNumPlayers):
                    if not gcgetPlayer(iPlayer).isAlive(): continue
