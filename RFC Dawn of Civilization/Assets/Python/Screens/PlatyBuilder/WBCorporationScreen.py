...
class WBCorporationScreen:
    ...
    def interfaceScreen(self, iPlayerX):
        ...
        screen.setText("WBCorporationExit", "Background", "<font=4>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper() + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, screen.getXResolution() - 30, screen.getYResolution() - 42, -0.1,
                       FontTypes.TITLE_FONT, WidgetTypes.WIDGET_CLOSE_SCREEN, -1, -1)
        screen.setLabel("CorporationHeader", "Background", "<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_CONCEPT_CORPORATIONS", ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, screen.getXResolution() / 2, 20, -0.1, FontTypes.GAME_FONT,
                        WidgetTypes.WIDGET_GENERAL, -1, -1)
        screen.setLabel("HeadquarterHeader", "Background", "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_CORPORATION_HEADQUARTERS", ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, screen.getXResolution() / 8, self.iTable_Y - 30, -0.1,
        ...
        screen.addPullDownString("CurrentPage", GlobalCyTranslator.getText("TXT_KEY_CONCEPT_CORPORATIONS", ()), 9, 9, True)
        ...
        sText = "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_CORPORATION_HEADQUARTERS", ()) + "</font>"
    ...
    def placeHeadquarter(self):
        ...
        for i in xrange(gc.getNumCorporationInfos()):
            ...
            pHeadquarter = GlobalCyGame.getHeadquarters(i)
            if not pHeadquarter.isNone():
                ...
                screen.setTableText("WBHeadquarter", 2, iRow, "<font=3>" + sColor + pHeadquarter.getName() + "</color></font>", gc.getCivilizationInfo(pHeadquarter.getCivilizationType()).getButton(), WidgetTypes.WIDGET_PYTHON,
                                    7200 + iPlayerX, pHeadquarter.getID(), CvUtil.FONT_LEFT_JUSTIFY)
    ...
    def editHeadquarter(self, item, pCity):
        if pCity.isHeadquartersByType(item):
            GlobalCyGame.clearHeadquarters(item)
        else:
            GlobalCyGame.setHeadquarters(item, pCity, False)
        self.placeHeadquarter()
