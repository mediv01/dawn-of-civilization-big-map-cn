...
class CvWorldBuilderScreen:
    ...
    def interfaceScreen(self):
        ...
        self.m_iCurrentTeam = gcgetPlayer(self.m_iCurrentPlayer).getTeam()
        ...
        if GlobalCyInterface.isInAdvancedStart():
            pPlayer = gcgetPlayer(self.m_iCurrentPlayer)
    ...
    def mouseOverPlot(self, argsList):
        screen = CyGInterfaceScreen("WorldBuilderScreen", CvScreenEnums.WORLDBUILDER_SCREEN)
        self.m_pCurrentPlot = GlobalCyInterface.getMouseOverPlot()
        self.m_iCurrentX = self.m_pCurrentPlot.getX()
        self.m_iCurrentY = self.m_pCurrentPlot.getY()
        if not GlobalCyInterface.isInAdvancedStart():
            sText = "<font=3b>%s, X: %d, Y: %d</font>" % (GlobalCyTranslator.getText("TXT_KEY_WB_LATITUDE", (self.m_pCurrentPlot.getLatitude(),)), self.m_iCurrentX, self.m_iCurrentY)
            ...
            if self.iPlayerAddMode in self.DoCMapMode:
                ...
                if self.iPlayerAddMode == "ReligionMap":
                    ...
                    if iRegion != -1:
                        ...
                        sDoCText += "<font=3b>%s</font>" % GlobalCyTranslator.getText(str(TextKey), ())
                    ...
                    if self.iPlayerAddMode == "WarMap":
                        ...
                        iPlotSettlerValue = utils.getSettlerValue((self.m_iCurrentX, self.m_iCurrentY),self.m_iCurrentPlayer)
        ...
        if self.iPlayerAddMode in self.RevealMode:
            if GlobalCyInterface.isLeftMouseDown():
                self.setMultipleReveal(True)
            elif GlobalCyInterface.isRightMouseDown():
    ...
    def updateScreen(self):
        ...
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.Advanced_Start_DIRTY_BIT) and not GlobalCyInterface.isFocusedWidget()):
            self.refreshAdvancedStartTabCtrl(True)
            GlobalCyInterface.setDirty(InterfaceDirtyBits.Advanced_Start_DIRTY_BIT, False)
    ...
    def refreshReveal(self):
        CyEngine().clearAreaBorderPlots(AreaBorderLayers.AREA_BORDER_LAYER_REVEALED_PLOTS)
        for i in xrange(GlobalCyMap.numPlots()):
            pPlot = GlobalCyMap.plotByIndex(i)
            if pPlot.isNone(): continue
    ...
    def refreshASItemCost(self):
        if (GlobalCyInterface.isInAdvancedStart()):
            self.m_iCost = 0
            if (self.m_pCurrentPlot != 0):
                if (self.m_pCurrentPlot.isRevealed(GlobalCyGame.getActiveTeam(), False)):

                    # Unit mode
                    if (self.getASActiveUnit() != -1):
                        self.m_iCost = gcgetPlayer(self.m_iCurrentPlayer).getAdvancedStartUnitCost(self.getASActiveUnit(), True, self.m_pCurrentPlot)
                    elif (self.getASActiveCity() != -1):
                        self.m_iCost = gcgetPlayer(self.m_iCurrentPlayer).getAdvancedStartCityCost(True, self.m_pCurrentPlot)
                    elif (self.getASActivePop() != -1 and self.m_pCurrentPlot.isCity()):
                        self.m_iCost = gcgetPlayer(self.m_iCurrentPlayer).getAdvancedStartPopCost(True, self.m_pCurrentPlot.getPlotCity())
                    elif (self.getASActiveCulture() != -1 and self.m_pCurrentPlot.isCity()):
                        self.m_iCost = gcgetPlayer(self.m_iCurrentPlayer).getAdvancedStartCultureCost(True, self.m_pCurrentPlot.getPlotCity())
                    elif (self.getASActiveBuilding() != -1 and self.m_pCurrentPlot.isCity()):
                        self.m_iCost = gcgetPlayer(self.m_iCurrentPlayer).getAdvancedStartBuildingCost(self.getASActiveBuilding(), True, self.m_pCurrentPlot.getPlotCity())
                    elif (self.getASActiveRoute() != -1):
                        self.m_iCost = gcgetPlayer(self.m_iCurrentPlayer).getAdvancedStartRouteCost(self.getASActiveRoute(), True, self.m_pCurrentPlot)
                    elif (self.getASActiveImprovement() != -1):
                        self.m_iCost = gcgetPlayer(self.m_iCurrentPlayer).getAdvancedStartImprovementCost(self.getASActiveImprovement(), True, self.m_pCurrentPlot)

                elif (self.m_pCurrentPlot.isAdjacentNonrevealed(GlobalCyGame.getActiveTeam())):
                    if (self.getASActiveVisibility() != -1):
                        self.m_iCost = gcgetPlayer(self.m_iCurrentPlayer).getAdvancedStartVisibilityCost(True, self.m_pCurrentPlot)
    ...
    def placeObject(self, bMulti=False):
        ...
        if GlobalCyInterface.isInAdvancedStart():
            pPlayer = gcgetPlayer(self.m_iCurrentPlayer)
            pPlot = GlobalCyMap.plot(self.m_iCurrentX, self.m_iCurrentY)
            iActiveTeam = GlobalCyGame.getActiveTeam()
                    ...
                    # Buildings List
                        ...
                        if pPlot.isCity():
                            ...
                            if (iBuildingType != -1 and pPlayer.getAdvancedStartBuildingCost(iBuildingType, True, pCity) != -1):
                                CyMessageControl().sendAdvancedStartAction(AdvancedStartActionTypes.ADVANCEDSTARTACTION_BUILDING, self.m_iCurrentPlayer, self.m_iCurrentX, self.m_iCurrentY, iBuildingType,
                ...
                # Improvements Tab
                    ...
                    # Improvements List
                        ...
                        if iImprovementType > -1 and pPlayer.getAdvancedStartImprovementCost(iImprovementType, True, pPlot) > -1:
                            CyMessageControl().sendAdvancedStartAction(AdvancedStartActionTypes.ADVANCEDSTARTACTION_IMPROVEMENT, self.m_iCurrentPlayer, self.m_iCurrentX, self.m_iCurrentY, iImprovementType,
                                                                       True)  # Action, Player, X, Y, Data, bAdd
        ...
        if self.iPlayerAddMode == "EraseAll":
            ...
            popup.setHeaderString(GlobalCyTranslator.getText("TXT_KEY_WB_LANDMARK_START", ()))
        ...
        ## Python Effects ##
            ...
            for i in xrange(abs(iChange)):
                unit = gcgetPlayer(self.m_iCurrentPlayer).initUnit(self.iSelection, self.m_pCurrentPlot.getX(), self.m_pCurrentPlot.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
            ...
            pCity = gcgetPlayer(self.m_iCurrentPlayer).initCity(x, y)
            sName = cnm.getFoundName(self.m_iCurrentPlayer, (x, y))
            if sName:
                pCity.setName(CvUtil.convertToUnicode(sName), True)
            if bPython:
        ...
        ## Python Effects ##
            ...
            if self.m_pRiverStartPlot == self.m_pCurrentPlot:
                ...
                if ((iXDiff == iYDiff) and (iXDiff == 1) and (self.m_pRiverStartPlot.getX() > self.m_pCurrentPlot.getX()) and (self.m_pRiverStartPlot.getY() < self.m_pCurrentPlot.getY())):
                    ...
                    self.m_pRiverStartPlot = GlobalCyMap.plot(self.m_pRiverStartPlot.getX() - 1, self.m_pRiverStartPlot.getY() + 1)
                    ...
                    # Buildings List
                        ...
                        if (pPlot.isCity()):
                            if (pPlot.getPlotCity().getOwner() == self.m_iCurrentPlayer):
                                ...
                                if -1 != iBuildingType:
                                    CyMessageControl().sendAdvancedStartAction(AdvancedStartActionTypes.ADVANCEDSTARTACTION_BUILDING, self.m_iCurrentPlayer, self.m_iCurrentX, self.m_iCurrentY, iBuildingType,
                                                                               False)  # Action, Player, X, Y, Data, bAdd

                # Unit Tab
                elif (self.m_advancedStartTabCtrl.getActiveTab() == self.m_iASUnitTabID):
                    iUnitType = getASUnit(self.m_iAdvancedStartCurrentIndexes[self.m_advancedStartTabCtrl.getActiveTab()])
                    if -1 != iUnitType:
                        CyMessageControl().sendAdvancedStartAction(AdvancedStartActionTypes.ADVANCEDSTARTACTION_UNIT, self.m_iCurrentPlayer, self.m_pCurrentPlot.getX(), self.m_pCurrentPlot.getY(), iUnitType,
                                                                   False)  # Action, Player, X, Y, Data, bAdd
    ...
    def placeRiverNW(self, bUseCurrent):
        if (bUseCurrent):
            pRiverStepPlot = GlobalCyMap.plot(self.m_pRiverStartPlot.getX(), self.m_pRiverStartPlot.getY())
            if (not pRiverStepPlot.isNone()):
                pRiverStepPlot.setNOfRiver(True, CardinalDirectionTypes.CARDINALDIRECTION_WEST)

        pRiverStepPlot = GlobalCyMap.plot(self.m_pRiverStartPlot.getX() - 1, self.m_pRiverStartPlot.getY())
        if (not pRiverStepPlot.isNone()):
            pRiverStepPlot.setWOfRiver(True, CardinalDirectionTypes.CARDINALDIRECTION_NORTH)
        pRiverStepPlot = GlobalCyMap.plot(self.m_pRiverStartPlot.getX() - 1, self.m_pRiverStartPlot.getY() + 1)
        if (not pRiverStepPlot.isNone()):
            pRiverStepPlot.setWOfRiver(True, CardinalDirectionTypes.CARDINALDIRECTION_NORTH)
        return
    ...
    def placeRiverN(self, bUseCurrent):
        ...
        pRiverStepPlot = GlobalCyMap.plot(self.m_pRiverStartPlot.getX(), self.m_pRiverStartPlot.getY() + 1)
        if (not pRiverStepPlot.isNone()):
            pRiverStepPlot.setWOfRiver(True, CardinalDirectionTypes.CARDINALDIRECTION_NORTH)
        return
    ...
    def getMultiplePlotData(self):
        ...
        if self.bRegion:
            ...
            lPlots = [gcmap.plot(x, y) for (x, y) in utils.getRegionPlots(iRegion)]
            return lPlots
        ...
        if self.iPlayerAddMode == "AreaExporter3":
            ...
            return [gcmap.plot(x, y) for (x, y) in utils.getPlotList(tTR, tBL)]
        ...
        if self.iBrushWidth == -1:
            iMinX = 0
            iMaxX = GlobalCyMap.getGridWidth()
        if self.iBrushHeight == -1:
            iMinY = 0
            iMaxY = GlobalCyMap.getGridHeight()
        if not GlobalCyMap.isWrapX():
            iMaxX = min(iMaxX, GlobalCyMap.getGridWidth())
        if not GlobalCyMap.isWrapY():
            iMinY = max(iMinY, 0)
        for x in range(iMinX, iMaxX):
            for y in range(iMinY, iMaxY):
                lPlots.append(gcmap.plot(x, y))
        return lPlots
    ...
    def setSideMenu(self):
        ...
        if GlobalCyInterface.isInAdvancedStart():
            ...
            iX = 50
            iY = 15
            szText = u"<font=4>" + GlobalCyTranslator.getText("TXT_KEY_WB_AS_POINTS", (gcgetPlayer(GlobalCyGame.getActivePlayer()).getAdvancedStartPoints(),)) + "</font>"
            ...
            iY += 30
            szText = GlobalCyTranslator.getText("TXT_KEY_ADVANCED_START_BEGIN_GAME", ())
            ...
            szText = u"<font=4>" + GlobalCyTranslator.getText("TXT_KEY_WB_AS_COST_THIS_LOCATION", (self.m_iCost,)) + u"</font>"
            iY = 85
            ...
            screen.setImageButton("Version", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_GENERAL_QUESTIONMARK").getPath(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 9)
            iX += iAdjust
            screen.setImageButton("WorldBuilderRegenerateMap", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_REVEAL_ALL_TILES").getPath(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_REGENERATE_MAP, -1, -1)
            iX += iAdjust
            screen.addCheckBoxGFC("WorldBuilderEraseButton", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_ERASE").getPath(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_ERASE_BUTTON, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.setImageButton("WorldBuilderSaveButton", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_SAVE").getPath(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_SAVE_BUTTON, -1, -1)
            iX += iAdjust
            screen.setImageButton("WorldBuilderLoadButton", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_LOAD").getPath(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_LOAD_BUTTON, -1, -1)
            iX += iAdjust
            screen.setImageButton("WorldBuilderExitButton", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_EXIT").getPath(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_EXIT_BUTTON, -1, -1)
            ...
            iX += iAdjust
            screen.addCheckBoxGFC("DoCMapsScreen", "Art/Interface/Buttons/DoCMaps.dds", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SMALLCIRCLE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 42, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("MoveMapScreen", "Art/Interface/Buttons/MoveMap.dds", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SMALLCIRCLE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 44, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("AreaExporterScreen", "Art/Interface/Buttons/AreaExport.dds", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SMALLCIRCLE").getPath(),
            ...
            iX = iXStart + 8
            iY += iAdjust
            screen.addCheckBoxGFC("WorldBuilderNormalPlayerModeButton", ",-,Art/Interface/Buttons/FinalFrontier1_Atlas.dds,2,15", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_NORMAL_PLAYER_TAB_MODE_BUTTON, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("WorldBuilderNormalMapModeButton", ",-,Art/Interface/Buttons/FinalFrontier2_Atlas.dds,3,6", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_NORMAL_MAP_TAB_MODE_BUTTON, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("WorldBuilderRevealTileModeButton", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_LOS").getPath(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_REVEAL_TAB_MODE_BUTTON, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("PythonEffectButton", ",-,Art/Interface/Buttons/FinalFrontier2_Atlas.dds,3,4", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 0, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("HideInactive", ",-,Art/Interface/Buttons/GodsOfOld_Atlas.dds,8,3", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
            ...
            screen.addCheckBoxGFC("EditUnitData", ",-,Art/Interface/Buttons/Afterworld_Atlas.dds,4,9", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_UNIT_EDIT_BUTTON, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("EditPromotions", ",-,Art/Interface/Buttons/Promotions_Atlas.dds,8,2", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 6, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("EditCityDataI", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_TOGGLE_CITY_EDIT_MODE").getPath(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_CITY_EDIT_BUTTON, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("EditCityDataII", ",-,Art/Interface/Buttons/FinalFrontier2_Atlas.dds,1,8", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 7, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("EditCityBuildings", ",-,Art/Interface/Buttons/FinalFrontier1_Atlas.dds,5,14", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                  iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 8, ButtonStyles.BUTTON_STYLE_LABEL)
            iX += iAdjust
            screen.addCheckBoxGFC("EditEvents", "", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
    ...
    def refreshSideMenu(self):
        ...
        if GlobalCyInterface.isInAdvancedStart():
            ...
            if self.iPlayerAddMode in self.PlayerMode:
                iX = iXStart + 8
                screen.addCheckBoxGFC("AddOwnershipButton", gc.getCivilizationInfo(gcgetPlayer(self.m_iCurrentPlayer).getCivilizationType()).getButton(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                ...
                for iPlayer in xrange(gc.getMAX_PLAYERS()):
                    if gcgetPlayer(iPlayer).isEverAlive():
                        sName = utils.getLeaderNameOrCivName(iPlayer)
                        if not gcgetPlayer(iPlayer).isAlive():
                            sName = "*" + sName
                        if gcgetPlayer(iPlayer).isTurnActive():
                            sName = "[" + sName + "]"
                ...
                iY += iAdjust
                screen.setImageButton("EditPlayerData", gc.getLeaderHeadInfo(gcgetPlayer(self.m_iCurrentPlayer).getLeaderType()).getButton(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 1)
                iX += iAdjust
                ...
                iX += iAdjust
                screen.addCheckBoxGFC("EditStartingPlot", ",-,Art/Interface/Buttons/Warlords_Atlas_1.dds,4,13", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 32, ButtonStyles.BUTTON_STYLE_LABEL)

                iY += iAdjust
                iX = iXStart + 8
                screen.addCheckBoxGFC("AddUnitsButton", ",-,Art/Interface/Buttons/Warlords_Atlas_1.dds,6,10", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 27, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("AddBuildingsButton", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_CITYSELECTION").getPath(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 19, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("AddCityButton", ",-,Art/Interface/Buttons/Charlemagne_Atlas.dds,4,2", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                ...
                while i < 1001:
                    ...
                    if str(i)[0] == "1":
                        ...
                        i *= 2
                sText = "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_HIDE_INACTIVE", ()) + "</font>"

            elif self.iPlayerAddMode in self.MapMode:
                iX = iXStart + 8
                screen.addCheckBoxGFC("EditPlotData", ",-,Art/Interface/Buttons/FinalFrontier1_Atlas.dds,4,16", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1027, -1, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("AddRiverButton", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_RIVER_PLACEMENT").getPath(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 11, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("WorldBuilderLandmarkButton", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_LANDMARK_MODE").getPath(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_LANDMARK_BUTTON, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addDropDownBoxGFC("WorldBuilderPlayerChoice", iX, iY, screen.getXResolution() - 8 - iX, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
                screen.addPullDownString("WorldBuilderPlayerChoice", GlobalCyTranslator.getText("TXT_KEY_WB_LANDMARKS", ()), gc.getBARBARIAN_PLAYER(), gc.getBARBARIAN_PLAYER(), self.m_iCurrentPlayer == gc.getBARBARIAN_PLAYER())
                for iPlayer in xrange(gc.getMAX_PLAYERS()):
                ...
                iX = iXStart + 8
                iY += iAdjust
                screen.addCheckBoxGFC("AddImprovementButton", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_FEATURE_PRODUCTION").getPath(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 12, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("AddBonusButton", ",-,Art/Interface/Buttons/Warlords_Atlas_1.dds,7,9", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 13, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("AddPlotTypeButton", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_CHANGE_ALL_PLOTS").getPath(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 14, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("AddTerrainButton", ",-,Art/Interface/Buttons/BaseTerrain_TerrainFeatures_Atlas.dds,3,1", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 15, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("AddRouteButton", "Art/Interface/Buttons/Builds/BuildRoad.dds", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 16, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("AddFeatureButton", ",-,Art/Interface/Buttons/BaseTerrain_TerrainFeatures_Atlas.dds,3,3", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 17, ButtonStyles.BUTTON_STYLE_LABEL)

                iX = iXStart + 8
                iY += iAdjust
                screen.addCheckBoxGFC("RegionButton", ",-,Art/Interface/Buttons/FinalFrontier2_Atlas.dds,1,6", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 43, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("SensibilityCheck", ",-,Art/Interface/Buttons/FinalFrontier1_Atlas.dds,1,16", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                ...
                iX = iXStart + 8
                screen.addCheckBoxGFC("FlipButton", ",-,Art/Interface/Buttons/GodsOfOld_Atlas.dds,1,1", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 35, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("CoreButton", ",-,Art/Interface/Buttons/GodsOfOld_Atlas.dds,5,1", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 36, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("SettlerValueButton", ",-,Art/Interface/Buttons/GodsOfOld_Atlas.dds,7,1", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 37, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("WarMapButton", ",-,Art/Interface/Buttons/GodsOfOld_Atlas.dds,4,1", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 38, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("ReligionMapButton", ",-,Art/Interface/Buttons/GodsOfOld_Atlas.dds,3,1", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 39, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("RegionMapButton", ",-,Art/Interface/Buttons/GodsOfOld_Atlas.dds,2,1", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 40, ButtonStyles.BUTTON_STYLE_LABEL)
                iX = iXStart + 8
                iY += iAdjust
                screen.addCheckBoxGFC("VictoryMapButton", ",-,Art/Interface/Buttons/GodsOfOld_Atlas.dds,6,1", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 46, ButtonStyles.BUTTON_STYLE_LABEL)

                iX = iXStart + 8
                iY += iAdjust
                screen.setImageButton("ToolHelp", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_GENERAL_QUESTIONMARK").getPath(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 51)
                iX += iAdjust
                if self.iPlayerAddMode == "Flip":
                    screen.addCheckBoxGFC("FlipAIButton", "Art/Interface/Buttons/TechTree/Artificial_Intelligence.dds", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                          iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 45, ButtonStyles.BUTTON_STYLE_LABEL)
                    iX += iAdjust
                elif self.iPlayerAddMode == "VictoryMap":
                    screen.addCheckBoxGFC("VictoryRectangleButton", "Art/Interface/Buttons/TechTree/Engineering.dds", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                          iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 47, ButtonStyles.BUTTON_STYLE_LABEL)
                    iX += iAdjust
                elif self.iPlayerAddMode == "RegionMap":
                    screen.addCheckBoxGFC("AllRegionsButton", ",-,Art/Interface/Buttons/FinalFrontier2_Atlas.dds,4,5", GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                    ...
                    if self.iPlayerAddMode == "ReligionMap":
                        for i in range(5):
                            sText = GlobalCyTranslator.getText(str(lReligionMapTexts[i]), ())
                ...
                if self.iPlayerAddMode == "ReligionMap":
                    ...
                    for iRegion in range(iNumRegions):
                        TextKey = u"TXT_KEY_REGION_%d" % iRegion
                        sName = GlobalCyTranslator.getText(str(TextKey), ())
                ...
                if self.iPlayerAddMode != "VictoryMap":
                    ...
                    iY += iAdjust
                    if self.iPlayerAddMode == "ReligionMap":
                        screen.setButtonGFC("ClearChanges", GlobalCyTranslator.getText("TXT_KEY_WB_REVERT_CHANGES", ()), "", iX, iY, screen.getXResolution() - 8 - iX, iButtonWidth, WidgetTypes.WIDGET_PYTHON, -1, -1,
                        ...
                        if self.iPlayerAddMode == "RegionMap":
                            ...
                            iSpan = 2
                        screen.setButtonGFC("ClearChanges", GlobalCyTranslator.getText("TXT_KEY_WB_REVERT_CHANGES", ()), "", iX, iY, iSpan * iAdjust - 3, iButtonWidth, WidgetTypes.WIDGET_PYTHON, -1, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
                        iX += iSpan * iAdjust
                        screen.setButtonGFC("Export", GlobalCyTranslator.getText("TXT_KEY_WB_EXPORT", ()), "", iX, iY, iSpan * iAdjust - 3, iButtonWidth, WidgetTypes.WIDGET_PYTHON, -1, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
                        iX += iSpan * iAdjust
                        if self.iPlayerAddMode != "RegionMap":
                            if self.iPlayerAddMode == "Flip":
                                ...
                                bExtended = (
                                        self.m_iCurrentPlayer in Areas.dChangedCoreArea or self.m_iCurrentPlayer in Areas.dChangedNormalArea or self.m_iCurrentPlayer in Areas.dChangedBroaderArea or self.m_iCurrentPlayer in SettlerMaps.dChangedSettlerMaps or self.m_iCurrentPlayer in WarMaps.dChangedWarMaps)
                            if bExtended:
                                screen.setButtonGFC("SwitchReborn", GlobalCyTranslator.getText("TXT_KEY_WB_EXTENDED", ()), "", iX, iY, 2 * iAdjust - 3, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 0, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
                            else:
                                screen.setButtonGFC("SwitchReborn", GlobalCyTranslator.getText("TXT_KEY_WB_NA", ()), "", iX, iY, 2 * iAdjust - 3, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
                ...
                if self.iPlayerAddMode == "MoveMap":
                    ...
                    screen.setButtonGFC("CancelMoveMap", GlobalCyTranslator.getText("TXT_KEY_SCREEN_CANCEL", ()), "", iX, iY, iWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, -1, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
                ...
                for i in range(3):
                    ...
                    screen.addPullDownString("MoveMapReset", GlobalCyTranslator.getText(szText, ()), i, i, self.iMoveMapReset == i)

            elif self.iPlayerAddMode in self.AreaExport:
                iX = iXStart + 8
                screen.addCheckBoxGFC("AreaExportWater", gc.getTerrainInfo(iOcean).getButton(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 49, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust
                screen.addCheckBoxGFC("AreaExportPeak", gc.getTerrainInfo(iTerrainPeak).getButton(), GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                                      iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 50, ButtonStyles.BUTTON_STYLE_LABEL)
                iX += iAdjust

                screen.addDropDownBoxGFC("WorldBuilderPlayerChoice", iX, iY, screen.getXResolution() - 8 - iX, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
                screen.addPullDownString("WorldBuilderPlayerChoice", GlobalCyTranslator.getText("TXT_KEY_WB_AREA_EXPORTER_CLICK", ()), 0, 0, self.iPlayerAddMode == "AreaExporter1")
                screen.addPullDownString("WorldBuilderPlayerChoice", GlobalCyTranslator.getText("TXT_KEY_WB_AREA_EXPORTER_TLBR", ()), 1, 1, self.iPlayerAddMode == "AreaExporter2")
                ...
                iX = iXStart + 8
                iY += iAdjust
                screen.setButtonGFC("ClearChanges", GlobalCyTranslator.getText("TXT_KEY_WB_REVERT_CHANGES", ()), "", iX, iY, 3 * iAdjust - 3, iButtonWidth, WidgetTypes.WIDGET_PYTHON, -1, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
                iX += 3 * iAdjust
                if self.iPlayerAddMode == "AreaExporter3":
                    screen.setButtonGFC("CancelMoveMap", GlobalCyTranslator.getText("TXT_KEY_SCREEN_CANCEL", ()), "", iX, iY, 3 * iAdjust - 3, iButtonWidth, WidgetTypes.WIDGET_PYTHON, -1, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
                else:
                    screen.setButtonGFC("Export", GlobalCyTranslator.getText("TXT_KEY_WB_EXPORT", ()), "", iX, iY, 3 * iAdjust - 3, iButtonWidth, WidgetTypes.WIDGET_PYTHON, -1, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
                ...
                screen.addPullDownString("RevealMode", GlobalCyTranslator.getText("TXT_KEY_REVEAL_PLOT", ()), 0, 0, self.iPlayerAddMode == self.RevealMode[0])
                screen.addPullDownString("RevealMode", GlobalCyTranslator.getText("TXT_KEY_REVEAL_SUBMARINE", ()), 1, 1, self.iPlayerAddMode == self.RevealMode[1])
                screen.addPullDownString("RevealMode", GlobalCyTranslator.getText("TXT_KEY_REVEAL_STEALTH", ()), 2, 2, self.iPlayerAddMode == self.RevealMode[2])
                ...
                iY += iAdjust
                screen.setImageButton("WorldBuilderRevealAll", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_REVEAL_ALL_TILES").getPath(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_REVEAL_ALL_BUTTON, -1, -1)
                iX += iAdjust
                screen.addDropDownBoxGFC("WorldBuilderPlayerChoice", iX, iY, screen.getXResolution() - 8 - iX, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
                for iPlayer in xrange(gc.getMAX_PLAYERS()):
                    if gcgetPlayer(iPlayer).isEverAlive():
                ...
                iX = iXStart + 8
                iY += iAdjust
                screen.setImageButton("WorldBuilderUnrevealAll", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_UNREVEAL_ALL_TILES").getPath(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_WB_UNREVEAL_ALL_BUTTON, -1, -1)
                iX += iAdjust
                iWidth = (screen.getXResolution() - 8 - iX - 3) / 2
                screen.addDropDownBoxGFC("BrushWidth", iX, iY, iWidth, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
                for i in range(1, 11):
                ...
                iX = iXStart + 8
                screen.setImageButton("WorldBuilderEraseAll", GlobalCyArtFileMgr.getInterfaceArtInfo("WORLDBUILDER_UNREVEAL_ALL_TILES").getPath(), iX, iY, iButtonWidth, iButtonWidth, WidgetTypes.WIDGET_PYTHON, 1029, 29)
                iX += iAdjust
                iWidth = (screen.getXResolution() - 8 - iX - 3) / 2
                screen.addDropDownBoxGFC("BrushWidth", iX, iY, iWidth, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
                for i in range(1, 11):
    ...
    def setSelectionTable(self):
        screen = CyGInterfaceScreen("WorldBuilderScreen", CvScreenEnums.WORLDBUILDER_SCREEN)
        iWidth = 200
        iCivilization = gcgetPlayer(self.m_iCurrentPlayer).getCivilizationType()
        if self.iPlayerAddMode == "Units":
            ...
            screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_KEY_WB_CITY_ALL", ()), -2, -2, -2 == self.iSelectClass)
            screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_PEDIA_NON_COMBAT", ()), -1, -1, -1 == self.iSelectClass)
            ...
            iY = 25
            sWonder = GlobalCyTranslator.getText("TXT_KEY_CONCEPT_WONDERS", ())
            screen.addDropDownBoxGFC("WBSelectClass", 0, iY, iWidth, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
            screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_KEY_WB_CITY_ALL", ()), 0, 0, 0 == self.iSelectClass)
            screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_BUILDING", ()), 1, 1, 1 == self.iSelectClass)
            screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_KEY_PEDIA_NATIONAL_WONDER", ()), 2, 2, 2 == self.iSelectClass)
            screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_KEY_PEDIA_TEAM_WONDER", ()), 3, 3, 3 == self.iSelectClass)
            screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_KEY_PEDIA_WORLD_WONDER", ()), 4, 4, 4 == self.iSelectClass)
            ...
            iY = 25
            screen.addDropDownBoxGFC("WBSelectClass", 0, iY, iWidth, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
            screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_KEY_WB_CITY_ALL", ()), -1, -1, -1 == self.iSelectClass)
            screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_KEY_GLOBELAYER_RESOURCES_GENERAL", ()), 0, 0, 0 == self.iSelectClass)
            iBonusClass = 1
    ...
    def refreshSelection(self):
        ...
        if self.iPlayerAddMode == "Units":
            ...
            sText = "<font=3>" + GlobalCyTranslator.getText("[COLOR_HIGHLIGHT_TEXT]", ()) + ItemInfo.getDescription() + "</color></font>"
            ...
            if ItemInfo.getNumVarieties() > 1:
                ...
                for i in xrange(ItemInfo.getNumVarieties()):
                    screen.addPullDownString("WBSelectClass", GlobalCyTranslator.getText("TXT_KEY_WB_FEATURE_VARIETY", (i,)), i, i, i == self.iSelectClass)
            else:
    ...
    def showRevealed(self, pPlot):
        if self.iPlayerAddMode == "RevealPlot":
            ...
            if gcgetTeam(self.m_iCurrentTeam).isAtWar(pPlot.getTeam()): return
    ...
    def showWarOverlay(self):
        utils.removeStabilityOverlay()
        if self.m_iCurrentPlayer < iNumPlayers:
            for i in range(GlobalCyMap.numPlots()):
                plot = GlobalCyMap.plotByIndex(i)
    ...
    def determineRegionColors(self):
        ...
        for (x, y) in utils.getWorldPlotsList():
            ...
            for (i, j) in utils.surroundingPlots((x, y)):
                surroundingPlot = gcmap.plot(i, j)
    ...
    def showToolHelp(self):
        if self.iPlayerAddMode == "Flip":
            sText = GlobalCyTranslator.getText("TXT_KEY_WB_FLIPMAP_HELP", ())
        elif self.iPlayerAddMode == "Core":
            sText = GlobalCyTranslator.getText("TXT_KEY_WB_COREMAP_HELP", ())
        elif self.iPlayerAddMode == "SettlerValue":
            sText = GlobalCyTranslator.getText("TXT_KEY_WB_SETTLERMAP_HELP", ())
        elif self.iPlayerAddMode == "WarMap":
            sText = GlobalCyTranslator.getText("TXT_KEY_WB_WARMAP_HELP", ())
        elif self.iPlayerAddMode == "ReligionMap":
            sText = GlobalCyTranslator.getText("TXT_KEY_WB_RELIGIONMAP_HELP", ())
        elif self.iPlayerAddMode == "RegionMap":
            sText = GlobalCyTranslator.getText("TXT_KEY_WB_REGIONMAP_HELP", ())
        elif self.iPlayerAddMode == "VictoryMap":
            sText = GlobalCyTranslator.getText("TXT_KEY_WB_VICTORYMAP_HELP", ())
        elif self.iPlayerAddMode in self.MoveMap:
            sText = GlobalCyTranslator.getText("TXT_KEY_WB_MOVEMAP_HELP", ())
        elif self.iPlayerAddMode in self.AreaExport:
            sText = GlobalCyTranslator.getText("TXT_KEY_WB_AREAEXPORT_HELP", ())
        else:
            return
        utils.show(sText)
    ...
    def getPlotItems(self, tPlot):
        x, y = tPlot
        plot = gcmap.plot(x, y)
        lInfo = [tPlot, plot.getPlotType(), plot.getTerrainType(), plot.getRouteType(), (plot.getFeatureType(), plot.getFeatureVariety()), plot.getBonusType(-1), plot.getImprovementType(),
                 (plot.getRiverNSDirection(), plot.getRiverWEDirection())]
        return lInfo
    ...
    def moveMapReset(self):
        ...
        if self.iMoveMapReset == 1:
            for j in range(len(lInfos)):
                for i in range(len(lInfos[j])):
                    item = lInfos[j][i]
                    plot = gcmap.plot(item[0][0], item[0][1])
                    plot.erase()
    ...
    def Exit(self):
        GlobalCyInterface.setWorldBuilder(False)
        return
    ...
    def refreshAdvancedStartTabCtrl(self, bReuse):
        if GlobalCyInterface.isInAdvancedStart():
            ...
            self.m_iCurrentPlayer = GlobalCyGame.getActivePlayer()
            self.m_iCurrentTeam = GlobalCyGame.getActiveTeam()
            self.m_iAdvancedStartCurrentIndexes = []
            self.m_iAdvancedStartCurrentList = []
            ...
            self.m_advancedStartTabCtrl.addTabSection(GlobalCyTranslator.getText("TXT_KEY_WB_AS_CITIES", ()));
            ...
            self.m_advancedStartTabCtrl.addTabSection(GlobalCyTranslator.getText("TXT_KEY_WB_AS_UNITS", ()));
            self.m_iAdvancedStartCurrentIndexes.append(0)

            self.m_iAdvancedStartCurrentList.append(0)

            self.m_advancedStartTabCtrl.setNumColumns((gc.getNumImprovementInfos() / 10) + 2);
            self.m_advancedStartTabCtrl.addTabSection(GlobalCyTranslator.getText("TXT_KEY_WB_AS_IMPROVEMENTS", ()));
            self.m_iAdvancedStartCurrentIndexes.append(0)

            self.m_iAdvancedStartCurrentList.append(self.m_iASRoutesListID)

            self.m_advancedStartTabCtrl.setNumColumns(1);
            self.m_advancedStartTabCtrl.addTabSection(GlobalCyTranslator.getText("TXT_KEY_WB_AS_VISIBILITY", ()));
            self.m_iAdvancedStartCurrentIndexes.append(0)

            self.m_iAdvancedStartCurrentList.append(0)

            self.m_advancedStartTabCtrl.setNumColumns(1);
            self.m_advancedStartTabCtrl.addTabSection(GlobalCyTranslator.getText("TXT_KEY_WB_AS_TECH", ()));
            self.m_iAdvancedStartCurrentIndexes.append(0)
    ...
    def leftMouseDown(self, argsList):
        ...
        if GlobalCyInterface.isInAdvancedStart():
            ...
            if len(self.TempInfo) >= 2:
                pPlayerX = gcgetPlayer(self.TempInfo[0])
                if pPlayerX:
            ...
            if pOldCity:
                ...
                pOldCity.setName(CvUtil.convertToUnicode("ToBeRazed"), False)
                pNewCity.setName(CvUtil.convertToUnicode(sName), True)
            ...
            if pOldCity:
                ...
                if self.iPlayerAddMode == "DuplicateCityPlus":
                    for item in self.lMoveUnit:
                        ...
                        pNewUnit.setName(CvUtil.convertToUnicode(loopUnit.getNameNoDesc()))
    ...
    def copyCityStats(self, pOldCity, pNewCity, bMove):
        ...
        for iReligion in xrange(gc.getNumReligionInfos()):
            ...
            if bMove and pOldCity.isHolyCityByType(iReligion):
                GlobalCyGame.setHolyCity(iReligion, pNewCity, False)
        ...
        for iCorporation in xrange(gc.getNumCorporationInfos()):
            ...
            if bMove and pOldCity.isHeadquartersByType(iCorporation):
                GlobalCyGame.setHeadquarters(iCorporation, pNewCity, False)
    ...
    def handleInput(self, inputClass):
        ...
        if inputClass.getFunctionName() == "WorldBuilderEraseAll":
            for i in xrange(GlobalCyMap.numPlots()):
                self.m_pCurrentPlot = GlobalCyMap.plotByIndex(i)
                self.placeObject()
            ...
            if self.iPlayerAddMode == "Flip":
                ...
                if CvEventInterface.getEventManager().bAlt:
                    ...
                    met.exportCore(self.m_iCurrentPlayer, GlobalCyInterface.shiftKey())
                self.showStabilityOverlay()
            elif self.iPlayerAddMode == "SettlerValue":
                if CvEventInterface.getEventManager().bAlt:
                    for iPlayer in range(iNumPlayers):
                    ...
                    met.exportSettlerMap(self.m_iCurrentPlayer, GlobalCyInterface.shiftKey())
                self.showStabilityOverlay()
            elif self.iPlayerAddMode == "WarMap":
                if CvEventInterface.getEventManager().bAlt:
                    for iPlayer in range(iNumPlayers):
                    ...
                    met.exportWarMap(self.m_iCurrentPlayer, GlobalCyInterface.shiftKey())
                self.showWarOverlay()
            elif self.iPlayerAddMode == "RegionMap":
                met.exportRegionMap(GlobalCyInterface.shiftKey())
                self.showRegionOverlay()
            elif self.iPlayerAddMode in self.AreaExport:
