...
class WBInfoScreen:
    ...
    def interfaceScreen(self, iPlayerX):
        ...
        screen.setText("WBInfoExit", "Background", "<font=4>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper() + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, screen.getXResolution() - 30, screen.getYResolution() - 42, -0.1,
        ...
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_UNIT", ()), 0, 0, 0 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_PROMOTION", ()), 1, 1, 1 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_BUILDING", ()), 2, 2, 2 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_SPECIALIST", ()), 3, 3, 3 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_RELIGION", ()), 4, 4, 4 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_CONCEPT_CORPORATIONS", ()), 5, 5, 5 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_TERRAIN", ()), 6, 6, 6 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_FEATURE", ()), 7, 7, 7 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_BONUS", ()), 8, 8, 8 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_IMPROVEMENT", ()), 9, 9, 9 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_ROUTE", ()), 10, 10, 10 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_CIVIC", ()), 11, 11, 11 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_TECH", ()), 12, 12, 12 == iMode)
        screen.addPullDownString("ItemType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_PROJECT", ()), 13, 13, 13 == iMode)
    ...
    def placePlotData(self):
        ...
        sText = "<font=3b>" + GlobalCyTranslator.getText("[COLOR_SELECTED_TEXT]", ())
        if iMode < 2:
            iPlayer = lSelectedItem[0]
            iUnit = lSelectedItem[1]
            pUnit = gcgetPlayer(iPlayer).getUnit(iUnit)
            if pUnit:
                sText += pUnit.getName()
    ...
    def placeMap(self):
        ...
        iHeight = iWidth * GlobalCyMap.getGridHeight() / GlobalCyMap.getGridWidth()
        if iHeight > iMaxHeight:
            iWidth = iMaxHeight * GlobalCyMap.getGridWidth() / GlobalCyMap.getGridHeight()
            iHeight = iMaxHeight
    ...
    def refreshMap(self):
        ...
        if iMode < 2:
            ...
            pUnit = gcgetPlayer(lSelectedItem[0]).getUnit(lSelectedItem[1])
            if pUnit:
            ...
            for lPlots in lItems[iItem][5]:
                ...
                iCount += 1
                sColor = ""
                sButton = GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_CANCEL").getPath()
                iOwner = pPlot.getOwner()
                if iOwner > -1:
                    pPlayer = gcgetPlayer(iOwner)
                    sColor = u"<color=%d,%d,%d,%d>" % (pPlayer.getPlayerTextColorR(), pPlayer.getPlayerTextColorG(), pPlayer.getPlayerTextColorB(), pPlayer.getPlayerTextColorA())
            ...
            for lPlots in lItems[iItem][5]:
                iPlayer = lPlots[0]
                if iMode > 11:
                    iPlayer = gcgetTeam(lPlots[0]).getLeaderID()
                iColumn = iCount % nColumns
                iRow = iCount / nColumns
                if iRow > iMaxRows:
                    screen.appendTableRow("PlotTable")
    ...
    def placeItems(self):
        ...
        screen.setTableColumnHeader("InfoTable", 0, "<font=3>" + GlobalCyTranslator.getText("TXT_KEY_DOMESTIC_ADVISOR_NAME", ()) + "</font>", iWidth / 2)
        screen.setTableColumnHeader("InfoTable", 1, "<font=3>" + gcgetPlayer(iSelectedPlayer).getName() + "</font>", iWidth / 4)
        screen.setTableColumnHeader("InfoTable", 2, "<font=3>" + GlobalCyTranslator.getText("TXT_KEY_WB_CITY_ALL", ()) + "</font>", iWidth / 4)
        ...
        if iMode == 0:
            ...
            iData1 = 7879
            pPlayer = gcgetPlayer(iSelectedPlayer)
            ...
            for i in xrange(gc.getNumReligionInfos()):
                ...
                lItems.append([Info.getDescription(), pPlayer.getHasReligionCount(i), GlobalCyGame.countReligionLevels(i), i, Info.getButton(), []])
            ...
            for i in xrange(gc.getNumCorporationInfos()):
                ...
                lItems.append([Info.getDescription(), pPlayer.getHasCorporationCount(i), GlobalCyGame.countCorporationLevels(i), i, Info.getButton(), []])
            for i in xrange(GlobalCyMap.numPlots()):
                pPlot = GlobalCyMap.plotByIndex(i)
                if pPlot.isNone(): continue
                if pPlot.isCity():
            ...
            for iTeamX in xrange(gc.getMAX_TEAMS()):
                ...
                if pTeamX.isAlive():
                    for iItemX in xrange(gc.getNumTechInfos()):
                        ...
                        if iCount > 0:
                            if iTeamX == gcgetPlayer(iSelectedPlayer).getTeam():
                                lItems[iItemX][1] += iCount
                            lItems[iItemX][2] += iCount
    ...
    def handleInput(self, inputClass):
        ...
        if inputClass.getFunctionName() == "PlotData":
            if iMode == 0:
                ...
                pCity = gcgetPlayer(lSelectedItem[0]).getCity(lSelectedItem[1])
                if pCity:
                    WBCityEditScreen.WBCityEditScreen(CvPlatyBuilderScreen.CvWorldBuilderScreen()).interfaceScreen(pCity)
            elif iMode < 11:
                pPlot = GlobalCyMap.plot(lSelectedItem[0], lSelectedItem[1])
                if not pPlot.isNone():
            ...
            if iMode < 2:
                ...
                pNewUnit = gcgetPlayer(iPlayer).getUnit(iUnit)
                if pNewUnit:
                ...
                pNewCity = gcgetPlayer(iPlayer).getCity(iCity)
                if pNewCity:
                ...
                iPlayerX = inputClass.getData2() / 10000
                lSelectedItem = [gcgetPlayer(iPlayerX).getTeam(), -1]
            self.placePlotData()
