...
class WBReligionScreen:
    ...
    def interfaceScreen(self, iPlayerX):
        ...
        screen.setText("WBReligionExit", "Background", "<font=4>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper() + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, screen.getXResolution() - 30, screen.getYResolution() - 42, -0.1,
                       FontTypes.TITLE_FONT, WidgetTypes.WIDGET_CLOSE_SCREEN, -1, -1)
        screen.setLabel("ReligionHeader", "Background", "<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_RELIGION", ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, screen.getXResolution() / 2, 20, -0.1, FontTypes.GAME_FONT,
                        WidgetTypes.WIDGET_GENERAL, -1, -1)
        screen.setLabel("HolyCityHeader", "Background", "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_RELIGION_SCREEN_HOLY_CITY", ())[:-1] + "</font>", CvUtil.FONT_CENTER_JUSTIFY, screen.getXResolution() / 8, self.iTable_Y - 30, -0.1,
                        FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        screen.setLabel("StateReligionHeader", "Background", "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_WB_STATE_RELIGION", ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, screen.getXResolution() / 8, screen.getYResolution() / 2, -0.1,
                        FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        ...
        screen.addPullDownString("CurrentPage", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_RELIGION", ()), 8, 8, True)
        ...
        sText = "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_RELIGION_SCREEN_HOLY_CITY", ())[:-1] + "</font>"
    ...
    def sortCities(self):
        global lCities
        lCities = []
        iSelectedTeam = gcgetPlayer(iSelectedPlayer).getTeam()
    ...
    def placeCityTable(self):
        ...
        sText = GlobalCyTranslator.getText("TXT_KEY_WB_CITY_ALL", ())
    ...
    def placeHolyCities(self):
        ...
        for i in xrange(gc.getNumReligionInfos()):
            ...
            pHolyCity = GlobalCyGame.getHolyCity(i)
            if not pHolyCity.isNone():
                ...
                screen.setTableText("WBHolyCity", 2, iRow, "<font=3>" + sColor + pHolyCity.getName() + "</color></font>", gc.getCivilizationInfo(pHolyCity.getCivilizationType()).getButton(), WidgetTypes.WIDGET_PYTHON, 7200 + iPlayerX,
                                    pHolyCity.getID(), CvUtil.FONT_LEFT_JUSTIFY)
    ...
    def placeStateReligion(self):
        ...
        for iPlayerX in xrange(gc.getMAX_PLAYERS()):
            ...
            screen.setTableText("WBStateReligion", 2, iRow, "<font=3>" + sColor + pPlayerX.getName() + "</font></color>", gc.getLeaderHeadInfo(iLeader).getButton(), WidgetTypes.WIDGET_PYTHON, 7876, iPlayerX * 10000 + iLeader,
    ...
    def handleInput(self, inputClass):
        ...
        if inputClass.getButtonType() == WidgetTypes.WIDGET_PYTHON:
            if inputClass.getData1() > 7199 and inputClass.getData1() < 7300:
                ...
                WBCityEditScreen.WBCityEditScreen(CvPlatyBuilderScreen.CvWorldBuilderScreen()).interfaceScreen(gcgetPlayer(iPlayerX).getCity(iCityID))
        ...
        if inputClass.getFunctionName() == "CurrentPage":
            ...
            if iIndex == 0:
                ...
                WBTeamScreen.WBTeamScreen().interfaceScreen(gcgetPlayer(iSelectedPlayer).getTeam())
            elif iIndex == 9:
    ...
    def editHolyCity(self, item, pCity):
        if pCity.isHolyCityByType(item):
            GlobalCyGame.clearHolyCity(item)
        else:
            GlobalCyGame.setHolyCity(item, pCity, False)
        self.placeHolyCities()
