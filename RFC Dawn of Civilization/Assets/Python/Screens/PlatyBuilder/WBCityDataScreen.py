...
class WBCityDataScreen:
    ...
    def interfaceScreen(self, pCityX):
        ...
        screen.setText("CityDataExit", "Background", "<font=4>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper() + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, screen.getXResolution() - 30, screen.getYResolution() - 42, -0.1,
        ...
        screen.addPullDownString("CurrentPage", GlobalCyTranslator.getText("TXT_KEY_WB_CITY_DATA", ()), 0, 0, False)
        screen.addPullDownString("CurrentPage", GlobalCyTranslator.getText("TXT_KEY_WB_CITY_DATA2", ()), 1, 1, True)
    ...
    def placeModify(self):
        ...
        sText = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_BUILDING", ())
        screen.setLabel("ModifyHeader", "Background", "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_WB_MODIFY", (sText,)) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, iX + iTableWidth / 2, iY, -0.1, FontTypes.TITLE_FONT,
                        WidgetTypes.WIDGET_GENERAL, -1, -1)

        iY += 30
        screen.addDropDownBoxGFC("BuildingType", iX, iY, 150, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
        screen.addPullDownString("BuildingType", GlobalCyTranslator.getText("TXT_KEY_CONCEPT_BUILDINGS", ()), 0, 0, not bWonder)
        screen.addPullDownString("BuildingType", GlobalCyTranslator.getText("TXT_KEY_CONCEPT_WONDERS", ()), 1, 1, bWonder)
        ...
        #	screen.addPullDownString("YieldType", GlobalCyTranslator.getText("TXT_KEY_CONCEPT_HAPPINESS", ()), iTotal, iTotal, iSelectedYield == iTotal)
        #	screen.addPullDownString("YieldType", GlobalCyTranslator.getText("TXT_KEY_CONCEPT_HEALTH", ()), iTotal + 1, iTotal + 1, iSelectedYield == iTotal + 1)
        ## Disabled because neither is working correctly with BTS dll ##
        ...
        for item in lBuilding:
            ...
            if iChange > 0:
                sText += u"%d%s" % (iChange, GlobalCyTranslator.getText("[ICON_HAPPY]", ()))
            elif iChange < 0:
                sText += u"%d%s" % (-iChange, GlobalCyTranslator.getText("[ICON_UNHAPPY]", ()))
            iChange = pCity.getBuildingHealthChange(iBuildingClass)
            if iChange > 0:
                sText += u"%d%s" % (iChange, GlobalCyTranslator.getText("[ICON_HEALTHY]", ()))
            elif iChange < 0:
                sText += u"%d%s" % (-iChange, GlobalCyTranslator.getText("[ICON_UNHEALTHY]", ()))
    ...
    def placeSpecialist(self):
        ...
        sText = "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_WB_BASE_RATE", (pCity.getBaseGreatPeopleRate(),)) + "</font>"
        ...
        screen.setLabel("SpecialistHeader", "Background", u"<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_WB_FREE_SPECIALISTS", ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, iX + iWidth / 2, iY, -0.1, FontTypes.TITLE_FONT,
                        WidgetTypes.WIDGET_GENERAL, -1, -1)
        iY += 30
    ...
    def placeGreatPeople(self):
        ...
        sText = "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_WB_PROGRESS", (pCity.getGreatPeopleProgress(),)) + "/" + str(gcgetPlayer(iPlayer).greatPeopleThreshold(False)) + "</font>"
        ...
        screen.setLabel("GreatPeopleHeader", "Background", u"<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_CONCEPT_GREAT_PEOPLE", ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, iX + iWidth / 2, iY, -0.1, FontTypes.TITLE_FONT,
                        WidgetTypes.WIDGET_GENERAL, -1, -1)
        iY += 30
        sText = GlobalCyTranslator.getText("[COLOR_SELECTED_TEXT]", ()) + "<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_WB_CITY_ALL", ()) + " (+/-)</font></color>"
    ...
    def placeBonus(self):
        ...
        screen.setLabel("BonusHeader", "Background", u"<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_BONUS", ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, iX + iWidth / 2, iY - 60, -0.1, FontTypes.TITLE_FONT,
                        WidgetTypes.WIDGET_GENERAL, -1, -1)
        sText = GlobalCyTranslator.getText("[COLOR_SELECTED_TEXT]", ()) + "<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_WB_CITY_ALL", ()) + " (+/-)</font></color>"
        ...
        for item in lBonus:
            ...
            if pCity.isNoBonus(item[1]):
                screen.setTableText("WBBonus", 0, iRow, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_CANCEL").getPath(), WidgetTypes.WIDGET_PYTHON, 1030, item[1], CvUtil.FONT_LEFT_JUSTIFY)
            else:
                sText = "<font=4>" + GlobalCyTranslator.getText("[ICON_TRADE]", ()) + "</font>"
    ...
    def editGreatPeopleFlat(self, iCount):
        ...
        iCount = min(iCount, gcgetPlayer(iPlayer).greatPeopleThreshold(False) - pCity.getGreatPeopleProgress())
    ...
    def editGreatPeopleProgress(self, item):
        ...
        pCity.changeGreatPeopleUnitProgress(item, min(iCount, gcgetPlayer(iPlayer).greatPeopleThreshold(False) - pCity.getGreatPeopleUnitProgress(item)))
