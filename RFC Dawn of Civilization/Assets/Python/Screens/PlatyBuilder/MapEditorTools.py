...
def changeCore(iPlayer, tPlot):
    x, y = tPlot
    bCore = gcmap.plot(x, y).isCore(iPlayer)
    plot = gcmap.plot(x, y)
...
def changeSettlerValue(iPlayer, tPlot, iValue):
    ...
    SettlerMaps.setMapValueByPlayer(iPlayer, x, y, iValue, True)
...
def exportCore(iPlayer, bForce=False):
    ...
    if not bCoreChanged:
        for (x, y) in utils.getWorldPlotsList():
            ...
            if gcmap.plot(x, y).isCore(iPlayer) != bOldCore:
                bCoreChanged = True
                break
    if bCoreChanged:
        lCorePlots = [(x, y) for (x, y) in utils.getWorldPlotsList() if gcmap.plot(x, y).isCore(iPlayer)]
            ...
            for y in reversed(range(iWorldY)):
                ...
                for x in range(iWorldX):
                    ...
                    if plot.isWater() or (plot.isPeak() and (x, y) not in Areas.lPeakExceptions):
                        ...
                        iValue = utils.getSettlerValue((x,y), iPlayer)
                    sLine += "%d,\t" % iValue
                if y == 0:
                    sLine += ")),"
                else:
...
def exportAreaExport(lPlots, bWaterException, bPeakException):
    if lPlots:
        BL, TR = getTLBR(lPlots)
        lExceptions = [(x, y) for (x, y) in utils.getPlotList(BL, TR) if (x, y) not in lPlots and (bWaterException and gcmap.plot(x, y).isWater()) and (bPeakException and gcmap.plot(x, y).isPeak())]

        if bWaterException:
            lExceptions.extend([(x, y) for (x, y) in lPlots if gcmap.plot(x, y).isWater() and (x, y) not in lExceptions])
        if bPeakException:
            lExceptions.extend([(x, y) for (x, y) in lPlots if gcmap.plot(x, y).isPeak() and (x, y) not in lExceptions])
