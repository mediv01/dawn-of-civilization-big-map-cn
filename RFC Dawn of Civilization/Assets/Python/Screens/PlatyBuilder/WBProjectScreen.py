...
class WBProjectScreen:
    ...
    def interfaceScreen(self, iTeamX):
        ...
        screen.setText("WBProjectExit", "Background", "<font=4>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper() + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, screen.getXResolution() - 30, screen.getYResolution() - 42, -0.1,
                       FontTypes.TITLE_FONT, WidgetTypes.WIDGET_CLOSE_SCREEN, -1, -1)
        screen.setLabel("ProjectHeader", "Background", "<font=4b>" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_PROJECT", ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, screen.getXResolution() / 2, 20, -0.1, FontTypes.TITLE_FONT,
        ...
        for i in xrange(gc.getMAX_TEAMS()):
            if gcgetTeam(i).isAlive():
                ...
                sName = gcgetPlayer(iLeader).getName()
        ...
        screen.addPullDownString("ProjectType", GlobalCyTranslator.getText("TXT_KEY_WB_CITY_ALL", ()), 0, 0, iProjectType == 0)
        screen.addPullDownString("ProjectType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_TEAM_PROJECT", ()), 1, 1, iProjectType == 1)
        screen.addPullDownString("ProjectType", GlobalCyTranslator.getText("TXT_KEY_PEDIA_WORLD_PROJECT", ()), 2, 2, iProjectType == 2)
        ...
        screen.addPullDownString("CurrentPage", GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_PROJECT", ()), 2, 2, True)
        ...
        sText = "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_GAME_OPTION_NO_BARBARIANS", ()) + "</font>"
        ...
        sText = "<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_WB_COPY_ALL", (GlobalCyTranslator.getText("TXT_KEY_MAIN_MENU_PLAYERS", ()),)) + "</font>"
        sColor = GlobalCyTranslator.getText("[COLOR_WARNING_TEXT]", ())
        if bApplyAll:
            sColor = GlobalCyTranslator.getText("[COLOR_POSITIVE_TEXT]", ())
    ...
    def handleInput(self, inputClass):
        ...
        if inputClass.getFunctionName() == "ChangeBy":
            ...
            sText = u"<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_WB_COPY_ALL", (GlobalCyTranslator.getText("TXT_KEY_MAIN_MENU_PLAYERS", ()),)) + "</font>"
            sColor = GlobalCyTranslator.getText("[COLOR_WARNING_TEXT]", ())
            if bApplyAll:
                sColor = GlobalCyTranslator.getText("[COLOR_POSITIVE_TEXT]", ())
            ...
            bNoBarb = not bNoBarb
            sText = u"<font=3b>" + GlobalCyTranslator.getText("TXT_KEY_GAME_OPTION_NO_BARBARIANS", ()) + "</font>"
            sColor = GlobalCyTranslator.getText("[COLOR_WARNING_TEXT]", ())
            if bNoBarb:
                sColor = GlobalCyTranslator.getText("[COLOR_POSITIVE_TEXT]", ())
    ...
    def editProject(self, item):
        ...
        if gc.getProjectInfo(item).isAllowsNukes() and GlobalCyGame.getProjectCreatedCount(item) == 0:
            GlobalCyGame.makeNukesValid(False)
    ...
    def modifyCount(self, item, pTeamX):
        ...
        if CvPlatyBuilderScreen.bPython and iCount > 0:
            pCapital = gcgetPlayer(pTeamX.getLeaderID()).getCapitalCity()
