...
class CvInfoScreen:
    ...
    def __init__(self, screenId):
        ...
        self.graphEnd = GlobalCyGame.getGameTurn() - 1
        self.graphZoom = self.graphEnd - GlobalCyGame.getStartTurn()
        self.nWidgetCount = 0
        self.nLineCount = 0
    ...
    def initText(self):
        ...
        self.TEXT_ECONOMY_MEASURE = (u"  %c" % GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)) + localText.getText("TXT_KEY_DEMO_SCREEN_ECONOMY_MEASURE", ())
        self.TEXT_INDUSTRY_MEASURE = (u"  %c" % GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)) + localText.getText("TXT_KEY_DEMO_SCREEN_INDUSTRY_MEASURE", ())
        self.TEXT_AGRICULTURE_MEASURE = (u"  %c" % GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)) + localText.getText("TXT_KEY_DEMO_SCREEN_AGRICULTURE_MEASURE", ())
        self.TEXT_MILITARY_MEASURE = ""
        self.TEXT_LAND_AREA_MEASURE = (u"  %c" % GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)) + localText.getText("TXT_KEY_DEMO_SCREEN_LAND_AREA_MEASURE", ())
        self.TEXT_POPULATION_MEASURE = ""
        self.TEXT_HAPPINESS_MEASURE = "%"
        self.TEXT_HEALTH_MEASURE = (u"  %c" % GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)) + localText.getText("TXT_KEY_DEMO_SCREEN_POPULATION_MEASURE", ())
        self.TEXT_IMP_EXP_MEASURE = (u"  %c" % GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)) + localText.getText("TXT_KEY_DEMO_SCREEN_ECONOMY_MEASURE", ())
        ...
        iTEXT_W = 0
        for i in range(self.NUM_SCORES):
            iTEXT_W += GlobalCyInterface.determineWidth(sTemp1[i])
        ...
        for i in range(self.NUM_SCORES):
            ...
            self.X_GRAPH_TEXT[i] = self.X_GRAPH_TEXT[i - 1] + GlobalCyInterface.determineWidth(sTemp1[i - 1]) + iText_Space
    ...
    def getLastTurn(self):
        return (gcgame.getReplayMessageTurn(gcgame.getNumReplayMessages() - 1))
    ...
    def showScreen(self, iTurn, iTabID, iEndGame):
        ...
        self.iStartTurn = 0
        for iI in range(gc.getGameSpeedInfo(gcgame.getGameSpeedType()).getNumTurnIncrements()):
            self.iStartTurn += gc.getGameSpeedInfo(gcgame.getGameSpeedType()).getGameTurnInfo(iI).iNumGameTurnsPerIncrement
        self.iStartTurn *= gc.getEraInfo(gcgame.getStartEra()).getStartPercent()
        self.iStartTurn /= 100
    ...
    def drawPermanentGraphWidgets(self):
        ...
        start = GlobalCyGame.getStartTurn()
        now = GlobalCyGame.getGameTurn()
        nTurns = now - start - 1
    ...
    def updateGraphButtons(self):
        screen = self.getScreen()
        screen.enable(self.graphLeftButtonID, self.graphEnd - self.graphZoom > GlobalCyGame.getStartTurn())
        screen.enable(self.graphRightButtonID, self.graphEnd < GlobalCyGame.getGameTurn() - 1)

    def checkGraphBounds(self):
        start = GlobalCyGame.getStartTurn()
        end = GlobalCyGame.getGameTurn() - 1
    ...
    def buildScoreCache(self, scoreType):
        ...
        for p in range(maxPlayer + 1):
            ...
            if (p not in self.aiPlayersMet) and not GlobalCyGame.isDebugMode():
                # Don't compute score for people we haven't met
                self.scoreCache[scoreType].append(None)
            else:
                self.scoreCache[scoreType].append([])
                firstTurn = utils.getScenarioStartTurn()  # GlobalCyGame.getStartTurn()
                thisTurn = GlobalCyGame.getGameTurn()
                turn = firstTurn
                while (turn <= thisTurn):
    ...
    def computeHistory(self, scoreType, iPlayer, iTurn):
        ...
        iScore = gcgetPlayer(iPlayer).getScoreHistory(iTurn)
        ...
        if (scoreType == self.TOTAL_SCORE):
            ...
            return gcgetPlayer(iPlayer).getEconomyHistory(iTurn)
        elif (scoreType == self.INDUSTRY_SCORE):
            return gcgetPlayer(iPlayer).getIndustryHistory(iTurn)
        elif (scoreType == self.AGRICULTURE_SCORE):
            return gcgetPlayer(iPlayer).getAgricultureHistory(iTurn)
        elif (scoreType == self.POWER_SCORE):
            return gcgetPlayer(iPlayer).getPowerHistory(iTurn)
        elif (scoreType == self.CULTURE_SCORE):
            return gcgetPlayer(iPlayer).getCultureHistory(iTurn)
        elif (scoreType == self.ESPIONAGE_SCORE):
            return gcgetPlayer(iPlayer).getEspionageHistory(iTurn)
        elif (scoreType == self.TECH_SCORE):
            return gcgetPlayer(iPlayer).getTechHistory(iTurn)
    ...
    def drawGraph(self, vGraphID_Locn):
        ...
        max = 0
        thisTurn = GlobalCyGame.getGameTurn()
        startTurn = utils.getScenarioStartTurn()  # GlobalCyGame.getStartTurn()
        ...
        for p in self.aiPlayersMet:
            ...
            # BUG: Change Graphs - start
            if AdvisorOpt.isGraphs():
                i = gcgetPlayer(p).getID()
                if not self.bPlayerInclude[i]:
                    continue
            # BUG: Change Graphs - end

            color = gc.getPlayerColorInfo(gcgetPlayer(p).getPlayerColor()).getColorTypePrimary()
            oldX = -1
            oldY = iH_GRAPH
            turn = lastTurn
    ...
    def drawLegend(self):
        ...
        if AdvisorOpt.isGraphs():
            for p in self.aiPlayersMet:
                ...
                if not gcgetPlayer(p).isAlive(): szPlayerName += " [" + self.BUG_LEGEND_DEAD + "]"

                if iW_LEGEND < self.X_LEGEND_TEXT + GlobalCyInterface.determineWidth(szPlayerName) + 10:
                    iW_LEGEND = self.X_LEGEND_TEXT + GlobalCyInterface.determineWidth(szPlayerName) + 10
        ...
        for p in self.aiPlayersMet:
            ...
            if AdvisorOpt.isGraphs():
                ...
                name = gcgetPlayer(p).getCivilizationShortDescription(0)

            i = gcgetPlayer(p).getID()
            if (self.bPlayerInclude[i]
                    or not AdvisorOpt.isGraphs()):
                textColorR = gcgetPlayer(p).getPlayerTextColorR()
                textColorG = gcgetPlayer(p).getPlayerTextColorG()
                textColorB = gcgetPlayer(p).getPlayerTextColorB()
                textColorA = gcgetPlayer(p).getPlayerTextColorA()

                lineColor = gc.getPlayerColorInfo(gcgetPlayer(p).getPlayerColor()).getColorTypePrimary()
        ...
        if AdvisorOpt.isGraphs():
            ...
            for p in self.aiPlayersMetNAEspionage:
                ...
                if not gcgetPlayer(p).isAlive():  # player is dead!
                    textColorR = 175
                    textColorG = 175
                    textColorB = 175
                    textColorA = gcgetPlayer(p).getPlayerTextColorA()
    ...
    def getPlayerName(self, ePlayer):
        ...
        if (ScoreOpt.isShowBothNames()):
            szPlayerName = szPlayerName + "/" + gcgetPlayer(ePlayer).getCivilizationShortDescription(0)
    ...
    def drawTextChart(self):
        ...
        pPlayer = gcgetPlayer(self.iActivePlayer)
        ...
        for iPlayerLoop in range(gc.getMAX_PLAYERS()):
            ...
            if (gcgetPlayer(iPlayerLoop).isAlive() and not gcgetPlayer(iPlayerLoop).isBarbarian() and not gcgetPlayer(iPlayerLoop).isMinorCiv()):

                iNumActivePlayers += 1

                pCurrPlayer = gcgetPlayer(iPlayerLoop)
    ...
    def drawTopCities(self):
        ...
        for iWidgetLoop in range(self.iNumCities):
            ...
            pPlot = GlobalCyMap.plot(iCityX, iCityY)
            ...
            if (pCity.isRevealed(gcgame.getActiveTeam(), False)):
                screen.addPlotGraphicGFC(self.szCityAnimWidgets[iWidgetLoop], self.X_CITY_ANIMATION, self.Y_ROWS_CITIES[iWidgetLoop] + self.Y_CITY_ANIMATION_BUFFER - self.H_CITY_ANIMATION / 2, self.W_CITY_ANIMATION, self.H_CITY_ANIMATION,
                                         pPlot, iDistance, False, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def determineCityData(self):
        ...
        for iRankLoop in range(5):
            ...
            if (pCity):
                ...
                pPlayer = gcgetPlayer(pCity.getOwner())

                iTurnYear = GlobalCyGame.getTurnYear(pCity.getGameTurnFounded())
                ...
                if (pCity.isRevealed(gcgame.getActiveTeam(), False) or gcgetTeam(pPlayer.getTeam()).isHasMet(gcgame.getActiveTeam())):
    ...
    def drawWondersList(self):
        ...
        if (self.iWonderID > -1):
            ...
            if (self.szWonderDisplayMode == self.szWDM_Project):
                ...
                szSpecialText = GlobalCyGameTextMgr.getProjectHelp(self.iWonderID, True, None)
                ...
                if (iTurnYear != -9999):  # -9999 used for wonders in progress
                    ...
                    pActivePlayer = gcgetPlayer(iActivePlayer)
                    tActivePlayer = gcgetTeam(pActivePlayer.getTeam())
                ...
                if (pWonderInfo.getGreatPeopleRateChange() != 0):
                    ...
                    szStatsText += szText + (u"%c" % GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR)) + "\n"
                ...
                szSpecialText = GlobalCyGameTextMgr.getBuildingHelp(self.iWonderID, True, False, False, None)
                splitText = string.split(szSpecialText, "\n")
                for special in splitText:
                    if len(special) != 0:
                        screen.appendListBoxString(listName, special, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
    ...
    def calculateWondersList(self):
        ...
        self.pActivePlayer = gcgetPlayer(GlobalCyGame.getActivePlayer())

        # Loop through players to determine Wonders
        for iPlayerLoop in range(gc.getMAX_PLAYERS()):

            pPlayer = gcgetPlayer(iPlayerLoop)
            iPlayerTeam = pPlayer.getTeam()

            # No barbs and only display national wonders for the active player's team
            if (pPlayer and not pPlayer.isBarbarian() and ((self.szWonderDisplayMode != self.szWDM_NatnlWonder) or (iPlayerTeam == gcgetTeam(gcgetPlayer(self.iActivePlayer).getTeam()).getID()))):
                ...
                for pCity in apCityList:
                    ...
                    pCityPlot = GlobalCyMap.plot(pCity.getX(), pCity.getY())
                    ...
                    if (self.szWonderDisplayMode == self.szWDM_Project):
                        for iProjectLoop in range(gc.getNumProjectInfos()):
                            ...
                            if (iProjectProd == iProjectLoop):
                                ...
                                # Project Mode
                                if (iPlayerTeam == gcgetTeam(gcgetPlayer(self.iActivePlayer).getTeam()).getID()):
                        ...
                        for iBuildingLoop in range(gc.getNumBuildingInfos()):
                            ...
                            if (self.szWonderDisplayMode == self.szWDM_WorldWonder and isWorldWonderClass(gc.getBuildingInfo(iBuildingLoop).getBuildingClassType())):
                                ...
                                if (iBuildingProd == iBuildingLoop):
                                    ...
                                    if (iPlayerTeam == gcgetPlayer(self.iActivePlayer).getTeam()):
                                        self.aaWondersBeingBuilt.append([iBuildingProd, pPlayer.getCivilizationShortDescription(0)])

                                if (pCity.getNumBuilding(iBuildingLoop) > 0):
                                    if (iPlayerTeam == gcgetPlayer(self.iActivePlayer).getTeam() or gcgetTeam(gcgetPlayer(self.iActivePlayer).getTeam()).isHasMet(iPlayerTeam)) or PYTHON_ALWAYS_SHOW_WONDER_BEING_BUILD == 1:
                            ...
                            # National/Team Wonder Mode
                            elif (self.szWonderDisplayMode == self.szWDM_NatnlWonder and (
                                    isNationalWonderClass(gc.getBuildingInfo(iBuildingLoop).getBuildingClassType()) or isTeamWonderClass(gc.getBuildingInfo(iBuildingLoop).getBuildingClassType()))):
                                ...
                                if (pCity.getNumBuilding(iBuildingLoop) > 0):
                                    ...
                                    if (iPlayerTeam == gcgetPlayer(self.iActivePlayer).getTeam() or gcgetTeam(gcgetPlayer(self.iActivePlayer).getTeam()).isHasMet(iPlayerTeam)):
                                        self.aaWondersBuilt.append([pCity.getBuildingOriginalTime(iBuildingLoop), iBuildingLoop, pPlayer.getCivilizationShortDescription(0), szCityName])
                                    else:
                                        self.aaWondersBuilt.append([pCity.getBuildingOriginalTime(iBuildingLoop), iBuildingLoop, localText.getText("TXT_KEY_UNKNOWN", ()), localText.getText("TXT_KEY_UNKNOWN", ())])
                                    self.iNumWonders += 1
                    ...
                    if (pTeam.isAlive() and not pTeam.isBarbarian()):
                        ...
                        for iProjectLoop in range(gc.getNumProjectInfos()):
                            ...
                            for iI in range(pTeam.getProjectCount(iProjectLoop)):
                                ...
                                if (iTeamLoop == gcgetPlayer(self.iActivePlayer).getTeam() or gcgetTeam(gcgetPlayer(self.iActivePlayer).getTeam()).isHasMet(iTeamLoop)):
                                    self.aaWondersBuilt.append([-9999, iProjectLoop, gcgetPlayer(iPlayerLoop).getCivilizationShortDescription(0), szCityName])
                                else:
                        ...
                        for iProjectLoop in range(gc.getNumProjectInfos()):
                            ...
                            if (iProjectProd == iProjectLoop):
                                ...
                                if (self.pActiveTeam.isHasMet(iPlayerTeam)
                                        ...
                                        and pCity.isRevealed(gcgame.getActiveTeam())):
                                    self.aaWondersBeingBuilt_BUG.append([iProjectLoop, pPlayer.getCivilizationShortDescription(0), pCity, iPlayerLoop])
                        ...
                        for iBuildingLoop in range(gc.getNumBuildingInfos()):
                            ...
                            if (self.szWonderDisplayMode == self.szWDM_WorldWonder and isWorldWonderClass(gc.getBuildingInfo(iBuildingLoop).getBuildingClassType())):
                                ...
                                if (iBuildingProd == iBuildingLoop):
                                    ...
                                    if (self.pActiveTeam.isHasMet(iPlayerTeam)
                                        and self.pActivePlayer.canDoEspionageMission(self.iInvestigateCityMission, pCity.getOwner(), pCity.plot(), -1)
                                        and pCity.isRevealed(gcgame.getActiveTeam())) or PYTHON_ALWAYS_SHOW_WONDER_BEING_BUILD == 1:
                                        self.aaWondersBeingBuilt_BUG.append([iBuildingLoop, pPlayer.getCivilizationShortDescription(0), pCity, iPlayerLoop])
                        ...
                        for iProjectLoop in range(gc.getNumProjectInfos()):
                            ...
                            for iI in range(pTeam.getProjectCount(iProjectLoop)):
                                ...
                                if (iTeamLoop == self.iActiveTeam or self.pActiveTeam.isHasMet(iTeamLoop)):
                                    self.aaWondersBuilt_BUG.append([-9999, iProjectLoop, True, gcgetPlayer(iPlayerLoop).getCivilizationShortDescription(0), None, iPlayerLoop])
                                else:
    ...
    def drawWondersList_BUG(self):
        ...
        for iWonderLoop in range(iWBB):
            ...
            color = -1
            ePlayerColor = gcgetPlayer(iPlayer).getPlayerColor()
            if ePlayerColor != -1:
            ...
            # Check to see if active player can see this city
            if pCity and pCity.isRevealed(gcgame.getActiveTeam()):
                szCityName = pCity.getName()
            else:
                szCityName = u""
        ...
        for iWonderLoop in range(self.iNumWonders):
            ...
            screen.appendTableRow(self.szWondersTable)
            if bKnown and pCity and pCity.isRevealed(gcgame.getActiveTeam()):
    ...
    def drawStatsTab(self):
        ...
        iMinutesPlayed = GlobalCyGame.getMinutesPlayed()
        ...
        iGridW = GlobalCyMap.getGridWidth()
        iGridH = GlobalCyMap.getGridHeight()
        for iX in range(iGridW):
            for iY in range(iGridH):
                plot = GlobalCyMap.plot(iX, iY)
        ...
        # Leaderhead graphic
        player = gcgetPlayer(gcgame.getActivePlayer())
        ...
        szText = u"<font=4b>" + gcgetPlayer(self.iActivePlayer).getCivilizationShortDescription(0) + u"</font>"
        ...
        if AdvisorOpt.isShowImprovements():
            ...
            screen.addTableControlGFC(szImprovementsTable, self.iNumImprovementStatsChartCols, self.X_STATS_BOTTOM_CHART + self.W_STATS_BOTTOM_CHART_UNITS + self.W_STATS_BOTTOM_CHART_BUILDINGS, self.Y_STATS_BOTTOM_CHART,
                                      self.W_STATS_BOTTOM_CHART_IMPROVEMENTS, self.H_STATS_BOTTOM_CHART,
                                      True, True, 32, 32, TableStyles.TABLE_STYLE_STANDARD)
    ...
    def getTurnDate(self, turn):
        ...
        year = GlobalCyGame.getTurnYear(turn)
            ...
            if (self.iActiveTab == self.iTopCitiesID):
                ...
                # BUG Wonders table
                    ...
                    if (inputClass.getMouseX() == 0):
                        screen.hideScreen()
                        pPlayer = gcgetPlayer(inputClass.getData1())
    ...
    def determineKnownPlayers(self, iEndGame=0):
        ...
        #	if (gcgetTeam(iLoopPlayerTeam).isEverAlive()):
        #		if (self.pActiveTeam.isHasMet(iLoopPlayerTeam) or GlobalCyGame.isDebugMode() or iEndGame != 0):
        #			if (	iDemographicsMission == -1 or
        #					self.pActivePlayer.canDoEspionageMission(iDemographicsMission, iLoopPlayer, None, -1) or
        #					iEndGame != 0 or
        #					iLoopPlayerTeam == GlobalCyGame.getActiveTeam()
        #				       or GlobalCyGame.isDebugMode()): #Rhye
        ...
        for iLoopPlayer in range(gc.getMAX_CIV_PLAYERS()):
            pLoopPlayer = gcgetPlayer(iLoopPlayer)
            iLoopPlayerTeam = pLoopPlayer.getTeam()
            if gcgetTeam(iLoopPlayerTeam).isEverAlive() and not pLoopPlayer.isMinorCiv():
                if self.pActiveTeam.isHasMet(iLoopPlayerTeam) or GlobalCyGame.isDebugMode() or iEndGame != 0:
