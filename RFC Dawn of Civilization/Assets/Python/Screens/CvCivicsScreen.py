...
class CvCivicsScreen:
    ...
    def interfaceScreen(self):
        ...
        self.setActivePlayer(gcgame.getActivePlayer())
        ...
        screen.addDDSGFC("CivicsBackground", GlobalCyArtFileMgr.getInterfaceArtInfo('MAINMENU_SLIDESHOW_LOAD').getPath(), 0, 0, self.W_SCREEN, self.H_SCREEN, WidgetTypes.WIDGET_GENERAL, -1, -1)
        ...
        if GlobalCyGame.isDebugMode():
            ...
            for iPlayer in xrange(gc.getMAX_PLAYERS()):
                if gcgetPlayer(iPlayer).isAlive():
                    screen.addPullDownString("DebugMenu", utils.getLeaderNameOrCivName(iPlayer), iPlayer, iPlayer, False)
    ...
    def placeContents(self):
        ''
        player = gcgetPlayer(self.iActivePlayer)
        screen = self.getScreen()
        ...
        for i in xrange(len(self.Categories)):
            ...
            screen.addDDSGFC(sName, GlobalCyArtFileMgr.getInterfaceArtInfo('SCREEN_BG').getPath(), iX, iY, self.W_CIVIC_CATEGORY, self.H_CIVIC_CATEGORY, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def showCivic(self, iCategory):
        ...
        screen.addMultilineText(sName, sText, iX + iSpacing, iY + self.BUTTON_LARGE + 20, self.BUTTON_LARGE + iSpacing, self.H_CIVIC_CATEGORY - self.BUTTON_LARGE - (iSpacing * 2), WidgetTypes.WIDGET_GENERAL, -1, -1,
        ...
        sText += u"<font=2>" + GlobalCyGameTextMgr.parseCivicInfo(iCivic, False, True, True) + "</font>"
    ...
    def updateCivicOptions(self, iCategory):
        ...
        for iCivic in xrange(gc.getNumCivicInfos()):
            if gc.getCivicInfo(iCivic).getCivicOptionType() == iCategory:
                ...
                screen.addCheckBoxGFC(sName, sButton, GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), xPos, iLine - 2, self.BUTTON_SMALL, self.BUTTON_SMALL, WidgetTypes.WIDGET_GENERAL, -1, -1,
                ...
                if self.SelectedCivics[iCategory] == iCivic:
                    ...
                    sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('COLOR_YELLOW'))
                elif player.canDoCivics(iCivic):
                    screen.show("CivicButton" + str(iCivic))
                else:
                    screen.hide("CivicButton" + str(iCivic))
                    sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('COLOR_LIGHT_GREY'))
    ...
    def updateCivicCosts(self):
        ...
        # Maintenance
        szText = GlobalCyTranslator.getText("TXT_KEY_CIVIC_SCREEN_UPKEEP", (player.getCivicUpkeep(self.DisplayedCivics, True),))
    ...
    def updateRevolution(self):
        ...
        if player.canRevolution(0):
            ...
            if iTurns > 0:
                # sAnarchy = GlobalCyTranslator.getText('TXT_KEY_CIVIC_SCREEN_ANARCHY_TIMER', (iTurns, ))
                sAnarchy = GlobalCyTranslator.getText('TXT_KEY_ANARCHY_TURNS', (iTurns,))
            ...
            if iTurns > 0:
                # sAnarchy = GlobalCyTranslator.getText('TXT_KEY_CIVIC_SCREEN_REVOLUTION_TIMER', (iTurns, ))
                sAnarchy = GlobalCyGameTextMgr.setRevolutionHelp(self.iActivePlayer)

        if STABILITY_SHOW_CIVICS_STABILITY_IN_CIVIC_SCREEN:
            iCivicCombinationStability = Stability.getCivicStability(self.iActivePlayer, self.DisplayedCivics)
            iCivicEraTechStability = Stability.getCivicEraStabilty(self.iActivePlayer, self.DisplayedCivics)
            sAnarchy += "   组合稳定度:" + str(iCivicCombinationStability) + "，时代稳定度:" + str(iCivicEraTechStability)
            ...
            if bHoverOn:
                if bGood:
                    if player.canDoCivics(iCivic):
                        sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('COLOR_GREEN'))
                    else:
                        sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('COLOR_PLAYER_MIDDLE_GREEN'))
                elif bBad:
                    if player.canDoCivics(iCivic):
                        sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('COLOR_RED'))
                    else:
                        sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('COLOR_PLAYER_CANADA_RED'))
                else:
                    if player.canDoCivics(iCivic):
                        sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('WHITE'))
                    else:
                        sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('COLOR_LIGHT_GREY'))
            else:
                if self.SelectedCivics[iCategory] == iCivic:
                    ...
                    screen.show("CivicButton" + str(iCivic))
                    sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('COLOR_WHITE'))
                else:
                    screen.hide("CivicButton" + str(iCivic))
                    sText = GlobalCyTranslator.changeTextColor(sText, gc.getInfoTypeForString('COLOR_LIGHT_GREY'))
            screen.setText(sName, "", sText, CvUtil.FONT_RIGHT_JUSTIFY, xPos - self.MARGIN, iLine, 0, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
