...
class CvForeignAdvisor:
        ...
        if (GlobalCyGame.isDebugMode()):
            ...
            for j in range(gc.getMAX_PLAYERS()):
                if (gcgetPlayer(j).isAlive()):
                    # screen.addPullDownString(self.szDropdownName, gcgetPlayer(j).getName(), j, j, False ) #Rhye
                    screen.addPullDownString(self.szDropdownName, gcgetPlayer(j).getCivilizationShortDescription(0), j, j, False)  # Rhye

        GlobalCyInterface.setDirty(InterfaceDirtyBits.Foreign_Screen_DIRTY_BIT, False)
    ...
    def drawContents(self, bInitial):
        ...
        if (self.iScreen != FOREIGN_RELATIONS_SCREEN):
            screen.setText(szRelationsId, "", u"<font=4>" + localText.getText("TXT_KEY_FOREIGN_ADVISOR_RELATIONS", ()).upper() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT,
                           WidgetTypes.WIDGET_FOREIGN_ADVISOR, FOREIGN_RELATIONS_SCREEN, -1)
        else:
            screen.setText(szRelationsId, "", u"<font=4>" + localText.getColorText("TXT_KEY_FOREIGN_ADVISOR_RELATIONS", (), gc.getInfoTypeForString("COLOR_YELLOW")).upper() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, xLink, self.Y_LINK, 0,
                           FontTypes.TITLE_FONT, WidgetTypes.WIDGET_FOREIGN_ADVISOR, -1, -1)
        xLink += self.DX_LINK

        szBonusId = self.getNextWidgetName()
        if (self.iScreen != FOREIGN_BONUS_SCREEN):
            screen.setText(szBonusId, "", u"<font=4>" + localText.getText("TXT_KEY_FOREIGN_ADVISOR_RESOURCES", ()).upper() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT,
                           WidgetTypes.WIDGET_FOREIGN_ADVISOR, FOREIGN_BONUS_SCREEN, -1)
        else:
            screen.setText(szBonusId, "", u"<font=4>" + localText.getColorText("TXT_KEY_FOREIGN_ADVISOR_RESOURCES", (), gc.getInfoTypeForString("COLOR_YELLOW")).upper() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, xLink, self.Y_LINK, 0,
                           FontTypes.TITLE_FONT, WidgetTypes.WIDGET_FOREIGN_ADVISOR, -1, -1)
        xLink += self.DX_LINK

        szTechId = self.getNextWidgetName()
        if (self.iScreen != FOREIGN_TECH_SCREEN):
            screen.setText(szTechId, "", u"<font=4>" + localText.getText("TXT_KEY_FOREIGN_ADVISOR_TECHS", ()).upper() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_FOREIGN_ADVISOR,
                           FOREIGN_TECH_SCREEN, -1)
        else:
            screen.setText(szTechId, "", u"<font=4>" + localText.getColorText("TXT_KEY_FOREIGN_ADVISOR_TECHS", (), gc.getInfoTypeForString("COLOR_YELLOW")).upper() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, xLink, self.Y_LINK, 0,
                           FontTypes.TITLE_FONT, WidgetTypes.WIDGET_FOREIGN_ADVISOR, -1, -1)
        xLink += self.DX_LINK

        szActiveId = self.getNextWidgetName()
        if (self.iScreen != FOREIGN_ACTIVE_TRADE_SCREEN):
            screen.setText(szActiveId, "", u"<font=4>" + localText.getText("TXT_KEY_FOREIGN_ADVISOR_ACTIVE", ()).upper() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT,
                           WidgetTypes.WIDGET_FOREIGN_ADVISOR, FOREIGN_ACTIVE_TRADE_SCREEN, -1)
        else:
            screen.setText(szActiveId, "", u"<font=4>" + localText.getColorText("TXT_KEY_FOREIGN_ADVISOR_ACTIVE", (), gc.getInfoTypeForString("COLOR_YELLOW")).upper() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, xLink, self.Y_LINK, 0,
                           FontTypes.TITLE_FONT, WidgetTypes.WIDGET_FOREIGN_ADVISOR, -1, -1)
        xLink += self.DX_LINK
    ...
    def drawActive(self):
        ...
        # Get the Players
        playerActive = gcgetPlayer(self.iActiveLeader)
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            if (gcgetPlayer(iLoopPlayer).isAlive() and iLoopPlayer != self.iActiveLeader and not gcgetPlayer(iLoopPlayer).isBarbarian() and not gcgetPlayer(iLoopPlayer).isMinorCiv()):
                if (gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isHasMet(gcgetPlayer(self.iActiveLeader).getTeam()) or gcgame.isDebugMode()):
                    nDeals = 0
                    for i in range(gcgame.getIndexAfterLastDeal()):
                        deal = gcgame.getDeal(i)
        ...
        for j in range(nNumPLayers):
            ...
            screen.attachPanel(mainPanelName, playerPanelName, gcgetPlayer(iLoopPlayer).getCivilizationShortDescription(0), "", False, True, PanelStyles.PANEL_STYLE_MAIN)

            screen.attachLabel(playerPanelName, "", "   ")

            screen.attachImageButton(playerPanelName, "", gc.getLeaderHeadInfo(gcgetPlayer(iLoopPlayer).getLeaderType()).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_LEADERHEAD, iLoopPlayer, -1, False)
            ...
            for i in range(gcgame.getIndexAfterLastDeal()):
                ...
                if (deal.getFirstPlayer() == iLoopPlayer and deal.getSecondPlayer() == self.iActiveLeader and not deal.isNone()) or (deal.getSecondPlayer() == iLoopPlayer and deal.getFirstPlayer() == self.iActiveLeader):
                    screen.appendListBoxString(dealPanelName, GlobalCyGameTextMgr.getDealString(deal, iLoopPlayer), WidgetTypes.WIDGET_DEAL_KILL, deal.getID(), -1, CvUtil.FONT_LEFT_JUSTIFY)
                    iRow += 1
    ...
    def drawPossibleDeals(self):
        ...
        # Get the Players
        playerActive = gcgetPlayer(self.iActiveLeader)
        playerSelected = gcgetPlayer(self.iSelectedLeader)
        ...
        if (gcgetTeam(playerActive.getTeam()).isGoldTrading() or gcgetTeam(playerSelected.getTeam()).isGoldTrading()):
            if (self.iScreen == FOREIGN_BONUS_SCREEN):
        ...
        if (self.iScreen == FOREIGN_BONUS_SCREEN):
            ...
            for iLoopBonus in range(gc.getNumBonusInfos()):
                ...
                if (self.iSelectedLeader == self.iActiveLeader):
                    ...
                    for iLoopPlayer in range(gc.getMAX_PLAYERS()):
                        if (gcgetPlayer(iLoopPlayer).isAlive() and not gcgetPlayer(iLoopPlayer).isBarbarian() and not gcgetPlayer(iLoopPlayer).isMinorCiv() and gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isHasMet(
                                gcgetPlayer(self.iActiveLeader).getTeam())):
                            if (iLoopPlayer != self.iActiveLeader and gcgetPlayer(self.iActiveLeader).canTradeItem(iLoopPlayer, tradeData, False)):
                                bTradeable = True
                                iLoopPlayer = gc.getMAX_PLAYERS()  # exit for loop
                else:
                    # display resources that you can trade to the selected leader
                    bTradeable = gcgetPlayer(self.iActiveLeader).canTradeItem(self.iSelectedLeader, tradeData, False)
        ...
        screen.addCheckBoxGFCAt(activePlayerPanelName, szName, gc.getLeaderHeadInfo(gcgetPlayer(self.iActiveLeader).getLeaderType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), 10, 0, self.W_LEADER,
                                self.H_LEADER, WidgetTypes.WIDGET_LEADERHEAD, self.iActiveLeader, -1, ButtonStyles.BUTTON_STYLE_LABEL, False)
        if (self.iSelectedLeader == self.iActiveLeader):
        ...
        # Their leaderheads
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            if (gcgetPlayer(iLoopPlayer).isAlive() and iLoopPlayer != self.iActiveLeader and (
                    gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isHasMet(gcgetPlayer(self.iActiveLeader).getTeam()) or gcgame.isDebugMode()) and not gcgetPlayer(iLoopPlayer).isBarbarian() and not gcgetPlayer(
                iLoopPlayer).isMinorCiv()):

                currentPlayerPanelName = self.getNextWidgetName()
                szPlayerName = gcgetPlayer(iLoopPlayer).getCivilizationShortDescription(0)
                if (gcgetTeam(playerActive.getTeam()).isGoldTrading() or gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isGoldTrading()):
                    if (self.iScreen == FOREIGN_BONUS_SCREEN):
                        szPlayerName += u" : " + localText.getText("TXT_KEY_FOREIGN_ADVISOR_GOLD_PER_TURN_FOR_TRADE", (gcgetPlayer(iLoopPlayer).AI_maxGoldPerTurnTrade(self.iActiveLeader),))
                    elif (self.iScreen == FOREIGN_TECH_SCREEN):
                        szPlayerName += u" : " + localText.getText("TXT_KEY_FOREIGN_ADVISOR_GOLD_FOR_TRADE", (gcgetPlayer(iLoopPlayer).AI_maxGoldTrade(self.iActiveLeader),))
                ...
                elif (not gcgetTeam(playerActive.getTeam()).isTechTrading() and not gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isTechTrading()):
                ...
                if (self.iScreen == FOREIGN_BONUS_SCREEN):
                    if (not playerActive.canTradeNetworkWith(iLoopPlayer) and not gcgame.isDebugMode()):
                        ...
                        for iLoopBonus in range(gc.getNumBonusInfos()):
                            tradeData.iData = iLoopBonus
                            if (gcgetPlayer(iLoopPlayer).canTradeItem(self.iActiveLeader, tradeData, False)):
                                if (gcgetPlayer(iLoopPlayer).getTradeDenial(self.iActiveLeader, tradeData) == DenialTypes.NO_DENIAL):
                    ...
                    if (not gcgetTeam(playerActive.getTeam()).isTechTrading() and not gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isTechTrading() and not gcgame.isDebugMode()):
                        screen.attachMultiListControlGFC(currentPlayerPanelName, "ChildTrade" + currentPlayerPanelName, "", 1, self.BUTTON_SIZE, self.BUTTON_SIZE, TableStyles.TABLE_STYLE_STANDARD)
                        screen.appendMultiListButton("ChildTrade" + currentPlayerPanelName, ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_CANCEL").getPath(), 0, WidgetTypes.WIDGET_GENERAL, -1, -1, False)
                    else:
                        listTradeable = []
                        ...
                        for iLoopTech in range(gc.getNumTechInfos()):
                            ...
                            elif (gcgetTeam(gcgetPlayer(iLoopPlayer).getTeam()).isHasTech(iLoopTech) and playerActive.canResearch(iLoopTech, False)):
                ...
                screen.attachLabel(currentPlayerPanelName, "", "   ")
                szName = self.getNextWidgetName()
                screen.addCheckBoxGFCAt(currentPlayerPanelName, szName, gc.getLeaderHeadInfo(gcgetPlayer(iLoopPlayer).getLeaderType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), 10, 0, self.W_LEADER,
                                        self.H_LEADER, WidgetTypes.WIDGET_LEADERHEAD, iLoopPlayer, -1, ButtonStyles.BUTTON_STYLE_LABEL, False)
    ...
    def drawRelations(self, bInitial):
        ...
        for iPlayer in range(gc.getMAX_PLAYERS()):
            ...
            if (player.isAlive() and iPlayer != self.iActiveLeader and (
                    gcgetTeam(player.getTeam()).isHasMet(gcgetPlayer(self.iActiveLeader).getTeam()) or gcgame.isDebugMode()) and not player.isBarbarian() and not player.isMinorCiv()):
        ...
        # screen.addCheckBoxGFC(szLeaderHead, gc.getLeaderHeadInfo(gcgetPlayer(self.iActiveLeader).getLeaderType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), self.X_LEADER_CIRCLE_TOP - iLeaderWidth/2, int(fLeaderTop), iLeaderWidth, iLeaderHeight, WidgetTypes.WIDGET_LEADERHEAD, self.iActiveLeader, -1, ButtonStyles.BUTTON_STYLE_LABEL) #Rhye
        screen.addCheckBoxGFC(szLeaderHead, gc.getCivilizationInfo(gcgetPlayer(self.iActiveLeader).getCivilizationType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
                              self.X_LEADER_CIRCLE_TOP - iLeaderWidth / 2, int(fLeaderTop), iLeaderWidth, iLeaderHeight, WidgetTypes.WIDGET_LEADERHEAD, self.iActiveLeader, -1, ButtonStyles.BUTTON_STYLE_LABEL)  # Rhye
        ...
        for iPlayer in leaderMap.keys():
            ...
            playerBase = gcgetPlayer(iBaseLeader)
            ...
            screen.addCheckBoxGFC(szLeaderHead, gc.getCivilizationInfo(player.getCivilizationType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), int(fX), int(fY), iLeaderWidth, iLeaderHeight,
                                  WidgetTypes.WIDGET_LEADERHEAD, iPlayer, iBaseLeader, ButtonStyles.BUTTON_STYLE_LABEL)  # Rhye
            ...
            szName = self.getNextWidgetName()
            if (gcgetTeam(player.getTeam()).isHasMet(playerBase.getTeam()) and iBaseLeader != iPlayer):
                # Rhye - start
                # szText = " (" + gc.getAttitudeInfo(gcgetPlayer(iPlayer).AI_getAttitude(iBaseLeader)).getDescription()
                # if (iBaseLeader != iPlayer):
                #	if (gcgetTeam(player.getTeam()).isVassal(playerBase.getTeam())):
                #		szText += ", " + localText.getText("TXT_KEY_MISC_VASSAL_SHORT", ())
                #	elif (gcgetTeam(playerBase.getTeam()).isVassal(player.getTeam())):
                #		szText += ", " + localText.getText("TXT_KEY_MISC_MASTER", ())
                # szText += ")"
                if (len(leaderMap.keys()) >= 16):
                    szText = u"<font=1>" + " (" + gc.getAttitudeInfo(gcgetPlayer(iPlayer).AI_getAttitude(iBaseLeader)).getDescription() + u"</font>"
                    if (iBaseLeader != iPlayer):
                        if (gcgetTeam(player.getTeam()).isVassal(playerBase.getTeam())):
        ...
        for iSelectedLeader in range(gc.getMAX_PLAYERS()):
            bDisplayed = (not gcgetPlayer(iSelectedLeader).isBarbarian() and not gcgetPlayer(iSelectedLeader).isMinorCiv() and gcgetPlayer(iSelectedLeader).isAlive() and (
                    gcgame.isDebugMode() or gcgetTeam(playerActive.getTeam()).isHasMet(gcgetPlayer(iSelectedLeader).getTeam())))
            if iSelectedLeader in self.listSelectedLeaders or (bNoLeadersSelected and bDisplayed):
                ...
                for iPlayer in leaderMap.keys():
                    ...
                    if (iSelectedLeader != iPlayer):
                        if (player.getTeam() == gcgetPlayer(iSelectedLeader).getTeam()):
                        ...
                        elif (gcgetTeam(player.getTeam()).isVassal(gcgetPlayer(iSelectedLeader).getTeam()) or gcgetTeam(gcgetPlayer(iSelectedLeader).getTeam()).isVassal(player.getTeam())):
                            szName = self.getNextLineName()
                        ...
                        elif (gcgetTeam(player.getTeam()).isHasMet(gcgetPlayer(iSelectedLeader).getTeam())):
                            if (gcgetTeam(player.getTeam()).isAtWar(gcgetPlayer(iSelectedLeader).getTeam())):
                                szName = self.getNextLineName()
                                ...
                                bJustPeace = True
                                if (gcgetTeam(player.getTeam()).isOpenBorders(gcgetPlayer(iSelectedLeader).getTeam())):
                                    fDy = fYSelected - fY
                                    fDx = fXSelected - fX
                                    ...
                                    szName = self.getNextLineName()
                                    screen.addLineGFC(self.BACKGROUND_ID, szName, int(fXSelected + fSecondLineOffsetX), int(fYSelected + fSecondLineOffsetY), int(fX + fSecondLineOffsetX), int(fY + fSecondLineOffsetY),
                                                      gc.getInfoTypeForString("COLOR_CITY_GREEN"))
                                    bJustPeace = False
                                if (gcgetTeam(player.getTeam()).isDefensivePact(gcgetPlayer(iSelectedLeader).getTeam())):
                                    szName = self.getNextLineName()
                ...
                if (player.getTeam() == gcgetPlayer(iSelectedLeader).getTeam()):
                    ...
                    if (gcgetTeam(player.getTeam()).isAtWar(gcgetPlayer(iSelectedLeader).getTeam())):
                        ...
                        if (gcgetTeam(player.getTeam()).isOpenBorders(gcgetPlayer(iSelectedLeader).getTeam())):
                            ...
                            fSecondLineOffsetX = -self.LINE_WIDTH * math.sin(fTheta)
                            szName = self.getNextLineName()
                            screen.addLineGFC(self.BACKGROUND_ID, szName, int(fXSelected + fSecondLineOffsetX), int(fYSelected + fSecondLineOffsetY), int(self.X_LEADER_CIRCLE_TOP + fSecondLineOffsetX),
                                              int(fLeaderTop + iLeaderHeight / 2 + fSecondLineOffsetY), gc.getInfoTypeForString("COLOR_CITY_GREEN"))
                            bJustPeace = False
                        if (gcgetTeam(player.getTeam()).isDefensivePact(gcgetPlayer(iSelectedLeader).getTeam())):
                            szName = self.getNextLineName()
    ...
    def update(self, fDelta):
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.Foreign_Screen_DIRTY_BIT) == True):
            GlobalCyInterface.setDirty(InterfaceDirtyBits.Foreign_Screen_DIRTY_BIT, False)
            self.drawContents(False)
        return
