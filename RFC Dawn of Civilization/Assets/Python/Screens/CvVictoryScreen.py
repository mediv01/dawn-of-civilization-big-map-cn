...
class CvVictoryScreen:
    ...
    def interfaceScreen(self):
        ...
        self.iActivePlayer = GlobalCyGame.getActivePlayer()
        self.activePlayer = gcgetPlayer(self.iActivePlayer)
        if self.iScreen == -1:
            self.iScreen = VICTORY_CONDITION_SCREEN
        ...
        # Header...
        screen.setLabel(self.HEADER_ID, "Background", u"<font=4b>" + localText.getText("TXT_KEY_VICTORY_SCREEN_TITLE", ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.Y_TITLE, self.Z_CONTROLS, FontTypes.TITLE_FONT,
    ...
    def drawTabs(self):
        ...
        xLink = self.X_LINK
        if (self.iScreen != VICTORY_CONDITION_SCREEN):
            screen.setText(self.VC_TAB_ID, "", u"<font=4>" + localText.getText("TXT_KEY_MAIN_MENU_VICTORIES", ()).upper() + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1,
                           -1)
        else:
            screen.setText(self.VC_TAB_ID, "", u"<font=4>" + localText.getColorText("TXT_KEY_MAIN_MENU_VICTORIES", (), gc.getInfoTypeForString("COLOR_YELLOW")).upper() + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0,
                           FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        xLink += self.DX_LINK

        if (self.iScreen != GAME_SETTINGS_SCREEN):
            screen.setText(self.SETTINGS_TAB_ID, "", u"<font=4>" + localText.getText("TXT_KEY_MAIN_MENU_SETTINGS", ()).upper() + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL,
                           -1, -1)
        else:
            screen.setText(self.SETTINGS_TAB_ID, "", u"<font=4>" + localText.getColorText("TXT_KEY_MAIN_MENU_SETTINGS", (), gc.getInfoTypeForString("COLOR_YELLOW")).upper() + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0,
                           FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        xLink += self.DX_LINK

        if self.bVoteTab:
            if (self.iScreen != UN_RESOLUTION_SCREEN):
                screen.setText(self.UN_RESOLUTION_TAB_ID, "", u"<font=4>" + localText.getText("TXT_KEY_VOTING_TITLE", ()).upper() + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT,
                               WidgetTypes.WIDGET_GENERAL, -1, -1)
            else:
                screen.setText(self.UN_RESOLUTION_TAB_ID, "", u"<font=4>" + localText.getColorText("TXT_KEY_VOTING_TITLE", (), gc.getInfoTypeForString("COLOR_YELLOW")).upper() + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0,
                               FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
            xLink += self.DX_LINK

            if (self.iScreen != UN_MEMBERS_SCREEN):
                screen.setText(self.UN_MEMBERS_TAB_ID, "", u"<font=4>" + localText.getText("TXT_KEY_MEMBERS_TITLE", ()).upper() + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0, FontTypes.TITLE_FONT,
                               WidgetTypes.WIDGET_GENERAL, -1, -1)
            else:
                screen.setText(self.UN_MEMBERS_TAB_ID, "", u"<font=4>" + localText.getColorText("TXT_KEY_MEMBERS_TITLE", (), gc.getInfoTypeForString("COLOR_YELLOW")).upper() + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xLink, self.Y_LINK, 0,
                               FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
            xLink += self.DX_LINK
    ...
    def showDebugInfoScreen(self):
        ...
        # logics - start
        activePlayer = gcgetPlayer(self.iActivePlayer)
        activeTeam = gcgetTeam(activePlayer.getTeam())
    ...
    def showVotingScreen(self):
            ...
            for j in range(gc.getNumVoteSourceInfos()):
                if (gc.getBuildingInfo(i).getVoteSourceType() == j):
                    ...
                    for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
                        if (gcgetTeam(iLoopTeam).isAlive() and not gcgetTeam(iLoopTeam).isMinorCiv() and not gcgetTeam(iLoopTeam).isBarbarian()):
                            if (gcgetTeam(iLoopTeam).getBuildingClassCount(gc.getBuildingInfo(i).getBuildingClassType()) > 0):
                                iUNTeam = iLoopTeam
                                if (iLoopTeam == iActiveTeam or gcgame.isDebugMode() or gcgetTeam(activePlayer.getTeam()).isHasMet(iLoopTeam)):
                                    bUnknown = False
                                break
        ...
        for (iVoteBuildingClass, iUNTeam, bUnknown) in aiVoteBuildingClass:
            ...
            if (iUNTeam != -1):
                if bUnknown:
                    ...
                    # Rhye - start
                    # szName = gcgetTeam(iUNTeam).getName()
                    szName = gcgetPlayer(iUNTeam).getCivilizationShortDescription(0)
                # Rhye - end
        ...
        for i in range(gc.getNumVoteSourceInfos()):
            if (gcgame.canHaveSecretaryGeneral(i) and -1 != gcgame.getSecretaryGeneral(i)):
                iRow = screen.appendTableRow(szTable)
                ...
                screen.setTableText(szTable, 1, iRow, gcgetPlayer(gcgame.getSecretaryGeneral(i)).getCivilizationShortDescription(0), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)

            for iLoop in range(gc.getNumVoteInfos()):
                if gcgame.countPossibleVote(iLoop, i) > 0:
                    info = gc.getVoteInfo(iLoop)
                    if gcgame.isChooseElection(iLoop):
                        iRow = screen.appendTableRow(szTable)
                        screen.setTableText(szTable, 0, iRow, info.getDescription(), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        if gcgame.isVotePassed(iLoop):
                            ...
                            screen.setTableText(szTable, 1, iRow, localText.getText("TXT_KEY_POPUP_ELECTION_OPTION", (u"", gcgame.getVoteRequired(iLoop, i), gcgame.countPossibleVote(iLoop, i))), "", WidgetTypes.WIDGET_GENERAL,
    ...
    def getVoteAvailable(self):
        ...
        for i in range(gc.getNumVoteSourceInfos()):
            if gcgame.isDiploVote(i):
                if (gcgame.getVoteSourceReligion(i) != -1):
                    iRelVote = i
                else:
                    iUNVote = i

            if (gcgame.canHaveSecretaryGeneral(i)
                    and gcgame.getSecretaryGeneral(i) != -1):
    ...
    def showMembersScreen_BUG(self, iRelVote, iUNVote, iActiveVote, iVoteIdx):
        ...
        if (gcgame.getVoteSourceReligion(iActiveVote) != -1):
            sTableHeader += " (" + gc.getReligionInfo(gcgame.getVoteSourceReligion(iActiveVote)).getDescription() + ")"
        ...
        for j in range(gc.getMAX_TEAMS()):
            ...
            if (gcgetTeam(j).isAlive()
                    and gcgame.isTeamVoteEligible(j, iActiveVote)):
                BugUtil.debug("CvVictoryScreen: Team %i, %s <- vote eligible ", j, gcgetPlayer(j).getCivilizationShortDescription(0))
                if iCandTeam1 == -1:
                    iCandTeam1 = j
                else:
                    iCandTeam2 = j
        ...
        iX -= 10 + GlobalCyInterface.determineWidth(sText)
        ...
        for j in range(gc.getMAX_PLAYERS()):
            ...
            if (pPlayer.isAlive()
                ...
                if (gcgame.canHaveSecretaryGeneral(iActiveVote)
                        and iAPUNTeam == pPlayer.getTeam()
                        and gcgame.getSecretaryGeneral(iActiveVote) == -1):
                    lPlayerStatus = 0
                    lPlayerLabel = gc.getVoteSourceInfo(iActiveVote).getSecretaryGeneralText()
                elif (gcgame.canHaveSecretaryGeneral(iActiveVote)
                      and gcgame.getSecretaryGeneral(iActiveVote) == pPlayer.getTeam()):
                    lPlayerStatus = 1
                    lPlayerLabel = gc.getVoteSourceInfo(iActiveVote).getSecretaryGeneralText()
        ...
        for lMember in lMembers:
            ...
            if (iCandTeam1 == gcgetPlayer(iMember).getTeam()
                    and iCand1Known == 1):
                iVoteCand[0] = lMemberVotes
            if (iCandTeam2 == gcgetPlayer(iMember).getTeam()
                    and iCand2Known == 1):
        ...
        iMaxVotes = 0
        for iLoop in range(gc.getNumVoteInfos()):
            if gcgame.countPossibleVote(iLoop, iActiveVote) > 0:
                iMaxVotes = gcgame.countPossibleVote(iLoop, iActiveVote)
                break
    ...
    def showMembersScreen_NonBUG(self):
        ...
        for i in range(gc.getNumVoteSourceInfos()):
            if gcgame.isDiploVote(i):
                ...
                if (gcgame.getVoteSourceReligion(i) != -1):
                    screen.setTableText(szTable, 1, iRow, gc.getReligionInfo(gcgame.getVoteSourceReligion(i)).getDescription(), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                ...
                print
                iSecretaryGeneralVote
                for j in range(gc.getMAX_PLAYERS()):
                    if gcgetPlayer(j).isAlive() and gcgetTeam(iActiveTeam).isHasMet(gcgetPlayer(j).getTeam()):
                        # szPlayerText = gcgetPlayer(j).getName() #Rhye
                        szPlayerText = gcgetPlayer(j).getCivilizationShortDescription(0)  # Rhye
                        if (-1 != iSecretaryGeneralVote):
                            szPlayerText += localText.getText("TXT_KEY_VICTORY_SCREEN_PLAYER_VOTES", (gcgetPlayer(j).getVotes(iSecretaryGeneralVote, i),))
                        if (gcgame.canHaveSecretaryGeneral(i) and gcgame.getSecretaryGeneral(i) == gcgetPlayer(j).getTeam()):
                            iRow = screen.appendTableRow(szTable)
                            screen.setTableText(szTable, 0, iRow, szPlayerText, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        ...
                        elif (gcgetPlayer(j).isFullMember(i)):
                            iRow = screen.appendTableRow(szTable)
                            screen.setTableText(szTable, 0, iRow, szPlayerText, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        ...
                        elif (gcgetPlayer(j).isVotingMember(i)):
                            iRow = screen.appendTableRow(szTable)
                            screen.setTableText(szTable, 0, iRow, szPlayerText, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
    ...
    def getVoteReq(self, i, iVote):
        ...
        iMinVotes = 999999
        for iLoop in range(gc.getNumVoteInfos()):
            iVoteReq = gcgame.getVoteRequired(iLoop, i)
            if iVoteReq > 0:
    ...
    def getCandStatusName(self, iCand):
        ...
        if iActiveTeam == iCand:
            return 1, gcgetPlayer(iCand).getCivilizationShortDescription(0)

        if gcgetTeam(iActiveTeam).isHasMet(iCand):
            return 1, gcgetPlayer(iCand).getCivilizationShortDescription(0)
        else:
    ...
    def getPlayerStatusName(self, iPlayer):
        ...
        if gcgetTeam(iActiveTeam).isHasMet(iPlayerTeam):
            return True, pPlayer.getCivilizationShortDescription(0)
        else:
    ...
    def getVoteTotalColor(self, iVoteReq, iVoteTotal, iVoteCand, bWinner, bVictoryVote):
        print
        "%i %i %i" % (iVoteReq, iVoteTotal, iVoteCand)
        if not bWinner:
            return -1
    ...
    def showGameSettingsScreen(self):
        ...
        screen.addPanel(szSettingsPanel, localText.getText("TXT_KEY_MAIN_MENU_SETTINGS", ()).upper(), "", True, True, self.SETTINGS_PANEL_X1, self.SETTINGS_PANEL_Y - 10, self.SETTINGS_PANEL_WIDTH, self.SETTINGS_PANEL_HEIGHT,
                        PanelStyles.PANEL_STYLE_MAIN)
        szSettingsTable = self.getNextWidgetName()
        screen.addListBoxGFC(szSettingsTable, "", self.SETTINGS_PANEL_X1 + self.MARGIN, self.SETTINGS_PANEL_Y + self.MARGIN, self.SETTINGS_PANEL_WIDTH - 2 * self.MARGIN, self.SETTINGS_PANEL_HEIGHT - 2 * self.MARGIN,
        ...
        screen.appendListBoxStringNoUpdate(szSettingsTable, localText.getText("TXT_KEY_LEADER_CIV_DESCRIPTION", (activePlayer.getNameKey(), activePlayer.getCivilizationShortDescriptionKey())), WidgetTypes.WIDGET_GENERAL, -1, -1,
                                           CvUtil.FONT_LEFT_JUSTIFY)
        screen.appendListBoxStringNoUpdate(szSettingsTable, u"     (" + GlobalCyGameTextMgr.parseLeaderTraits(activePlayer.getLeaderType(), activePlayer.getCivilizationType(), True, False) + ")", WidgetTypes.WIDGET_GENERAL, -1, -1,
                                           CvUtil.FONT_LEFT_JUSTIFY)
        ...
        screen.appendListBoxStringNoUpdate(szSettingsTable, " ", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
        #		screen.appendListBoxStringNoUpdate(szSettingsTable, gcmap.getMapScriptName(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )

        ## Start HOF MOD V1.61.001  4/8
        if (showHOFSettingChecks and not GameSetUpCheck.isMapScriptOK()):
            screen.appendListBoxStringNoUpdate(szSettingsTable, gcmap.getMapScriptName() + " " + self.BuffyWarningNotAllowed, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
            failedHOFChecks = True
        else:
            screen.appendListBoxStringNoUpdate(szSettingsTable, gcmap.getMapScriptName(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
        ...
        screen.appendListBoxStringNoUpdate(szSettingsTable, localText.getText("TXT_KEY_SETTINGS_MAP_SIZE", (gc.getWorldInfo(gcmap.getWorldSize()).getTextKey(),)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
        screen.appendListBoxStringNoUpdate(szSettingsTable, localText.getText("TXT_KEY_SETTINGS_CLIMATE", (gc.getClimateInfo(gcmap.getClimate()).getTextKey(),)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
        screen.appendListBoxStringNoUpdate(szSettingsTable, localText.getText("TXT_KEY_SETTINGS_SEA_LEVEL", (gc.getSeaLevelInfo(gcmap.getSeaLevel()).getTextKey(),)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
        screen.appendListBoxStringNoUpdate(szSettingsTable, " ", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
        screen.appendListBoxStringNoUpdate(szSettingsTable, localText.getText("TXT_KEY_SETTINGS_STARTING_ERA", (gc.getEraInfo(gcgame.getStartEra()).getTextKey(),)), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
        screen.appendListBoxStringNoUpdate(szSettingsTable, localText.getText("TXT_KEY_SETTINGS_GAME_SPEED", (gc.getGameSpeedInfo(gcgame.getGameSpeedType()).getTextKey(),)), WidgetTypes.WIDGET_GENERAL, -1, -1,
                                           CvUtil.FONT_LEFT_JUSTIFY)
        ...
        if showHOFSettingChecks:
            ...
            for iVCLoop in range(gc.getNumVictoryInfos()):
                if not gcgame.isVictoryValid(iVCLoop):
                    if text is not None:
                        text += ", "
                    else:
        ...
        screen.addPanel(szOptionsPanel, localText.getText("TXT_KEY_MAIN_MENU_CUSTOM_SETUP_OPTIONS", ()).upper(), "", True, True, self.SETTINGS_PANEL_X2, self.SETTINGS_PANEL_Y - 10, self.SETTINGS_PANEL_WIDTH, self.SETTINGS_PANEL_HEIGHT,
                        PanelStyles.PANEL_STYLE_MAIN)
        szOptionsTable = self.getNextWidgetName()
        screen.addListBoxGFC(szOptionsTable, "", self.SETTINGS_PANEL_X2 + self.MARGIN, self.SETTINGS_PANEL_Y + self.MARGIN, self.SETTINGS_PANEL_WIDTH - 2 * self.MARGIN, self.SETTINGS_PANEL_HEIGHT - 2 * self.MARGIN,
                             TableStyles.TABLE_STYLE_EMPTY)
        screen.enableSelect(szOptionsTable, False)

        ## Start HOF MOD V1.61.001  6/8
        if (showHOFSettingChecks and gcgame.isGameMultiPlayer()):
        ...
        if showHOFSettingChecks and not Buffy.isDllInCorrectPath():
            failedHOFChecks = True
            text = "\n" + self.BuffyWarningInstallLocation + "\n" + gcgame.getExePath() + "\Mods"
            screen.appendListBoxStringNoUpdate(szOptionsTable, text, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)

        if (gcgame.isOption(GameOptionTypes.GAMEOPTION_ADVANCED_START)):
            szNumPoints = u"%s %d" % (localText.getText("TXT_KEY_ADVANCED_START_POINTS", ()), gcgame.getNumAdvancedStartPoints())
        ...
        if (gcgame.isGameMultiPlayer()):
            ...
            if (gcgame.getMaxTurns() > 0):
                szMaxTurns = u"%s %d" % (localText.getText("TXT_KEY_TURN_LIMIT_TAG", ()), gcgame.getMaxTurns())
                screen.appendListBoxStringNoUpdate(szOptionsTable, szMaxTurns, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)

            if (gcgame.getMaxCityElimination() > 0):
                szMaxCityElimination = u"%s %d" % (localText.getText("TXT_KEY_CITY_ELIM_TAG", ()), gcgame.getMaxCityElimination())
        ...
        if (gcgame.hasSkippedSaveChecksum()):
        ...
        screen.addPanel(szCivsPanel, localText.getText("TXT_KEY_RIVALS_MET", ()).upper(), "", True, True, self.SETTINGS_PANEL_X3, self.SETTINGS_PANEL_Y - 10, self.SETTINGS_PANEL_WIDTH, self.SETTINGS_PANEL_HEIGHT,
                        PanelStyles.PANEL_STYLE_MAIN)

        szCivsTable = self.getNextWidgetName()
        screen.addListBoxGFC(szCivsTable, "", self.SETTINGS_PANEL_X3 + self.MARGIN, self.SETTINGS_PANEL_Y + self.MARGIN, self.SETTINGS_PANEL_WIDTH - 2 * self.MARGIN, self.SETTINGS_PANEL_HEIGHT - 2 * self.MARGIN,
                             TableStyles.TABLE_STYLE_EMPTY)
        ...
        if showHOFSettingChecks:
            ...
            if (GameSetUpCheck.isMapSizeOK() and not GameSetUpCheck.isOpponentCountOK(opponentCount)):
                failedHOFChecks = True
                zsMapSize = localText.getText("TXT_KEY_SETTINGS_MAP_SIZE", (gc.getWorldInfo(gcmap.getWorldSize()).getTextKey(),))
        ...
        for iLoopPlayer in range(gc.getMAX_CIV_PLAYERS()):
            player = gcgetPlayer(iLoopPlayer)
            if (player.isEverAlive() and iLoopPlayer != self.iActivePlayer and (gcgetTeam(player.getTeam()).isHasMet(activePlayer.getTeam()) or gcgame.isDebugMode()) and not player.isBarbarian() and not player.isMinorCiv()):
                screen.appendListBoxStringNoUpdate(szCivsTable, localText.getText("TXT_KEY_LEADER_CIV_DESCRIPTION", (player.getNameKey(), player.getCivilizationShortDescriptionKey())), WidgetTypes.WIDGET_GENERAL, -1, -1,
                                                   CvUtil.FONT_LEFT_JUSTIFY)
                screen.appendListBoxStringNoUpdate(szCivsTable, u"     (" + GlobalCyGameTextMgr.parseLeaderTraits(player.getLeaderType(), player.getCivilizationType(), True, False) + ")", WidgetTypes.WIDGET_GENERAL, -1, -1,
                                                   CvUtil.FONT_LEFT_JUSTIFY)
    ...
    def showVictoryConditionScreen(self):
        ...
        iActiveTeam = gcgetPlayer(self.iActivePlayer).getTeam()
        ...
        for i in range(gc.getMAX_CIV_TEAMS()):
            if (gcgetTeam(i).isAlive() and not gcgetTeam(i).isMinorCiv() and not gcgetTeam(i).isBarbarian()):
                nRivals += 1
                # BUG Additions Start
                if i != iActiveTeam:
                    if gcgetTeam(i).isHasMet(iActiveTeam):
                        nknown += 1
                    if gcgetTeam(i).isVassal(iActiveTeam):
                        nVassaled += 1
        # BUG Additions End

        # Population
        totalPop = gcgame.getTotalPopulation()
        ...
        for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
            if (gcgetTeam(iLoopTeam).isAlive() and not gcgetTeam(iLoopTeam).isMinorCiv() and not gcgetTeam(iLoopTeam).isBarbarian()):
                if (iLoopTeam != iActiveTeam and (activePlayer.getTeam().isHasMet(iLoopTeam) or gcgame.isDebugMode())):
                    teamPop = gcgetTeam(iLoopTeam).getTotalPopulation()
                    if (teamPop > bestPop):
                        bestPop = teamPop
                        iBestPopTeam = iLoopTeam

        # Score
        ourScore = gcgame.getTeamScore(iActiveTeam)
        ...
        for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
            if (gcgetTeam(iLoopTeam).isAlive() and not gcgetTeam(iLoopTeam).isMinorCiv() and not gcgetTeam(iLoopTeam).isBarbarian()):
                if (iLoopTeam != iActiveTeam and (activePlayer.getTeam().isHasMet(iLoopTeam) or gcgame.isDebugMode())):
                    teamScore = gcgame.getTeamScore(iLoopTeam)
                    if (teamScore > bestScore):
                        bestScore = teamScore
                        iBestScoreTeam = iLoopTeam

        # Land Area
        totalLand = gcmap.getLandPlots()
        ...
        for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
            if (gcgetTeam(iLoopTeam).isAlive() and not gcgetTeam(iLoopTeam).isMinorCiv() and not gcgetTeam(iLoopTeam).isBarbarian()):
                if (iLoopTeam != iActiveTeam and (activePlayer.getTeam().isHasMet(iLoopTeam) or gcgame.isDebugMode())):
                    teamLand = gcgetTeam(iLoopTeam).getTotalLand()
        ...
        for iLoopReligion in range(gc.getNumReligionInfos()):
            if (activePlayer.getTeam().hasHolyCity(iLoopReligion)):
                religionPercent = gcgame.calculateReligionPercent(iLoopReligion)
        ...
        for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
            if (gcgetTeam(iLoopTeam).isAlive() and not gcgetTeam(iLoopTeam).isMinorCiv() and not gcgetTeam(iLoopTeam).isBarbarian()):
                if (iLoopTeam != iActiveTeam and (activePlayer.getTeam().isHasMet(iLoopTeam) or gcgame.isDebugMode())):
                    teamCulture = gcgetTeam(iLoopTeam).countTotalCulture()
                ...
                if (gc.getBuildingInfo(i).getVoteSourceType() == j):
                    ...
                    for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
                        if (gcgetTeam(iLoopTeam).isAlive() and not gcgetTeam(iLoopTeam).isMinorCiv() and not gcgetTeam(iLoopTeam).isBarbarian()):
                            if (gcgetTeam(iLoopTeam).getBuildingClassCount(gc.getBuildingInfo(i).getBuildingClassType()) > 0):
                                iUNTeam = iLoopTeam
                                if (iLoopTeam == iActiveTeam or gcgame.isDebugMode() or activePlayer.getTeam().isHasMet(iLoopTeam)):
                                    bUnknown = False
                                break
        ...
        for iLoopVC in range(gc.getNumVictoryInfos()):
            ...
            if gcgame.isVictoryValid(iLoopVC):

                iNumRows = screen.getTableNumRows(szTable)
                szVictoryType = u"<font=4b>" + victory.getDescription().upper() + u"</font>"
                if (victory.isEndScore() and (gcgame.getMaxTurns() > gcgame.getElapsedGameTurns())):
                    szVictoryType += "    (" + localText.getText("TXT_KEY_MISC_TURNS_LEFT", (gcgame.getMaxTurns() - gcgame.getElapsedGameTurns(),)) + ")"
                ...
                if (victory.isTargetScore() and gcgame.getTargetScore() != 0):

                    iRow = screen.appendTableRow(szTable)
                    screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_TARGET_SCORE", (gcgame.getTargetScore(),)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    # Rhye - start
                    ...
                    if (iBestScoreTeam != -1):
                        # Rhye - start
                        # screen.setTableText(szTable, 4, iRow, gcgetTeam(iBestScoreTeam).getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        screen.setTableText(szTable, 4, iRow, gcgetPlayer(iBestScoreTeam).getCivilizationShortDescription(0) + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        # Rhye - end
                ...
                if (victory.isEndScore()):
                    ...
                    szText1 = localText.getText("TXT_KEY_VICTORY_SCREEN_HIGHEST_SCORE", (GlobalCyGameTextMgr.getTimeStr(gcgame.getStartTurn() + gcgame.getMaxTurns(), False),))
                ...
                if (gcgame.getAdjustedPopulationPercent(iLoopVC) > 0):
                    iRow = screen.appendTableRow(szTable)
                    screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_PERCENT_POP", (gcgame.getAdjustedPopulationPercent(iLoopVC),)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    # Rhye - start
                    # screen.setTableText(szTable, 2, iRow, activePlayer.getTeam().getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    screen.setTableText(szTable, 2, iRow, activePlayer.getCivilizationShortDescription() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    # Rhye - end
                    ...
                    if (iBestPopTeam != -1):
                        # Rhye - start
                        # screen.setTableText(szTable, 4, iRow, gcgetTeam(iBestPopTeam).getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        screen.setTableText(szTable, 4, iRow, gcgetPlayer(iBestPopTeam).getCivilizationShortDescription(0) + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        # Rhye - end
                        screen.setTableText(szTable, 5, iRow, (u"%.2f%%" % (bestPop * 100 / totalPop)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    bEntriesFound = True

                if (gcgame.getAdjustedLandPercent(iLoopVC) > 0):
                    iRow = screen.appendTableRow(szTable)
                    screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_PERCENT_LAND", (gcgame.getAdjustedLandPercent(iLoopVC),)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    # Rhye - start
                    # screen.setTableText(szTable, 2, iRow, activePlayer.getTeam().getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    screen.setTableText(szTable, 2, iRow, activePlayer.getCivilizationShortDescription() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    ...
                    if (iBestLandTeam != -1):
                        # Rhye - start
                        # screen.setTableText(szTable, 4, iRow, gcgetTeam(iBestLandTeam).getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        screen.setTableText(szTable, 4, iRow, gcgetPlayer(iBestLandTeam).getCivilizationShortDescription(0) + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        # Rhye - end
                ...
                if (victory.getTotalCultureRatio() > 0):
                    iRow = screen.appendTableRow(szTable)
                    screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_PERCENT_CULTURE", (int((100.0 * bestCulture) / victory.getTotalCultureRatio()),)), "", WidgetTypes.WIDGET_GENERAL, -1, -1,
                                        CvUtil.FONT_LEFT_JUSTIFY)
                    # Rhye - start
                    # screen.setTableText(szTable, 2, iRow, activePlayer.getTeam().getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    screen.setTableText(szTable, 2, iRow, activePlayer.getCivilizationShortDescription(0) + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                    ...
                    if (iBestLandTeam != -1):
                        screen.setTableText(szTable, 4, iRow, gcgetPlayer(iBestCultureTeam).getCivilizationShortDescription(0) + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        ...
                        if (iLoopTeam != iActiveTeam and (activePlayer.getTeam().isHasMet(iLoopTeam) or gcgame.isDebugMode())):
                            ...
                            for i in range(gc.getNumBuildingClassInfos()):
                                if (gc.getBuildingClassInfo(i).getVictoryThreshold(iLoopVC) > 0):
                                    teamBuilding += gcgetTeam(iLoopTeam).getBuildingClassCount(i)
                ...
                for i in range(gc.getNumBuildingClassInfos()):
                    if (gc.getBuildingClassInfo(i).getVictoryThreshold(iLoopVC) > 0):
                        ...
                        if (iBestBuildingTeam != -1):
                            # Rhye - start
                            # screen.setTableText(szTable, 4, iRow, gcgetTeam(iBestBuildingTeam).getName() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                            screen.setTableText(szTable, 4, iRow, gcgetPlayer(iBestBuildingTeam).getCivilizationShortDescription(0) + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                            # Rhye - end
                            screen.setTableText(szTable, 5, iRow, gcgetTeam(iBestBuildingTeam).getBuildingClassCount(i), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                        bEntriesFound = True
                ...
                for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
                    if (gcgetTeam(iLoopTeam).isAlive() and not gcgetTeam(iLoopTeam).isMinorCiv() and not gcgetTeam(iLoopTeam).isBarbarian()):
                        if (iLoopTeam != iActiveTeam
                                and (activePlayer.getTeam().isHasMet(iLoopTeam) or gcgame.isDebugMode())
                                and self.isApolloBuiltbyTeam(gcgetTeam(iLoopTeam))):
                            teamProject = 0
                            for i in range(gc.getNumProjectInfos()):
                                if (gc.getProjectInfo(i).getVictoryThreshold(iLoopVC) > 0):
                                    teamProject += gcgetTeam(iLoopTeam).getProjectCount(i)
                ...
                if AdvisorOpt.isVictories():
                            ...
                            if not self.isApolloBuilt():
                                ...
                                if not bApolloShown:
                                    ...
                                    if self.isApolloBuiltbyTeam(activePlayer.getTeam()):
                                        screen.setTableText(szTable, 2, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_BUILT", (activePlayer.getCivilizationShortDescription(),)), "", WidgetTypes.WIDGET_GENERAL, -1, -1,
                                                            CvUtil.FONT_LEFT_JUSTIFY)
                                    else:
                                        screen.setTableText(szTable, 2, iRow, activePlayer.getCivilizationShortDescription() + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                                        screen.setTableText(szTable, 3, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_NOT_BUILT", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                                    ...
                                    if (iBestProjectTeam != -1):
                                        screen.setTableText(szTable, 4, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_BUILT", (gcgetPlayer(iBestProjectTeam).getCivilizationShortDescription(0),)), "", WidgetTypes.WIDGET_GENERAL, -1, -1,
                                                            CvUtil.FONT_LEFT_JUSTIFY)
                                ...
                                if self.isApolloBuiltbyTeam(activePlayer.getTeam()):
                                    ...
                                    bHasTech = gcgetTeam(iActiveTeam).isHasTech(iReqTech)
                                ...
                                if (iBestProjectTeam != -1):
                                    pTeam = gcgetTeam(iBestProjectTeam)
                                    sSSPlayer = gcgetPlayer(iBestProjectTeam).getCivilizationShortDescription(0) + ":"
                    ...
                    for i in range(gc.getNumProjectInfos()):
                        if (gc.getProjectInfo(i).getVictoryThreshold(iLoopVC) > 0):
                            ...
                            if (iBestProjectTeam != -1):
                                screen.setTableText(szTable, 3, iRow, gcgetPlayer(iBestProjectTeam).getCivilizationShortDescription(0) + ":", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                                screen.setTableText(szTable, 4, iRow, unicode(gcgetTeam(iBestProjectTeam).getProjectCount(i)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
                ...
                if (bSpaceshipFound):
                    if gcgetTeam(self.activePlayer.getTeam()).canLaunch(iLoopVC):
                        screen.setButtonGFC("SpaceShipButton" + str(iLoopVC), localText.getText("TXT_KEY_BUTTON_LAUNCH_SPACESHIP", ()), "", 0, 0, 15, 10, WidgetTypes.WIDGET_GENERAL, self.SPACESHIP_LAUNCH_BUTTON, iLoopVC,
                    ...
                    victoryDelay = gcgetTeam(iActiveTeam).getVictoryCountdown(iLoopVC)
                    if ((victoryDelay > 0) and (gcgame.getGameState() != GameStateTypes.GAMESTATE_EXTENDED)):
                        victoryDate = GlobalCyGameTextMgr.getTimeStr(utils.getGameTurn() + victoryDelay, False)
                ...
                if (victory.isDiploVote()):
                    for (iVoteBuildingClass, iUNTeam, bUnknown) in aiVoteBuildingClass:
                        iRow = screen.appendTableRow(szTable)
                        screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_ELECTION", (gc.getBuildingClassInfo(iVoteBuildingClass).getTextKey(),)), "", WidgetTypes.WIDGET_GENERAL, -1, -1,
                                            CvUtil.FONT_LEFT_JUSTIFY)
                        if (iUNTeam != -1):
                            if bUnknown:
                                szName = localText.getText("TXT_KEY_TOPCIVS_UNKNOWN", ())
                ...
                if (victory.getCityCulture() != CultureLevelTypes.NO_CULTURELEVEL and victory.getNumCultureCities() > 0):
                    ...
                    for iLoopPlayer in range(gc.getMAX_PLAYERS()):
                        if (gcgetPlayer(iLoopPlayer).isAlive() and not gcgetPlayer(iLoopPlayer).isMinorCiv() and not gcgetPlayer(iLoopPlayer).isBarbarian()):
                            if (iLoopPlayer != self.iActivePlayer and (activePlayer.getTeam().isHasMet(gcgetPlayer(iLoopPlayer).getTeam()) or gcgame.isDebugMode())):
                    ...
                    iRow = screen.appendTableRow(szTable)
                    screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_CITY_CULTURE", (victory.getNumCultureCities(), gc.getCultureLevelInfo(victory.getCityCulture()).getTextKey())), "",
                ...
                if (iLoopVC == 7):
                    for i in range(3):
                        ...
                        sGoalTurn = ''
                        if not gcgetTeam(self.iActivePlayer).isHasTech(iCalendar) or AdvisorOpt.isUHVFinishDateTurn():
                            iVictoryYear = dVictoryYears[gcgetPlayer(self.iActivePlayer).getCivilizationType()][i]
                            if iVictoryYear != -1: sGoalTurn = ' ' + localText.getText("TXT_KEY_VICTORY_UHV_END_TURN", (utils.getTurnForYear(iVictoryYear) - utils.getScenarioStartTurn(),))
                        ...
                        if self.iActivePlayer == iPhilippines and i == 0:
                            ...
                            if lEmbassies:
                                ...
                                for iCiv in lEmbassies:
                                    if szHelp != "":
                                        szHelp += ", "
                                    szHelp += gcgetPlayer(iCiv).getCivilizationShortDescription(0)
                            else:
                    ...
                    if iVictoryType == -1:
                        ...
                        for i in range(3):
                            ...
                            if iVictoryType == iVictoryPaganism and i == 1:
                                # 中文转英文
                                paganReligionName = gc.getCivilizationInfo(gcgetPlayer(self.iActivePlayer).getCivilizationType()).getPaganReligionName(0)
                                ...
                                if self.iActivePlayer == iMaya and not gcgetPlayer(self.iActivePlayer).isReborn():
                                    sGoalText += "_MAYA"
                                elif self.iActivePlayer == iTeotihuacan:
                                    sGoalText += "_TEOTIHUACAN"
                                sGoalText = localText.getText(str(sGoalText), ())
    ...
    def getVotesForWhichCandidate(self, iPlayer, iCand1, iCand2, iVote):
        ...
        iPTeam = gcgetPlayer(iPlayer).getTeam()
        ...
        # * AI votes for its master, if it is a vassal
        if gcgetTeam(iPTeam).isVassal(iCand1): return 1
        if gcgetTeam(iPTeam).isVassal(iCand2): return 2
    ...
    def getPlayerOnTeam(self, iTeam):
        for i in range(gc.getMAX_PLAYERS()):
            if iTeam == gcgetPlayer(i).getTeam():
                return i
    ...
    def isApolloBuilt(self):
        ...
        for iLoopTeam in range(gc.getMAX_CIV_TEAMS()):
            pLoopTeam = gcgetTeam(iLoopTeam)
            if (pLoopTeam.isAlive()
                    and not pLoopTeam.isMinorCiv()
                    and not pLoopTeam.isBarbarian()):
                if iLoopTeam == iActiveTeam:
                    bContact = True
                elif (gcgetTeam(iActiveTeam).isHasMet(iLoopTeam)
                      or gcgame.isDebugMode()):
                    bContact = True
                else:
                    bContact = False
    ...
    def handleInput(self, inputClass):
        ...
        if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_LISTBOX_ITEM_SELECTED):
            ...
            # wunshare
                ...
                GlobalCyInterface.clearSelectedCities()
