...
class CvTopCivs:
    ...
    def makeList(self, szType):
        ...
        for iPlayerLoop in range(iNumMajorPlayers):  # Rhye
            ...
            if (gcgetPlayer(iPlayerLoop).isAlive()):

                if (szType == localText.getText("TXT_KEY_TOPCIVS_WEALTH", ())):
                    self.aiTopCivsValues.append([gcgetPlayer(iPlayerLoop).getGold(), iPlayerLoop])
                    print("Player %d Num Gold: %d" % (iPlayerLoop, gcgetPlayer(iPlayerLoop).getGold()))

                if (szType == localText.getText("TXT_KEY_TOPCIVS_POWER", ())):
                    self.aiTopCivsValues.append([gcgetPlayer(iPlayerLoop).getPower(), iPlayerLoop])
                ...
                if (szType == localText.getText("TXT_KEY_TOPCIVS_TECH", ())):
                    ...
                    for iTechLoop in range(iNumTotalTechs):
                        ...
                        bPlayerHasTech = gcgetTeam(gcgetPlayer(iPlayerLoop).getTeam()).isHasTech(iTechLoop)
                ...
                if (szType == localText.getText("TXT_KEY_TOPCIVS_CULTURE", ())):
                    self.aiTopCivsValues.append([gcgetPlayer(iPlayerLoop).countTotalCulture(), iPlayerLoop])

                if (szType == localText.getText("TXT_KEY_TOPCIVS_SIZE", ())):
                    self.aiTopCivsValues.append([gcgetPlayer(iPlayerLoop).getTotalLand(), iPlayerLoop])
    ...
    def printList(self, szType):
        ...
        for iRankLoop in range(8):
            ...
            szPlayerName = gcgetPlayer(iPlayer).getNameKey()

            if (szPlayerName != ""):

                pActivePlayerTeam = gcgetTeam(gcgetPlayer(GlobalCyGame.getActivePlayer()).getTeam())
                iPlayerTeam = gcgetPlayer(iPlayer).getTeam()
                szCivText = ""

                # Does the Active player know this player exists?
                if (iPlayer == GlobalCyGame.getActivePlayer() or pActivePlayerTeam.isHasMet(iPlayerTeam)):
                    # szCivText = localText.getText("TXT_KEY_TOPCIVS_TEXT3", (szPlayerName, self.RankList[iRankLoop])) #Rhye
                    szCivText = gcgetPlayer(iPlayer).getCivilizationDescription(0)  # Rhye
                else:
    ...
    def turnChecker(self, iTurnNum):
        ...
        if (not GlobalCyGame.isNetworkMultiPlayer() and GlobalCyGame.getActivePlayer() >= 0):
            if (iTurnNum % 50 == 0 and iTurnNum > 0 and gcgetPlayer(GlobalCyGame.getActivePlayer()).isAlive()):
                self.showScreen()
