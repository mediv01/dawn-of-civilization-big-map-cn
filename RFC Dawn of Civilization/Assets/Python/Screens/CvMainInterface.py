...
class CvMainInterface:
    ...
    # I know that this is redundent, but GlobalCyInterface.getPlotListOffset() (and prob the column one too)
    # uses this function
    ...
    def initState(self, screen=None):
        ...
        gcgame.setXResolution(self.xResolution)
        gcgame.setYResolution(self.yResolution)
        ...
        # BUG - Progress Bar - Tick Marks - start
        self.pBarPopulationBar = ProgressBarUtil.ProgressBar("PopulationBar-Canvas", iCityCenterRow1X, iCityCenterRow1Y - 4, self.xResolution - (iCityCenterRow1X * 2), iStackBarHeight, gc.getYieldInfo(YieldTypes.YIELD_FOOD).getColorType(),
        ...
        self.pBarProductionBar = ProgressBarUtil.ProgressBar("ProductionBar-Canvas", iCityCenterRow2X, iCityCenterRow2Y - 4, self.xResolution - (iCityCenterRow2X * 2), iStackBarHeight,
                                                             gc.getYieldInfo(YieldTypes.YIELD_PRODUCTION).getColorType(), ProgressBarUtil.TICK_MARKS, True)
        self.pBarProductionBar.addBarItem("ProductionBar")
        self.pBarProductionBar.addBarItem("ProductionText")
        self.pBarProductionBar_Whip = ProgressBarUtil.ProgressBar("ProductionBar-Whip-Canvas", iCityCenterRow2X, iCityCenterRow2Y - 4, self.xResolution - (iCityCenterRow2X * 2), iStackBarHeight, gc.getInfoTypeForString("COLOR_YELLOW"),
    ...
    def interfaceScreen(self):
        ...
        gcmap.updateMinimapColor()

        self.createMinimapButtons()

        # Help button (always visible)
        screen.setImageButton("InterfaceHelpButton", ArtFileMgr.getInterfaceArtInfo("INTERFACE_GENERAL_CIVILOPEDIA_ICON").getPath(), xResolution - 28, 2, 24, 24, WidgetTypes.WIDGET_ACTION,
        ...
        for j in range(self.numPlotListRows()):
            ...
            for i in range(self.numPlotListButtonsPerRow()):
                ...
                screen.addCheckBoxGFCAt(szStringPanel, szString, ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_GOVERNOR").getPath(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), xOffset + 3, 3, 32, 32,
                                        WidgetTypes.WIDGET_PLOT_LIST, k, -1, ButtonStyles.BUTTON_STYLE_LABEL, True)
        ...
        screen.setImageButton("EndTurnButton", "", xResolution - (iEndOfTurnButtonSize / 2) - iEndOfTurnPosX, yResolution - (iEndOfTurnButtonSize / 2) - iEndOfTurnPosY, iEndOfTurnButtonSize, iEndOfTurnButtonSize,
        ...
        #	if gcgame.isOption(GameOptionTypes.GAMEOPTION_PICK_RELIGION):
        ...
        screen.setImageButton(self.PLE.PLOT_LIST_UP_NAME, ArtFileMgr.getInterfaceArtInfo("PLE_ARROW_UP").getPath(), 315 + (xResolution - (iMultiListXL + iMultiListXR) - 68) + 5, yResolution - 171 + 5, 20, 20, WidgetTypes.WIDGET_GENERAL, -1,
                              -1)
        screen.hide(self.PLE.PLOT_LIST_UP_NAME)
        screen.setImageButton(self.PLE.PLOT_LIST_DOWN_NAME, ArtFileMgr.getInterfaceArtInfo("PLE_ARROW_DOWN").getPath(), 298 + (xResolution - (iMultiListXL + iMultiListXR) - 34) + 5, yResolution - 171 + 5, 20, 20, WidgetTypes.WIDGET_GENERAL,
                              -1, -1)
    ...
    def updateScreen(self):
        ...
        #				utils.getGameTurn(), gcgame.getActivePlayer(), GlobalCyInterface.getShowInterface(), GlobalCyInterface.getEndTurnState())
        ...
        if (GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY):
            if (gcgame.isPaused()):
                # Pause overrides other messages
                acOutput = localText.getText("SYSTEM_GAME_PAUSED", (gcgetPlayer(gcgame.getPausePlayer()).getNameKey(),))
                ...
                if (messageControl.GetConnState(messageControl.GetFirstBadConnection()) == 1):
                    if (gcgame.isMPOption(MultiplayerOptionTypes.MPOPTION_ANONYMOUS)):
                        acOutput = localText.getText("SYSTEM_WAITING_FOR_PLAYER", (gcgetPlayer(messageControl.GetFirstBadConnection()).getNameKey(), 0))
                    else:
                        acOutput = localText.getText("SYSTEM_WAITING_FOR_PLAYER", (gcgetPlayer(messageControl.GetFirstBadConnection()).getNameKey(), (messageControl.GetFirstBadConnection() + 1)))
                    # screen.modifyLabel( "EndTurnText", acOutput, CvUtil.FONT_CENTER_JUSTIFY )
                    screen.setEndTurnState("EndTurnText", acOutput)
                    bShow = True
                elif (messageControl.GetConnState(messageControl.GetFirstBadConnection()) == 2):
                    if (gcgame.isMPOption(MultiplayerOptionTypes.MPOPTION_ANONYMOUS)):
                        acOutput = localText.getText("SYSTEM_PLAYER_JOINING", (gcgetPlayer(messageControl.GetFirstBadConnection()).getNameKey(), 0))
                    else:
                        acOutput = localText.getText("SYSTEM_PLAYER_JOINING", (gcgetPlayer(messageControl.GetFirstBadConnection()).getNameKey(), (messageControl.GetFirstBadConnection() + 1)))
                    # screen.modifyLabel( "EndTurnText", acOutput, CvUtil.FONT_CENTER_JUSTIFY )
                    screen.setEndTurnState("EndTurnText", acOutput)
                    bShow = True
            else:
                # Flash select messages if no popups are present
                if (GlobalCyInterface.shouldDisplayReturn()):
                    acOutput = localText.getText("SYSTEM_RETURN", ())
                    # screen.modifyLabel( "EndTurnText", acOutput, CvUtil.FONT_CENTER_JUSTIFY )
                    screen.setEndTurnState("EndTurnText", acOutput)
                    bShow = True
                elif (GlobalCyInterface.shouldDisplayWaitingOthers()):
                    acOutput = localText.getText("SYSTEM_WAITING", ())
                    # screen.modifyLabel( "EndTurnText", acOutput, CvUtil.FONT_CENTER_JUSTIFY )
                    screen.setEndTurnState("EndTurnText", acOutput)
                    bShow = True
                elif (GlobalCyInterface.shouldDisplayEndTurn()):
                    # BUG - Reminders - start
                    ...
                    screen.setEndTurnState("EndTurnText", acOutput)
                    bShow = True
                elif (GlobalCyInterface.shouldDisplayWaitingYou()):
        ...
        if (bShow):
            ...
            if (GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_SHOW or GlobalCyInterface.isCityScreenUp()):
        ...
        if (
                GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START):
        ...
        self.PLE.checkInfoPane(GlobalCyInterface.getMousePos())
        # BUG - PLE - end
    ...
    def redraw(self):
        ...
        self.setFieldofView(screen, GlobalCyInterface.isCityScreenUp())
        # BUG - Field of View - end

        # Check Dirty Bits, see what we need to redraw...
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.PercentButtons_DIRTY_BIT) == True):
            # Percent Buttons
            self.updatePercentButtons()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.PercentButtons_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.Flag_DIRTY_BIT) == True):
            # Percent Buttons
            self.updateFlag()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.Flag_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.MiscButtons_DIRTY_BIT) == True):
            # Miscellaneous buttons (civics screen, etc)
            self.updateMiscButtons()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.MiscButtons_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.InfoPane_DIRTY_BIT) == True):
            # Info Pane Dirty Bit
            ...
            GlobalCyInterface.setDirty(InterfaceDirtyBits.InfoPane_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.PlotListButtons_DIRTY_BIT) == True):
            ...
            GlobalCyInterface.setDirty(InterfaceDirtyBits.PlotListButtons_DIRTY_BIT, False)
        ...
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.SelectionButtons_DIRTY_BIT) == True):
            # Selection Buttons Dirty
            self.updateSelectionButtons()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.SelectionButtons_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.ResearchButtons_DIRTY_BIT) == True):
            # Research Buttons Dirty
            self.updateResearchButtons()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.ResearchButtons_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.CitizenButtons_DIRTY_BIT) == True):
            ...
            GlobalCyInterface.setDirty(InterfaceDirtyBits.CitizenButtons_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.GameData_DIRTY_BIT) == True):
            # Game Data Strings Dirty
            self.updateGameDataStrings()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.GameData_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.Help_DIRTY_BIT) == True):
            # Help Dirty bit
            self.updateHelpStrings()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.Help_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.CityScreen_DIRTY_BIT) == True):
            # Selection Data Dirty Bit
            self.updateCityScreen()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.Domestic_Advisor_DIRTY_BIT, True)
            GlobalCyInterface.setDirty(InterfaceDirtyBits.CityScreen_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.Score_DIRTY_BIT) == True or GlobalCyInterface.checkFlashUpdate()):
            # Scores!
            self.updateScoreStrings()
            GlobalCyInterface.setDirty(InterfaceDirtyBits.Score_DIRTY_BIT, False)
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.GlobeInfo_DIRTY_BIT) == True):
            # Globeview and Globelayer buttons
            GlobalCyInterface.setDirty(InterfaceDirtyBits.GlobeInfo_DIRTY_BIT, False)
    ...
    def updatePercentButtons(self):
        ...
        pHeadSelectedCity = GlobalCyInterface.getHeadSelectedCity()

        if (not GlobalCyInterface.isCityScreenUp() or (pHeadSelectedCity.getOwner() == gcgame.getActivePlayer()) or gcgame.isDebugMode()):
            iCount = 0
            ...
            if (
                ...
                for iI in range(CommerceTypes.NUM_COMMERCE_TYPES):
                    ...
                    if (gc.getActivePlayer().isCommerceFlexible(eCommerce) or (GlobalCyInterface.isCityScreenUp() and (eCommerce == CommerceTypes.COMMERCE_GOLD))):
                        # BUG - Min/Max Sliders - start
                        bEnable = gc.getActivePlayer().isCommerceFlexible(eCommerce)
                        if MainOpt.isShowMinMaxCommerceButtons() and not GlobalCyInterface.isCityScreenUp():
                            iMinMaxAdjustX = 20
                        ...
                        screen.setButtonGFC(szString, u"", "", 70 + iMinMaxAdjustX, 50 + (19 * iCount), 20, 20, WidgetTypes.WIDGET_CHANGE_PERCENT, eCommerce, gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS"),
                                            ButtonStyles.BUTTON_STYLE_CITY_PLUS)
                        screen.show(szString)
                        screen.enable(szString, bEnable)
                        szString = "DecreasePercent" + str(eCommerce)
                        screen.setButtonGFC(szString, u"", "", 90 + iMinMaxAdjustX, 50 + (19 * iCount), 20, 20, WidgetTypes.WIDGET_CHANGE_PERCENT, eCommerce, -gc.getDefineINT("COMMERCE_PERCENT_CHANGE_INCREMENTS"),
                                            ButtonStyles.BUTTON_STYLE_CITY_MINUS)
                        screen.show(szString)
                        screen.enable(szString, bEnable)
    ...
    def updateEndTurnButton(self):
        ...
        if (GlobalCyInterface.shouldDisplayEndTurnButton() and GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):

            eState = GlobalCyInterface.getEndTurnState()
    ...
    def updateMiscButtons(self):
        ...
        if (GlobalCyInterface.shouldDisplayFlag() and GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):
        ...
        if (GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_HIDE_ALL or GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_MINIMAP_ONLY):
            screen.hide("InterfaceLeftBackgroundWidget")
        ...
        elif (GlobalCyInterface.isCityScreenUp()):
        ...
        elif (GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_HIDE):
            screen.hide("InterfaceLeftBackgroundWidget")
            screen.show("InterfaceTopBackgroundWidget")
            screen.hide("InterfaceCenterBackgroundWidget")
            screen.hide("InterfaceRightBackgroundWidget")
        ...
        elif (GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_ADVANCED_START):
            screen.hide("InterfaceLeftBackgroundWidget")
            screen.hide("InterfaceTopBackgroundWidget")
            screen.hide("InterfaceCenterBackgroundWidget")
            screen.hide("InterfaceRightBackgroundWidget")
    ...
    def updatePlotListButtons_Common(self, screen):
        ...
        bHandled = False
        if (GlobalCyInterface.shouldDisplayUnitModel() and not CyEngine().isGlobeviewUp() and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL):
            if (GlobalCyInterface.isCitySelection()):

                iOrders = GlobalCyInterface.getNumOrdersQueued()

                for i in range(iOrders):
                    if (bHandled == False):
                        eOrderNodeType = GlobalCyInterface.getOrderNodeType(i)
                        if (eOrderNodeType == OrderTypes.ORDER_TRAIN):
                            gcgame.setCityScreenOwner(GlobalCyInterface.getHeadSelectedCity().getOwner())
                            screen.addUnitGraphicGFC("InterfaceUnitModel", GlobalCyInterface.getOrderNodeData1(i), 175, yResolution - 138, 123, 132, WidgetTypes.WIDGET_HELP_SELECTED, 0, -1, -20, 30, 1, False)
                            gcgame.resetCityScreenOwner()
                            bHandled = True
                        elif (eOrderNodeType == OrderTypes.ORDER_CONSTRUCT):
                            screen.addBuildingGraphicGFC("InterfaceUnitModel", GlobalCyInterface.getOrderNodeData1(i), 175, yResolution - 138, 123, 132, WidgetTypes.WIDGET_HELP_SELECTED, 0, -1, -20, 30, 0.8, False)
                            bHandled = True
                        elif (eOrderNodeType == OrderTypes.ORDER_CREATE):
                            if (gc.getProjectInfo(GlobalCyInterface.getOrderNodeData1(i)).isSpaceship()):
                                modelType = 0
                                screen.addSpaceShipWidgetGFC("InterfaceUnitModel", 175, yResolution - 138, 123, 132, GlobalCyInterface.getOrderNodeData1(i), modelType, WidgetTypes.WIDGET_HELP_SELECTED, 0, -1)
                            else:
            ...
            elif (GlobalCyInterface.getHeadSelectedUnit()):
                screen.addUnitGraphicGFC("InterfaceUnitModel", GlobalCyInterface.getHeadSelectedUnit().getUnitType(), 175, yResolution - 138, 123, 132, WidgetTypes.WIDGET_UNIT_MODEL, GlobalCyInterface.getHeadSelectedUnit().getUnitType(), -1, -20,
                                         30, 1, False)
                #				screen.addSpecificUnitGraphicGFC( "InterfaceUnitModel", GlobalCyInterface.getHeadSelectedUnit(), 175, yResolution - 138, 123, 132, WidgetTypes.WIDGET_UNIT_MODEL, GlobalCyInterface.getHeadSelectedUnit().getUnitType(), -1,  -20, 30, 1, False )
    ...
    def updatePlotListButtons_Orig(self, screen):
        ...
        pPlot = GlobalCyInterface.getSelectionPlot()
        ...
        BugUtil.debug("updatePlotListButtons_Orig - column %i, offset %i", GlobalCyInterface.getPlotListColumn(), GlobalCyInterface.getPlotListOffset())

        if (pPlot and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyEngine().isGlobeviewUp() == False):

            iVisibleUnits = GlobalCyInterface.getNumVisibleUnits()
            iCount = -(GlobalCyInterface.getPlotListColumn())

            bLeftArrow = False
            bRightArrow = False

            if (GlobalCyInterface.isCityScreenUp()):
                iMaxRows = 1
                iSkipped = (self.numPlotListRows() - 1) * self.numPlotListButtonsPerRow()
                iCount += iSkipped
            else:
                iMaxRows = self.numPlotListRows()
                iCount += GlobalCyInterface.getPlotListOffset()
                iSkipped = 0

            BugUtil.debug("updatePlotListButtons_Orig - iCount(%i), iSkipped(%i)", iCount, iSkipped)

            GlobalCyInterface.cacheInterfacePlotUnits(pPlot)
            for i in range(GlobalCyInterface.getNumCachedInterfacePlotUnits()):
                pLoopUnit = GlobalCyInterface.getCachedInterfacePlotUnit(i)
                if (pLoopUnit):

                    if ((iCount == 0) and (GlobalCyInterface.getPlotListColumn() > 0)):
                        bLeftArrow = True
                    elif ((iCount == (self.numPlotListRows() * self.numPlotListButtonsPerRow() - 1)) and ((iVisibleUnits - iCount - GlobalCyInterface.getPlotListColumn() + iSkipped) > 1)):
                        bRightArrow = True

                    if ((iCount >= 0) and (iCount < self.numPlotListButtonsPerRow() * self.numPlotListRows())):
                        if ((pLoopUnit.getTeam() != gcgame.getActiveTeam()) or pLoopUnit.isWaiting()):
                        ...
                        if (pLoopUnit.getOwner() == gcgame.getActivePlayer()):
                            bEnable = True
                        else:
                            bEnable = False
                        screen.enable(szString, bEnable)
    ...
    def updatePlotListButtons_BUG(self, screen):
        ...
        #		if not (pPlot
        #		and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL
        #		and CyEngine().isGlobeviewUp() == False):
        if (not pPlot
                or GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_HIDE_ALL
        ...
        #				if ((iCount == 0) and (GlobalCyInterface.getPlotListColumn() > 0)):
        #					bLeftArrow = True
        #				elif ((iCount == (gc.getMAX_PLOT_LIST_ROWS() * self.numPlotListButtons() - 1)) and ((iVisibleUnits - iCount - GlobalCyInterface.getPlotListColumn() + iSkipped) > 1)):
        #					bRightArrow = True
    ...
    def updateSelectionButtons(self):
        ...
        pHeadSelectedCity = GlobalCyInterface.getHeadSelectedCity()
        pHeadSelectedUnit = GlobalCyInterface.getHeadSelectedUnit()
        ...
        if (not CyEngine().isGlobeviewUp() and pHeadSelectedCity):
            ...
            if ((pHeadSelectedCity.getOwner() == gcgame.getActivePlayer()) or gcgame.isDebugMode()):
                ...
                for i in range(g_NumUnitClassInfos):
                    ...
                    if (pHeadSelectedCity.canTrain(eLoopUnit, False, True)):
                        szButton = gcgetPlayer(pHeadSelectedCity.getOwner()).getUnitButton(eLoopUnit)
                ...
                screen.selectMultiList("BottomButtonContainer", GlobalCyInterface.getCityTabSelectionRow())

        elif (not CyEngine().isGlobeviewUp() and pHeadSelectedUnit and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY):

            self.setMinimapButtonVisibility(True)

            if (GlobalCyInterface.getInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION):

                if (pHeadSelectedUnit.getOwner() == gcgame.getActivePlayer() and g_pSelectedUnit != pHeadSelectedUnit):
                    ...
                    if (GlobalCyInterface.canCreateGroup()):
                    ...
                    if (GlobalCyInterface.canDeleteGroup()):
                    ...
                    if pUnit.getUnitType() == iAztecSlave and pUnit.getOwner() == iAztecs:
                        plot = GlobalCyMap.plot(pUnit.getX(), pUnit.getY())
                        if plot.isCity():
        ...
        elif (GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY):
    ...
    def updateResearchButtons(self):
        ...
        i = 0
        if (GlobalCyInterface.shouldShowResearchButtons() and GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):
            iCount = 0
            ...
            for i in range(gc.getNumTechInfos()):
                if (gc.getActivePlayer().canResearch(i, False)):
                    if (iCount < 20):
                        ...
                        #			if not (gcgame.isReligionSlotTaken(j)):
    ...
    def updateCitizenButtons(self):
        ...
        if not GlobalCyInterface.isCityScreenUp(): return 0

        pHeadSelectedCity = GlobalCyInterface.getHeadSelectedCity()
        if not (pHeadSelectedCity and GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_SHOW): return 0
    ...
    def updateCitizenButtons_Stacker(self):
        ...
        for i in range(currentAngryCitizenCount):
            bHandled = True
            szName = "AngryCitizen" + str(i)
            screen.setImageButton(szName, ArtFileMgr.getInterfaceArtInfo("INTERFACE_ANGRYCITIZEN_TEXTURE").getPath(), xResolution - SPECIALIST_AREA_MARGIN - (stackWidth * i), yResolution - (282 - SPECIALIST_ROW_HEIGHT), 30, 30,
        ...
        for i in range(gc.getNumSpecialistInfos()):
            for j in range(pHeadSelectedCity.getFreeSpecialistCount(i)):
                szName = "FreeSpecialist" + str(iCount)
                screen.setImageButton(szName, gc.getSpecialistInfo(i).getTexture(), (xResolution - SPECIALIST_AREA_MARGIN - (stackWidth * iCount)), yResolution - (282 - SPECIALIST_ROW_HEIGHT * 2), 30, 30, WidgetTypes.WIDGET_FREE_CITIZEN, i,
                                      1)
                screen.show(szName)
                bHandled = True
        ...
        for i in range(gc.getNumSpecialistInfos()):
            ...
            if (gcgetPlayer(pHeadSelectedCity.getOwner()).isSpecialistValid(i) or i == 0):
            ...
            for k in range(iCount):
                if (k >= pHeadSelectedCity.getSpecialistCount(i)):
                    szName = "IncresseCitizenBanner" + str((i * 100) + k)
                    screen.addCheckBoxGFC(szName, gc.getSpecialistInfo(i).getTexture(), "", xResolution - (SPECIALIST_AREA_MARGIN + iXShiftVal) - (HorizontalSpacing * k), (yResolution - 282 - (SPECIALIST_ROW_HEIGHT * iYShiftVal)), 30, 30,
                                          WidgetTypes.WIDGET_CHANGE_SPECIALIST, i, 1, ButtonStyles.BUTTON_STYLE_LABEL)
                    screen.enable(szName, False)
                    screen.show(szName)

                    szName = "IncresseCitizenButton" + str((i * 100) + k)
                    screen.addCheckBoxGFC(szName, "", "", xResolution - (SPECIALIST_AREA_MARGIN + iXShiftVal) - (HorizontalSpacing * k), (yResolution - 282 - (SPECIALIST_ROW_HEIGHT * iYShiftVal)), 30, 30,
                                          WidgetTypes.WIDGET_CHANGE_SPECIALIST, i, 1, ButtonStyles.BUTTON_STYLE_LABEL)
                    screen.show(szName)
    ...
    def updateGameDataStrings(self):
        ...
        bShift = GlobalCyInterface.shiftKey()
        ...
        if (pHeadSelectedCity):
            ...
            ePlayer = gcgame.getActivePlayer()
        ...
        if (
            ...
            # Percent of commerce
            if (gcgetPlayer(ePlayer).isAlive()):
                iCount = 0
                for iI in range(CommerceTypes.NUM_COMMERCE_TYPES):
                    eCommerce = (iI + 1) % CommerceTypes.NUM_COMMERCE_TYPES
                    if (gcgetPlayer(ePlayer).isCommerceFlexible(eCommerce) or (GlobalCyInterface.isCityScreenUp() and (eCommerce == CommerceTypes.COMMERCE_GOLD))):
                        szOutText = u"<font=2>%c:%d%%</font>" % (gc.getCommerceInfo(eCommerce).getChar(), gcgetPlayer(ePlayer).getCommercePercent(eCommerce))
                        szString = "PercentText" + str(iI)
                        ...
                        if not GlobalCyInterface.isCityScreenUp():
                            szOutText = u"<font=2>" + localText.getText("TXT_KEY_MISC_POS_GOLD_PER_TURN", (gcgetPlayer(ePlayer).getCommerceRate(CommerceTypes(eCommerce)),)) + u"</font>"
                            szString = "RateText" + str(iI)
                            # BUG - Min/Max Sliders - start
            ...
            if (gcgetPlayer(ePlayer).isAlive()):
                ...
                if MainOpt.isGoldRateWarning():
                    ...
                    szText = GlobalCyGameTextMgr.getGoldStr(ePlayer)
                ...
                if (((gcgetPlayer(ePlayer).calculateGoldRate() != 0) and not (gcgetPlayer(ePlayer).isAnarchy())) or (gcgetPlayer(ePlayer).getGold() != 0)):
                    screen.show("GoldText")

                # BUG - NJAGC - start
                if (ClockOpt.isEnabled()
                        and ClockOpt.isShowEra()):
                    szText = localText.getText("TXT_KEY_BUG_ERA", (gc.getEraInfo(gcgetPlayer(ePlayer).getCurrentEra()).getDescription(),))
                    if (ClockOpt.isUseEraColor()):
                        iEraColor = ClockOpt.getEraColor(gc.getEraInfo(gcgetPlayer(ePlayer).getCurrentEra()).getType())
                        if (iEraColor >= 0):
                ...
                if (gcgetPlayer(ePlayer).isAnarchy()):
                    ...
                    szText = localText.getText("INTERFACE_ANARCHY", (gcgetPlayer(ePlayer).getAnarchyTurns(),))
                    ...
                    if (gcgetPlayer(ePlayer).getCurrentResearch() != -1):
                        screen.show("ResearchText")
                    else:
                        screen.hide("ResearchText")

                elif (gcgetPlayer(ePlayer).getCurrentResearch() != -1):

                    szText = GlobalCyGameTextMgr.getResearchStr(ePlayer)
                    ...
                    researchProgress = gcgetTeam(gcgetPlayer(ePlayer).getTeam()).getResearchProgress(gcgetPlayer(ePlayer).getCurrentResearch())
                    overflowResearch = (gcgetPlayer(ePlayer).getOverflowResearch() * gcgetPlayer(ePlayer).calculateResearchModifier(gcgetPlayer(ePlayer).getCurrentResearch())) / 100
                    researchCost = gcgetTeam(gcgetPlayer(ePlayer).getTeam()).getResearchCost(gcgetPlayer(ePlayer).getCurrentResearch())
                    researchRate = gcgetPlayer(ePlayer).calculateResearchRate(-1)
    ...
    def updateGreatPersonBar(self, screen):
        if (not GlobalCyInterface.isCityScreenUp() and MainOpt.isShowGPProgressBar()):
            ...
            if (pGPCity):
                fThreshold = float(gcgetPlayer(pGPCity.getOwner()).greatPeopleThreshold(False))
    ...
    def updateGreatGeneralBar(self, screen):
        if (not GlobalCyInterface.isCityScreenUp() and MainOpt.isShowGGProgressBar()):
        ...
        if (ClockOpt.isEnabled()):
            ...
            if (bShowGameTurn):
                if (bFirst):
                    ...
                    g_szTimeText += u" - "
                g_szTimeText += u"%d" % (gcgame.getElapsedGameTurns())
                if (bShowTotalTurns):
                    g_szTimeText += u"/%d" % (gcgame.getMaxTurns())
            ...
            if (bShowPercentComplete):
                if (bFirst):
                    ...
                    if (not bShowGameTurn):
                        ...
                        g_szTimeText += u" "
                g_szTimeText += u"%2.2f%%" % (100 * (float(gcgame.getElapsedGameTurns()) / float(gcgame.getMaxTurns())))
            ...
            if (bShowDateGA):
                if (bFirst):
                    ...
                    g_szTimeText += u" - "
                szDateGA = unicode(GlobalCyGameTextMgr.getInterfaceTimeStr(ePlayer))
                if (ClockOpt.isUseEraColor()):
                    iEraColor = ClockOpt.getEraColor(gc.getEraInfo(gcgetPlayer(ePlayer).getCurrentEra()).getType())
                    if (iEraColor >= 0):
			...
			"""
            g_szTimeText = localText.getText("TXT_KEY_TIME_TURN", (GlobalCyGame.getGameTurn(),)) + u" - " + unicode(GlobalCyGameTextMgr.getInterfaceTimeStr(ePlayer))
    ...
    def updateCityScreen(self):
        ...
        if (GlobalCyInterface.isCityScreenUp()):
            if (pHeadSelectedCity):
                ...
                if (pHeadSelectedCity.getTeam() == gcgame.getActiveTeam()):
                ...
                if (pHeadSelectedCity.isCapital()):
                    szBuffer += u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.STAR_CHAR))
                elif (pHeadSelectedCity.isGovernmentCenter()):
                    szBuffer += u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.SILVER_STAR_CHAR))

                if (pHeadSelectedCity.isDirtyPower()):
                    szBuffer += u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.POWER_CHAR))
                elif (pHeadSelectedCity.isPower()):
                    szBuffer += u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.CLEAN_POWER_CHAR))

                szBuffer += u"%s: %d" % (pHeadSelectedCity.getName(), pHeadSelectedCity.getPopulation())

                if (pHeadSelectedCity.isOccupation()):
                    szBuffer += u" (%c:%d)" % (GlobalCyGame.getSymbolID(FontSymbols.OCCUPATION_CHAR), pHeadSelectedCity.getOccupationTimer())
                ...
                if (not pHeadSelectedCity.isDisorder() and not pHeadSelectedCity.isFoodProduction()):
                    ...
                    if (CityScreenOpt.isShowFoodAssist()):
                        ...
                        szBuffer = u"%d%c - %d%c" % (
                            pHeadSelectedCity.getYieldRate(YieldTypes.YIELD_FOOD), gc.getYieldInfo(YieldTypes.YIELD_FOOD).getChar(), pHeadSelectedCity.foodConsumption(False, 0), GlobalCyGame.getSymbolID(FontSymbols.EATEN_FOOD_CHAR))
                # BUG - Food Assist - end
                ...
                if ((pHeadSelectedCity.happyLevel() >= 0) or (pHeadSelectedCity.unhappyLevel(0) > 0)):
                    if (pHeadSelectedCity.isDisorder()):
                        szBuffer = u"%d%c" % (pHeadSelectedCity.angryPopulation(0), GlobalCyGame.getSymbolID(FontSymbols.ANGRY_POP_CHAR))
                    ...
                    if (CityScreenOpt.isShowAngerCounter()
                            and pHeadSelectedCity.getTeam() == gcgame.getActiveTeam()):
                        iAngerTimer = max(pHeadSelectedCity.getHurryAngerTimer(), pHeadSelectedCity.getConscriptAngerTimer())
                        if not gcgetPlayer(pHeadSelectedCity.getOwner()).isNoTemporaryUnhappiness() and iAngerTimer > 0:
                ...
                for i in range(CommerceTypes.NUM_COMMERCE_TYPES):
                    ...
                    if ((gcgetPlayer(pHeadSelectedCity.getOwner()).isCommerceFlexible(eCommerce)) or (eCommerce == CommerceTypes.COMMERCE_GOLD)):
                        ...
                        if (iHappiness != 0):
                            if (iHappiness > 0):
                                szTempBuffer = u", %d%c" % (iHappiness, GlobalCyGame.getSymbolID(FontSymbols.HAPPY_CHAR))
                            else:
                                szTempBuffer = u", %d%c" % (-iHappiness, GlobalCyGame.getSymbolID(FontSymbols.UNHAPPY_CHAR))
                        ...
                        for k in range(pHeadSelectedCity.getNumBuilding(i)):
                            ...
                            if (pHeadSelectedCity.getNumActiveBuilding(i) > 0):
                                ...
                                if (iHealth != 0):
                                    ...
                                    if (iHealth > 0):
                                        szTempBuffer = u"+%d%c" % (iHealth, GlobalCyGame.getSymbolID(FontSymbols.HEALTHY_CHAR))
                                        szRightBuffer = szRightBuffer + szTempBuffer
                                    else:
                                        szTempBuffer = u"+%d%c" % (-(iHealth), GlobalCyGame.getSymbolID(FontSymbols.UNHEALTHY_CHAR))
                                        szRightBuffer = szRightBuffer + szTempBuffer
                ...
                for i in range(gc.getDefineINT("MAX_TRADE_ROUTES")):
                    ...
                    if (pLoopCity and pLoopCity.getOwner() >= 0):
                        player = gcgetPlayer(pLoopCity.getOwner())
                ...
                for i in range(gc.getNumBonusInfos()):
                    ...
                    if (pHeadSelectedCity.hasBonus(i)):
                        ...
                        iPlotIndex = gcmap.plotIndex(pHeadSelectedCity.getX(), pHeadSelectedCity.getY())
                        ...
                        # wunshare
                        iAffectedCities = gcgetPlayer(pHeadSelectedCity.getOwner()).getBonusAffectedCities(i)  # bonusInfo.getAffectedCities()
                        ...
                        if (iHappiness != 0 or iCityHappiness != 0):
                            ...
                            if iCityHappiness > 0:
                                szTempBuffer = u"<font=1>+%d%c</font>" % (iCityHappiness, GlobalCyGame.getSymbolID(FontSymbols.HAPPY_CHAR))
                            elif iCityHappiness < 0:
                                szTempBuffer = u"<font=1>+%d%c</font>" % (-iCityHappiness, GlobalCyGame.getSymbolID(FontSymbols.UNHAPPY_CHAR))

                            if iCityHealth > 0:
                                szTempBuffer += u"<font=1>, +%d%c</font>" % (iCityHealth, GlobalCyGame.getSymbolID(FontSymbols.HEALTHY_CHAR))
                            elif iCityHealth < 0:
                                szTempBuffer += u"<font=1>, +%d%c</font>" % (iCityHealth, GlobalCyGame.getSymbolID(FontSymbols.UNHEALTHY_CHAR))
                ...
                szBuffer = u"%d%% %s" % (pHeadSelectedCity.calculateOverallCulturePercent(pHeadSelectedCity.getOwner()), gcgetPlayer(pHeadSelectedCity.getOwner()).getCivilizationAdjective(0))
                ...
                for i in range(len(lBars)):
                    ...
                    screen.setStackedBarColorsRGB("NationalityBar", i, gcgetPlayer(iPlayer).getPlayerTextColorR(), gcgetPlayer(iPlayer).getPlayerTextColorG(), gcgetPlayer(iPlayer).getPlayerTextColorB(),
                                                  gcgetPlayer(iPlayer).getPlayerTextColorA())
                ...
                #	if True: #( gcgetPlayer(h).isAlive() ):
                #		iPercent = pHeadSelectedCity.plot().calculateOverallCulturePercent(h)
                #		if ( iPercent > 0 ):
                #			screen.setStackedBarColorsRGB( "NationalityBar", iWhichBar, gcgetPlayer(h).getPlayerTextColorR(), gcgetPlayer(h).getPlayerTextColorG(), gcgetPlayer(h).getPlayerTextColorB(), gcgetPlayer(h).getPlayerTextColorA() )
                #			if ( iRemainder <= 0):
                ...
                if (iDefenseModifier != 0):
                    szBuffer = localText.getText("TXT_KEY_MAIN_CITY_DEFENSE", (GlobalCyGame.getSymbolID(FontSymbols.DEFENSE_CHAR), iDefenseModifier))
                ...
                szBuffer = localText.getText("INTERFACE_CITY_WONDER_LIMIT", (iWorldWonders, iWorldWondersLimit, GlobalCyGame.getSymbolID(FontSymbols.STAR_CHAR)))
                ...
                szBuffer = localText.getText("INTERFACE_CITY_WONDER_LIMIT", (iNationalWonders, iNationalWondersLimit, GlobalCyGame.getSymbolID(FontSymbols.SILVER_STAR_CHAR)))
                ...
                if ((pHeadSelectedCity.getGreatPeopleProgress() > 0) or (pHeadSelectedCity.getGreatPeopleRate() > 0)):
                    ...
                    if CityScreenOpt.isShowCityGreatPersonInfo():
                        ...
                        szBuffer = localText.getText("INTERFACE_CITY_GREATPEOPLE_RATE", (GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR), pHeadSelectedCity.getGreatPeopleRate()))
                    ...
                    iFirst = float(pHeadSelectedCity.getGreatPeopleProgress()) / float(gcgetPlayer(pHeadSelectedCity.getOwner()).greatPeopleThreshold(False))
                    screen.setBarPercentage("GreatPeopleBar", InfoBarTypes.INFOBAR_STORED, iFirst)
                    if (iFirst == 1):
                        screen.setBarPercentage("GreatPeopleBar", InfoBarTypes.INFOBAR_RATE, (float(pHeadSelectedCity.getGreatPeopleRate()) / float(gcgetPlayer(pHeadSelectedCity.getOwner()).greatPeopleThreshold(False))))
                    else:
                        screen.setBarPercentage("GreatPeopleBar", InfoBarTypes.INFOBAR_RATE, ((float(pHeadSelectedCity.getGreatPeopleRate()) / float(gcgetPlayer(pHeadSelectedCity.getOwner()).greatPeopleThreshold(False)))) / (1 - iFirst))
                ...
                print
                "Culture costs: " + str(lCultureCosts)
            ...
            # Help Text Area
            if (GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):
                screen.setHelpTextArea(350, FontTypes.SMALL_FONT, 7, yResolution - 172, -0.1, False, "", True, False, CvUtil.FONT_LEFT_JUSTIFY, HELP_TEXT_MINIMUM_WIDTH)
            else:
    ...
    def updateInfoPaneStrings(self):
        ...
        if (pHeadSelectedCity):
            ...
            for i in range(iOrders):
                ...
                if (GlobalCyInterface.getOrderNodeType(i) == OrderTypes.ORDER_TRAIN):
                    szLeftBuffer = gc.getUnitInfo(GlobalCyInterface.getOrderNodeData1(i)).getDescription()
                    szRightBuffer = "(" + str(pHeadSelectedCity.getUnitProductionTurnsLeft(GlobalCyInterface.getOrderNodeData1(i), i)) + ")"

                    if (GlobalCyInterface.getOrderNodeSave(i)):
                        szLeftBuffer = u"*" + szLeftBuffer

                    # BUG - Production Started - start
                    if CityScreenOpt.isShowProductionStarted():
                        eUnit = GlobalCyInterface.getOrderNodeData1(i)
                ...
                elif (GlobalCyInterface.getOrderNodeType(i) == OrderTypes.ORDER_CONSTRUCT):
                    szLeftBuffer = gc.getBuildingInfo(GlobalCyInterface.getOrderNodeData1(i)).getDescription()
                    szRightBuffer = "(" + str(pHeadSelectedCity.getBuildingProductionTurnsLeft(GlobalCyInterface.getOrderNodeData1(i), i)) + ")"

                    # BUG - Production Started - start
                    if CityScreenOpt.isShowProductionStarted():
                        eBuilding = GlobalCyInterface.getOrderNodeData1(i)
                ...
                elif (GlobalCyInterface.getOrderNodeType(i) == OrderTypes.ORDER_CREATE):
                    szLeftBuffer = gc.getProjectInfo(GlobalCyInterface.getOrderNodeData1(i)).getDescription()
                    szRightBuffer = "(" + str(pHeadSelectedCity.getProjectProductionTurnsLeft(GlobalCyInterface.getOrderNodeData1(i), i)) + ")"

                    # BUG - Production Started - start
                    if BugDll.isVersion(3) and CityScreenOpt.isShowProductionStarted():
                        eProject = GlobalCyInterface.getOrderNodeData1(i)
                        if pHeadSelectedCity.getProjectProduction(eProject) > 0:
                            szRightBuffer = BugUtil.colorText(szRightBuffer, "COLOR_CYAN")
                # BUG - Production Started - end

                elif (GlobalCyInterface.getOrderNodeType(i) == OrderTypes.ORDER_MAINTAIN):
                    szLeftBuffer = gc.getProcessInfo(GlobalCyInterface.getOrderNodeData1(i)).getDescription()
        ...
        elif (pHeadSelectedUnit and GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):
            ...
            if (GlobalCyInterface.mirrorsSelectionGroup()):
                pSelectedGroup = pHeadSelectedUnit.getGroup()
            else:
                pSelectedGroup = 0

            if (GlobalCyInterface.getLengthSelectionList() > 1):

                # BUG - Stack Movement Display - start
                szBuffer = localText.getText("TXT_KEY_UNIT_STACK", (GlobalCyInterface.getLengthSelectionList(),))
                if MainOpt.isShowStackMovementPoints():
                    iMinMoves = 100000
                    iMaxMoves = 0
                    for i in range(GlobalCyInterface.getLengthSelectionList()):
                        pUnit = GlobalCyInterface.getSelectionUnit(i)
                    ...
                    if (iMinMoves == iMaxMoves):
                        ...
                        szBuffer += u" %.1f%c" % (fMinMoves, GlobalCyGame.getSymbolID(FontSymbols.MOVES_CHAR))
                    else:
                        fMinMoves = float(iMinMoves) / gc.getMOVE_DENOMINATOR()
                        fMaxMoves = float(iMaxMoves) / gc.getMOVE_DENOMINATOR()
                        szBuffer += u" %.1f - %.1f%c" % (fMinMoves, fMaxMoves, GlobalCyGame.getSymbolID(FontSymbols.MOVES_CHAR))
                ...
                if ((pSelectedGroup == 0) or (pSelectedGroup.getLengthMissionQueue() <= 1)):
                    if (pHeadSelectedUnit):
                        for i in range(gc.getNumUnitInfos()):
                            iCount = GlobalCyInterface.countEntities(i)
                ...
                if ((pSelectedGroup == 0) or (pSelectedGroup.getLengthMissionQueue() <= 1)):
                    ...
                    if (pHeadSelectedUnit.getDomainType() == DomainTypes.DOMAIN_AIR):
                        if (pHeadSelectedUnit.airBaseCombatStr() > 0):
                            ...
                            if (pHeadSelectedUnit.isFighting()):
                                szRightBuffer = u"?/%d%c" % (pHeadSelectedUnit.airBaseCombatStr(), GlobalCyGame.getSymbolID(FontSymbols.STRENGTH_CHAR))
                            elif (pHeadSelectedUnit.isHurt()):
                                szRightBuffer = u"%.1f/%d%c" % (((float(pHeadSelectedUnit.airBaseCombatStr() * pHeadSelectedUnit.currHitPoints())) / (float(pHeadSelectedUnit.maxHitPoints()))), pHeadSelectedUnit.airBaseCombatStr(),
                                                                GlobalCyGame.getSymbolID(FontSymbols.STRENGTH_CHAR))
                            else:
                                szRightBuffer = u"%d%c" % (pHeadSelectedUnit.airBaseCombatStr(), GlobalCyGame.getSymbolID(FontSymbols.STRENGTH_CHAR))
                    else:
                        if (pHeadSelectedUnit.canFight()):
                            ...
                            if (pHeadSelectedUnit.isFighting()):
                                szRightBuffer = u"?/%d%c" % (pHeadSelectedUnit.baseCombatStr(), GlobalCyGame.getSymbolID(FontSymbols.STRENGTH_CHAR))
                            elif (pHeadSelectedUnit.isHurt()):
                                szRightBuffer = u"%.1f/%d%c" % (((float(pHeadSelectedUnit.baseCombatStr() * pHeadSelectedUnit.currHitPoints())) / (float(pHeadSelectedUnit.maxHitPoints()))), pHeadSelectedUnit.baseCombatStr(),
                                                                GlobalCyGame.getSymbolID(FontSymbols.STRENGTH_CHAR))
                            else:
                                szRightBuffer = u"%d%c" % (pHeadSelectedUnit.baseCombatStr(), GlobalCyGame.getSymbolID(FontSymbols.STRENGTH_CHAR))
                    ...
                    if MainOpt.isShowUnitMovementPointsFraction():
                        szRightBuffer = u"%d%c" % (pHeadSelectedUnit.baseMoves(), GlobalCyGame.getSymbolID(FontSymbols.MOVES_CHAR))
                        ...
                        if (pHeadSelectedUnit.baseMoves() == iCurrMoves):
                            ...
                            szRightBuffer = u"%d/%d%c" % (iCurrMoves, pHeadSelectedUnit.baseMoves(), GlobalCyGame.getSymbolID(FontSymbols.MOVES_CHAR))
    ...
    def CivNameSuffx(self, iPlayer):
        txt = ''


        canOpenBroader = utils.canOpenBordersTrades(iPlayer, utils.getHumanID())
        if (canOpenBroader):
            txt += '[O]'

        canVassal1 = utils.canVassalTrades(iPlayer, utils.getHumanID())

        if (canVassal1):
            txt += '[V]'   # '附庸'

        canVassal2 = utils.canSurrenderTrades(iPlayer, utils.getHumanID())
        if (canVassal2):
            txt += '[S]'   # '投降'

        import Stability
        bFall = Stability.isDecline(iPlayer)
        if bFall:
            txt += '[F]'

        CollapseTurn = data.players[iPlayer].iTurnsToCollapse
        if CollapseTurn >= 0:
            txt += '[正在崩溃][' + str(CollapseTurn) + ']'
    ...
    def updateScoreStrings(self):
        ...
        if ((GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY)):
            if (GlobalCyInterface.isScoresVisible() and not GlobalCyInterface.isCityScreenUp() and CyEngine().isGlobeviewUp() == False):
                ...
                while (i > -1):
                    eTeam = gcgame.getRankTeam(i)

                    if (gcgetTeam(gcgame.getActiveTeam()).isHasMet(eTeam) or gcgetTeam(eTeam).isHuman() or gcgame.isDebugMode()):
                        # BUG - Align Icons - start
                        if (bAlignIcons):
                            scores.addTeam(gcgetTeam(eTeam), i)
                        # BUG - Align Icons - end
                        j = gc.getMAX_CIV_PLAYERS() - 1
                        while (j > -1):
                            ePlayer = gcgame.getRankPlayer(j)

                            if (not GlobalCyInterface.isScoresMinimized() or gcgame.getActivePlayer() == ePlayer):
                                # BUG - Dead Civs - start
                                if (gcgetPlayer(ePlayer).isEverAlive() and not gcgetPlayer(ePlayer).isBarbarian()
                                        and (gcgetPlayer(ePlayer).isAlive() or ScoreOpt.isShowDeadCivs())):
                                    # BUG - Dead Civs - end
                                    # BUG - Minor Civs - start			# 排除野蛮人拜占庭
                                    if ((not gcgetPlayer(ePlayer).isMinorCiv() or ScoreOpt.isShowMinorCivs()) and ePlayer <= iCanada + 3):
                                        # BUG - Minor Civs - end
                                        if (gcgetPlayer(ePlayer).getTeam() == eTeam):
                                            szBuffer = u"<font=2>"
                                            # BUG - Align Icons - start
                                            if (bAlignIcons):
                                                scores.addPlayer(gcgetPlayer(ePlayer), j)
                                            # BUG: Align Icons continues throughout -- if (bAlignIcons): scores.setFoo(foo)
                                            # BUG - Align Icons - end

                                            if (gcgame.isGameMultiPlayer()):
                                                if (not (gcgetPlayer(ePlayer).isTurnActive())):
                                                    szBuffer = szBuffer + "*"
                                                    if (bAlignIcons):
                                                        scores.setWaiting()
                                            ...
                                            if (ScoreOpt.isUsePlayerName()):
                                                szPlayerName = gcgetPlayer(ePlayer).getCivilizationShortDescription(0)
                                            else:
                                                szPlayerName = gc.getLeaderHeadInfo(gcgetPlayer(ePlayer).getLeaderType()).getDescription()
                                            if (ScoreOpt.isShowBothNames()):
                                                szCivName = gcgetPlayer(ePlayer).getCivilizationShortDescription(0)
                                                szPlayerName = szPlayerName + "/" + szCivName
                                            elif (ScoreOpt.isShowBothNamesShort()):
                                                szCivName = gcgetPlayer(ePlayer).getCivilizationDescription(0)
                                                szPlayerName = szPlayerName + "/" + szCivName
                                            ...
                                            szPlayerName += self.CivNameSuffx(ePlayer)

                                            if (not gcgetPlayer(ePlayer).isAlive() and ScoreOpt.isShowDeadTag()):
                                                szPlayerScore = localText.getText("TXT_KEY_BUG_DEAD_CIV", ())
                                                if (bAlignIcons):
                                                    scores.setScore(szPlayerScore)
                                            else:
                                                iScore = gcgame.getPlayerScore(ePlayer)
                                                ...
                                                if (ScoreOpt.isShowScoreDelta()):
                                                    ...
                                                    if (ePlayer >= gcgame.getActivePlayer()):
                                                        iGameTurn -= 1
                                                    if (ScoreOpt.isScoreDeltaIncludeCurrentTurn()):
                                                        iScoreDelta = iScore
                                                    elif (iGameTurn >= 0):
                                                        iScoreDelta = gcgetPlayer(ePlayer).getScoreHistory(iGameTurn)
                                                    else:
                                                        iScoreDelta = 0
                                                    iPrevGameTurn = iGameTurn - 1
                                                    if (iPrevGameTurn >= 0):
                                                        iScoreDelta -= gcgetPlayer(ePlayer).getScoreHistory(iPrevGameTurn)
                                                    if (iScoreDelta != 0):
                                                        if (iScoreDelta > 0):
                                            ...
                                            if (not GlobalCyInterface.isFlashingPlayer(ePlayer) or GlobalCyInterface.shouldFlash(ePlayer)):
                                                if (ePlayer == gcgame.getActivePlayer()):
                                                    szPlayerName = u"[<color=%d,%d,%d,%d>%s</color>]" % (
                                                        gcgetPlayer(ePlayer).getPlayerTextColorR(), gcgetPlayer(ePlayer).getPlayerTextColorG(), gcgetPlayer(ePlayer).getPlayerTextColorB(), gcgetPlayer(ePlayer).getPlayerTextColorA(),
                                                        szPlayerName)
                                                else:
                                                    if (not gcgetPlayer(ePlayer).isAlive() and ScoreOpt.isGreyOutDeadCivs()):
                                                        szPlayerName = u"<color=%d,%d,%d,%d>%s</color>" % (175, 175, 175, gcgetPlayer(ePlayer).getPlayerTextColorA(), szPlayerName)
                                                    else:
                                                        szPlayerName = u"<color=%d,%d,%d,%d>%s</color>" % (
                                                            gcgetPlayer(ePlayer).getPlayerTextColorR(), gcgetPlayer(ePlayer).getPlayerTextColorG(), gcgetPlayer(ePlayer).getPlayerTextColorB(), gcgetPlayer(ePlayer).getPlayerTextColorA(),
                                                            szPlayerName)
                                            ...
                                            if (gcgetPlayer(ePlayer).isAlive()):
                                                ...
                                                if (gcgetTeam(eTeam).isAlive()):
                                                    if (not (gcgetTeam(gcgame.getActiveTeam()).isHasMet(eTeam))):
                                                        szBuffer = szBuffer + (" ?")
                                                        if (bAlignIcons):
                                                            scores.setNotMet()
                                                    if (gcgetTeam(eTeam).isAtWar(gcgame.getActiveTeam())):
                                                        szBuffer = szBuffer + "(" + localText.getColorText("TXT_KEY_CONCEPT_WAR", (), gc.getInfoTypeForString("COLOR_RED")).upper() + ")"
                                                        if (bAlignIcons):
                                                            scores.setWar()
                                                    elif (gcgetTeam(gcgame.getActiveTeam()).isForcePeace(eTeam)):
                                                        if (bAlignIcons):
                                                            scores.setPeace()
                                                    elif (gcgetTeam(eTeam).isAVassal()):
                                                        for iOwnerTeam in range(gc.getMAX_TEAMS()):
                                                            if (gcgetTeam(eTeam).isVassal(iOwnerTeam) and gcgetTeam(gcgame.getActiveTeam()).isForcePeace(iOwnerTeam)):
                                                                if (bAlignIcons):
                                                                    scores.setPeace()
                                                                break
                                                    if (gcgetPlayer(ePlayer).canTradeNetworkWith(gcgame.getActivePlayer()) and (ePlayer != gcgame.getActivePlayer())):
                                                        szTempBuffer = u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.TRADE_CHAR))
                                                        szBuffer = szBuffer + szTempBuffer
                                                        if (bAlignIcons):
                                                            scores.setTrade()
                                                    if (gcgetTeam(eTeam).isOpenBorders(gcgame.getActiveTeam())):
                                                        szTempBuffer = u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.OPEN_BORDERS_CHAR))
                                                        szBuffer = szBuffer + szTempBuffer
                                                        if (bAlignIcons):
                                                            scores.setBorders()
                                                    if (gcgetTeam(eTeam).isDefensivePact(gcgame.getActiveTeam())):
                                                        szTempBuffer = u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.DEFENSIVE_PACT_CHAR))
                                                        szBuffer = szBuffer + szTempBuffer
                                                        if (bAlignIcons):
                                                            scores.setPact()
                                                    if gcgetPlayer(ePlayer).isStateReligion():
                                                        if gcgetPlayer(ePlayer).getStateReligion() == -1:
                                                            szTempBuffer = self.getPaganReligionChar(ePlayer)
                                                        elif (gcgetPlayer(ePlayer).hasHolyCity(gcgetPlayer(ePlayer).getStateReligion())):
                                                            szTempBuffer = u"%c" % (gc.getReligionInfo(gcgetPlayer(ePlayer).getStateReligion()).getHolyCityChar())
                                                        else:
                                                            szTempBuffer = u"%c" % (gc.getReligionInfo(gcgetPlayer(ePlayer).getStateReligion()).getChar())
                                                        szBuffer = szBuffer + szTempBuffer
                                                        if (bAlignIcons):
                                                            scores.setReligion(szTempBuffer)

                                                    if (bEspionage and gcgetTeam(eTeam).getEspionagePointsAgainstTeam(gcgame.getActiveTeam()) < gcgetTeam(gcgame.getActiveTeam()).getEspionagePointsAgainstTeam(eTeam)):
                                                    ...
                                                    if (gcgetTeam(gcgame.getActiveTeam()).getCounterespionageTurnsLeftAgainstTeam(eTeam) != 0):
                                                        szTempBuffer = u"(%d)%c" % (gcgetTeam(gcgame.getActiveTeam()).getCounterespionageTurnsLeftAgainstTeam(eTeam), GlobalCyGame.getSymbolID(FontSymbols.OCCUPATION_CHAR))
                                                        szBuffer = szBuffer + szTempBuffer
                                                        if (bAlignIcons):
                                                ...
                                                if (((gcgetPlayer(ePlayer).getTeam() == gcgame.getActiveTeam()) and (gcgetTeam(gcgame.getActiveTeam()).getNumMembers() > 1)) or (
                                                        gcgetTeam(gcgetPlayer(ePlayer).getTeam()).isVassal(gcgame.getActiveTeam())) or gcgame.isDebugMode() or bEspionageCanSeeResearch):
                                                    if (gcgetPlayer(ePlayer).getCurrentResearch() != -1):
                                                        szTempBuffer = u"-%s (%d)" % (
                                                            gc.getTechInfo(gcgetPlayer(ePlayer).getCurrentResearch()).getDescription(), gcgetPlayer(ePlayer).getResearchTurnsLeft(gcgetPlayer(ePlayer).getCurrentResearch(), True))
                                                        szBuffer = szBuffer + szTempBuffer
                                                        if (bAlignIcons):
                                                            scores.setResearch(gcgetPlayer(ePlayer).getCurrentResearch(), gcgetPlayer(ePlayer).getResearchTurnsLeft(gcgetPlayer(ePlayer).getCurrentResearch(), True), ePlayer)
                                                # BUG: ...end of indentation
                                                # BUG - Dead Civs - end
                                                # BUG - Power Rating - start
                                                # if on, show according to espionage "see demographics" mission
                                                bshowEspionage  = not bEspionage or gc.getActivePlayer().canDoEspionageMission(iDemographicsMission, ePlayer, None, -1)
                                                if PYTHON_CIV4_ALWAYS_SHOW_STRNGTH_IN_SCRREN:
                                                    bshowEspionage  = True

                                                if (bShowPower
                                                        and (gcgame.getActivePlayer() != ePlayer
                                                             and bshowEspionage)):
                                                    iPower = gcgetPlayer(ePlayer).getPower()
                                                    if (iPower > 0):  # avoid divide by zero
                                                        ...
                                                        if (ScoreOpt.isPowerThemVersusYou()):
                                                            if (fPowerRatio > 0):
                                                                ...
                                                                fPowerRatio = 99.0
                                                        cPower = gcgame.getSymbolID(FontSymbols.STRENGTH_CHAR)
                                                ...
                                                if (ScoreOpt.isShowAttitude()):
                                                    if (not gcgetPlayer(ePlayer).isHuman()):
                                                        iAtt = gcgetPlayer(ePlayer).AI_getAttitude(gcgame.getActivePlayer())
                                                        cAtt = unichr(ord(unichr(GlobalCyGame.getSymbolID(FontSymbols.POWER_CHAR) + 4)) + iAtt)
                                                        szBuffer += cAtt
                                                        if (bAlignIcons):
                                                            scores.setAttitude(cAtt)
                                                # BUG - Attitude Icons - end
                                                # Leoreth - Plague Icon - start
                                                if gcgetPlayer(ePlayer).getBuildingClassCount(gc.getInfoTypeForString("BUILDINGCLASS_PLAGUE")) > 0:
                                                    cPlague = unichr(GlobalCyGame.getSymbolID(FontSymbols.PLAGUE_CHAR))
                                                    szBuffer += cPlague
                                                    if bAlignIcons:
                                                        scores.setPlague()
                                                ...
                                                if ePlayer < iNumPlayers:
                                                    ...
                                                    if iStabilityLevel > iStabilityShaky:
                                                        cStab = unichr(GlobalCyGame.getSymbolID(FontSymbols.SOLID_CHAR))
                                                    elif iStabilityLevel > iStabilityUnstable:
                                                        cStab = unichr(GlobalCyGame.getSymbolID(FontSymbols.STABLE_CHAR))
                                                    else:
                                                        cStab = unichr(GlobalCyGame.getSymbolID(FontSymbols.UNSTABLE_CHAR))
                                                    szBuffer += cStab
                                                    if (PYTHON_SHOW_CIV_STABILITY_ON_PANNEL > 0):
                                                        # import Stability
                                                ...
                                                # BUG - Refuses to Talk - start
                                                if (not DiplomacyUtil.isWillingToTalk(ePlayer, gcgame.getActivePlayer())):
                                                    cRefusesToTalk = u"!"
                                                ...
                                                if (ScoreOpt.isShowWorstEnemy()):
                                                    if (AttitudeUtil.isWorstEnemy(ePlayer, gcgame.getActivePlayer())):
                                                        cWorstEnemy = u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.ANGRY_POP_CHAR))
                                                ...
                                                if (ScoreOpt.isShowWHEOOH()):
                                                    if (PlayerUtil.isWHEOOH(ePlayer, PlayerUtil.getActivePlayerID())):
                                                        szTempBuffer = u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.OCCUPATION_CHAR))
                                                        szBuffer = szBuffer + szTempBuffer
                                                        if (bAlignIcons):
                                                            scores.setWHEOOH()
                                                # BUG - WHEOOH - end
                                            ...
                                            if (GlobalCyGame.isNetworkMultiPlayer()):
                                                szTempBuffer = GlobalCyGameTextMgr.getNetStats(ePlayer)
                                                szBuffer = szBuffer + szTempBuffer
                                                if (bAlignIcons):
                                                    scores.setNetStats(szTempBuffer)

                                            if (gcgetPlayer(ePlayer).isHuman() and GlobalCyInterface.isOOSVisible()):
                                                szTempBuffer = u" <color=255,0,0>* %s *</color>" % (GlobalCyGameTextMgr.getOOSSeeds(ePlayer))
                                                szBuffer = szBuffer + szTempBuffer
                                                if (bAlignIcons):
                                                    scores.setNetStats(szTempBuffer)
                                            ...
                                            # BUG - Align Icons - start
                                            if (not bAlignIcons):
                                                if (GlobalCyInterface.determineWidth(szBuffer) > iWidth):
                                                    iWidth = GlobalCyInterface.determineWidth(szBuffer)

                                                szName = "ScoreText" + str(ePlayer)
                                                if (GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_SHOW or GlobalCyInterface.isInAdvancedStart()):
                                                ...
                                                GlobalCyInterface.checkFlashReset(ePlayer)
    ...
    def screen_showmoney(self, bAlignIcons, ePlayer, scores, szBuffer):
        ...
        # 金钱部分
        iGoldThrehold = PYTHON_SHOW_CIV_MONEY_ON_PANNEL
        if (iGoldThrehold > 0):
            iGold = gcgetPlayer(ePlayer).AI_maxGoldTrade(gcgame.getActivePlayer())
            iGoldPerTurn = gcgetPlayer(ePlayer).AI_maxGoldPerTurnTrade(gcgame.getActivePlayer())
            if ePlayer is utils.getHumanID():
                iGold = 0
                iGoldPerTurn = 0

            maxtradegold = self.getMaxTradeMoney()
            maxlen1 = len(str(maxtradegold))

            maxtradegold_per = self.getMaxTradeMoney_PerTurn()
            maxlen2 = len(str(maxtradegold_per))
            if (iGold >= 0):
                iGold = min(iGold, 9999999)
                if (iGold > 1000):
                    szTempBuffer += BugUtil.colorText(u"- %s" % (FillNumberToText(iGold, maxlen1)), "COLOR_YELLOW")
                elif (iGold > 500):
                    szTempBuffer += BugUtil.colorText(u"- %s" % (FillNumberToText(iGold, maxlen1)), "COLOR_CYAN")
                else:
                    szTempBuffer += BugUtil.colorText(u"- %s" % (FillNumberToText(iGold, maxlen1)), "COLOR_WHITE")

                if (iGoldPerTurn > 20):
                    szTempBuffer += BugUtil.colorText(u"   (%s)" % (FillNumberToText(iGoldPerTurn, maxlen2)), "COLOR_YELLOW")
                elif (iGoldPerTurn > 10):
                    szTempBuffer += BugUtil.colorText(u"   (%s)" % (FillNumberToText(iGoldPerTurn, maxlen2)), "COLOR_CYAN")
                else:
                    szTempBuffer += BugUtil.colorText(u"   (%s)" % (FillNumberToText(iGoldPerTurn, maxlen2)), "COLOR_WHITE")

        # 可勒索金币部分
        if (PYTHON_SHOW_CIV_ASKMONEY_ON_PANNEL > 0):
            iCiv = ePlayer
            if (gcgetPlayer(iCiv).isAlive()):
                [askMoney, isfullask] = self.getAskMoney(iCiv)
                maxAskMoney = self.getMaxAskMoney()

                if maxAskMoney>0:
                    maxlen2 = len(str(maxAskMoney))
                    letter = 'A'
                    if (not isfullask):
                        letter = 'P'
                    ...
                    if askMoney > 0:
                        color = "COLOR_WHITE"
                        if (gcgetPlayer(iCiv).AI_getAttitude(utils.getHumanID()) >= AttitudeTypes.ATTITUDE_PLEASED):
                            color = "COLOR_YELLOW"
                        if (gcgetPlayer(iCiv).AI_getAttitude(utils.getHumanID()) == AttitudeTypes.ATTITUDE_ANNOYED):
                            color = "COLOR_LIGHT_GREY"
                        if (gcgetPlayer(iCiv).AI_getAttitude(utils.getHumanID()) < AttitudeTypes.ATTITUDE_ANNOYED):
                            color = "COLOR_RED"
                        szTempBuffer += BugUtil.colorText(u"- %s(%s)" % (letter, FillNumberToText(askMoney, maxlen2)), color)
                    else:
                        szTempBuffer += BugUtil.colorText(u"-       "+'  '*maxlen2 +'', "COLOR_WHITE")
    ...
    def getMaxTradeMoney(self):
        maxTradeMoney = 0
        for iCiv2 in range(iNumPlayers):
            if (gcgetPlayer(iCiv2).isAlive()):
                if iCiv2 is utils.getHumanID():
                    iGold = 0
                else:
                    iGold = gcgetPlayer(iCiv2).AI_maxGoldTrade(gcgame.getActivePlayer())
                    maxTradeMoney = max(maxTradeMoney, iGold)
        return maxTradeMoney

    def getMaxTradeMoney_PerTurn(self):
        maxTradeMoney = 0
        for iCiv2 in range(iNumPlayers):
            if (gcgetPlayer(iCiv2).isAlive()):
                if iCiv2 is utils.getHumanID():
                    ...
                    iGoldPerTurn = gcgetPlayer(iCiv2).AI_maxGoldPerTurnTrade(gcgame.getActivePlayer())
                    maxTradeMoney = max(maxTradeMoney, iGoldPerTurn)
        return maxTradeMoney

    def getMaxAskMoney(self):
        maxAskMoney = 0
        for iCiv2 in range(iNumPlayers):
            if (gcgetPlayer(iCiv2).isAlive()):
                [askMoney2,isfullask] = self.getAskMoney(iCiv2)
                maxAskMoney = max(maxAskMoney, askMoney2)
        return maxAskMoney

    def getAskMoney(self, iCiv):
        iPlayer = iCiv
        human = utils.getHumanID()
        cantrade = gcgetTeam(human).canDeclareWar(iPlayer)
        ...
        if iCiv is human:
            c = 0
        if utils.isMinorOrBarbaian(iPlayer):
            c = 0
        if not cantrade:
            c = 0
        return [c,isfullask]
    ...
    def updateHelpStrings(self):
        ...
        if (GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_HIDE_ALL):
            screen.setHelpTextString("")
        else:
            screen.setHelpTextString(GlobalCyInterface.getHelpString())
    ...
    def updateGlobeviewButtons(self):
        ...
        if GlobalCyInterface.getShowInterface() == InterfaceVisibility.INTERFACE_HIDE:
        ...
        iNumLayers = kGLM.getNumLayers()
        if kEngine.isGlobeviewUp() and GlobalCyInterface.getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL:
            # set up panel
            ...
            if bHasOptions:
                ...
                for iTmp in range(iNumOptions):
                    ...
                    iTextWidth = GlobalCyInterface.determineWidth(szBuffer)
    ...
    def handleInput(self, inputClass):
        ...
        if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED and inputClass.getFunctionName().startswith("GreatPersonBar")):
            ...
            if pCity and not pCity.isNone():
                GlobalCyInterface.selectCity(pCity, False)
            return 1
        # BUG - Great Person Bar - end
        ...
        if (inputClass.getNotifyCode() == 11 and inputClass.getData1() == 10000 and inputClass.getData2() == 10000):
            ...
            city = gcmap.plot(iX, iY).getPlotCity()
    ...
    def handleRawYieldsButtons(self, inputClass):
        ...
        if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CURSOR_MOVE_ON):
            ...
            if iButton == 0:
                ...
                return 0
            GlobalCyInterface.setDirty(InterfaceDirtyBits.CityScreen_DIRTY_BIT, True)
            return 1
        return 0
    ...
    def forward(self):
        if (not GlobalCyInterface.isFocused() or GlobalCyInterface.isCityScreenUp()):
            if (GlobalCyInterface.isCitySelection()):
                GlobalCyGame.doControl(ControlTypes.CONTROL_NEXTCITY)
            else:
                GlobalCyGame.doControl(ControlTypes.CONTROL_NEXTUNIT)

    def back(self):
        if (not GlobalCyInterface.isFocused() or GlobalCyInterface.isCityScreenUp()):
            if (GlobalCyInterface.isCitySelection()):
                GlobalCyGame.doControl(ControlTypes.CONTROL_PREVCITY)
            else:
                GlobalCyGame.doControl(ControlTypes.CONTROL_PREVUNIT)
