...
class CvTechChooser:
    ...
    def interfaceScreen(self):
        ''
        self.iPlayer = GlobalCyGame.getActivePlayer()
        if GlobalCyGame.isPitbossHost():
            return

        screen = self.getScreen()
        player = gcgetPlayer(self.iPlayer)
        ...
        screen.addDDSGFC("ScreenBackground", GlobalCyArtFileMgr.getInterfaceArtInfo('SCREEN_BG_OPAQUE').getPath(), 0, 0, self.W_SCREEN, self.H_SCREEN, WidgetTypes.WIDGET_GENERAL, -1, -1)
        ...
        # Filters
        iSize = 28
        szBorder = GlobalCyArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath()
        ...
        screen.addCheckBoxGFC("FilterBuilt", ",Art/Interface/Buttons/Actions/FoundCity.dds,Art/Interface/Buttons/Charlemagne_Atlas.dds,8,1", szBorder, 10 + iSize, 10, iSize, iSize, WidgetTypes.WIDGET_PYTHON, 7801, 2,
        ...
        if player.getAdvancedStartPoints() > -1:
            ...
            szText = GlobalCyTranslator.getText("TXT_KEY_WB_AS_ADD_TECH", ())
        ...
        if GlobalCyGame.isDebugMode():
            ...
            for j in xrange(gc.getMAX_PLAYERS()):
                if gcgetPlayer(j).isAlive():
                    screen.addPullDownString("CivDropDown", gcgetPlayer(j).getName(), j, j, self.iPlayer == j)
    ...
    def placeTechs(self):
        ...
        ARROW_X = GlobalCyArtFileMgr.getInterfaceArtInfo("ARROW_X").getPath()
        ARROW_Y = GlobalCyArtFileMgr.getInterfaceArtInfo("ARROW_Y").getPath()
        ARROW_MXMY = GlobalCyArtFileMgr.getInterfaceArtInfo("ARROW_MXMY").getPath()
        ARROW_XY = GlobalCyArtFileMgr.getInterfaceArtInfo("ARROW_XY").getPath()
        ARROW_MXY = GlobalCyArtFileMgr.getInterfaceArtInfo("ARROW_MXY").getPath()
        ARROW_XMY = GlobalCyArtFileMgr.getInterfaceArtInfo("ARROW_XMY").getPath()
        ARROW_HEAD = GlobalCyArtFileMgr.getInterfaceArtInfo("ARROW_HEAD").getPath()
        ...
        for tech in xrange(gc.getNumTechInfos()):
            ...
            for index in xrange(len(self.TechEffects[tech])):
                ...
                if type == "Civic":
                    ...
                    if GlobalCyGame.isOption(GameOptionTypes.GAMEOPTION_PICK_RELIGION):
                        szButton = GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_POPUPBUTTON_RELIGION").getPath()
                    else:
                    ...
                    if self.bBuilt and not team.isHasTech(tech):
                        ...
                        if iLimit > -1 and GlobalCyGame.getUnitClassCreatedCount(iClass) >= iLimit:
                            continue
                    ...
                    if self.bBuilt and not team.isHasTech(tech):
                        ...
                        if iLimit > -1 and GlobalCyGame.getBuildingClassCreatedCount(iClass) >= iLimit:
                            continue
                    ...
                    screen.addDDSGFCAt(szObsolete, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_RED_X").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_OBSOLETE, item, -1,
                                       False)
                    ...
                    screen.addDDSGFCAt(szObsolete, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_RED_X").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_OBSOLETE_SPECIAL,
                                       item, -1, False)

                elif type == "Project":
                    if self.bBuilt and not team.isHasTech(tech):
                        iLimit = gc.getProjectInfo(item).getMaxGlobalInstances()
                        if iLimit > -1 and GlobalCyGame.getProjectCreatedCount(item) >= iLimit:
                            continue
                    ...
                    if self.bBuilt and not team.isHasTech(tech):
                        if GlobalCyGame.isCorporationFounded(item):
                            continue
                    ...
                    screen.addDDSGFCAt(szObsolete, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_RED_X").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_OBSOLETE_BONUS, item,
                                       -1, False)

                elif type == "ResourceTrade":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_GOLDTRADING").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_BONUS_PLAYER_TRADE,
                                       tech, -1, False)

                elif type == "EnableIrrigation":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_IRRIGATION").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_IRRIGATION, tech, -1,
                                       False)

                elif type == "IgnoreIrrigation":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_NOIRRIGATION").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_IGNORE_IRRIGATION,
                                       tech, -1, False)

                elif type == "WorkerSpeed":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_WORKER_SPEED").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_WORKER_RATE, tech,
                                       -1, False)

                elif type == "FeatureProduction":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_FEATURE_PRODUCTION").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE,
                                       WidgetTypes.WIDGET_HELP_FEATURE_PRODUCTION, tech, -1, False)

                elif type == "RouteMovement":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_MOVE_BONUS").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_MOVE_BONUS, tech, -1,
                                       False)

                elif type == "BridgeBuilding":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_BRIDGEBUILDING").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_BUILD_BRIDGE, tech,
                                       -1, False)

                elif type == "RiverTrade":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_RIVERTRADE").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_TERRAIN_TRADE, tech,
                                       gc.getNumTerrainInfos(), False)

                elif type == "WaterTrade":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_WATERTRADE").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_TERRAIN_TRADE, tech,
                                       item, False)

                elif type == "WaterWork":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_WATERWORK").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_WATER_WORK, tech, -1,
                                       False)

                elif type == "WaterSight":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_LOS").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_LOS_BONUS, tech, -1, False)

                elif type == "WaterMoves":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_WATERMOVES").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_DOMAIN_EXTRA_MOVES,
                                       tech, item, False)

                elif type == "MapCenter":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_MAPCENTER").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_MAP_CENTER, tech, -1,
                                       False)

                elif type == "MapReveal":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_MAPREVEAL").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_MAP_REVEAL, tech, -1,
                                       False)

                elif type == "MapTrading":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_MAPTRADING").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_MAP_TRADE, tech, -1,
                                       False)

                elif type == "WealthTrading":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_GOLDTRADING").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_GOLD_TRADE, tech, -1,
                                       False)

                elif type == "TechTrading":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_TECHTRADING").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_TECH_TRADE, tech, -1,
                                       False)

                elif type == "TradeRoute":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_TRADE_ROUTES").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_TRADE_ROUTES, tech,
                                       -1, False)

                elif type == "OpenBorders":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_OPENBORDERS").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_OPEN_BORDERS, tech,
                                       -1, False)

                elif type == "DefensivePact":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_DEFENSIVEPACT").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_DEFENSIVE_PACT,
                                       tech, -1, False)

                elif type == "PermanentAlliance":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_PERMALLIANCE").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_PERMANENT_ALLIANCE,
                                       tech, -1, False)

                elif type == "VassalStates":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_VASSAL").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_VASSAL_STATE, tech, -1,
                                       False)

                elif type == "Happiness":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_HAPPINESS").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_HAPPINESS_RATE, tech,
                                       -1, False)

                elif type == "Health":
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_HEALTH").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_HEALTH_RATE, tech, -1,
                                       False)

                elif type == "CommerceAdjust":
                    szFileName = GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_GENERAL_QUESTIONMARK").getPath()
                    if item in [CommerceTypes.COMMERCE_GOLD, CommerceTypes.COMMERCE_RESEARCH, CommerceTypes.COMMERCE_CULTURE]:
                        ...
                        szFileName = GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_ESPIONAGE").getPath()
                    ...
                    screen.addDDSGFCAt(szItem, szTechBox, GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_FREETECH").getPath(), iX + fX, iY + self.Y_ITEMS, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_HELP_FREE_TECH, tech, -1,
                                       False)

                elif type == "FreeUnit":
                    if gc.getUnitInfo(item).getEspionagePoints() == 0 or not GlobalCyGame.isOption(GameOptionTypes.GAMEOPTION_NO_ESPIONAGE):
            ...
            for j in xrange(gc.getNUM_AND_TECH_PREREQS()):
                ...
                if yDiff == 0:
                    ...
                    if yDiff == -6:
                        ...
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_Y, iX + (self.getWidth(xDiff) / 2), iY + self.getYStart(1) + 8 - self.getHeight(yDiff, 0), 8, self.getHeight(yDiff, 0) - 8, WidgetTypes.WIDGET_GENERAL,
                                           -1, -1, False)
                        ...
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_X, iX + 8 + (self.getWidth(xDiff) / 2), iY + self.getYStart(1) - self.getHeight(yDiff, 0), (self.getWidth(xDiff) / 2) - 8, 8, WidgetTypes.WIDGET_GENERAL,
                                           -1, -1, False)
                        ...
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_Y, iX + (self.getWidth(xDiff) * 5 / 6), iY + self.getYStart(2) + 8 - self.getHeight(yDiff, 3), 8, self.getHeight(yDiff, 3) - 8,
                        ...
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_X, iX + 8 + (self.getWidth(xDiff) * 5 / 6), iY + self.getYStart(2) - self.getHeight(yDiff, 3), (self.getWidth(xDiff) / 6) - 8, 8,
                                           WidgetTypes.WIDGET_GENERAL, -1, -1, False)
                        ...
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_Y, iX + (self.getWidth(xDiff) / 2), iY + self.getYStart(2) + 8 - self.getHeight(yDiff, 3), 8, self.getHeight(yDiff, 3) - 8, WidgetTypes.WIDGET_GENERAL,
                                           -1, -1, False)
                        ...
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_X, iX + 8 + (self.getWidth(xDiff) / 2), iY + self.getYStart(2) - self.getHeight(yDiff, 3), (self.getWidth(xDiff) / 2) - 8, 8, WidgetTypes.WIDGET_GENERAL,
                                           -1, -1, False)
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_HEAD, iX + self.getWidth(xDiff), iY + self.getYStart(2) - self.getHeight(yDiff, 3), 8, 8, WidgetTypes.WIDGET_GENERAL, -1, -1, False)
                    ...
                    if yDiff == 2 and xDiff == 2:
                        ...
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_Y, iX + self.BOX_WIDTH + (self.getWidth(xDiff) / 6), iY + self.getYStart(4) + 8, 8, self.getHeight(yDiff, 3) - 8, WidgetTypes.WIDGET_GENERAL, -1, -1,
                                           False)
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_MXY, iX + self.BOX_WIDTH + (self.getWidth(xDiff) / 6), iY + self.getYStart(4) + self.getHeight(yDiff, 3), 8, 8, WidgetTypes.WIDGET_GENERAL, -1, -1,
                                           False)
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_X, iX + 8 + self.BOX_WIDTH + (self.getWidth(xDiff) / 6), iY + self.getYStart(4) + self.getHeight(yDiff, 3), (self.getWidth(xDiff) * 5 / 6) - 8, 8,
                                           WidgetTypes.WIDGET_GENERAL, -1, -1, False)
                        ...
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_X, iX + 8 + (self.getWidth(xDiff) / 3), iY + self.getYStart(5) + self.getHeight(yDiff, 0), (self.getWidth(xDiff) * 2 / 3) - 8, 8,
                                           WidgetTypes.WIDGET_GENERAL, -1, -1, False)
                        ...
                        screen.addDDSGFCAt(self.getNextWidgetName(), "TechList", ARROW_X, iX + 8 + (self.getWidth(xDiff) / 2), iY + self.getYStart(4) + self.getHeight(yDiff, 3), (self.getWidth(xDiff) / 2) - 8, 8, WidgetTypes.WIDGET_GENERAL,
                                           -1, -1, False)
    ...
    def updateTechs(self, bForce):
        ...
        for tech in ChangedList:
            ...
            # Tech Box Colours
            szTechBox = "TechBox" + str(tech)
            if gcgetTeam(player.getTeam()).isHasTech(tech):
    ...
    def updateSelectedTech(self):
        ''
        player = gcgetPlayer(GlobalCyGame.getActivePlayer())
        screen = self.getScreen()

        szName = ""
        iCost = 0
        if self.iSelectedTech > -1:
            iCost = gcgetPlayer(GlobalCyGame.getActivePlayer()).getAdvancedStartTechCost(self.iSelectedTech, True)
    ...
    def placeGreatPeople(self):
        ''
        if GlobalCyGame.isInAdvancedStart():
            return
        ...
        for iUnit in self.GreatPeopleList:
            ...
            lTechs = [i for i in xrange(gc.getNumTechInfos()) if gcgetPlayer(self.iPlayer).canResearch(i, False)]
            iFirstDiscovery = utils.getHighestEntry(lTechs, flavor)

            iSecondDiscovery = None
            if iFirstDiscovery:
                lTechsGiven = [i for i in xrange(gc.getNumTechInfos()) if gcgetPlayer(self.iPlayer).canResearchGiven(i, False, iFirstDiscovery) and i != iFirstDiscovery]
            ...
            for i, iTech in enumerate([iFirstDiscovery, iSecondDiscovery]):
                if iTech:
                    ...
                    screen.addDDSGFCAt("GreatPeoplePrereq" + str(iUnit) + str(i), "TechBottomPanel", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_CANCEL").getPath(), iGPX, 16, self.GP_ICON_SIZE, self.GP_ICON_SIZE,
                                       WidgetTypes.WIDGET_GENERAL, -1, -1, False)
                iGPX += self.GP_ICON_SIZE
    ...
    def onClose(self):
        player = gcgetPlayer(self.iPlayer)
        if player.getAdvancedStartPoints() > -1:
            GlobalCyInterface.setDirty(InterfaceDirtyBits.Advanced_Start_DIRTY_BIT, True)
        return 0
    ...
    def update(self, fDelta):
        if GlobalCyInterface.isDirty(InterfaceDirtyBits.Advanced_Start_DIRTY_BIT):
            ...
            if gcgetPlayer(self.iPlayer).getAdvancedStartPoints() < 0:
                screen = self.getScreen()
                screen.hide("AddTechButton")
                screen.hide("AddTechCost")
