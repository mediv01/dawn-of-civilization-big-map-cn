...
class CvReligionScreen:
    ...
    def interfaceScreen(self):
        ...
        # Header...
        screen.setLabel(self.HEADER_NAME, "Background", u"<font=4b>" + localText.getText("TXT_KEY_RELIGION_SCREEN_TITLE", ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.Y_TITLE, self.Z_TEXT, FontTypes.TITLE_FONT,
        ...
        if AdvisorOpt.isReligious():
            ...
            if AdvisorOpt.isShowAllReligions():
                ...
                self.RELIGIONS = ReligionUtil.getPlayerReligions(gcgetPlayer(self.iActivePlayer))
        else:
            self.X_RELIGION_AREA = 45
            self.Y_RELIGION_AREA = 84
            self.W_RELIGION_AREA = 934
    ...
    def drawReligionInfo(self):
        ...
        screen.addCheckBoxGFCAt(szArea, szButtonName, self.getPaganismButtonArt(self.iActivePlayer), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), self.X_SCROLLABLE_RELIGION_AREA + xLoop - 25,
                                self.Y_SCROLLABLE_RELIGION_AREA + 5, self.BUTTON_SIZE, self.BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL, False)
        ...
        for iRel in self.RELIGIONS:
            ...
            if gcgame.getReligionGameTurnFounded(iRel) >= 0:
                screen.addCheckBoxGFCAt(szArea, szButtonName, gc.getReligionInfo(iRel).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), self.X_SCROLLABLE_RELIGION_AREA + xLoop - 25,
                                        self.Y_SCROLLABLE_RELIGION_AREA + 5, self.BUTTON_SIZE, self.BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL, False)
            else:
                screen.setImageButtonAt(szButtonName, szArea, gc.getReligionInfo(iRel).getButtonDisabled(), self.X_SCROLLABLE_RELIGION_AREA + xLoop - 25, self.Y_SCROLLABLE_RELIGION_AREA + 5, self.BUTTON_SIZE, self.BUTTON_SIZE,
                                        WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def drawHelpInfo(self):
        ...
        for iRel in self.RELIGIONS:
            if (gcgame.getReligionGameTurnFounded(iRel) >= 0):
                szFounded = GlobalCyGameTextMgr.getTimeStr(gcgame.getReligionGameTurnFounded(iRel), False)
        ...
        for iRel in self.RELIGIONS:
            if (gcgame.getReligionGameTurnFounded(iRel) >= 0):
                pHolyCity = gcgame.getHolyCity(iRel)
                if pHolyCity.isNone():
                ...
                elif not pHolyCity.isRevealed(gcgetPlayer(self.iActivePlayer).getTeam(), False):
                    szFounded = localText.getText("TXT_KEY_UNKNOWN", ())
                    screen.setLabelAt("", szArea, szFounded, CvUtil.FONT_CENTER_JUSTIFY, xLoop, self.Y_HOLY_CITY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
                else:
                    szFounded = pHolyCity.getName()
                    screen.setLabelAt("", szArea, u"(%s)" % gcgetPlayer(pHolyCity.getOwner()).getCivilizationAdjective(0), CvUtil.FONT_CENTER_JUSTIFY, xLoop, self.Y_HOLY_CITY + 8, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL,
                                      -1, -1)
        ...
        for iRel in self.RELIGIONS:
            if (gcgame.getReligionGameTurnFounded(iRel) >= 0):
                szFounded = str(gcgame.calculateReligionPercent(iRel)) + "%"
        ...
        self.iReligionSelected = gcgetPlayer(self.iActivePlayer).getStateReligion()
    ...
    def drawCityInfo(self, iReligion):
        ...
        if AdvisorOpt.isReligious():
            ...
            screen.addPanel(self.AREA1_ID, "", "", True, True, self.X_RELIGION_AREA, self.Y_RELIGION_AREA + self.H_RELIGION_AREA + self.H_SCROLL_OFFSET + 3, self.W_RELIGION_AREA, self.H_CITY_AREA - self.H_SCROLL_OFFSET + 20,
        ...
        if AdvisorOpt.isReligious():
            ...
            screen.addTableControlGFC(self.TABLE_ID, self.TABLE_COLUMNS, self.X_RELIGION_AREA + 15, self.Y_RELIGION_AREA + self.H_RELIGION_AREA + self.H_SCROLL_OFFSET + 3 + 15, self.W_RELIGION_AREA - 2 * 15,
            ...
            for iCity in range(len(cityList)):
                ...
                if (iLinkReligion == -1):
                    ...
                    for iI in range(len(lReligions)):
                        szTempBuffer = GlobalCyGameTextMgr.getReligionHelpCity(lReligions[iI], pLoopCity.GetCy(), False, False, False, True)
                        if (szTempBuffer):
                            if (not bFirst):
                                sHelp += u", "
                            sHelp += szTempBuffer
                    ...
                    sHelp = GlobalCyGameTextMgr.getReligionHelpCity(iLinkReligion, pLoopCity.GetCy(), False, False, True, False)
        ...
        # start of BUG indent of original code
            ...
            for i in range(len(cityList)):
                ...
                if pLoopCity.isCapital():
                    szCityName += u"%c" % GlobalCyGame.getSymbolID(FontSymbols.STAR_CHAR)
                ...
                if (iLinkReligion == -1):
                    ...
                    szCityName += GlobalCyGameTextMgr.getReligionHelpCity(iLinkReligion, pLoopCity.GetCy(), False, False, True, False)
        ...
        # Convert Button....
        iLink = 0
        if (gcgetPlayer(self.iActivePlayer).canChangeReligion()):
            iLink = 1
        ...
        if (not self.canConvert(iLinkReligion) or iLinkReligion == self.iReligionOriginal):
            ...
            szAnarchyTime = GlobalCyGameTextMgr.setConvertHelp(self.iActivePlayer, iLinkReligion)
        else:
            ...
            szAnarchyTime = localText.getText("TXT_KEY_ANARCHY_TURNS", (gcgetPlayer(self.iActivePlayer).getReligionAnarchyLength(),))
    ...
    def canConvert(self, iReligion):
        iCurrentReligion = gcgetPlayer(self.iActivePlayer).getStateReligion()
        if (iReligion == gc.getNumReligionInfos()):
            iConvertReligion = -1
        else:
            iConvertReligion = iReligion

        return (iConvertReligion != iCurrentReligion and gcgetPlayer(self.iActivePlayer).canConvert(iConvertReligion))
    ...
    def handleInput(self, inputClass):
        ...
        # BUG Zoom to City
            ...
            if (inputClass.getMouseX() == 0):
                ...
                GlobalCyInterface.selectCity(pCity, True);
    ...
    def ReligionScreenButton(self, inputClass):
        if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED):
            if (inputClass.getID() == gc.getNumReligionInfos() or gcgame.getReligionGameTurnFounded(inputClass.getID()) >= 0):
    ...
    def getPaganismName(self, iPlayer):
        if not gcgetPlayer(iPlayer).isStateReligion():
            return localText.getText("TXT_KEY_RELIGION_SCREEN_NO_STATE", ())

        sName = gc.getCivilizationInfo(gcgetPlayer(iPlayer).getCivilizationType()).getPaganReligionName(0)
    ...
    def getPaganismButtonArt(self, iPlayer):
        ...
        sButton = gc.getCivilizationInfo(gcgetPlayer(iPlayer).getCivilizationType()).getPaganReligionButton()
