...
class CvCorporationScreen:
    ...
    def drawCorporationInfo(self):
        ...
        for i in range(gc.getNumCorporationInfos()):
            ...
            for iUnit in range(gc.getNumUnitInfos()):
                if gc.getUnitInfo(iUnit).getBuildings(iBuilding) or gc.getUnitInfo(iUnit).getForceBuildings(iBuilding):
                    szGreatPerson = gc.getUnitInfo(iUnit).getDescription()
                    break
            screen.addCheckBoxGFCAt(szArea, szButtonName, gc.getCorporationInfo(i).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), self.X_SCROLLABLE_CORPORATION_AREA + xLoop - 25,
                                    self.Y_SCROLLABLE_CORPORATION_AREA + 5, self.BUTTON_SIZE, self.BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL, False)
            ...
            iNum = 0
            szList = u""
            for iRequired in range(NUM_CORPORATION_PREREQ_BONUSES):
        ...
        # xLoop = self.X_CORPORATION_START
        # for i in range(gc.getNumCorporationInfos()):
        # if (gcgame.getCorporationGameTurnFounded(i) < 0):
        # szFounded = localText.getText("TXT_KEY_RELIGION_SCREEN_NOT_FOUNDED", ())
        # else:
        # szFounded = GlobalCyGameTextMgr.getTimeStr(gcgame.getCorporationGameTurnFounded(i), False)
        ...
        # xLoop = self.X_CORPORATION_START
        # for i in range(gc.getNumCorporationInfos()):
        #	pHeadquarters = gcgame.getHeadquarters(i)
        ...
        #	elif not pHeadquarters.isRevealed(gcgetPlayer(self.iActivePlayer).getTeam(), False):
        #		szFounded = localText.getText("TXT_KEY_UNKNOWN", ())
        #		screen.setLabelAt("", szArea, szFounded, CvUtil.FONT_CENTER_JUSTIFY, xLoop, self.Y_HEADQUARTERS, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        #	else:
        #		szFounded = pHeadquarters.getName()
        #		screen.setLabelAt("", szArea, "(%s)" % gcgetPlayer(pHeadquarters.getOwner()).getCivilizationAdjective(0), CvUtil.FONT_CENTER_JUSTIFY, xLoop, self.Y_HEADQUARTERS+8, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def drawCityInfo(self, iCorporation):
        ...
        for i in range(len(cityList)):
            ...
            if (iLinkCorporation == -1):
                bFirst = True
                for iI in range(len(lCorporations)):
                    szTempBuffer = GlobalCyGameTextMgr.getCorporationHelpCity(lCorporations[iI], pLoopCity.GetCy(), False, False)
                    if (szTempBuffer):
                        if (not bFirst):
                            szCityName += u", "
                ...
                szCityName += GlobalCyGameTextMgr.getCorporationHelpCity(iLinkCorporation, pLoopCity.GetCy(), False, True)
        ...
        if self.iCorporationExamined != -1:
            screen.setLabel(self.HEADER_NAME, "Background", u"<font=4b>" + gc.getCorporationInfo(self.iCorporationExamined).getDescription().upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.Y_TITLE, self.Z_TEXT,
                            FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
        else:
            screen.setLabel(self.HEADER_NAME, "Background", u"<font=4b>" + localText.getText("TXT_KEY_CORPORATION_SCREEN_TITLE", ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.Y_TITLE, self.Z_TEXT,
                            FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def drawRequirements(self, iCorporation):
        ...
        pActivePlayer = gcgetPlayer(GlobalCyGame.getActivePlayer())
        iActivePlayer = GlobalCyGame.getActivePlayer()
        teamCiv = gcgetTeam(pActivePlayer.getTeam())
        ...
        if (iLinkCorporation != -1):
            ...
            screen.addDDSGFC(szButtonName, gc.getTechInfo(iRequiredTech).getButton(), self.X_REQUIREMENTS_AREA + 15, self.Y_REQUIREMENTS_AREA + 10, self.BUTTON_SIZE, self.BUTTON_SIZE, WidgetTypes.WIDGET_TECH_TREE,
                             gc.getCorporationInfo(iLinkCorporation).getTechPrereq(), -1)

            szList = u""
            if teamCiv.isHasTech(iRequiredTech):
                szList += u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.SUCCESS_CHAR))
            else:
                szList += u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.FAILURE_CHAR))
            screen.setLabel("", szRequirementsArea, szList, CvUtil.FONT_CENTER_JUSTIFY, self.X_REQUIREMENTS_AREA + self.BUTTON_SIZE + 20, self.Y_REQUIREMENTS_AREA + self.BUTTON_SIZE - 8, self.DZ, FontTypes.SMALL_FONT,
                            WidgetTypes.WIDGET_GENERAL, -1, -1)

            if (iLinkCorporation > 1):
                szButtonName = self.TECH_REQUIRED_BUTTON_CORPORATION
                screen.addDDSGFC(szButtonName, gc.getTechInfo(iEconomicsTech).getButton(), self.X_REQUIREMENTS_AREA + 20 + self.BUTTON_SIZE + 15, self.Y_REQUIREMENTS_AREA + 10, self.BUTTON_SIZE, self.BUTTON_SIZE,
                                 WidgetTypes.WIDGET_TECH_TREE, iEconomicsTech, -1)
                ...
                if teamCiv.isHasTech(iEconomicsTech):
                    ...
                    szList += u"%c" % (GlobalCyGame.getSymbolID(FontSymbols.FAILURE_CHAR))
                screen.setLabel("", szRequirementsArea, szList, CvUtil.FONT_CENTER_JUSTIFY, self.X_REQUIREMENTS_AREA + self.BUTTON_SIZE + self.BUTTON_SIZE + 40, self.Y_REQUIREMENTS_AREA + self.BUTTON_SIZE - 8, self.DZ, FontTypes.SMALL_FONT,
                                WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def drawInfluences(self, iCorporation):
        ...
        if (iLinkCorporation != -1):
            screen.addMultilineText("Child" + self.INFLUENCES_ID, localText.getText("TXT_KEY_CORPORATION_INFLUENCES_" + str(iLinkCorporation), ()), self.X_INFLUENCES_AREA + 10, self.Y_INFLUENCES_AREA + 10, self.W_INFLUENCES_AREA - 20,
                                    self.H_INFLUENCES_AREA - 20, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
