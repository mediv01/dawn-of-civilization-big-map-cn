...
class CvDawnOfMan:
    ...
    def interfaceScreen(self):
        ...
        self.player = gcgetPlayer(gcgame.getActivePlayer())
        self.EXIT_TEXT = localText.getText("TXT_KEY_SCREEN_CONTINUE", ())

        # Create screen

        screen = CyGInterfaceScreen("CvDawnOfMan", self.iScreenID)
        screen.showScreen(PopupStates.POPUPSTATE_IMMEDIATE, False)  # wunshare 2022.03.12
        ...
        ##
        ##		self.Text_BoxText = GlobalCyGameTextMgr.parseLeaderTraits(self.player.getLeaderType(), self.player.getCivilizationType(), True, False)
        ##		self.Text_BoxText += "\n" + GlobalCyGameTextMgr.parseCivInfos(self.player.getCivilizationType(), True)
        ##
        ...
        if utils.getTurnForYear(tBirth[iActivePlayer]) <= utils.getScenarioStartTurn():
        ...
        year = str(abs(iYear)) + ' '
        if iYear >= 0:
            year += GlobalCyTranslator.getText("TXT_KEY_AD", ())
        else:
            year += GlobalCyTranslator.getText("TXT_KEY_BC", ())
        ...
        ##Rhye - end
        screen.addMultilineText("BodyText", bodyString, self.X_TEXT_PANEL + self.iMarginSpace, self.Y_TEXT_PANEL + self.iMarginSpace + self.iTEXT_PANEL_MARGIN, self.W_TEXT_PANEL - (self.iMarginSpace * 2),
                                self.H_TEXT_PANEL - (self.iMarginSpace * 2) - 75, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
    ...
    def update(self, fDelta):
        ...
        if tBirth[iActivePlayer] <= utils.getScenarioStartYear():
            ...
            screen.setLabel("Text", "", GlobalCyTranslator.getText("TXT_KEY_AUTOPLAY_TURNS_REMAINING", (0,)), CvUtil.FONT_CENTER_JUSTIFY, 530, 445, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
            screen.show("Exit")  # Rhye
        else:
            iGameTurn = GlobalCyGame.getGameTurn()

            iNumAutoPlayTurns = utils.getTurnForYear(tBirth[GlobalCyGame.getActiveTeam()])
            ...
            screen.setLabel("Text", "", GlobalCyTranslator.getText("TXT_KEY_AUTOPLAY_TURNS_REMAINING", (max(0, iNumTurnsRemaining),)), CvUtil.FONT_CENTER_JUSTIFY, 530, 445, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def onClose(self):
        # GlobalCyInterface.DoSoundtrack("AS2D_R_F_C") #Rhye
