...
class CvCustomizableDomesticAdvisor:
    ...
    def createDictionaries(self):
        ...
        self.angryIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.ANGRY_POP_CHAR)
        ...
        self.defenseIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.DEFENSE_CHAR)
        self.espionageIcon = u"%c" % (gc.getCommerceInfo(CommerceTypes.COMMERCE_ESPIONAGE).getChar())
        self.fistIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.OCCUPATION_CHAR)
        self.foodIcon = u"%c" % (gc.getYieldInfo(YieldTypes.YIELD_FOOD).getChar())
        self.footIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.MOVES_CHAR)
        self.goldIcon = u"%c" % (gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar())
        self.redGoldIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.BAD_GOLD_CHAR)
        self.figureheadIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR)
        self.hammerIcon = u"%c" % (gc.getYieldInfo(YieldTypes.YIELD_PRODUCTION).getChar())
        self.happyIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.HAPPY_CHAR)
        self.healthIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.HEALTHY_CHAR)
        self.lawIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.DEFENSIVE_PACT_CHAR)
        self.militaryIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.STRENGTH_CHAR)
        self.powerIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.POWER_CHAR)
        self.redfoodIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.BAD_FOOD_CHAR)
        self.religionIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.RELIGION_CHAR)
        self.researchIcon = u"%c" % (gc.getCommerceInfo(CommerceTypes.COMMERCE_RESEARCH).getChar())
        self.sickIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.UNHEALTHY_CHAR)
        self.tradeIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.TRADE_CHAR)
        self.unhappyIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.UNHAPPY_CHAR)
        ...
        self.starIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.STAR_CHAR)
        self.silverStarIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.SILVER_STAR_CHAR)
        self.bulletIcon = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)
        ...
        self.corpMaintPercent = gc.getWorldInfo(gcmap.getWorldSize()).getCorporationMaintenancePercent()
    ...
    def createPositions(self, screen):
        ...
        nExitTextWidth = GlobalCyInterface.determineWidth(localText.getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper())
    ...
    def getCurrentCity(self):
        ...
        iPlayer = PyPlayer(GlobalCyGame.getActivePlayer())
    ...
    def drawBasicScreen(self):
        ...
        # Text Buttons
        screen.setText(self.EXIT_NAME, "Background", localText.getText("TXT_KEY_PEDIA_SCREEN_EXIT", ()).upper(), CvUtil.FONT_RIGHT_JUSTIFY, self.X_EXIT, self.Y_EXIT, self.Z_TEXT, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_CLOSE_SCREEN, -1,
                       -1)
        ...
        screen.addCheckBoxGFC(self.START_CUSTOMIZING_NAME, ArtFileMgr.getInterfaceArtInfo("INTERFACE_BTN_FOREIGN").getPath(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), x, self.Y_SPECIAL, self.nControlSize,
                              self.nControlSize, WidgetTypes.WIDGET_CDA_EDIT_PAGE, -1, -1, ButtonStyles.BUTTON_STYLE_IMAGE)
        x += self.nControlSize + 2
        ...
        screen.addCheckBoxGFC(self.TOGGLE_SPECS_NAME, info.getTexture(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), x, self.Y_SPECIAL, self.nControlSize, self.nControlSize, WidgetTypes.WIDGET_CDA_TOGGLE_SPECIALISTS,
                              -1, -1, ButtonStyles.BUTTON_STYLE_IMAGE)
        x += self.nControlSize + 2
        ...
        for i in range(gc.getNumCultureLevelInfos()):
            ...
            if (nValue != 0):
                # Set text
                screen.setText(self.CULTURE_TEXT_NAME + str(i), "Background", "<font=2>" + pCultureLevel.getText() + "</font>", CvUtil.FONT_LEFT_JUSTIFY, self.nCultureLevelX, self.nCultureLevelY + (self.nCultureLevelDistance * iCount),
                               self.Z_TEXT, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
                # Set value
                screen.setText(self.CULTURE_TEXT_NAME + self.NUMBER_TEXT + str(i), "Background", "<font=2>" + str(nValue) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.nCultureLevelX + self.nCultureLevelTextOffset,
                               self.nCultureLevelY + (self.nCultureLevelDistance * iCount), self.Z_TEXT, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
                # Increment counter
                iCount += 1
        ...
        screen.setText(self.GP_TEXT_NAME + self.NUMBER_TEXT, "Background", "<font=2>" + str(iPlayer.player.greatPeopleThreshold(False)) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.nGPLevelX, self.nGPLevelY + self.nGPLevelDistance,
                       self.Z_TEXT, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
    ...
    def drawSpecialists(self):
        ...
        for i in range(self.getNumSpecialistInfos()):
            ...
            if (gc.getSpecialistInfo(i).isVisible()):
                ...
                screen.setImageButton(szName, gc.getSpecialistInfo(i).getTexture(), self.nFirstSpecialistX + (self.nSpecialistDistance * i), self.nSpecialistY, self.nSpecialistWidth, self.nSpecialistLength, WidgetTypes.WIDGET_CITIZEN, i,
                                      -1)
                screen.hide(szName)
        ...
        for i in range(self.getNumSpecialistInfos()):
            if (gc.getSpecialistInfo(i).isVisible()):
                ...
                screen.setButtonGFC(szName, u"", "", self.nFirstSpecialistX + (self.nSpecialistDistance * i) + self.nPlusOffsetX, self.nSpecialistY + self.nPlusOffsetY, self.nPlusWidth, self.nPlusHeight,
                                    WidgetTypes.WIDGET_CHANGE_SPECIALIST, i, 1, ButtonStyles.BUTTON_STYLE_CITY_PLUS)
                screen.hide(szName)
        ...
        for i in range(self.getNumSpecialistInfos()):
            if (gc.getSpecialistInfo(i).isVisible()):
                ...
                screen.setButtonGFC(szName, u"", "", self.nFirstSpecialistX + (self.nSpecialistDistance * i) + self.nMinusOffsetX, self.nSpecialistY + self.nMinusOffsetY, self.nMinusWidth, self.nMinusHeight,
                                    WidgetTypes.WIDGET_CHANGE_SPECIALIST, i, -1, ButtonStyles.BUTTON_STYLE_CITY_MINUS)
                screen.hide(szName)
        ...
        for i in range(self.getNumSpecialistInfos()):
            if (gc.getSpecialistInfo(i).isVisible()):
                ...
                screen.setText(szName, "Background", "", CvUtil.FONT_LEFT_JUSTIFY, self.nFirstSpecialistX + (self.nSpecialistDistance * i) + self.nSpecTextOffsetX, self.nSpecialistY + self.nSpecTextOffsetY, self.Z_TEXT, FontTypes.GAME_FONT,
    ...
    def showSpecialists(self):
        ...
        if (city):
            ...
            for i in range(self.getNumSpecialistInfos()):
                ...
                szName = self.SPECIALIST_TEXT_NAME + str(i)
                screen.setText(szName, "Background", str(city.getSpecialistCount(i)) + "/" + str(city.getMaxSpecialistCount(i)) + u" %c" % self.SPECIALIST_ICON_DICT[i], CvUtil.FONT_LEFT_JUSTIFY,
                               self.nFirstSpecialistX + (self.nSpecialistDistance * i) + self.nSpecTextOffsetX, self.nSpecialistY + self.nSpecTextOffsetY, self.Z_TEXT, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
                screen.show(szName)
    ...
    def calculateFounded(self, city, szKey, arg):
        ...
        return unicode(GlobalCyGameTextMgr.getTimeStr(iTurnTime, False))
    ...
    def calculateFeatures(self, city, szKey, arg):
        ...
        if city.isDisorder():
            ...
            if city.isOccupation():
                szOccu = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.OCCUPATION_CHAR)
    ...
    def calculateThreats(self, city, szKey, arg):
        ...
        player = PyPlayer(GlobalCyGame.getActivePlayer())

        for i in range(gc.getMAX_PLAYERS()):
            if player.getTeam().isAtWar(gcgetPlayer(i).getTeam()):
                if city.isVisible(gcgetPlayer(i).getTeam(), False):
                    szReturn = self.angryIcon
                    break
    ...
    def calculateTotalCulture(self, city, szKey, arg):
        ...
        return city.getCulture(GlobalCyGame.getActivePlayer())

    def calculateCultureTurns(self, city, szKey, arg):

        iCultureTimes100 = city.getCultureTimes100(gcgame.getActivePlayer())
    ...
    def calculateGreatPeopleTurns(self, city, szKey, arg):
        ...
        if iGreatPersonRate > 0:
            iGPPLeft = gcgetPlayer(gcgame.getActivePlayer()).greatPeopleThreshold(False) - city.getGreatPeopleProgress()
    ...
    def calculateHasBonus(self, city, szKey, arg):
        ...
        # wunshare
        if city.hasBonusEffect(arg) or gcgetPlayer(city.getOwner()).getBonusAffectedCities(arg) == 0:  # gc.getBonusInfo(arg).getAffectedCities() == 0:
            return self.objectHave
    ...
    def canAdviseToConstruct(self, city, i):
        ...
        if info.getObsoleteTech() != TechTypes.NO_TECH and PyPlayer(GlobalCyGame.getActivePlayer()).getTeam().isHasTech(info.getObsoleteTech()):
            return False

        sinfo = gc.getSpecialBuildingInfo(info.getSpecialBuildingType())

        if sinfo:
            if sinfo.getObsoleteTech() != TechTypes.NO_TECH and PyPlayer(GlobalCyGame.getActivePlayer()).getTeam().isHasTech(sinfo.getObsoleteTech()):
                return False
        ...
        if (code == NotifyCode.NOTIFY_LISTBOX_ITEM_SELECTED):
            ...
            if (szWidget == self.PAGES_DD_NAME + "0"):
                ...
                if (inputClass.getMouseX() == 0):
                    ...
                    city = self.getCurrentCity()
                    if (city):
                        GlobalCyInterface.lookAtCityOffset(city.getID())

                    if self.PAGES[self.currentPageNum]["showSpecControls"]:
                        self.showSpecialists()

                    # And pass it back to the screen
                    self.updateAppropriateCitySelection(self.currentPage, len(PyPlayer(GlobalCyGame.getActivePlayer()).getCityList()))
    ...
    def update(self, fDelta):
        """ Update the Advisor."""
        if (GlobalCyInterface.isDirty(InterfaceDirtyBits.Domestic_Advisor_DIRTY_BIT) == True):
            GlobalCyInterface.setDirty(InterfaceDirtyBits.Domestic_Advisor_DIRTY_BIT, False)
