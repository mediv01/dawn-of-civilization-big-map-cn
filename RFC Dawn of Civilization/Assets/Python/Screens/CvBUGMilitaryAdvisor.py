...
class CvMilitaryAdvisor:
    ...
    def interfaceScreen(self):
        ...
        # Header...
        screen.setLabel(self.HEADER_ID, "Background", u"<font=4b>" + localText.getText("TXT_KEY_MILITARY_ADVISOR_TITLE", ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.Y_TITLE, self.Z_CONTROLS,
    ...
    def drawTabs(self):
        ...
        xLink += GlobalCyInterface.determineWidth(szText) + self.SPACING
    ...
    def showSituationReport(self):
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            pPlayer = gcgetPlayer(iLoopPlayer)

            if (pPlayer.isAlive()
                    and (gcgetTeam(pPlayer.getTeam()).isHasMet(gcgetPlayer(self.iActivePlayer).getTeam())
        ...
        for iLoopPlayer in range(gc.getMAX_PLAYERS()):
            ...
            if (pPlayer.isAlive()
                ...
                # show WHEOOH
                if bWHEOOH:
                    sWHEOOH = u" %c" % GlobalCyGame.getSymbolID(FontSymbols.OCCUPATION_CHAR)
                else:
                    sWHEOOH = ""
    ...
    def Grid_ThreatIndex(self, iRow, iPlayer, bCurrentWar, bWHEOOH):
        ...
        if gcgetTeam(pPlayer.getTeam()).isAVassal():
        ...
        if gcgetTeam(pPlayer.getTeam()).isDefensivePact(gcgetPlayer(self.iActivePlayer).getTeam()):
            fThreat = fThreat * 0.2
    ...
    def fillStratAdvGrid(self):
        ...
        for player in PlayerUtil.players(alive=True, barbarian=False, minor=False):
            ...
            if (ePlayer != self.iActivePlayer
                    and (activeTeam.isHasMet(player.getTeam()) or gcgame.isDebugMode())):
    ...
    def addStratAdvUnits(self, activePlayer, player, iRow):
        if (not gcgetTeam(activePlayer.getTeam()).isTechTrading()
                and not gcgetTeam(player.getTeam()).isTechTrading()):
            szButton = ArtFileMgr.getInterfaceArtInfo("QUESTION_MARK").getPath()
    ...
    def UL_setMinimapVisibility(self, screen, bVisibile):
        ...
        if bVisibile:
            ...
            GlobalCyInterface.setShowInterface(InterfaceVisibility.INTERFACE_HIDE)
        ...
        if bRedraw:
            ...
            for iIndex in range(iNumLeaders):
                ...
                if player.isBarbarian():
                    ...
                    szButton = gc.getLeaderHeadInfo(gcgetPlayer(iLoopPlayer).getLeaderType()).getButton()

                szLeaderButton = self.getLeaderButtonWidget(iLoopPlayer)  # self.getNextWidgetName()
                screen.addCheckBoxGFC(szLeaderButton, szButton, ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), x, y, iButtonSize, iButtonSize, WidgetTypes.WIDGET_MINIMAP_HIGHLIGHT, 2, iLoopPlayer,
    ...
    def drawCombatExperience(self):
        ...
        if (gcgetPlayer(self.iActivePlayer).greatPeopleThreshold(True) > 0):
            ...
            screen.setBarPercentage(szGGBar_ID, InfoBarTypes.INFOBAR_STORED, float(iExperience) / float(gcgetPlayer(self.iActivePlayer).greatPeopleThreshold(True)))

            screen.setLabel(szGGTxt_ID, "", localText.getText("TXT_KEY_MISC_COMBAT_EXPERIENCE", ()), CvUtil.FONT_CENTER_JUSTIFY, self.X_GREAT_GENERAL_BAR + self.W_GREAT_GENERAL_BAR / 2, self.Y_GREAT_GENERAL_BAR + 6, 0, FontTypes.GAME_FONT,
