...
class BugEventManager(CvEventManager.CvEventManager):
    ...
    def __init__(self, logging=None, noLogEvents=None):
        ...
        self.CustomEvents = {
            ...
            4568: ('看海模式', self.rnfEventApply4568, self.rnfEventBegin4568),
            4569: ('回合计算器', self.rnfEventApply4569, self.rnfEventBegin4569),
            4570: ('历史上的今日', self.rnfEventApply4570, self.rnfEventBegin4570),
            8001: ('波立联统', HistoryEvents.rnfEventApply8001, HistoryEvents.rnfEventBegin8001),
        }
    ...
    def onUnitCaptured(self, argsList):
        ...
        BugUtil.debug("%s %s captured as %s by %s",
                      gcgetPlayer(eOwner).getCivilizationShortDescription(0), gc.getUnitInfo(eUnitType).getDescription(),
                      pNewUnit.getName(), gcgetPlayer(pNewUnit.getOwner()).getCivilizationShortDescription(0))
    ...
    def onPlayerRevolution(self, argsList):
        ...
        BugUtil.debug("Revolution for %s, %d turns: %s", gcgetPlayer(ePlayer).getCivilizationShortDescription(0), iAnarchyTurns, ", ".join(civics))
    ...
    def rnfEventBegin4569(self):
        pass

    def rnfEventApply4569(self, playerID, netUserData, popupReturn):

        import GameTurnCalculator
        GameTurnCalculator.rnfEventApply4569(playerID, netUserData, popupReturn)
        pass

    def rnfEventBegin4570(self):
        pass

    def rnfEventApply4570(self, playerID, netUserData, popupReturn):

        import HistoryDay
        HistoryDay.rnfEventApply4570(playerID, netUserData, popupReturn)
        pass
