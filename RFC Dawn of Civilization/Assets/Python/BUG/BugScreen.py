...
class BugScreen:
    ...
    def evenlySpaceTabs(self):
        ...
        for tab in self.tabs:
            if tab.show:
                sparewidth -= GlobalCyInterface.determineWidth(tab.getLabel())
                num_gaps += 1
        #				BugUtil.debug("CvEspionage Advisor: evenlySpaceTabs %i", GlobalCyInterface.determineWidth(tab.getLabel()))
        ...
        for itab in range(len(self.tabs)):
            ...
            if self.tabs[itab].show:
                ...
                iStart += GlobalCyInterface.determineWidth(self.tabs[itab].getLabel())
                iStart += gapwidth
