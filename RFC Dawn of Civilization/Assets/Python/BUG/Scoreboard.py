...
def onDealCanceled(argsList):
    ...
    GlobalCyInterface.setDirty(InterfaceDirtyBits.Score_DIRTY_BIT, True)
...
class Column:
    ...
    def __init__(self, key, id, type=SKIP, text=None, alt=None):
        ...
        self.alt = alt
        if (type == FIXED):
            self.width = GlobalCyInterface.determineWidth(text)
        else:
            self.width = 0
        if (key):
...
class Scoreboard:
    ...
    def __init__(self):
        self._activePlayer = gcgame.getActivePlayer()
        self._teamScores = []
        self._playerScores = []
    ...
    def setResearch(self, tech, turns, iPlayer):
        ...
        '''
        if (PYTHON_SHOW_CIV_MONEY_ON_PANNEL > 0):
            iGold=gcgetPlayer(iPlayer).AI_maxGoldTrade(utils.getHumanID())
...
class PlayerScore:
    ...
    def isActive(self):
        return self.getID() == gcgame.getActivePlayer()
