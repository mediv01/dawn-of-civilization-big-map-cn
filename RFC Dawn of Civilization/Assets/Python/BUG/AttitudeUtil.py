...
def hasAttitude(nPlayer, nTarget):
    ...
    return (nPlayer != -1 and nTarget != -1 and nPlayer != nTarget
            and gcgetTeam(gcgetPlayer(nPlayer).getTeam()).isHasMet(gcgetPlayer(nTarget).getTeam()))
...
def getAttitudeString(nPlayer, nTarget):
    ...
    if hasAttitude(nPlayer, nTarget):
        return GlobalCyGameTextMgr.getAttitudeString(nPlayer, nTarget)
    return None
...
def getAttitudeCategory(nPlayer, nTarget):
    ...
    if hasAttitude(nPlayer, nTarget):
        return gcgetPlayer(nPlayer).AI_getAttitude(nTarget)
    return None
...
class Attitude:
    ...
    def __init__(self, iThisPlayer, iTargetPlayer):
        pActiveTeam = gcgetTeam(gc.getActivePlayer().getTeam())
        iThisTeam = gcgetPlayer(iThisPlayer).getTeam()
        pThisTeam = gcgetTeam(iThisTeam)
        iTargetTeam = gcgetPlayer(iTargetPlayer).getTeam()
        self.iThisPlayer = iThisPlayer
        self.iTargetPlayer = iTargetPlayer
        self.iAttitudeSum = 0
        self.iAttitudeModifiers = {}
        self.bHasActiveMetBoth = GlobalCyGame.isDebugMode() or (pActiveTeam.isHasMet(iThisTeam) and pActiveTeam.isHasMet(iTargetTeam))
        ...
        if (iThisPlayer != iTargetPlayer and pThisTeam.isHasMet(iTargetTeam)):
            self.eAttitudeType = gcgetPlayer(iThisPlayer).AI_getAttitude(iTargetPlayer)
            self.sAttitudeString = GlobalCyGameTextMgr.getAttitudeString(iThisPlayer, iTargetPlayer)
