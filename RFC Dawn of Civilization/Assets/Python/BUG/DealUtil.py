...
class Deal(object):
    ...
    def turnsToCancel(self, eByPlayer=-1):
        if BugDll.isPresent():
            ...
            return self.getInitialGameTurn() + gc.getDefineINT("PEACE_TREATY_LENGTH") - utils.getGameTurn()
...
def test():
    ...
    for player, deals in allDeals.iteritems():
        print
        PlayerUtil.getPlayer(player).getCivilizationShortDescription(0)
        for type, deal in deals.iteritems():
            print
            "%s: %r" % (TradeUtil.TRADE_FORMATS[type].name, deal)
