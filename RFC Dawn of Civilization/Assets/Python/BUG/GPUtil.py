...
def getUnitIcon(iUnit):
    ...
    try:
        ...
        return u"%c" % GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR)
...
def getDisplayCity():
    ...
    if (pHeadSelectedCity and pHeadSelectedCity.getTeam() == gcgame.getActiveTeam()):
...
def getCityTurns(city):
    if (city):
        player = gcgetPlayer(city.getOwner())
        iThreshold = player.greatPeopleThreshold(False)
        iRate = city.getGreatPeopleRate()
        if (iRate > 0):
            iProgress = city.getGreatPeopleProgress()
...
def getHoverText(eWidgetType, iData1, iData2, bOption):
    ...
    iThreshold = gcgetPlayer(city.getOwner()).greatPeopleThreshold(False)
    iProgress = city.getGreatPeopleProgress()
    iRate = city.getGreatPeopleRate()
...
def getGreatPeopleText(city, iGPTurns, iGPBarWidth, bGPBarTypesNone, bGPBarTypesOne, bIncludeCityName):
    sGreatPeopleChar = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR)
    if (not city):
        ...
        if (len(lPercents) == 0):
            ...
            if (bGPBarTypesOne or len(lPercents) == 1):
                ...
                for iPercent, iUnit in lPercents:
                    ...
                    if (GlobalCyInterface.determineWidth(szNewText) > iGPBarWidth - 10):
                        # Keep under width
                        break
                    szTypes = szNewTypes
                if (len(szTypes) > 0):
