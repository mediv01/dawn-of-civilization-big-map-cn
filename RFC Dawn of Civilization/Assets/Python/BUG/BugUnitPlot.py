...
class BupPanel:
    ...
    def Draw(self):
        ...
        if (self._hasPlotChanged()
                or GlobalCyInterface.isCityScreenUp() != self.CityUp_Prior):
            #			BugUtil.debug("BupPanel plot has changed or CityScreenUp status has changed - %s %s", GlobalCyInterface.isCityScreenUp(), self.CityUp_Prior)
            self.BupUnits_Prior = []
            self.PlotListOffset_Prior = 0
            self.PlotListColumn_Prior = 0
            self.Hide()
        ...
        self.PlotListOffset = GlobalCyInterface.getPlotListOffset()
        ...
        iIndex = -(GlobalCyInterface.getPlotListColumn())
        ...
        BugUtil.debug("BupPanel plotlistcolumn, current(%i), prior(%i)", GlobalCyInterface.getPlotListColumn(), self.PlotListColumn_Prior)
        ...
        for iUnit in range(iMaxUnits):
            iUnit_Prior = iUnit - (GlobalCyInterface.getPlotListColumn() - self.PlotListColumn_Prior)
            ...
            if iIndex == iFirstPlot:  # looking at the top left cell
                if GlobalCyInterface.getPlotListColumn() > 0:  # still have units to show
                    bLeftArrow = True
            elif iIndex == iLastPlot:  # the bottom right cell
                if (iVisibleUnits - iIndex - GlobalCyInterface.getPlotListColumn() + iSkippedCells) > 1:  # more units to show to right
                    bRightArrow = True
            ...
            if BupUnit.Owner == gcgame.getActivePlayer():
        ...
        # save prior stuff
        self.CityUp_Prior = GlobalCyInterface.isCityScreenUp()
        self.PlotListOffset_Prior = GlobalCyInterface.getPlotListOffset()
        self.PlotListColumn_Prior = GlobalCyInterface.getPlotListColumn()
    ...
    def _drawUnitIcon(self, BupUnit, iIndex, szCell):
        ...
        self.screen.enable(szCell, BupUnit.Owner == gcgame.getActivePlayer())
...
class BupUnit:
    def __init__(self, pUnit):
        ...
        self.isFortified = pUnit.getTeam() != gcgame.getActiveTeam() or pUnit.isWaiting()
