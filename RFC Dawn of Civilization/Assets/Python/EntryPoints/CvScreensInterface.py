...
def CheckStability(argsList):
    # tlist=RFCEMapShowInDll.SearchCore(x,y)
    x = argsList[0]  # player
    y = argsList[1]  # recalculate
    tList = DocMapShowInDll.CheckStability(x, y)
    return tList
...
def getUHVTech(argsList):
    # tlist=RFCEMapShowInDll.SearchCore(x,y)
    x = argsList[0]
    y = argsList[1]
    tList = DocMapShowInDll.getUHVTech(x, y)
    return tList

def getinitTech(argsList):
    # tlist=RFCEMapShowInDll.SearchCore(x,y)
    iTech = argsList[0]
    y = argsList[1]
    tList = DocMapShowInDll.getinitTech(iTech)
    return tList

def getUHVBuilding(argsList):
    # tlist=RFCEMapShowInDll.SearchCore(x,y)
    x = argsList[0]
    y = argsList[1]
    tList = DocMapShowInDll.getUHVBuilding(x, y)
    return tList


def getUHVTiles(argsList):
    # tlist=RFCEMapShowInDll.SearchCore(x,y)
    x = argsList[0]
    y = argsList[1]
    tList = DocMapShowInDll.getUHVTiles(x, y)
    return tList
...
def showTechChooser():
    if GlobalCyGame.getActivePlayer() > -1:
...
# if (-1 != GlobalCyGame.getActivePlayer()):
...
def showReplay(argsList):
    if argsList[0] > -1:
        GlobalCyGame.saveReplay(argsList[0])
...
def WorldBuilderDiplomacyModeCB():
    WBDiplomacyScreen.WBDiplomacyScreen().interfaceScreen(GlobalCyGame.getActivePlayer(), False)
...
def cityWarningOnClickedCallback(argsList):
    ...
    if (not city.isNone()):
        if (iButtonId == 0):
            ...
            GlobalCyInterface.selectCity(city, False)


def cityWarningOnFocusCallback(argsList):
    GlobalCyInterface.playGeneralSound("AS2D_ADVISOR_SUGGEST")
    GlobalCyInterface.lookAtCityOffset(argsList[0])
    return 0
...
def featAccomplishedOnFocusCallback(argsList):
    ...
    GlobalCyInterface.playGeneralSound("AS2D_FEAT_ACCOMPLISHED")
    if ((iData1 >= FeatTypes.FEAT_UNITCOMBAT_ARCHER) and (iData1 <= FeatTypes.FEAT_FOOD_CONNECTED)):
        GlobalCyInterface.lookAtCityOffset(iData2)
...
def isForeignCorePlot(argsList):
    ...
    for iCiv in range(iNumPlayers):
        if CyGlobalContext().getGame().getGameTurn() >= utils.getTurnForYear(tBirth[iCiv]):
            if (x, y) in Areas.getCoreArea(iCiv):
                iResult = 1
                break
    return iResult
...
def isNeighbor(argsList):
    ...
    if iNeighbor in lNeighbours[iPlayer]:
        return 1
    else:
        return 0
...
def getUHVTileInfo(argsList):
    ...
    if iPlayer == iGreece:
        ...
        if gcmap.plot(x, y).getRegionID() in lAfrica:
            return 33
        ...
        if utils.isPlotInArea((x, y), vic.tEuropeTL, vic.tEuropeBR):
            return 29
        elif utils.isPlotInArea((x, y), vic.tEasternEuropeTL, vic.tEasternEuropeBR):
            return 29
        ...
        if utils.isPlotInArea((x, y), vic.tPacific1TL, vic.tPacific1BR) or utils.isPlotInArea((x, y), vic.tPacific2TL, vic.tPacific2BR) or utils.isPlotInArea((x, y), vic.tPacific3TL, vic.tPacific3BR) or utils.isPlotInArea((x, y),
                                                                                                                                                                                                                              vic.tHawaiiTL,
                                                                                                                                                                                                                              vic.tHawaiiBR):
            return 76

    elif iPlayer == iMamluks:
        if utils.isPlotInArea((x, y), vic.tLowerNileTL, vic.tLowerNileBR) and gcmap.plot(x, y).isRiver():
            if utils.isPlotInArea((x, y), vic.tNorthAfricaTL, vic.tNorthAfricaBR):
                return 80
            return 79
        ...
        if utils.getSettlerValue((x,y), iManchuria) >= 90:
            return 81
        ...
        if gcmap.plot(x, y).getRegionID() == rAustralia:
            return 74
        ...
        if gcmap.plot(x, y).getRegionID() in [rIberia, rEurope, rItaly, rBalkans]:
            return 29
        ...
        if gcmap.plot(x, y).getRegionID() == rItaly:
            return 91

        if gcmap.plot(x, y).getRegionID() == rBritain:
            return 92

        if gcmap.plot(x, y).getRegionID() == rIberia:
            return 8
...
def getCityName(argsList):
    ...
    result = cnm.getFoundNameCN(iPlayer, (x, y))
