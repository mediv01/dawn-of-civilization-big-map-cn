...
import wunshare_test  # wunshare: for speed up
...
def pyPrint(stuff):
    stuff = convertToStr(stuff)
    stuff = 'PY:' + stuff + "\n"
    if (wunshare_test.CvUtilOutput):
...
def getScoreComponent(iRawScore, iInitial, iMax, iFactor, bExponential, bFinal, bVictory):
    if gcgame.getEstimateEndTurn() == 0:
        return 0
    from RFCUtils import utils
    if bFinal and bVictory:
        # Alexius08: Begin new score calculation formula

        iHumanCiv = gcgame.getActivePlayer()
        if gcgetPlayer(iHumanCiv).isReborn() and iHumanCiv in dRebirth:
            iHumanSpawnTurn = utils.getTurnForYear(dRebirth[iHumanCiv])  # Get spawn turn for reborn civs
        else:
            iHumanSpawnTurn = utils.getTurnForYear(tBirth[iHumanCiv])  # Get spawn turn for others
        fTurnRatio = float(utils.getGameTurn() - iHumanSpawnTurn) / float(gcgame.getEstimateEndTurn() - iHumanSpawnTurn)  # Compensate for player's delayed spawn
        # End new formula
    ...
    import GlobalDefine
    iFree = (GlobalDefine.SCORE_FREE_PERCENT* iMax) / 100
    if (iFree + iMax) != 0:
    ...
    if bFinal:
        iScore = ((100 + gc.getDefineINT("SCORE_HANDICAP_PERCENT_OFFSET") + (gcgame.getHandicapType() * gc.getDefineINT("SCORE_HANDICAP_PERCENT_PER"))) * iScore) / 100
...
def combatDetailMessageBuilder(cdUnit, ePlayer, iChange):
    if (cdUnit.iExtraCombatPercent != 0):
        ...
        GlobalCyInterface.addCombatMessage(ePlayer, msg)
...
def combatMessageBuilder(cdAttacker, cdDefender, iCombatOdds):
    ...
    if (cdAttacker.eOwner == cdAttacker.eVisualOwner):
        ...
        #		combatMessage += "%s's" %(gcgetPlayer(cdAttacker.eOwner).getName(),)
        combatMessage += "%s%s" % (gcgetPlayer(cdAttacker.eOwner).getName(), localText.getText("TXT_KEY_APOSTROPHE_S", ()),)
    # <<< CYBERFRONT
    ...
    if (cdDefender.eOwner == cdDefender.eVisualOwner):
        # >>> CYBERFRONT // message: combat
        #		combatMessage += "%s's" %(gcgetPlayer(cdDefender.eOwner).getName(),)
        combatMessage += "%s%s" % (gcgetPlayer(cdDefender.eOwner).getName(), localText.getText("TXT_KEY_APOSTROPHE_S", ()),)
    # <<< CYBERFRONT
...
def initDynamicFontIcons():
    ...
    for key in OtherFontIcons.keys():
        # print key
        FontIconMap[key] = (u"%c" % GlobalCyGame.getSymbolID(OtherFontIcons.get(key)))
