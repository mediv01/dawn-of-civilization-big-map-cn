...
class CvPediaCorporation:
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), CorporationInfo.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE,
    ...
    def placeEffects(self):
        ...
        if iWealthHQ > 0:
            szSpecialText += GlobalCyTranslator.getText('TXT_KEY_CORPORATION_HQ_WEALTH', (iWealthHQ, ""))
        iResearchHQ = gc.getCorporationInfo(self.iCorporation).getHeadquarterCommerce(1)
        if iResearchHQ > 0:
            szSpecialText += GlobalCyTranslator.getText('TXT_KEY_CORPORATION_HQ_RESEARCH', (iResearchHQ, ""))
        iCultureHQ = gc.getCorporationInfo(self.iCorporation).getHeadquarterCommerce(2)
        if iCultureHQ > 0:
            szSpecialText += GlobalCyTranslator.getText('TXT_KEY_CORPORATION_HQ_CULTURE', (iCultureHQ, ""))
        iEspionageHQ = gc.getCorporationInfo(self.iCorporation).getHeadquarterCommerce(3)
        if iEspionageHQ > 0:
            szSpecialText += GlobalCyTranslator.getText('TXT_KEY_CORPORATION_HQ_ESPIONAGE', (iEspionageHQ, ""))

        if szSpecialText: szSpecialText += "\n"
        szSpecialText += GlobalCyGameTextMgr.parseCorporationInfo(self.iCorporation, True)[1:]
    ...
    def placeResources(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_CONCEPT_RESOURCES", ()), "", False, True, self.X_RESOURCES, self.Y_RESOURCES, self.W_RESOURCES, self.H_RESOURCES, PanelStyles.PANEL_STYLE_BLUE50)
        screen.attachLabel(panel, "", "  ")
