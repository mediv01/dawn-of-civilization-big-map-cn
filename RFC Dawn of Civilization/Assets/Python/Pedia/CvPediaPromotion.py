...
class CvPediaPromotion:
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), PromotionInfo.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE,
    ...
    def placeEffects(self):
        ...
        szText = GlobalCyGameTextMgr.getPromotionHelp(self.iPromotion, True)[1:]
    ...
    def placeUnitTypes(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_PROMOTION_UNITS", ()), "", True, True, self.X_UNIT_TYPES, self.Y_UNIT_TYPES, self.W_UNIT_TYPES, self.H_UNIT_TYPES, PanelStyles.PANEL_STYLE_BLUE50)
        ...
        for iUnitCombat in xrange(gc.getNumUnitCombatInfos()):
            if gc.getPromotionInfo(self.iPromotion).getUnitCombat(iUnitCombat):
                ...
                screen.setTableText(table, 0, iRow, "<font=3>" + gc.getUnitCombatInfo(iUnitCombat).getDescription() + "</font>", gc.getUnitCombatInfo(iUnitCombat).getButton(), WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT_COMBAT, iUnitCombat, -1,
