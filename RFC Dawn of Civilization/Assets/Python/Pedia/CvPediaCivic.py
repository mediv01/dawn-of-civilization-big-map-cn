...
class CvPediaCivic:
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), CivicInfo.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE,
    ...
    def placeEffects(self):
        ...
        szText += GlobalCyGameTextMgr.parseCivicInfo(self.iCivic, True, False, True).strip()
    ...
    def placeHistory(self):
        ...
        if self.iCivic == iSecularism:
            victorytext = GlobalCyTranslator.getText("TXT_KEY_PEDIA_RELIGIOUS_VICTORY", ())
            iVictory = iVictorySecularism
            bullet = u"%c" % GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)
