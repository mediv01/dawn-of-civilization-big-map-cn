...
class CvPediaBuilding:
    ...
    def interfaceScreen(self, iBuilding):
        ...
        if iBuilding == iHubbleSpaceTelescope:
            ...
            screen.addBuildingGraphicGFC(self.top.getNextWidgetName(), self.iBuilding, self.X_BUILDING_ANIMATION, self.Y_BUILDING_ANIMATION, self.W_BUILDING_ANIMATION, self.H_BUILDING_ANIMATION, WidgetTypes.WIDGET_GENERAL, -1, -1,
                                         self.X_ROTATION_BUILDING_ANIMATION, self.Z_ROTATION_BUILDING_ANIMATION, self.SCALE_ANIMATION, True)
    ...
    def spaceGraphic(self, iBuilding):
        ...
        screen.addModelGraphicGFC(szImage, "Art/Interface/Screens/Civilopedia/Space_Environment.nif", self.X_BUILDING_ANIMATION, self.Y_BUILDING_ANIMATION, self.W_BUILDING_ANIMATION, self.H_BUILDING_ANIMATION, WidgetTypes.WIDGET_GENERAL,
                                  -1, -1, self.X_ROTATION_BUILDING_ANIMATION, self.Z_ROTATION_BUILDING_ANIMATION, 1.0)
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), BuildingInfo.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE,
        ...
        if self.top.iActivePlayer != -1:
            pPlayer = gcgetPlayer(self.top.iActivePlayer)
        ...
        if iHappiness > 0:
            szStats += u"+%d%c  " % (iHappiness, GlobalCyGame.getSymbolID(FontSymbols.HAPPY_CHAR))
        elif iHappiness < 0:
            szStats += u"+%d%c  " % (abs(iHappiness), GlobalCyGame.getSymbolID(FontSymbols.UNHAPPY_CHAR))
        ...
        if iHealth > 0:
            szStats += u"+%d%c  " % (iHealth, GlobalCyGame.getSymbolID(FontSymbols.HEALTHY_CHAR))
        elif iHealth < 0:
            szStats += u"+%d%c  " % (abs(iHealth), GlobalCyGame.getSymbolID(FontSymbols.UNHEALTHY_CHAR))
    ...
    def placeRequires(self):
        ...
        if iPrereq >= 0:
            screen.attachImageButton(panel, "", gc.getReligionInfo(iPrereq).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_RELIGION, iPrereq,
                                     self.isStateReligionRequirement([iPrereq, iOrPrereq], [iStatePrereq, iOrStatePrereq]), False)

        if iOrPrereq >= 0:
            screen.attachLabel(panel, "", GlobalCyTranslator.getText("TXT_KEY_OR", ()))
            screen.attachImageButton(panel, "", gc.getReligionInfo(iOrPrereq).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_RELIGION, iOrPrereq,
                                     self.isStateReligionRequirement([iPrereq, iOrPrereq], [iStatePrereq, iOrStatePrereq]), False)
        ...
        if gc.getBuildingInfo(self.iBuilding).isPagan():
            if self.top.iActivePlayer != -1:
                button = gc.getCivilizationInfo(gcgetPlayer(self.top.iActivePlayer).getCivilizationType()).getPaganReligionButton()
                if button:
                    screen.attachImageButton(panel, "", button, GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_MINOR_RELIGION, gcgetPlayer(self.top.iActivePlayer).getCivilizationType(), 1, False)
    ...
    def placeEffects(self):
        ...
        BuildingInfo = gc.getBuildingInfo(self.iBuilding)
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_EFFECTS", ()), "", True, False, self.X_EFFECTS, self.Y_EFFECTS, self.W_EFFECTS, self.H_EFFECTS, PanelStyles.PANEL_STYLE_BLUE50)
        szText = GlobalCyGameTextMgr.getBuildingHelp(self.iBuilding, True, False, False, CyCity()).lstrip()
