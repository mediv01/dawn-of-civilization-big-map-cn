...
class CvPediaImprovement:
    ...
    def interfaceScreen(self, iImprovement):
        ...
        screen.addImprovementGraphicGFC(self.top.getNextWidgetName(), self.iImprovement, self.X_ANIMATION, self.Y_ANIMATION, self.W_ANIMATION, self.H_ANIMATION, WidgetTypes.WIDGET_GENERAL, -1, -1, self.X_ROTATION, self.Z_ROTATION,
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), info.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_GENERAL, -1,
                         -1)
        ...
        for iBuild in xrange(gc.getNumBuildInfos()):
            ...
            if BuildInfo.getImprovement() == self.iImprovement:
                ...
                iTime *= gc.getGameSpeedInfo(GlobalCyGame.getGameSpeedType()).getBuildPercent() / 100
                iTime *= gc.getEraInfo(GlobalCyGame.getStartEra()).getBuildPercent() / 100
                if iTime > 0:
        ...
        if iHappinessPercent != 0:
            symbol = GlobalCyGame.getSymbolID(FontSymbols.UNHAPPY_CHAR)
            if iHappinessPercent > 0:
                symbol = GlobalCyGame.getSymbolID(FontSymbols.HAPPY_CHAR)
            szStats += (u"+%.2f%c  " % (0.01 * abs(iHappinessPercent), symbol))

        iHealthPercent = info.getHealthPercent()
        if iHealthPercent != 0:
            symbol = GlobalCyGame.getSymbolID(FontSymbols.UNHEALTHY_CHAR)
            if iHealthPercent > 0:
                symbol = GlobalCyGame.getSymbolID(FontSymbols.HEALTHY_CHAR)
    ...
    def placeDetails(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_DETAILS", ()), "", True, False, self.X_DETAILS, self.Y_DETAILS, self.W_DETAILS, self.H_DETAILS, PanelStyles.PANEL_STYLE_BLUE50)

        szText = u""
        szBullet = GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR)
        ...
        szText += GlobalCyGameTextMgr.getImprovementHelp(self.iImprovement, True)
    ...
    def placeResources(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_CONCEPT_RESOURCES", ()), "", True, True, self.X_RESOURCES, self.Y_RESOURCES, self.W_RESOURCES, self.H_RESOURCES, PanelStyles.PANEL_STYLE_BLUE50)
