...
class CvPediaCultureLevel:
    ...
    def placeInfo(self):
        ...
        if self.top.iActivePlayer == -1:
            ...
            iCulture = CultureLevelInfo.getSpeedThreshold(gcgame.getGameSpeedType())
    ...
    def placeEffects(self):
        ...
        if iDefense > 0:
            szText += u"+%d%s %c\n" % (iDefense, "%", GlobalCyGame.getSymbolID(FontSymbols.DEFENSE_CHAR),)

        iUnrest = self.iCultureLevel
        if iUnrest > 0:
            szText += u"%d turns of unrest %c on conquest\n" % (iUnrest, GlobalCyGame.getSymbolID(FontSymbols.OCCUPATION_CHAR),)
    ...
    def placeBuildings(self):
        screen = self.top.getScreen()
        panel = self.top.getNextWidgetName()
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_BUILDINGS", ()), "", False, True, self.X_BUILDINGS, self.Y_BUILDINGS, self.W_BUILDINGS, self.H_BUILDINGS, PanelStyles.PANEL_STYLE_BLUE50)
        text = self.top.getNextWidgetName()
        screen.attachListBoxGFC(panel, text, "", TableStyles.TABLE_STYLE_EMPTY)
        screen.enableSelect(text, False)
        ...
        for iBuilding in range(gc.getNumBuildingInfos()):
            ...
            if iGreatPeopleRateModifier > 0:
                ...
                szBuildingEffects += u"+%d%s %c" % (self.iCultureLevel * iGreatPeopleRateModifier, "%", GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR))
            ...
            if iTradeRouteModifier > 0:
                ...
                szBuildingEffects += u"+%d%s %c" % (self.iCultureLevel * iTradeRouteModifier, "%", GlobalCyGame.getSymbolID(FontSymbols.TRADE_CHAR))
    ...
    def placeSpecialists(self):
        ...
        for iSpecialist in range(gc.getNumSpecialistInfos()):
            ...
            if iGreatPeopleRateChange > 0:
                ...
                szSpecialistEffects += u"+%d%c" % (iGreatPeopleRateChange, GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR))
