...
class CvPediaMain(CvPediaScreen.CvPediaScreen):
    ...
    def createScreen(self, screen):
        ...
        screen.addDDSGFC(self.BACKGROUND_ID, GlobalCyArtFileMgr.getInterfaceArtInfo("SCREEN_BG_OPAQUE").getPath(), 0, 0, self.W_SCREEN, self.H_SCREEN, WidgetTypes.WIDGET_GENERAL, -1, -1)
        ...
        self.szCategoryCivs = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_CIV", ())
        self.szCategoryLeaders = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_LEADER", ())
        self.szCategoryCivics = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_CIVIC", ())
        self.szCategoryReligions = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_RELIGION", ())
        self.szCategoryCorporations = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_CORPORATIONS", ())
        self.szCategorySpecialists = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_SPECIALIST", ())
        self.szCategoryTechs = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_TECH", ())
        self.szCategoryUnits = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_UNIT", ())
        self.szCategoryMilitaryUnits = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_MILITARY_UNITS", ())
        self.szCategoryUniqueUnits = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_UNIQUE_UNITS", ())
        self.szCategoryUnitCategories = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_UNIT_CHARTS", ())
        self.szCategoryUnitUpgrades = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_UNIT_UPGRADES", ())
        self.szCategoryPromotions = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_PROMOTION", ())
        self.szCategoryPromotionTree = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_PROMOTION_TREE", ())
        self.szCategoryBuildings = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_BUILDING", ())
        self.szCategoryReligiousBuildings = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_RELIGIOUS_BUILDINGS", ())
        self.szCategoryUniqueBuildings = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_UNIQUE_BUILDINGS", ())
        self.szCategoryGreatPeopleBuildings = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_GREAT_PEOPLE_BUILDINGS", ())
        self.szCategoryNationalWonders = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_NATIONAL_WONDERS", ())
        self.szCategoryGreatWonders = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_WORLD_WONDERS", ())
        self.szCategoryProjects = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_PROJECT", ())
        self.szCategoryTerrains = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_TERRAIN", ())
        self.szCategoryFeatures = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_FEATURE", ())
        self.szCategoryBonuses = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_BONUS", ())
        self.szCategoryImprovements = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_IMPROVEMENT", ())
        self.szCategoryRoutes = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_ROUTE", ())
        self.szCategoryCultureLevels = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_CULTURE_LEVELS", ())
        self.szCategoryConcepts = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_CONCEPTS", ())
        self.szCategoryHints = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_HINTS", ())
        self.szCategoryShortcuts = GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_SHORTCUTS", ())
        ...
        self.categoryGraphics = {
            "CIVS": u"%c  " % GlobalCyGame.getSymbolID(FontSymbols.MAP_CHAR),
            "CIVICS": u"%c  " % GlobalCyGame.getSymbolID(FontSymbols.GOLDEN_AGE_CHAR),
            "RELIGIONS": u"%c  " % GlobalCyGame.getSymbolID(FontSymbols.RELIGION_CHAR),
            "CORPORATIONS": u"%c  " % gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar(),
            "SPECIALISTS": u"%c  " % GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR),
            ...
            "UNITS": u"%c  " % GlobalCyGame.getSymbolID(FontSymbols.STRENGTH_CHAR),
            "PROMOTIONS": u"%c  " % GlobalCyGame.getSymbolID(FontSymbols.SILVER_STAR_CHAR),
    ...
    def pediaShow(self):
        ...
        self.iActivePlayer = gcgame.getActivePlayer()
        self.iCategory = -1
    ...
    def placeCivs(self):
        ...
        for iCivilization in xrange(gc.getNumCivilizationInfos()):
            ...
            lCivilizations.append((CivilizationInfo.getText(), iCivilization, CivilizationInfo.getStartingYear()))
        lCivilizations.append(("远古文明组",-1,-4000))
        lCivilizations.append(("古典文明组",-1,-900))
        lCivilizations.append((" ",-1,-901))
        lCivilizations.append(("中古文明组",-1,500))
        lCivilizations.append((" ",-1,499))
        lCivilizations.append(("中世纪文明组",-1,897))
        lCivilizations.append((" ",-1,896))
        lCivilizations.append(("近代文明组",-1,1500))
        lCivilizations.append((" ",-1,1499))
        lCivilizations.sort(key=lambda x: x[2])
    ...
    def placeLeaders(self):
        ...
        for iLeader in xrange(gc.getNumLeaderHeadInfos()):
            ...
            if not iCiv is None and gc.getCivilizationInfo(iCiv).isPlayable():
                lLeaders.append((LeaderInfo.getDescription(), iLeader, gc.getCivilizationInfo(iCiv).getStartingYear()))

        lLeaders.append(("远古文明组",-1,-4000))
        lLeaders.append(("古典文明组",-1,-900))
        lLeaders.append((" ",-1,-901))
        lLeaders.append(("中古文明组",-1,500))
        lLeaders.append((" ",-1,499))
        lLeaders.append(("中世纪文明组",-1,897))
        lLeaders.append((" ",-1,896))
        lLeaders.append(("近代文明组",-1,1500))
        lLeaders.append((" ",-1,1499))

        lLeaders.sort(key=lambda x: x[2])
        self.list = lLeaders
    ...
    def placeSpecialists(self):
        lSpecialists = []
        hSpecialists = GlobalCyTranslator.getText("TXT_KEY_PEDIA_HEADER_SPECIALIST", ())
        lSatellites = []
        hSatellites = GlobalCyTranslator.getText("TXT_KEY_PEDIA_HEADER_SATELLITE", ())
        lGreatSpecialists = []
        hGreatSpecialists = GlobalCyTranslator.getText("TXT_KEY_PEDIA_HEADER_GREAT_SPECIALIST", ())
    ...
    def placeTechs(self):
        ...
        for iTech in xrange(gc.getNumTechInfos()):
            x = gc.getTechInfo(iTech).getGridX()
            y = gc.getTechInfo(iTech).getGridY()
            if (x>0 and y>0):
                techvalue = x*100+y
                #iTechList.append((gc.getTechInfo(iTech).getDescription(),iTech,techvalue))
                iTechList.append((gc.getTechInfo(iTech).getDescription(),iTech,techvalue))

        iTechList.append(("远古时代",-1,-1))
        iTechList.append(("古典时代",-1,400))
        iTechList.append((" ",-1,399))
        iTechList.append(("中古时代",-1,700))
        iTechList.append((" ",-1,699))
        iTechList.append(("启蒙时代",-1,1000))
        iTechList.append((" ",-1,999))
        iTechList.append(("工业时代",-1,1300))
        iTechList.append((" ",-1,1299))
        iTechList.append(("全球化时代",-1,1600))
        iTechList.append((" ",-1,1599))
        iTechList.append(("信息化时代",-1,1900))
        iTechList.append((" ",-1,1899))

        iTechList.sort(key=lambda x: x[2])


        iTechList2 = []
        for iTech in iTechList:
            iTechList2.append((iTech[0],iTech[1]))



        self.list = iTechList2
    ...
    def placeUniqueUnits(self):
        UnitList = []
        BuildingClassTypeBefore = ''
        inum = 0

        for iUnit in xrange(gc.getNumUnitInfos()):
            if self.getUnitCategory(iUnit) == 2:
                if BuildingClassTypeBefore is not gc.getUnitInfo(iUnit).getUnitClassType():
                    if BuildingClassTypeBefore is not '':
                        UnitList.append((" ", -1, gc.getUnitInfo(iUnit).getUnitClassType()-0.6))

                    BuildingClassTypeBefore = gc.getUnitInfo(iUnit).getUnitClassType()
                    inum +=1
                    UnitList.append(("UU分组 %d" % inum, -1, gc.getUnitInfo(iUnit).getUnitClassType() - 0.5))
                UnitList.append((gc.getUnitInfo(iUnit).getDescription(), iUnit, gc.getUnitInfo(iUnit).getUnitClassType()))

        UnitList.sort(key=lambda x: x[2])
        self.list = UnitList
        self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT, gc.getUnitInfo)
            ...
            if utils.getBuildingCategory(iBuilding) == 0:
                if iBuilding == utils.getUniqueBuilding(self.iActivePlayer, iBuilding):
                    ...
                    if (gc.getTechInfo(itech)):
                        techid = gc.getTechInfo(itech).getGridX() * 100 + gc.getTechInfo(itech).getGridY()
                    lBuildings.append((gc.getBuildingInfo(iBuilding).getDescription(), iBuilding, techid))
        lBuildings.append(("远古时代",-1,-1))
        lBuildings.append(("古典时代",-1,400))
        lBuildings.append((" ",-1,399))
        lBuildings.append(("中古时代",-1,700))
        lBuildings.append((" ",-1,699))
        lBuildings.append(("启蒙时代",-1,1000))
        lBuildings.append((" ",-1,999))
        lBuildings.append(("工业时代",-1,1300))
        lBuildings.append((" ",-1,1299))
        lBuildings.append(("全球化时代",-1,1600))
        lBuildings.append((" ",-1,1599))
        lBuildings.append(("信息化时代",-1,1900))
        lBuildings.append((" ",-1,1899))
        lBuildings.sort(key = lambda x:x[2])
        ...
        for iBuilding in xrange(gc.getNumBuildingInfos()):
            if utils.getBuildingCategory(iBuilding) == 1:
                ...
                if iReligion != prevReligion:
                    hReligion = GlobalCyTranslator.getText("TXT_KEY_PEDIA_HEADER_RELIGION", (gc.getReligionInfo(iReligion).getDescription(), ''))
                    lBuildings.append(("", -1))
    ...
    def placeUniqueBuildings(self):
        ...
        for iBuilding in xrange(gc.getNumBuildingInfos()):
            if utils.getBuildingCategory(iBuilding) == 2 and not gc.getBuildingInfo(iBuilding).isGraphicalOnly():
                if BuildingClassTypeBefore is not gc.getBuildingInfo(iBuilding).getBuildingClassType():
                    if BuildingClassTypeBefore is not '':
                        lBuildings.append((" ", -1, gc.getBuildingInfo(iBuilding).getBuildingClassType()-0.6))

                    BuildingClassTypeBefore = gc.getBuildingInfo(iBuilding).getBuildingClassType()
                    inum +=1
                    lBuildings.append(("UB分组 %d" % inum, -1, gc.getBuildingInfo(iBuilding).getBuildingClassType() - 0.5))
                lBuildings.append((gc.getBuildingInfo(iBuilding).getDescription(), iBuilding, gc.getBuildingInfo(iBuilding).getBuildingClassType()))
    ...
    def placeGreatWonders(self):
        ...
        for iBuilding in xrange(gc.getNumBuildingInfos()):
            if utils.getBuildingCategory(iBuilding) == 5:
                ...
                techid = gc.getTechInfo(itech).getGridX()*100 + gc.getTechInfo(itech).getGridY()
                szDescription = gc.getBuildingInfo(iBuilding).getDescription().replace("The ", "")
                lBuildings.append((szDescription, iBuilding,techid))
    ...
    def placeHints(self):
        ...
        szHintsText = GlobalCyGameTextMgr.buildHintsList()
    ...
    def placeItems(self, itemwidget, info):
        ...
        for item in self.list:
            ...
            if item[1] == -1:
                sTitle = GlobalCyTranslator.changeTextColor(item[0], gc.getInfoTypeForString('COLOR_HIGHLIGHT_TEXT'))
            ...
            screen.setTableText(self.ITEM_LIST_ID, 0, i, u"<font=1>" + sTitle + u"</font>", button, widget, data1, data2, CvUtil.FONT_LEFT_JUSTIFY)
            i += 1
