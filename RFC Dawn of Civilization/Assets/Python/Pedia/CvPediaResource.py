...
class CvPediaResource:
    ...
    def interfaceScreen(self, iResource):
        ...
        screen.addBonusGraphicGFC(self.top.getNextWidgetName(), self.iResource, self.X_ANIMATION, self.Y_ANIMATION, self.W_ANIMATION, self.H_ANIMATION, WidgetTypes.WIDGET_GENERAL, -1, -1, self.X_ROTATION, self.Z_ROTATION,
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), ResourceInfo.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE,
    ...
    #	screen.appendListBoxString(panel, GlobalCyTranslator.getText("TXT_KEY_AFFECTED_CITIES", (ResourceInfo.getAffectedCities(),)), WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)

    def placeTech(self):
        screen = self.top.getScreen()
        panel = self.top.getNextWidgetName()

        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_CONCEPT_TECHNOLOGY", ()), "", False, True, self.X_TECH, self.Y_TECH, self.W_TECH, self.H_TECH, PanelStyles.PANEL_STYLE_BLUE50)
        ...
        if iTech > -1:
            ...
            screen.attachLabel(panel, "", u" (" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_BONUS_APPEARANCE", ()) + u")   ")

        iTech = gc.getBonusInfo(self.iResource).getTechCityTrade()
        if iTech > -1:
            screen.attachImageButton(panel, "", gc.getTechInfo(iTech).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TECH, iTech, 1, False)
            screen.attachLabel(panel, "", u" (" + GlobalCyTranslator.getText("TXT_KEY_PEDIA_BONUS_TRADE", ()) + u")   ")

        iTech = gc.getBonusInfo(self.iResource).getTechPlayerTrade()
        if iTech > -1:
            screen.attachImageButton(panel, "", gc.getTechInfo(iTech).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TECH, iTech, 1, False)
            screen.attachLabel(panel, "", u" (" + GlobalCyTranslator.getText("Trade", ()) + u")   ")

        iTech = gc.getBonusInfo(self.iResource).getTechObsolete()
        if iTech > -1:
            screen.attachImageButton(panel, "", gc.getTechInfo(iTech).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_TECH, iTech, 1, False)
            screen.attachLabel(panel, "", u" (" + GlobalCyTranslator.getText("Obsoletes", ()) + u")   ")

    def placeImprovements(self):
        screen = self.top.getScreen()
        panel = self.top.getNextWidgetName()

        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_IMPROVEMENT", ()), "", True, False, self.X_IMPROVEMENTS, self.Y_IMPROVEMENTS, self.W_IMPROVEMENTS, self.H_IMPROVEMENTS, PanelStyles.PANEL_STYLE_BLUE50)
    ...
    def placeUses(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_USED_BY", ()), "", False, True, self.X_USES, self.Y_USES, self.W_USES, self.H_USES, PanelStyles.PANEL_STYLE_BLUE50)
        screen.attachLabel(panel, "", "  ")
        ...
        for iCorporation in xrange(gc.getNumCorporationInfos()):
            ...
            for i in xrange(NUM_CORPORATION_PREREQ_BONUSES):
    ...
    def placeHistory(self):
        ...
        screen.addMultilineText(textName, gc.getBonusInfo(self.iResource).getCivilopedia(), self.X_HISTORY + 15, self.Y_HISTORY + 40, self.W_HISTORY - (30), self.H_HISTORY - (15 * 2) - 25, WidgetTypes.WIDGET_GENERAL, -1, -1,
