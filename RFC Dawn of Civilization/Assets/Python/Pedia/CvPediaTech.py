...
class CvPediaTech(CvPediaScreen.CvPediaScreen):
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), TechInfo.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_GENERAL,
                         -1, -1)
        ...
        if self.top.iActivePlayer == -1:
            ...
            szCost = u"Cost: %d%c" % (gcgetTeam(GlobalCyGame.getActiveTeam()).getResearchCost(self.iTech), gc.getCommerceInfo(CommerceTypes.COMMERCE_RESEARCH).getChar())
    ...
    def placeLeadsTo(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_LEADS_TO", ()), "", False, True, self.X_LEADS_TO, self.Y_LEADS_TO, self.W_LEADS_TO, self.H_LEADS_TO, PanelStyles.PANEL_STYLE_BLUE50)
    ...
    def placePrereqs(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_REQUIRES", ()), "", False, True, self.X_REQUIRES, self.Y_REQUIRES, self.W_REQUIRES, self.H_REQUIRES, PanelStyles.PANEL_STYLE_BLUE50)
        screen.attachLabel(panel, "", "  ")
        ...
        for j in xrange(gc.getNUM_AND_TECH_PREREQS()):
            ...
            if eTech > -1:
                if not bFirst:
                    screen.attachLabel(panel, "", GlobalCyTranslator.getText("TXT_KEY_AND", ()))
                else:
                    bFirst = False
        ...
        if not bFirst:
            if nOrTechs > 1:
                szLeftDelimeter = GlobalCyTranslator.getText("TXT_KEY_AND", ()) + "("
                szRightDelimeter = ") "
            elif nOrTechs > 0:
                szLeftDelimeter = GlobalCyTranslator.getText("TXT_KEY_AND", ())
            else:
        ...
        for j in xrange(gc.getNUM_OR_TECH_PREREQS()):
            ...
            if eTech > -1:
                if not bFirst:
                    screen.attachLabel(panel, "", GlobalCyTranslator.getText("TXT_KEY_OR", ()))
                else:
                    bFirst = False
                screen.attachImageButton(panel, "", gc.getTechInfo(eTech).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_REQUIRED_TECH, eTech, j, False)
    ...
    def placeEffects(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_EFFECTS", ()), "", False, True, self.X_EFFECTS, self.Y_EFFECTS, self.W_EFFECTS, self.H_EFFECTS, PanelStyles.PANEL_STYLE_BLUE50)
        screen.attachLabel(panel, "", "  ")

        # Worker Speed
        if TechInfo.getWorkerSpeedModifier() != 0:
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_FEATURE_PRODUCTION").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_WORKER_RATE, self.iTech, -1, False)

        # Irrigation
        if TechInfo.isIrrigation():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_IRRIGATION").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_IRRIGATION, self.iTech, -1, False)

        # Ignore Irrigation
        if TechInfo.isIgnoreIrrigation():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_NOIRRIGATION").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_IGNORE_IRRIGATION, self.iTech, -1, False)
        ...
        if TechInfo.getFeatureProductionModifier() != 0:
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_FEATURE_PRODUCTION").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_FEATURE_PRODUCTION, self.iTech, -1, False)

        # Route Movement
        for j in xrange(gc.getNumRouteInfos()):
            if gc.getRouteInfo(j).getTechMovementChange(self.iTech) != 0:
                screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_MOVE_BONUS").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_MOVE_BONUS, self.iTech, -1, False)

        # Bridge Building
        if TechInfo.isBridgeBuilding():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_BRIDGEBUILDING").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_BUILD_BRIDGE, self.iTech, -1, False)

        # River Trade
        if TechInfo.isRiverTrade():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_RIVERTRADE").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_TERRAIN_TRADE, self.iTech, gc.getNumTerrainInfos(),
                                     False)

        # Water Trade
        for j in xrange(gc.getNumTerrainInfos()):
            if TechInfo.isTerrainTrade(j):
                screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_WATERTRADE").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_TERRAIN_TRADE, self.iTech, j, False)

        # Water Work
        if TechInfo.isWaterWork():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_WATERWORK").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_WATER_WORK, self.iTech, -1, False)

        # Water Sight
        if TechInfo.isExtraWaterSeeFrom():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_LOS").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_LOS_BONUS, self.iTech, -1, False)

        # Water Moves
        for j in xrange(DomainTypes.NUM_DOMAIN_TYPES):
            if TechInfo.getDomainExtraMoves(j) != 0:
                screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_WATERMOVES").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_DOMAIN_EXTRA_MOVES, self.iTech, j, False)

        # Map Centre
        if TechInfo.isMapCentering():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_MAPCENTER").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_MAP_CENTER, self.iTech, -1, False)

        # Map Reveal
        if TechInfo.isMapVisible():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_MAPREVEAL").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_MAP_REVEAL, self.iTech, -1, False)

        # Map Trading
        if TechInfo.isMapTrading():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_MAPTRADING").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_MAP_TRADE, self.iTech, -1, False)

        # Tech Trading
        if TechInfo.isTechTrading():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_TECHTRADING").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_TECH_TRADE, self.iTech, -1, False)

        # Wealth Trading
        if TechInfo.isGoldTrading():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_GOLDTRADING").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_GOLD_TRADE, self.iTech, -1, False)

        # Trade Routes
        if TechInfo.getTradeRoutes() != 0:
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_TRADE_ROUTES").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_TRADE_ROUTES, self.iTech, -1, False)

        # Open Borders
        if TechInfo.isOpenBordersTrading():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_OPENBORDERS").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_OPEN_BORDERS, self.iTech, -1, False)

        # Defensive Pacts
        if TechInfo.isDefensivePactTrading():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_DEFENSIVEPACT").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_DEFENSIVE_PACT, self.iTech, -1, False)

        # Permanent Alliances
        if TechInfo.isPermanentAllianceTrading():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_PERMALLIANCE").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_PERMANENT_ALLIANCE, self.iTech, -1, False)

        # Vassal States
        if TechInfo.isVassalStateTrading():
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_VASSAL").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_VASSAL_STATE, self.iTech, -1, False)

        # Happiness
        if TechInfo.getHappiness() != 0:
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_HAPPINESS").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_HAPPINESS_RATE, self.iTech, -1, False)

        # Health
        if TechInfo.getHealth() != 0:
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_HEALTH").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_HEALTH_RATE, self.iTech, -1, False)
        ...
        for j in xrange(CommerceTypes.NUM_COMMERCE_TYPES):
            if TechInfo.isCommerceFlexible(j):
                if j == CommerceTypes.COMMERCE_CULTURE:
                    ...
                    szFileName = GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_DEFENSIVEPACT").getPath()
                else:
        ...
        if TechInfo.getFirstFreeTechs() > 0:
            screen.attachImageButton(panel, "", GlobalCyArtFileMgr.getInterfaceArtInfo("INTERFACE_TECH_FREETECH").getPath(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_HELP_FREE_TECH, self.iTech, -1, False)

        # Free Units
        if TechInfo.getFirstFreeUnitClass() != UnitClassTypes.NO_UNITCLASS:
            iCivilization = GlobalCyGame.getActiveCivilizationType()
            if iCivilization > -1:
    ...
    def placeEnables(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_BONUS_TRADE", ()), "", False, True, self.X_ENABLES, self.Y_ENABLES, self.W_ENABLES, self.H_ENABLES, PanelStyles.PANEL_STYLE_BLUE50)
        screen.attachLabel(panel, "", "  ")
    ...
    def placeHistory(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_CIVILOPEDIA_HISTORY", ()), "", True, True, self.X_HISTORY, self.Y_HISTORY, self.W_HISTORY, self.H_HISTORY, PanelStyles.PANEL_STYLE_BLUE50)
