...
class CvPediaTerrain:
    ...
    def placeDetails(self):
        ...
        if self.iTerrain != gc.getInfoTypeForString('TERRAIN_HILL'):
            szText = info.getHelp()
            szText += GlobalCyGameTextMgr.getTerrainHelp(self.iTerrain, True)
    ...
    def placeFeatures(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_MISC_FEATURES", ()), "", False, True, self.X_FEATURES, self.Y_FEATURES, self.W_FEATURES, self.H_FEATURES, PanelStyles.PANEL_STYLE_BLUE50)
