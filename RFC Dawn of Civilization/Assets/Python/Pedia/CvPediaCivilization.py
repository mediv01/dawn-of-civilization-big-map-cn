...
class CvPediaCivilization:
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), CivInfo.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_GENERAL,
                         -1, -1)
    ...
    def placeDescription(self):
        ...
        szText += GlobalCyGameTextMgr.parseCivInfos(self.iCivilization, True)
    ...
    def placeLeaders(self):
        screen = self.top.getScreen()
        panel = self.top.getNextWidgetName()
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_LEADERS", ()), "", False, True, self.X_LEADERS, self.Y_LEADERS, self.W_LEADERS, self.H_LEADERS, PanelStyles.PANEL_STYLE_BLUE50)
    ...
    def placeTechs(self):
        screen = self.top.getScreen()
        panel = self.top.getNextWidgetName()
        screen.addPanel(panel, GlobalCyTranslator.getText('TXT_KEY_STARTING_TECHS', ()), "", False, True, self.X_TECHS, self.Y_TECHS, self.W_TECHS, self.H_TECHS, PanelStyles.PANEL_STYLE_BLUE50)
        screen.attachLabel(panel, "", "  ")
    ...
    def placeBuildings(self):
        screen = self.top.getScreen()
        panel = self.top.getNextWidgetName()
        screen.addPanel(panel, GlobalCyTranslator.getText('TXT_KEY_UNIQUE_BUILDINGS', ()), "", False, True, self.X_BUILDINGS, self.Y_BUILDINGS, self.W_BUILDINGS, self.H_BUILDINGS, PanelStyles.PANEL_STYLE_BLUE50)
        screen.attachLabel(panel, "", "  ")
    ...
    def placeUnits(self):
        screen = self.top.getScreen()
        panel = self.top.getNextWidgetName()
        screen.addPanel(panel, GlobalCyTranslator.getText('TXT_KEY_FREE_UNITS', ()), "", False, True, self.X_UNITS, self.Y_UNITS, self.W_UNITS, self.H_UNITS, PanelStyles.PANEL_STYLE_BLUE50)
        screen.attachLabel(panel, "", "  ")
