...
class CvPediaUnit:
    ...
    def interfaceScreen(self, iUnit):
        ...
        if iUnit == iSatellite:
            ...
            screen.addUnitGraphicGFC(self.top.getNextWidgetName(), self.iUnit, self.X_UNIT_ANIMATION, self.Y_UNIT_ANIMATION, self.W_UNIT_ANIMATION, self.H_UNIT_ANIMATION, WidgetTypes.WIDGET_GENERAL, -1, -1, self.X_ROTATION_UNIT_ANIMATION,
                                     self.Z_ROTATION_UNIT_ANIMATION, self.SCALE_ANIMATION, True)
    ...
    def spaceGraphic(self, iUnit):
        ...
        screen.addModelGraphicGFC(szImage, "Art/Interface/Screens/Civilopedia/Space_Environment.nif", self.X_UNIT_ANIMATION, self.Y_UNIT_ANIMATION, self.W_UNIT_ANIMATION, self.H_UNIT_ANIMATION, WidgetTypes.WIDGET_GENERAL, -1, -1,
                                  self.X_ROTATION_UNIT_ANIMATION, self.Z_ROTATION_UNIT_ANIMATION, self.SCALE_ANIMATION)
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), UnitInfo.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE, WidgetTypes.WIDGET_GENERAL,
                         -1, -1)
        ...
        szStats = u"%d%c" % (iStrength, GlobalCyGame.getSymbolID(FontSymbols.STRENGTH_CHAR))

        # Movement
        if UnitInfo.getAirCombat() > 0 and UnitInfo.getAirRange() > 0:
            szStats += u"  %d%c" % (UnitInfo.getAirRange(), GlobalCyGame.getSymbolID(FontSymbols.MOVES_CHAR))
        else:
            szStats += u"  %d%c" % (UnitInfo.getMoves(), GlobalCyGame.getSymbolID(FontSymbols.MOVES_CHAR))

        # Cost
        if UnitInfo.getProductionCost() >= 0:
            if self.top.iActivePlayer == -1:
                iCost = (UnitInfo.getProductionCost() * 100) / 100
            else:
    ...
    def placeReplacements(self):
        screen = self.top.getScreen()
        panel = self.top.getNextWidgetName()
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_REPLACEMENTS_UPGRADES", ()), "", False, True, self.X_REPLACEMENTS, self.Y_REPLACEMENTS, self.W_REPLACEMENTS, self.H_REPLACEMENTS, PanelStyles.PANEL_STYLE_BLUE50)
    ...
    def placeAbilities(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_ABILITIES", ()), "", True, False, self.X_ABILITIES, self.Y_ABILITIES, self.W_ABILITIES, self.H_ABILITIES, PanelStyles.PANEL_STYLE_BLUE50)
        szText = GlobalCyGameTextMgr.getUnitHelp(self.iUnit, True, False, False, None)[1:]
        ...
        if iMax > 0:
            szText += u"\n%cMaximum %d Active" % (GlobalCyGame.getSymbolID(FontSymbols.BULLET_CHAR), iMax)
    ...
    def placePromotions(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_CATEGORY_PROMOTION", ()), "", True, True, self.X_PROMOTIONS, self.Y_PROMOTIONS, self.W_PROMOTIONS, self.H_PROMOTIONS, PanelStyles.PANEL_STYLE_BLUE50)
