...
class CvPediaSpecialist:
    ...
    def placeInfo(self):
        ...
        screen.addDDSGFC(self.top.getNextWidgetName(), SpecialistInfo.getButton(), self.X_ICON + self.W_ICON / 2 - self.ICON_SIZE / 2, self.Y_ICON + self.H_ICON / 2 - self.ICON_SIZE / 2, self.ICON_SIZE, self.ICON_SIZE,
    ...
    def placeEffects(self):
        ...
        szEffects = GlobalCyGameTextMgr.getSpecialistHelp(self.iSpecialist, True)[1:]
        szEffects = szEffects.replace("Birth Rate", szGP)
        if iGPClass == gc.getInfoTypeForString('UNITCLASS_PROPHET'):
            szEffects = szEffects.replace(u"%c" % GlobalCyGame.getSymbolID(FontSymbols.GREAT_PEOPLE_CHAR), u"%c" % GlobalCyGame.getSymbolID(FontSymbols.RELIGION_CHAR))
    ...
    def placeSources(self):
        ...
        screen.addPanel(panel, GlobalCyTranslator.getText("TXT_KEY_PEDIA_SOURCES", ()), "", False, True, self.X_SOURCES, self.Y_SOURCES, self.W_SOURCES, self.H_SOURCES, PanelStyles.PANEL_STYLE_BLUE50)
