...
[[[119, 47], [129, 56]],         610 ,    [(119, 54), (119, 55), (119, 56), (120, 54), (120, 56), (121, 55), (121, 56), (122, 55), (122, 56), (128, 56)],  "贞观之治开启了中华盛世，江南地区也被中华文明进一步开发为帝国的核心区域"] ,
...
tCoreAreaAdjust = {
    iChina: tCoreAreaChina,
    iByzantium:tCoreAreaByzantium,
}


def checkturn(iGameTurn):
    if (PYTHON_ALLOW_DYNAMIC_CORE== 0):return
    adjustCore()


def UpdateCoreArea(iPlayer):
    tCoreAreaCountry = tCoreAreaAdjust.get(iPlayer)
    for iNum in range(1,len(tCoreAreaCountry)): # 0是初始值！
        iYear =  tCoreAreaCountry[iNum][1]
        iTips =  tCoreAreaCountry[iNum][3]
        if utils.getGameTurn() == utils.getTurnForYear(iYear):
            tPlotList = tCoreAreaCountry[iNum][0]
            lExceptions =  tCoreAreaCountry[iNum][2]
            updateCore(iPlayer, tPlotList , lExceptions)
...
def adjustCore():
    UpdateCoreArea(iChina)
    UpdateCoreArea(iByzantium)
    pass



def getCoreArea(iPlayer,iCoreArea, lExceptions):
...
def updateCore(iPlayer, iCoreArea, lExceptions=None):
    if lExceptions is None:
        lExceptions = []
    lCore = getCoreArea(iPlayer,iCoreArea, lExceptions)
