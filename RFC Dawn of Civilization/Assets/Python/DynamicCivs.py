...
def text(sTextKey, tInput=()):
	...
	if not isValidKey(sTextKey): # wunshare: fixed 直接返回中文
	...
	#else:
	#	ret = key
	ret = localText.getText(sTextKey.encode(encoding), tInput)
...
def desc(iPlayer, sTextKey="%s1".encode('utf-8')):
	ret = ""
	#file = open("wunshareDbg.log", 'a+') 
	if isVassal(iPlayer): 
