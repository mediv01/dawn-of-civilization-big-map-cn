...
lRepublicOf = [iEgypt, iIndia, iChina, iPersia, iJapan, iEthiopia, iKorea, iVikings, iTurks, iKhazars, iTibet, iIndonesia, iKhmer, iHolyRome, iMali, iPoland, iLithuania, iMughals, iOttomans, iThailand, iMamluks, iPhilippines, iBoers,
               iVietnam, iZimbabwe, iSwahili, iSweden, iNigeria, iOman, iChad, iCeltia, iMississippi]
...
def setup():
    ...
    for iPlayer in range(iNumPlayers):
        ...
        if gcgetPlayer(iPlayer).getNumCities() > 0:
            checkName(iPlayer)

        if (tBirth[iPlayer] >= gcgame.getGameTurnYear() or gcgetPlayer(iPlayer).getNumCities() > 0) and not gcgetPlayer(iPlayer).isHuman():
...
def onCivRespawn(iPlayer, tOriginalOwners):
    ...
    if iPlayer in lRespawnNameChanges:
        if iPlayer == iCarthage:
            if gcgame.getGameTurnYear() >= 1200:
...
def onPalaceMoved(iPlayer):
    capital = gcgetPlayer(iPlayer).getCapitalCity()
    iEra = gcgetPlayer(iPlayer).getCurrentEra()
...
def setDesc(iPlayer, sName):
    try:
        gcgetPlayer(iPlayer).setCivDescription(sName)
    except:
        pass


def setShort(iPlayer, sShort):
    gcgetPlayer(iPlayer).setCivShortDescription(sShort)


def setAdjective(iPlayer, sAdj):
    gcgetPlayer(iPlayer).setCivAdjective(sAdj)


def setLeader(iPlayer, iLeader):
    if not iLeader: return
    if gcgetPlayer(iPlayer).getLeader() == iLeader: return
    gcgetPlayer(iPlayer).setLeader(iLeader)


def setLeaderName(iPlayer, sName):
    if not sName: return
    if gc.getLeaderHeadInfo(gcgetPlayer(iPlayer).getLeader()).getText() != sName:
        gcgetPlayer(iPlayer).setLeaderName(sName)
...
def key(iPlayer, sSuffix):
    ...
    # 兼容未翻译英文版
    converted = short(iPlayer).replace(" ", "_").upper()  # 返回文明Short名称(Unicode)

    # 将'TXT_KEY_CIV_美利坚'转换成对应的'TXT_KEY_CIV_AMERICA'，为了兼容%s1中文输入
    queryKey = u"TXT_KEY_CIV_%s" % converted
    converted = CvUtil.convertToStr(localText.getText(CvUtil.convertToStr(queryKey), ()))

    if not isValidKey(converted):
        file = open("wunshareDbg.log", 'a+')
        toFile('DynamicCivs:lost key:%s\n' % converted, file)  # 输出缺失的KEY
        file.close()


    retVal = "%s%s" % (converted, sSuffix)  # wunshare: fixed
    return retVal
...
def isValidCity(key):
    vaildChar = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz '
    for word in key:
        if word not in vaildChar:
            return False
    return True
...
def text(sTextKey, tInput=()):
    if not isValidKey(sTextKey) and len(tInput) == 0:  # wunshare 2022.03.12 直接返回中文
        # toScr(sTextKey) # wunshare
        return sTextKey

    if isValidKey(sTextKey):
        return localText.getText(CvUtil.convertToStr(sTextKey), tInput)

    for i in range(len(tInput)):
        sTextKey = sTextKey.replace('%s' + str(i + 1), tInput[i])
    return sTextKey
...
# nFormat = list(tInput) # format是Tuple，不能修改
# if len(tInput) and isinstance(tInput[0], (str, unicode)):
#	cityTrans = tInput[0].encode('utf-8')
#	if cityTrans in trans: # wusnhare: fixed
#		nFormat[0] = trans[cityTrans].decode('utf-8') # 城市名称转码
#	elif isValidCity(cityTrans): # 输出需要翻译的城市名称
#		file = open("wunshareDbg.log", 'a+')
#		toFile('需要翻译的城市名：-%s-\n'%cityTrans, file)
#		file.close()

# if isValidKey(key): # wunshare: fixed '%s2帝国'% tInput[1](Adj)等组合名称
#	ret = translator.getText(key.encode('utf-8'), tuple(encode(f) for f in nFormat)) # wunshare: fixed
# elif isValidKey(tInput[1]): # wunshare: fixed tInput[1]='TXT_KEY_CIV_XXX'等完整名称
#	ret = translator.getText(tInput[1].encode('utf-8'), ()) # wunshare: fixed
# else:
#	ret = key

# if isValidCity(ret):
#	file = open("wunshareDbg.log", 'a+')
#	toFile('DynamicCivs:lost city text:%s\n'%ret, file) # 输出缺失的城市名称
#	file.close()
...
# 核心逻辑函数
def desc(iPlayer, sTextKey):
    ret = ""
    # file = open("wunshareDbg.log", 'a+')
    if isVassal(iPlayer):
        ret = text(sTextKey, (name(iPlayer), adjective(iPlayer), name(iPlayer, True), adjective(iPlayer, True)))
        # toScr('DynamicCivs.desc:%s'%ret)
        # toFile('DynamicCivs.desc:%s\n'%ret, file)
        return ret
    ret = text(sTextKey, (name(iPlayer), adjective(iPlayer)))

    # 缺失KEY的情况，记录下来，直接返回Short Name
    if ret.find('TXT_KEY_CIV_')>=0:
        utils.log_release_error("以下的文本翻译缺失:"+ret)
        return utils.getCivChineseName(iPlayer)
    # toScr('DynamicCivs.desc:%s'%ret)
    # toFile('DynamicCivs.desc:%s\n'%ret, file)
    # file.close()
    return ret
...
def civkey(iPlayer):
    sName = CivEnglishNameMap.get(iPlayer)
    if sName:
        return sName
    else:
        base = gcgetPlayer(iPlayer).getCivilizationShortDescriptionKey()
        base = base.replace('TXT_KEY_CIV_', "")
        base = base.replace('_SHORT_DESC', "")
        return base


def short(iPlayer):
    return civkey(iPlayer)  # wunshare 2022.03.12


def civAdjective(iPlayer):
    return gcgetPlayer(iPlayer).getCivilizationAdjective(0)


def capitalName(iPlayer):
    return gcgetPlayer(iPlayer).getCivilizationShortDescriptionKey()
    capital = gcgetPlayer(iPlayer).getCapitalCity()
    if capital:
        sCapitalName = capital.getName()
        if iPlayer not in [iBurma, iNubia]:
            sCapitalName = cnm.getRenameName(iEngland, CvUtil.convertToStr(sCapitalName))
        if sCapitalName:
            return sCapitalName
        else:
...
def getColumn(iPlayer):
    lTechs = [gc.getTechInfo(iTech).getGridX() for iTech in range(iNumTechs) if gcgetTeam(iPlayer).isHasTech(iTech)]
...
def isCapitulated(iPlayer):
    return isVassal(iPlayer) and gcgetTeam(iPlayer).isCapitulated()
...
def isEmpire(iPlayer):
    ...
    return gcgetPlayer(iPlayer).getNumCities() >= getEmpireThreshold(iPlayer)


def getEmpireThreshold(iPlayer):
    if iPlayer in dEmpireThreshold: return dEmpireThreshold[iPlayer]

    if iPlayer == iEthiopia and not gcgame.isReligionFounded(iIslam):
        return 4

    if gcgetPlayer(iPlayer).isReborn():
...
def isAtWar(iPlayer):
    for iTarget in range(iNumPlayers):
        if gcgetTeam(iPlayer).isAtWar(iTarget):
            return True
    return False
...
def controlsHolyCity(iPlayer, iReligion):
    holyCity = gcgame.getHolyCity(iReligion)
...
def vassalName(iPlayer, iMaster):
    ...
    if iPlayer == iNetherlands: return gcgetPlayer(iPlayer).getCivilizationShortDescriptionKey()

    if gcgetPlayer(iPlayer).isReborn(): return gcgetPlayer(iPlayer).getCivilizationShortDescriptionKey()
...
def republicName(iPlayer):
    ...
    if iPlayer in (iChina, iManchuria): return 'TXT_KEY_CIV_REPUBLIC_OF_CHINA'
...
def specificName(iPlayer):
    ...
    if iNumCities == 0: return gcgetPlayer(iPlayer).getCivilizationShortDescriptionKey()
    ...
    if iPlayer == iNubia:
        ...
        if bEmpire:
            ...
            if iEra == iRenaissance and iGameTurn >= utils.getTurnForYear(1368):
    ...
    # return "TXT_KEY_CIV_LOMBARD"
        ...
        if not bEmpire:
            if iGameTurn < utils.getTurnForYear(tBirth[iGermany]):
...
def specificAdjective(iPlayer):
    ...
    iNumCities = pPlayer.getNumCities()
    if iNumCities == 0: return gcgetPlayer(iPlayer).getCivilizationAdjective(0)
    ...
    if iPlayer == iIndia:
        ...
        if bEmpire and controlsCity(iPlayer, Areas.getCapital(iRome)):
            return gcgetPlayer(iRome).getCivilizationAdjective(0)
        ...
        if not bEmpire and iGameTurn < utils.getTurnForYear(tBirth[iGermany]):
...
def vassalTitle(iPlayer, iMaster):
    ...
    if iMaster not in lRebirths or not gcgetPlayer(iMaster).isReborn():
...
def republicTitle(iPlayer):
    ...
    if iPlayer == iPoland:
        if gcgetPlayer(iPlayer).getCurrentEra() <= iIndustrial:
    ...
    if gcgetPlayer(iPlayer).getStateReligion() == iIslam:
...
def specificTitle(iPlayer, lPreviousOwners=[]):
    ...
    # pass
        ...
        if gcgame.getHolyCity(iIslam).getOwner() == iYemen:
    ...
    # Nothing for Mali
        ...
        if iReligion == iIslam:
            if bTheocracy and gcgame.getHolyCity(iIslam) and gcgame.getHolyCity(iIslam).getOwner() == iOttomans:
                return "TXT_KEY_CALIPHATE_ADJECTIVE"
...
def leader(iPlayer):
    ...
    if not gcgetPlayer(iPlayer).isAlive(): return None

    if gcgetPlayer(iPlayer).isHuman(): return None
    ...
    if iPlayer == iEgypt:
        ...
        if iEra >= iRenaissance and iGameTurn >= utils.getTurnForYear(1368): return iHongwu
        ...
        if iGameTurn >= utils.getTurnForYear(-1600): return iHammurabi
        ...
        if iGameTurn >= utils.getTurnForYear(1000): return iBasil
        ...
        if iGameTurn >= utils.getTurnForYear(1000): return iAlpArslan

    elif iPlayer == iArabia:
        if iGameTurn >= utils.getTurnForYear(1200): return iMustasim

    elif iPlayer == iTibet:
        if iGameTurn >= utils.getTurnForYear(1500): return iLobsangGyatso
        ...
        if iEra >= iRenaissance:
            if iGameTurn >= utils.getTurnForYear(1750): return iCatherine
        ...
        if iGameTurn >= utils.getTurnForYear(1650): return iWilliam
        ...
        if bResurrected and iGameTurn >= utils.getTurnForYear(1600): return iCastilla
        ...
        if iGameTurn >= utils.getTurnForYear(1400): return iKublaiKhan
        ...
        if iGameTurn >= utils.getTurnForYear(1850): return iLincoln
...
def leaderName(iPlayer):
    ...
    if iPlayer == iChina:
        ...
        if gcgame.getGameTurnYear() >= 1100:
