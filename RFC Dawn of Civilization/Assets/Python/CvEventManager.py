...
class CvEventManager(object):
    ...
    def onKbdEvent(self, argsList):
        ...
        if (self.bAllowCheats):
            ...
            argsList = (eventType, key, self.bCtrl, self.bShift, self.bAlt, mx, my, px, py, gcgame.isNetworkMultiPlayer())
        ...
        if (eventType == self.EventKeyDown):
            ...
            if (self.bAllowCheats):
                ...
                # Shift - ] (Debug - currently mouse-overd unit, health += 10
                    ...
                    unit = GlobalCyMap.plot(px, py).getUnit(0)
                    if (not unit.isNone()):
    ...
    def onModNetMessage(self, argsList):
        ...
        if iData1 == lNetworkEvents["CHANGE_COMMERCE_PERCENT"]:
            ...
            gcgetPlayer(iData2).changeCommercePercent(CommerceType[iData3], iData4)
            if iData2 == GlobalCyGame.getActivePlayer():
    ...
    def onGameStart(self, argsList):
        ...
        # if (gcgame.getGameTurnYear() == gc.getDefineINT("START_YEAR") and not gcgame.isOption(GameOptionTypes.GAMEOPTION_ADVANCED_START)):
        if (gcgame.getStartEra() == gc.getDefineINT("STANDARD_ERA") or gcgame.isOption(GameOptionTypes.GAMEOPTION_ADVANCED_START)):
            ...
            GlobalCyInterface.setSoundSelectionReady(True)
    ...
    def onEndPlayerTurn(self, argsList):
        ...
        if (gcgame.getElapsedGameTurns() == 1):
            if (gcgetPlayer(iPlayer).isHuman()):
                if (gcgetPlayer(iPlayer).canRevolution(0)):
                    popupInfo = CyPopupInfo()
    ...
    def onFirstContact(self, argsList):
        ## Platy Builder ##
        if GlobalCyGame.GetWorldBuilderMode() and not CvPlatyBuilderScreen.bPython: return
        ## Platy Builder ##
        'Contact'
    ...
    def onCombatLogHit(self, argsList):
        ...
        if cdDefender.eOwner == cdDefender.eVisualOwner:
            szDefenderName = gcgetPlayer(cdDefender.eOwner).getNameKey()
        else:
            szDefenderName = localText.getText("TXT_KEY_TRAIT_PLAYER_UNKNOWN", ())
        if cdAttacker.eOwner == cdAttacker.eVisualOwner:
            szAttackerName = gcgetPlayer(cdAttacker.eOwner).getNameKey()
        else:
        ...
        if (iIsAttacker == 0):
            ...
            GlobalCyInterface.addCombatMessage(cdAttacker.eOwner, combatMessage)
            GlobalCyInterface.addCombatMessage(cdDefender.eOwner, combatMessage)
    ...
    def onBuildingBuilt(self, argsList):
        ...
        game = gcgame
        if ((not gcgame.isNetworkMultiPlayer()) and (pCity.getOwner() == gcgame.getActivePlayer()) and (gcgetPlayer(pCity.getOwner()).countNumBuildings(iBuildingType) <= 1)):
            # If this is a wonder...
            if not GlobalCyGame.GetWorldBuilderMode():  ## Platy Builder ##
                popupInfo = CyPopupInfo()
                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON_SCREEN)
        ...
        CvUtil.pyPrint('%s was finished by Player %d Civilization %s'
                       % (PyInfo.BuildingInfo(iBuildingType).getDescription(), pCity.getOwner(), gcgetPlayer(pCity.getOwner()).getCivilizationDescription(0)))

    def onProjectBuilt(self, argsList):
        'Project Completed'
        pCity, iProjectType = argsList
        game = gcgame
        if ((not gcgame.isNetworkMultiPlayer()) and (pCity.getOwner() == gcgame.getActivePlayer())):
            if not GlobalCyGame.GetWorldBuilderMode():  # Platy Builder
                popupInfo = CyPopupInfo()
                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON_SCREEN)
    ...
    def onUnitPillage(self, argsList):
        ...
        iPlotX = pUnit.getX()
        iPlotY = pUnit.getY()
        pPlot = GlobalCyMap.plot(iPlotX, iPlotY)
    ...
    def onGoodyReceived(self, argsList):
        ...
        if (not self.__LOG_GOODYRECEIVED):
            return
        CvUtil.pyPrint('%s received a goody' % (gcgetPlayer(iPlayer).getCivilizationDescription(0)), )
    ...
    def onTechAcquired(self, argsList):
        ...
        if (iPlayer > -1 and bAnnounce and not GlobalCyInterface.noTechSplash()):
            if (gcgame.isFinalInitialized() and not gcgame.GetWorldBuilderMode()):
                if ((not gcgame.isNetworkMultiPlayer()) and (iPlayer == gcgame.getActivePlayer())):
                    popupInfo = CyPopupInfo()
                    popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON_SCREEN)
    ...
    def onReligionFounded(self, argsList):
        ...
        iCityId = gcgame.getHolyCity(iReligion).getID()
        if (gcgame.isFinalInitialized() and not gcgame.GetWorldBuilderMode()):
            if ((not gcgame.isNetworkMultiPlayer()) and (iFounder == gcgame.getActivePlayer())):
                popupInfo = CyPopupInfo()
                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON_SCREEN)
    ...
    def onCityBuilt(self, argsList):
        'City Built'
        city = argsList[0]
        # if (city.getOwner() == gcgame.getActivePlayer()):
    ...
    def onCityRazed(self, argsList):
        ...
        if city.getPopulation > 1 and iOwner != -1 and iPlayer != -1:
            owner = gcgetPlayer(iOwner)
            if not owner.isBarbarian() and owner.getNumCities() > 0:
                if gcgetTeam(owner.getTeam()).isAtWar(gcgetPlayer(iPlayer).getTeam()):
                    if gc.getNumEventTriggerInfos() > 0:  # prevents mods that don't have events from getting an error
                        iEvent = CvUtil.findInfoTypeNum(gc.getEventTriggerInfo, gc.getNumEventTriggerInfos(), 'EVENTTRIGGER_PARTISANS')
                        if iEvent != -1 and gcgame.isEventActive(iEvent) and owner.getEventTriggerWeight(iEvent) < 0:
    ...
    def onCityRename(self, argsList):
        'City is renamed'
        pCity = argsList[0]
        if (pCity.getOwner() == gcgame.getActivePlayer()):
    ...
    def onMouseEvent(self, argsList):
        ...
        if (px != -1 and py != -1):
            if (eventType == self.EventLButtonDown):
                if (self.bAllowCheats and self.bCtrl and self.bAlt and GlobalCyMap.plot(px, py).isCity() and not interfaceConsumed):
    ...
    def __eventEditCityNameApply(self, playerID, userData, popupReturn):
        ...
        iCityID = userData[0]
        bRename = userData[1]
        player = gcgetPlayer(playerID)
        city = player.getCity(iCityID)
        cityName = popupReturn.getEditBoxString(0)
        if (len(cityName) > 30):
            cityName = cityName[:30]
        city.setName(CvUtil.convertToUnicode(cityName), not bRename)
    ...
    def __eventEditUnitNameBegin(self, argsList):
        ...
        popup.setUserData((pUnit.getID(), GlobalCyGame.getActivePlayer()))
    ...
    def __eventEditUnitNameApply(self, playerID, userData, popupReturn):
        unit = gcgetPlayer(userData[1]).getUnit(userData[0])
        newName = popupReturn.getEditBoxString(0)
        unit.setName(CvUtil.convertToUnicode(newName))
    ...
    def __eventEditCityNameBegin(self, city, bRename):
        popup = PyPopup.PyPopup(CvUtil.EventEditCityName, EventContextTypes.EVENTCONTEXT_ALL)
        popup.setUserData((city.getID(), bRename, GlobalCyGame.getActivePlayer()))
        popup.setHeaderString(localText.getText("TXT_KEY_NAME_CITY", ()))
        popup.setBodyString(localText.getText("TXT_KEY_SETTLE_NEW_CITY_NAME", ()))
        popup.createEditBox(city.getName())
        popup.setEditBoxMaxCharCount(15)
    ...
    def __eventEditCityNameApply(self, playerID, userData, popupReturn):
        city = gcgetPlayer(userData[2]).getCity(userData[0])
        cityName = popupReturn.getEditBoxString(0)
        city.setName(CvUtil.convertToUnicode(cityName), not userData[1])
        if GlobalCyGame.GetWorldBuilderMode() and not GlobalCyGame.isInAdvancedStart():
    ...
    def __eventWBPlayerScriptPopupApply(self, playerID, userData, popupReturn):
        ...
        gcgetPlayer(userData[0]).setScriptData(CvUtil.convertToStr(sScript))
    ...
    def __eventWBCityScriptPopupApply(self, playerID, userData, popupReturn):
        sScript = popupReturn.getEditBoxString(0)
        pCity = gcgetPlayer(userData[0]).getCity(userData[1])
    ...
    def __eventWBUnitScriptPopupApply(self, playerID, userData, popupReturn):
        sScript = popupReturn.getEditBoxString(0)
        pUnit = gcgetPlayer(userData[0]).getUnit(userData[1])
    ...
    def __eventWBGameScriptPopupApply(self, playerID, userData, popupReturn):
        sScript = popupReturn.getEditBoxString(0)
        GlobalCyGame.setScriptData(CvUtil.convertToStr(sScript))
    ...
    def __eventWBPlotScriptPopupApply(self, playerID, userData, popupReturn):
        sScript = popupReturn.getEditBoxString(0)
        pPlot = GlobalCyMap.plot(userData[0], userData[1])
